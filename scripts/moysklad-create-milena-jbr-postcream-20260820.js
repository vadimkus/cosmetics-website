...
/**
 * Milena AESTHETIC CLINIC LLC (JBR Branch) — SO → invoice → shipment (unpaid).
 *
 *   54465 Soothing Repair Post Cream 100g ×2 @ 220 clinic
 *   Delivery Dubai ×1 @ 45, 100% off
 *   Total: 440.00 AED
 *
 *   node --import dotenv/config scripts/moysklad-create-milena-jbr-postcream-20260820.js
 *   node --import dotenv/config scripts/moysklad-create-milena-jbr-postcream-20260820.js --commit
 */
...
const ORDER = {
  name: `GENCardM${uaeShortDate()}MILJPC2`,
  marker: `MILENA-JBR-POSTCREAM-100G-X2-${uaeToday()}`,
}

const PRODUCT_LINES = [['54465', 2, 220]]
const DELIVERY_AED = 45
const EXPECTED_SUM_MINOR = 44000

function shipmentAddress() {
  return {
    country: href('country', COUNTRY_UAE_ID),
    city: 'Dubai',
    street: 'RIMAL 03, PLAZA - LEVEL, SHOP R03, Marsa Dubai, Jumeirah Beach Residence',
    addInfo: '',
  }
}
...
async function main() {
  console.log('====================================================================')
  console.log('  Milena JBR — Post Cream 100g ×2 @ 220, delivery free')
  console.log('====================================================================')
  console.log(`  Mode: ${COMMIT ? 'COMMIT' : 'DRY RUN'}`)

  const agent = await api('GET', `/entity/counterparty/${AGENT_ID}`)
  console.log(`  Customer: ${agent.name} (${agent.id})`)
  console.log(`  Phone: ${agent.phone || '0503516612'}`)
  ...
  if (sumMinor !== EXPECTED_SUM_MINOR) throw new Error(`Sum ${money(sumMinor)} ≠ ${money(EXPECTED_SUM_MINOR)}`)
  ...
  const clean = positions.map(({ quantity, price, discount, assortment, vat, vatEnabled }) => ({
    quantity,
    price,
    discount,
    assortment,
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      '54465 Soothing Repair Post Cream 100g x2 @220 clinic; delivery free.',
      'JBR Branch Rimal 03 Plaza Level Shop R03. Unpaid. Chain: invoice → shipment.',
    ].join('\n'),
