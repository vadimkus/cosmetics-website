...
/**
 * Miss Anna (Al Qusais, +971 50 505 1583) — paid retail SO → invoice → shipment → paymentin.
 *
 *   00122 Multi-Vita Radiance Cream 50g ×1 @ 290
 *   Delivery Dubai ×1 @ 45
 *   Total: 335 AED
 *
 *   Agent: 98a6f84e-46ea-11f1-0a80-03cb00049ccc
 *
 *   node --import dotenv/config scripts/moysklad-create-miss-anna-radiance-paid-20260819.js
 *   node --import dotenv/config scripts/moysklad-create-miss-anna-radiance-paid-20260819.js --commit
 */
...
const AGENT_ID = '98a6f84e-46ea-11f1-0a80-03cb00049ccc'
...
const PHONE = '+971 50 505 1583'
const SHIPMENT = {
  city: 'Dubai',
  street: 'Al Qusais industrial area 5, BlueDiamond building 704',
}

const ORDER = {
  name: `GENCardM${uaeShortDate()}1583`,
  moment: uaeMomentNow(),
  marker: `MISS-ANNA-RADIANCE-CREAM-50-RETAIL-${uaeToday()}`,
}

const PRODUCT_LINES = [['00122', 1, 290]]
...
async function exportInvoicePdf(invoiceId, invoiceName) {
  ...
  const outPath = path.join(ORDERS_DIR, `GENOSYS_Miss_Anna_${safe}.pdf`)
...
async function main() {
  console.log('====================================================================')
  console.log('  Miss Anna — radiance cream 50g + delivery, paid')
  console.log('====================================================================')
  ...
  if (agent.name !== 'Miss Anna') throw new Error(`Unexpected agent: ${agent.name}`)
  const digits = String(agent.phone || '').replace(/\D/g, '')
  if (!digits.endsWith('505051583')) throw new Error(`Phone mismatch: ${agent.phone}`)
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      `Phone ${PHONE}. 00122 Multi-Vita Radiance Cream 50g x1 @ 290; delivery Dubai 45. Paid.`,
