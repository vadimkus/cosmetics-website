...
async function updateProblemSkinBeautyBoxPricingReadable() {
  try {
    console.log('🚀 Updating Problem Skin Care Beauty Box pricing for better readability...')
    ...
    // Replace the pricing line with more readable format
    const updatedDescription = currentDescription.replace(
      /Regular price: [\d,]+ AED \| Bundle price: [\d.]+ AED \| Save 15% \([-\d.]+ AED\)/,
      `Regular price: ${individualPrices.toLocaleString()} AED
...
updateProblemSkinBeautyBoxPricingReadable()
  .then(() => {
    console.log('🎉 Done!')
    process.exit(0)
  })
