...
/**
 * SHAKIROVNA ESTHETIC CLINIC L.L.C (Business Bay) — unpaid clinic SO → invoice → shipment.
 *
 *   54461 Skin Defender ×1 @ 145
 *   Delivery Dubai ×1 @ 45, 100% off
 *   Total: 145.00 AED
 *
 *   node --import dotenv/config scripts/moysklad-create-shakirovna-clinic-defender-20260827.js
 *   node --import dotenv/config scripts/moysklad-create-shakirovna-clinic-defender-20260827.js --commit
 */
...
const AGENT_ID = 'a187255f-a9b6-11f0-0a80-09900022125b'
...
const ORDER = {
  name: `GENCardM${uaeShortDate()}SHKDEF`,
  marker: `SHAKIROVNA-CLINIC-BB-DEFENDER-X1-${uaeToday()}`,
}

const PRODUCT_LINES = [['54461', 1, 145]]
const DELIVERY_AED = 45
const EXPECTED_SUM_MINOR = 14500
...
function shipmentAddress() {
  return {
    ...
    city: 'Dubai',
    street: 'One by Omniyat, Al Asayel Street - Business Bay',
    addInfo: '',
  }
}
...
async function exportInvoicePdf(invoiceId, invoiceName) {
  ...
  const outPath = path.join(ORDERS_DIR, `GENOSYS_Shakirovna_Esthetic_Clinic_${safe}.pdf`)
...
async function main() {
  console.log('====================================================================')
  console.log('  Shakirovna Clinic BB — Defender ×1, delivery free (clinic, unpaid)')
  console.log('====================================================================')
  console.log(`  Mode: ${COMMIT ? 'COMMIT' : 'DRY RUN'}`)

  const agent = await api('GET', `/entity/counterparty/${AGENT_ID}`)
  if (agent.name !== 'SHAKIROVNA ESTHETIC CLINIC L.L.C') {
  ...
  console.log(`    00089 Excellent Delivery Dubai x1 @ ${DELIVERY_AED} 100% off = 0.00`)
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      '54461 Defender x1 @145 clinic; delivery Dubai free. Unpaid.',
    ].join(' | '),
