 ...
 *
 *   Peptide Gel Mask 39g (00012) ×20
 *   Order comment: Abu Dhabi Branch: Brau
 *   State: Доставлен - Ждем оплату
 *   Invoice Legal_TAX PDF → ~/Desktop/orders/
 *
 *   node --import dotenv/config scripts/moysklad-create-brau-ladies-peptide-abu-dhabi-20260728.js
 *   node --import dotenv/config scripts/moysklad-create-brau-ladies-peptide-abu-dhabi-20260728.js --commit
 */
...
const ORDER = {
  name: `GENCardM${uaeShortDate()}BRAUADUP20`,
  moment: uaeMomentNow(),
  marker: `BRAU-PEPTIDE-ADU-P20-${uaeToday()}`,
  comment: 'Abu Dhabi Branch: Brau',
}

const PRODUCT_LINES = [['00012', 20]]
const EXPECTED_SUM_MINOR = 76000 // 20 × 38 AED clinic
...
function buildShipmentAddress(agent) {
  ...
  return {
    country: countryHref(),
    city: 'Abu Dhabi',
    street: addInfo || 'Brau Ladies Salon LLC — Abu Dhabi Branch',
  }
}
...
async function main() {
  console.log('====================================================================')
  console.log('  Brau Ladies — peptide x20 | Abu Dhabi Branch | delivered awaiting pay')
  console.log('====================================================================')
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.comment,
      ORDER.marker,
      'Peptide Gel Mask 00012 x20 @ clinic.',
  ...
  const outPath = path.join(ordersDir, `GENOSYS_Brau_Ladies_Abu_Dhabi_${safeInv}.pdf`)
