# Fayy Health — basic cleanse SO + invoice + shipment (2026-08-17)

**Customer:** Fayy Health FZCO (`ee20d7e3-d46d-11ed-0a80-0df400228557`)  
**Phone:** +971 50 634 7171  
**Ship:** One Central, The Offices 2, 6th Floor, Unit 6.02, Dubai  

Address on this chain was duplicated in MoySklad (`street` also in `addInfo`). Cleaned 21 Aug — see `SESSION_CHANGES_2026-08-21_FAYY_HEALTH_PEPTIDE_X100.md`.  
**Pricing:** Clinic list. Hydro Cool excluded. Unpaid.

| Doc | № | Sum |
|-----|---|-----|
| Sales order | **GENCardM260817FAYY** | 1,220.00 |
| Invoice | **04938** | 1,220.00 |
| Shipment | **06693** | 1,220.00 |

Links: SO → INV 04938 → SHIP 06693. No `demand.customerOrder`. No payment. SO → Доставлен - Ждем оплату. No print.

| Code | Product | Qty | Clinic |
|------|---------|----:|-------:|
| 54461 | Skin Defender Lip & Eye Makeup Remover 200ml | 1 | 145 |
| 00024 | Snow O₂ Cleanser 500ml | 1 | 255 |
| 00025 | Snow Booster Toner 1000ml | 1 | 245 |
| 00129 | EPI Turnover Boosting Peeling Gel 100g | 1 | 125 |
| 54465 | Soothing Repair Post Cream 100g | 1 | 220 |
| 00041 | Multi Sun Cream SPF40 | 1 | 105 |
| 54457 | Ultra Shield SPF50 | 1 | 125 |

Postcream is the **100g** pro size (cleanser/booster are pro). SPF row listed both products, so both went on the invoice.

PDF: `~/Desktop/orders/GENOSYS_Fayy_Health_04938.pdf`

Script: `scripts/moysklad-create-fayy-health-basic-cleanse-order-20260817.js`
