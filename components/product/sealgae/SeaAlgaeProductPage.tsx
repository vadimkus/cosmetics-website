...
/**
 * Bespoke product page for SOOTHING BOMB SEA ALGAE MASK (product 36).
 *
...
export default function SeaAlgaeProductPage({ product, unitsSold = 0, routineProducts = [] }: Props) {
  ...
  const isRtl = dir === 'rtl'
  const copy = getSeaAlgaeCopy(locale)
      ...
      try {
        ...
        try {
          trackAddToCart({
            id: product.id,
            name: product.name,
            category: product.category || 'Mask',
            price: product.price,
            quantity: qty,
          })
        } catch { /* analytics is best-effort */ }
      } catch (error) {
        errorLog('SeaAlgae: add to cart failed', error)
        throw error
      }
    },
      ...
      <section className="mx-auto max-w-[1200px] px-4 pt-4 sm:px-6 md:pt-8 lg:pt-12">
        ...
        <div className="mt-5 grid grid-cols-1 gap-8 lg:mt-9 lg:grid-cols-[minmax(0,1.4fr)_minmax(0,1fr)] lg:gap-12 xl:gap-16">
          ...
          <div>
            ...
            <h1 className="cera-serif mt-3 text-[32px] leading-[1.06] sm:text-[42px] lg:text-[48px]">
              {product.name}
            </h1>
                ...
                <button
                  ...
                  className={`inline-flex h-[54px] min-w-[220px] flex-1 items-center justify-center gap-2.5 rounded-full px-8 text-[15px] font-semibold tracking-wide transition-all duration-300 disabled:cursor-not-allowed disabled:opacity-45 ${
                    justAdded
                      ...
                      : 'bg-[var(--cera-cta)] text-white hover:bg-[var(--cera-rose-ink)] hover:shadow-[0_18px_38px_-20px_rgba(20,32,26,0.8)]'
                  }`}
                >
          ...
          <div className="grid grid-cols-1 gap-10 lg:grid-cols-[minmax(0,0.8fr)_minmax(0,1fr)] lg:gap-16">
            ...
            <CeraReveal className="cera-card overflow-hidden">
              <div className="overflow-x-auto">
                <table className="sea-table w-full border-collapse text-start">
                  <caption className="sr-only">{copy.formula.title}</caption>
                  <thead>
                    <tr className="border-b border-[var(--cera-line)]">
                      {[copy.formula.columns.name, copy.formula.columns.amount, copy.formula.columns.role].map(head => (
                        <th
                          key={head}
                          scope="col"
                  ...
                  <tbody>
                    {copy.formula.rows.map(row => (
                      <tr key={row.name} className="border-b border-[var(--cera-line)] last:border-b-0">
                        ...
                        <td
                          dir="ltr"
                          className="sea-figure whitespace-nowrap px-4 py-4 text-start align-top text-[14px] text-[var(--cera-rose-ink)]"
                        >
                          {row.amount}
                        </td>
          ...
          {actives.length > 0 ? (
            <div className="mt-10 grid grid-cols-1 gap-x-10 gap-y-px sm:grid-cols-2 lg:mt-14 lg:gap-x-16">
              {actives.map((active, i) => (
                <CeraReveal key={active.name} delay={(i % 4) * 60} className="border-b border-[var(--cera-line)] py-6">
                  <div className="flex items-baseline gap-3">
      ...
      {companions.length > 0 && (
        <section className="mx-auto max-w-[1040px] px-4 py-16 sm:px-6 lg:py-20">
          <CeraSectionHeader
            ...
            title={locale === 'ar' ? 'ما يعمل معه' : locale === 'ru' ? 'Что рядом с ней' : 'What goes with it'}
          />
