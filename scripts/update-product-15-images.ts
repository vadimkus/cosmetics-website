/**
 * Update product 15 (INTENSIVE PROBLEM CONTROL TONER) to the new
 * public/images/problem/ set (Main + S1–S6).
 *
 * Dry run:
 *   npx tsx --env-file=.env.local scripts/update-product-15-images.ts
 * Apply:
 *   npx tsx --env-file=.env.local scripts/update-product-15-images.ts --apply
 */
...
const NEW_MAIN = '/images/problem/Main.jpg'
const NEW_GALLERY = [
...
async function main() {
  const p = await prisma.product.findFirst({
    where: { OR: [{ id: '15' }, { productNumber: '15' }] },
