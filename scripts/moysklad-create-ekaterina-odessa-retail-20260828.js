...
/**
 * Miss Ekaterina (Odessa) — new retail customer, SO → invoice → shipment (unpaid).
 *
 *   00044 ND Cell Anti-Wrinkle Cream 50ml ×1 @ 370 −10%
 *   54458 Moisture Replenishing Hyaluron Cream 50g ×1 @ 290 −10%
 *   Excellent Delivery Dubai ×1 @ 45 (no discount)
 *   Total: 639.00 AED
 *
 *   Phone +971 58 560 0763. Referred by Alena. Address TBC.
 *
 *   node --import dotenv/config scripts/moysklad-create-ekaterina-odessa-retail-20260828.js
 *   node --import dotenv/config scripts/moysklad-create-ekaterina-odessa-retail-20260828.js --commit
 */
...
const ORDER = {
  name: `GENCardM${uaeShortDate()}0763`,
  moment: uaeMomentNow(),
  marker: `EKATERINA-ODESSA-RETAIL-10-${uaeToday()}`,
}
...
const EXPECTED_SUM_MINOR = 63900
...
function customerNote() {
  return `Retail — from Odessa. Referred by Alena. Phone ${CUSTOMER.phone}. Address TBC.`
}
...
async function findExistingCounterparty() {
  for (const ph of [CUSTOMER.phone, '+971 58 560 0763', '0585600763', '585600763']) {
...
async function exportInvoicePdf(invoiceId, invoiceName) {
  ...
  const out = path.join(ORDERS_DIR, `GENOSYS_Miss_Ekaterina_${safe}.pdf`)
...
async function main() {
  console.log('====================================================================')
  console.log('  Miss Ekaterina (Odessa) — retail −10% + delivery 45, unpaid')
  console.log('====================================================================')
  ...
  console.log(`  Ship: ${CUSTOMER.city} (address TBC)`)
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      '00044 ND Cell x1 @370 −10%; 54458 Hyaluron Cream 50g x1 @290 −10%; delivery Dubai 45 no discount.',
      `Total 639 AED unpaid. From Odessa. Referred by Alena. Address TBC. Phone ${CUSTOMER.phone}.`,
    ].join('\n'),
