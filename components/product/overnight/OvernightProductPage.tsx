...
/**
 * Bespoke product page for SKIN RESCUE OVERNIGHT CREAM MASK (product 34).
 *
 ...
 * ballet-slipper pink palette on top via overnight.css, taken from the tube.
 *
 * Section order:
 *
 *   effects  Soothe · Hold water · Brighter look · Smoother lines
 *   engine   Niacinamide 2% + Adenosine 0.04%, the distinctive documented fact
 *   howTo    last step, leave on overnight, do not wash off
 ...
 *   routine  cleanse → booster → hyaluron serum → mask 34
 *
 * There is a dedicated DTS MG deck with a four-week TEWL / erythema trial.
 * There is no safety assessment under this SKU name. EGF REPAIR OXYMASK CREAM
 * is a different formula - do not borrow its SA. See overnightCopy.ts.
 * In particular: Niacinamide 2% leads, not oxygen therapy and not the
 * growth-factor list at 0%. Leave-on, not rinse-off. Do not use near eyes.
 */
...
import { getOvernightCopy } from './overnightCopy'
...
export default function OvernightProductPage({
  product,
  unitsSold = 0,
  routineProducts = [],
}: Props) {
  ...
  const isRtl = dir === 'rtl'
  const copy = getOvernightCopy(locale)
  ...
  const routineSteps = useMemo(() => {
    const routine = PRODUCT_ROUTINES['34']
    if (!routine) return []
  ...
  const addToCart = useCallback(
    async (qty: number) => {
      ...
      try {
        ...
        errorLog('Overnight mask: add to cart failed', error)
        throw error
      }
    },
  ...
  const handleAddRoutineProduct = useCallback(
    async (item: Product, selection?: ProductOptionSelection, quantity = 1) => {
      ...
      try {
        ...
        errorLog('Overnight mask: routine add to cart failed', error)
      } finally {
  ...
  return (
    <div className={`cera-page sro-page ${ceraSerif.variable} min-h-[100dvh]`} dir={dir}>
      {/* ───────────────────────────── Hero ─────────────────────────────── */}
        ...
        <div>
          ...
          {product.videoUrl ? (
            <CeraReveal className="mt-10">
              ...
              <div className="sro-video relative mt-4 mx-auto aspect-[9/16] w-full max-w-[340px] overflow-hidden rounded-[28px]">
                <video
      ...
      <section className="mx-auto max-w-[1000px] px-4 py-16 sm:px-6 lg:py-24">
        ...
        <div className="mt-10 grid grid-cols-1 gap-4 lg:mt-14 lg:grid-cols-2 lg:gap-6">
          ...
          <CeraReveal delay={90} className="cera-card sro-not p-6 lg:p-7">
            <h3 className="cera-serif text-[21px] leading-tight text-[var(--cera-ink)]">
