  ...
  return (
    <div className="rounded-3xl border border-[var(--cera-line)] bg-white p-5 shadow-[0_14px_40px_-28px_rgba(23,20,15,0.26)] md:p-6 lg:p-8">
      ...
      <div className="md:hidden">
        <div className="flex items-center gap-3">
          ...
          <div className="relative flex-shrink-0">
            ...
            {isEditing && (
              <button
                ...
                className="absolute bottom-0 right-0 w-6 h-6 bg-white border-2 border-[var(--cera-rose)] text-[var(--cera-rose-ink)] rounded-full shadow-lg flex items-center justify-center hover:bg-[var(--cera-blush)] active:bg-[var(--cera-blush)] transition-colors touch-manipulation"
          ...
          <div className="flex-1 min-w-0">
            <p className="truncate text-base font-bold text-[var(--cera-ink)]">{user.name}</p>
            <p className="text-xs text-[var(--cera-muted)] truncate">{displayEmail}</p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-1.5 mt-3">
          ...
          <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-[var(--cera-cream-deep)] text-[var(--cera-body)] rounded-full text-[10px] font-medium">
      ...
      <div className="hidden md:flex md:flex-row md:items-center md:gap-6 lg:gap-8">
        {/* Profile Picture */}
        <div className="relative flex-shrink-0">
          <div className="flex h-24 w-24 items-center justify-center overflow-hidden rounded-3xl border border-[var(--cera-line)] bg-[var(--cera-cream-deep)] shadow-sm md:h-24 md:w-24 lg:h-28 lg:w-28">
            {previewImage ? (
          ...
          {isEditing && (
            <div className="absolute -bottom-1 -right-1 flex gap-1.5">
              <button onClick={() => fileInputRef.current?.click()} className="bg-[var(--cera-rose)] text-white p-2.5 rounded-full hover:bg-[var(--cera-rose-ink)] transition-colors shadow-md" title={t('profile.uploadPhoto')}>
                <Camera className="h-3.5 w-3.5" />
              </button>
              {previewImage && (
                <button onClick={onRemoveImage} className="bg-white text-[var(--cera-body)] border border-[var(--cera-line)] p-2.5 rounded-full hover:text-red-600 hover:border-red-200 hover:bg-red-50 transition-colors shadow-md" title={t('profile.removePhoto')}>
        ...
        <div className="flex-1 min-w-0 text-left">
          <p className="mb-1 truncate text-2xl font-bold text-[var(--cera-ink)] lg:text-3xl">{user.name}</p>
          <p className="text-[var(--cera-body)] text-sm lg:text-base break-all">{displayEmail}</p>
          {/* User Type Badges */}
          <div className="flex flex-wrap gap-2 justify-start mt-3">
            ...
            <div className="flex items-center gap-1.5 px-2.5 py-1 bg-[var(--cera-cream-deep)] text-[var(--cera-body)] rounded-full text-xs lg:text-sm font-medium">
