...
/**
 * Miss Olga Pikina — paid retail: Beige Cushion ×1 @ 270 + delivery Dubai @ 45 = 315 AED.
 * SO → invoice → shipment → paymentin. Invoice PDF → ~/Desktop/orders/
 *
 *   Ship: The Greens and Views, Fairways East tower, Apartment 1804, Dubai
 *
 *   node --import dotenv/config scripts/moysklad-create-miss-olga-pikina-beige-order-paymentin-20260805.js
 *   node --import dotenv/config scripts/moysklad-create-miss-olga-pikina-beige-order-paymentin-20260805.js --commit
 */
...
const CUSTOMER = {
  name: 'Miss Olga Pikina',
  city: 'Dubai',
  street: 'The Greens and Views, Fairways East tower, Apartment 1804',
}

const ORDER = {
  name: `GENCardM${uaeShortDate()}PIKI`,
  moment: uaeMomentNow(),
  marker: `MISS-OLGA-PIKINA-BEIGE-PAID-${uaeToday()}`,
}

/** [code, qty, retailAed, discountPct] */
const PRODUCT_LINES = [['00144', 1, 270, 0]]
...
async function findOrCreateCustomer() {
  for (const q of ['Pikina', 'Пикина', 'Olga Pikina', 'Miss Olga Pikina']) {
    const d = await api('GET', `/entity/counterparty?search=${encodeURIComponent(q)}&limit=20`)
    const hit = (d.rows || []).find((r) => /pikina|пикина/i.test(r.name || ''))
    if (hit) {
      console.log(`  Customer exists: ${hit.name} (${hit.id})`)
      if (COMMIT) {
        await api('PUT', `/entity/counterparty/${hit.id}`, {
          name: CUSTOMER.name,
...
async function exportInvoicePdf(invoiceId, invoiceName) {
  ...
  const outPath = path.join(ORDERS_DIR, `GENOSYS_Miss_Olga_Pikina_${safe}.pdf`)
...
async function main() {
  console.log('====================================================================')
  console.log('  Miss Olga Pikina — Beige cushion + delivery paid')
  console.log('====================================================================')
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      '00144 Beige cushion x1 @270 retail; delivery Dubai 45.',
      `Ship: ${CUSTOMER.street}, Dubai.`,
    ].join(' | '),
