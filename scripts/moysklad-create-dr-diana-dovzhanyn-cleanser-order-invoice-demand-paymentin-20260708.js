...
/**
 * Dr. Diana Dovzhanyn — retail SO → invoice → shipment → paymentin (paid).
 *   Repeat of invoice 04541 pattern:
 *     00024 Snow O₂ Cleanser 500ml x1 @ 510 AED (retail)
 *     Excellent Delivery Dubai x1 @ 45 AED
 *     Total: 555 AED
 *
 *   Ship: Park Ridge Tower C, app 902, Dubai Hills
 *
 *   node --import dotenv/config scripts/moysklad-create-dr-diana-dovzhanyn-cleanser-order-invoice-demand-paymentin-20260708.js
 *   node --import dotenv/config scripts/moysklad-create-dr-diana-dovzhanyn-cleanser-order-invoice-demand-paymentin-20260708.js --commit
 */
...
const CUSTOMER = {
  name: 'Dr. Diana Dovzhanyn',
  phone: '+971558552397',
  city: 'Dubai Hills',
  street: 'Park Ridge Tower C, app 902',
}

const ORDER = {
  name: `GENCardM${uaeShortDate()}2397`,
  moment: uaeMomentNow(),
  marker: `DR-DIANA-DOVZHANYN-CLEANSER-RETAIL-${uaeToday()}`,
}

const PRODUCT_LINES = [['00024', 1, 510]]
const DELIVERY_AED = 45
const EXPECTED_SUM_MINOR = 55500
...
async function main() {
  console.log('====================================================================')
  console.log('  Dr. Diana Dovzhanyn — SO → invoice → shipment → paymentin (paid)')
  console.log('====================================================================')
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      '00024 Snow O₂ Cleanser 500ml x1 @ 510 retail; delivery Dubai 45 AED. Paid — paymentin posted.',
