...
/**
 * TONETRENDZ — paymentin for consignment report 01438 (1,128 AED).
 * Links to commissioner report, marks Paid.
 *
 *   node --import dotenv/config scripts/moysklad-create-tonetrendz-01438-paymentin-20260825.js
 *   node --import dotenv/config scripts/moysklad-create-tonetrendz-01438-paymentin-20260825.js --commit
 */
...
const REPORT_ID = 'a198933c-a052-11f1-0a80-028b0021be78'
const REPORT_NAME = '01438'
const EXPECTED_SUM_MINOR = 112800
const STATE_REPORT_PAID_ID = 'fd15289c-c3c4-11eb-0a80-065200268290'
const MARKER = `TONETRENDZ-01438-PAYMENTIN-${uaeToday()}`
...
async function main() {
  console.log('====================================================================')
  console.log('  TONETRENDZ — paymentin for consignment report 01438')
  console.log('====================================================================')
  ...
  const dup = pays.find(
    (p) => (p.description || '').includes(MARKER) || (p.description || '').includes(`report ${REPORT_NAME}`),
  )
  ...
  const paymentIn = await api('POST', '/entity/paymentin', {
    ...
    description: [
      ...
      'TONETRENDZ consignment — Square Aug — 1,128 AED paid in full.',
    ].join(' | '),
