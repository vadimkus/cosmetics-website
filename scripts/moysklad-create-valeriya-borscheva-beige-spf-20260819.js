...
/**
 * Dr. Valeria Borscheva — unpaid clinic SO → invoice → shipment.
 *
 *   00144 Cushion #2 Beige ×2 @ 150
 *   00041 Multi Sun Cream SPF40 ×2 @ 105
 *   Delivery Dubai ×1 @ 45
 *   Total: 555 AED
 *
 *   node --import dotenv/config scripts/moysklad-create-valeriya-borscheva-beige-spf-20260819.js
 *   node --import dotenv/config scripts/moysklad-create-valeriya-borscheva-beige-spf-20260819.js --commit
 */
...
const ORDER = {
  ...
  moment: uaeMomentNow(),
  marker: `VALERIYA-BORSCHEVA-BEIGE-SPF40-${uaeToday()}`,
}
...
function shipmentAddress(agent) {
  ...
  return {
    ...
    city: 'Dubai',
    street: 'Garden View Apartments, building 82, app 7',
  }
}
...
async function exportInvoicePdf(invoiceId, invoiceName) {
  ...
  const out = path.join(ORDERS_DIR, `GENOSYS_Valeriya_Borscheva_${safe}.pdf`)
...
async function main() {
  console.log('====================================================================')
  console.log('  Valeriya Borscheva — beige ×2 + SPF40 ×2 + delivery, unpaid')
  console.log('====================================================================')
  console.log(`  Mode: ${COMMIT ? 'COMMIT' : 'DRY RUN'}`)

  const agent = await api('GET', `/entity/counterparty/${AGENT_ID}`)
  console.log(`  Customer: ${agent.name} | ${agent.phone || '—'}`)
  if (!/borschev/i.test(agent.name)) throw new Error(`Unexpected agent: ${agent.name}`)
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      '00144 Beige x2 @150; 00041 SPF40 x2 @105; delivery 45. Clinic list. Unpaid.',
