 ...
 * Section order:
 *
 *   effects  Meet · Activate · Rinse
 *   engine   bicarbonate 9% + acidic gel, the reaction is the product
 *   howTo    dry skin, gel, sheet, ten minutes
 *   actives  ingredient cards from the product record, plus both INCIs
 *   suited   who it is for and who should buy something else
 *   routine  cleanse → kit → mist → cream
 *
 * There is a dedicated DTS MG deck and a safety assessment, and no quantified
 * clinical trial, so this page has no proof chart. See ezco2Copy.ts for the
 * sourcing rules. In particular: ten minutes, not 15-20; no peptide mask;
 * lactic acid is in the sheet, not the gel.
 */
...
import { getEzCo2Copy } from './ezco2Copy'
...
/** Tube packshot for the engine section. Main still titles the box PROFESSIONAL KIT, so the engine
 *  figure is s8, which prints the official name and the spatula. */
...
export default function EzCo2ProductPage({
  product,
  unitsSold = 0,
  routineProducts = [],
}: Props) {
  ...
  const isRtl = dir === 'rtl'
  const copy = getEzCo2Copy(locale)
  ...
  const routineSteps = useMemo(() => {
    const routine = PRODUCT_ROUTINES['38']
    if (!routine) return []
  ...
  const addToCart = useCallback(
    async (qty: number) => {
      ...
      try {
        ...
        errorLog('EZ CO2: add to cart failed', error)
        throw error
      }
    },
  ...
  const handleAddRoutineProduct = useCallback(
    async (item: Product, selection?: ProductOptionSelection, quantity = 1) => {
      ...
      try {
        ...
        errorLog('EZ CO2: routine add to cart failed', error)
      } finally {
  ...
  return (
    <div className={`cera-page ez-page ${ceraSerif.variable} min-h-[100dvh]`} dir={dir}>
      {/* ───────────────────────────── Hero ─────────────────────────────── */}
        ...
        <div>
          ...
          {product.videoUrl ? (
            <CeraReveal className="mt-10">
              ...
              <div className="ez-video relative mt-4 aspect-square overflow-hidden rounded-[28px] sm:aspect-video">
                <video
      ...
      <section className="mx-auto max-w-[1000px] px-4 py-16 sm:px-6 lg:py-24">
        ...
        <div className="mt-10 grid grid-cols-1 gap-4 lg:mt-14 lg:grid-cols-2 lg:gap-6">
          ...
          <CeraReveal delay={90} className="cera-card ez-not p-6 lg:p-7">
            <h3 className="cera-serif text-[21px] leading-tight text-[var(--cera-ink)]">
