...
/**
 * Persona Marina — unpaid clinic SO → invoice → shipment.
 *
 *   00025 Snow Booster Toner 1000ml ×1 @ 245
 *   Total: 245.00 AED
 *
 *   node --import dotenv/config scripts/moysklad-create-persona-marina-booster-1000-20260904.js
 *   node --import dotenv/config scripts/moysklad-create-persona-marina-booster-1000-20260904.js --commit
 */
...
const ORDER = {
  name: `GENCardM${uaeShortDate()}PMMB`,
  marker: `PERSONA-MARINA-BOOSTER1000-${uaeToday()}`,
}

const PRODUCT_LINES = [['00025', 1, 245]]
const EXPECTED_SUM_MINOR = 24500
...
async function main() {
  console.log('====================================================================')
  console.log('  Persona Marina — booster 1000ml 245 AED unpaid')
  console.log('====================================================================')
  console.log(`  Mode: ${COMMIT ? 'COMMIT' : 'DRY RUN'}`)

  const agent = await api('GET', `/entity/counterparty/${AGENT_ID}`)
  if (!/First Person Ladies Salon/i.test(agent.name) || !/Marina/i.test(agent.name + (agent.actualAddress || ''))) {
    if (agent.id !== AGENT_ID) throw new Error(`Unexpected agent: ${agent.name}`)
  }
  const phone = agent.phone || agent.phones || ''
  console.log(`  Customer: ${agent.name} | ${phone}`)
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      'Clinic list booster 1000ml 245 AED unpaid. No delivery. Not consignment.',
