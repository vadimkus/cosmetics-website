...
/**
 * Face Care Pro Spa LTD — clinic SO → invoice → shipment + Legal_TAX PDF.
 *
 *   00050 HR³ Matrix Scalp Peeling 100ml ×1 @ 145
 *   00024 Snow O₂ Cleanser 500ml ×4 @ 255
 *   00025 Snow Booster Toner 1000ml ×5 @ 245
 *   Excellent Delivery Dubai ×1 @ 45, 100% off (free)
 *   Total: 2,390 AED
 *
 *   Ship: Index Tower, Zaa'beel Second, DIFC, Floor R1 unit 215, Dubai
 *
 *   node --import dotenv/config scripts/moysklad-create-face-care-pro-spa-order-invoice-demand-20260722.js
 *   node --import dotenv/config scripts/moysklad-create-face-care-pro-spa-order-invoice-demand-20260722.js --commit
 */
...
const AGENT_ID = 'e0eb8f5c-29c5-11f1-0a80-11a200380fea' // Face Care Pro Spa LTD
...
const SHIP_STREET = "Index Tower - Zaa'beel Second - DIFC - Floor R1, unit 215"

const ORDER = {
  name: `GENCardM${uaeShortDate()}FCPS`,
  moment: uaeMomentNow(),
  marker: `FACE-CARE-PRO-SPA-SCALP-SNOW-${uaeToday()}`,
}
...
const EXPECTED_SUM_MINOR = 239000 // 145+1020+1225; delivery free
...
async function exportInvoicePdf(invoiceId, invoiceName) {
  ...
  const outPath = path.join(ORDERS_DIR, `GENOSYS_Face_Care_Pro_Spa_${safe}.pdf`)
...
async function main() {
  console.log('====================================================================')
  console.log('  Face Care Pro Spa LTD — clinic SO + invoice + shipment')
  console.log('====================================================================')
  ...
  const shipmentAddressFull = shipmentAddress()
  ...
  console.log(`  Ship to: ${shipmentAddressFull.street}, ${shipmentAddressFull.city}`)
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      '00050 scalp peeling x1 @145; 00024 Snow O2 500ml x4 @255; 00025 Snow Booster 1000ml x5 @245; delivery FREE.',
      `Ship: ${SHIP_STREET}, Dubai. Phone +971501504735.`,
    ].join(' | '),
