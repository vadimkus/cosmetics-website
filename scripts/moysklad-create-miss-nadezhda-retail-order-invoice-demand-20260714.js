...
/**
 * Miss Nadezhda — retail SO → invoice → shipment + PDF.
 * Phone +971521667701 (existing counterparty Miss Nadi).
 *
 *   00188 Microbiome Energy Infusing Mist 80ml ×1 @ 160, −10%
 *   54473 Revita Glow BB Cream #02 Natural 50g ×1 @ 250, −10%
 *   00194 Multi Vita Radiance Serum 30ml ×1 @ 330, −10%
 *   54458 Moisture Replenishing Hyaluron Cream 50g ×1 @ 290, −10%
 *   54467 Skin Reboot PDRN mask Pack ×1 @ 400, −10%
 *   Excellent Delivery Dubai ×1 @ 45
 *   Total: 1,332.00 AED
 *
 *   Ship: Beach Resort, JBR, Tower 1, 904, Dubai
 *
 *   node --import dotenv/config scripts/moysklad-create-miss-nadezhda-retail-order-invoice-demand-20260714.js
 *   node --import dotenv/config scripts/moysklad-create-miss-nadezhda-retail-order-invoice-demand-20260714.js --commit
 */
...
const EXISTING_AGENT_ID = '52990f1a-665c-11f1-0a80-1f27004459e1' // Miss Nadi, same phone
...
const ORDER = {
  ...
  moment: uaeMomentNow(),
  marker: `MISS-NADEZHDA-RETAIL-${uaeToday()}`,
}
...
const EXPECTED_SUM_MINOR = 133200
...
async function findOrUpdateCounterparty() {
  let agent = null
  ...
  if (!agent) {
    agent = await api('GET', `/entity/counterparty/${EXISTING_AGENT_ID}`)
  }
  ...
  const needsUpdate =
    ...
    agent.actualAddressFull?.street !== CUSTOMER.street

  console.log(`  Counterparty: ${agent.name} (${agent.id})`)
  if (needsUpdate) {
    console.log(`  Update name/address → ${CUSTOMER.name} | ${CUSTOMER.street}`)
    if (COMMIT) {
      agent = await api('PUT', `/entity/counterparty/${agent.id}`, {
        meta: agent.meta,
        name: CUSTOMER.name,
        ...
        description: [
          agent.description || '',
          `Retail — ${CUSTOMER.street}. Updated ${uaeToday()}.`,
        ]
          .filter(Boolean)
          .join(' ')
          .trim(),
...
function ordersPdfPath(invoiceName) {
  ...
  return path.join(ordersDir, `GENOSYS_Miss_Nadezhda_${safe}.pdf`)
}

async function main() {
  console.log('====================================================================')
  console.log('  Miss Nadezhda — retail SO + invoice + shipment')
  console.log('====================================================================')
  console.log(`  Mode: ${COMMIT ? 'COMMIT' : 'DRY RUN'}`)

  const agent = await findOrUpdateCounterparty()
  ...
  for (const [code, qty, retailAed, discountPct] of PRODUCT_LINES) {
    const item = stock.get(code)
    const net = lineNet(Math.round(retailAed * 100), qty, discountPct)
    console.log(
      `    ${code} ${item.name.slice(0, 48)} x${qty} @ ${retailAed.toFixed(2)} (−${discountPct}%) = ${money(net)}`
    )
  }
  ...
  const orderDesc = [
    ORDER.marker,
    '00188 mist, 54473 Revita Natural, 00194 radiance serum, 54458 hyaluron cream 50g, 54467 PDRN pack — 10% off products.',
    'Delivery Dubai 45. Total 1332 AED.',
    `Contact ${CUSTOMER.phone}. Ship: ${CUSTOMER.street}.`,
    'Chain: invoice → shipment (no paymentin).',
  ].join(' | ')
  ...
  if (pdfBuf) {
    ...
    console.log(`\n  PDF: ${outPath} (${pdfBuf.length} bytes)`)
  }
}
