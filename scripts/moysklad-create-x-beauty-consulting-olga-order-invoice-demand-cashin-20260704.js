...
/**
 * X BEAUTY CONSULTING - F.Z.C — full cycle, cash in, delivered:
 *   00022 Snow Booster Toner 200ml ×1 @ 130 AED
 *   00189 Skin Rescue Overnight Cream Mask 100g ×1 @ 170 AED
 *   00030 All For Sensitive Serum 30ml ×1 @ 165 AED
 *   Excellent Delivery Dubai ×1 @ 45 AED
 *   Total: 510 AED | customer note: Olga
 *
 *   node --import dotenv/config scripts/moysklad-create-x-beauty-consulting-olga-order-invoice-demand-cashin-20260704.js
 *   node --import dotenv/config scripts/moysklad-create-x-beauty-consulting-olga-order-invoice-demand-cashin-20260704.js --commit
 */
...
const CUSTOMER_NOTE = 'Olga'

const ORDER = {
  name: `GENCardM${uaeShortDate()}XBCO`,
  moment: uaeMomentNow(),
  marker: `X-BEAUTY-CONSULTING-OLGA-ORDER-${uaeToday()}`,
}
...
const EXPECTED_SUM_MINOR = 51000
...
async function main() {
  console.log('====================================================================')
  console.log('  X BEAUTY CONSULTING — SO → invoice → shipment → cash in (Olga)')
  console.log('====================================================================')
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ...
      '00022 booster 200ml x1 @130; 00189 overnight mask x1 @170; 00030 sensitive serum x1 @165; delivery 45.',
