...
/**
 * Bespoke product page for INTENSIVE HYDRO SOOTHING CREAM (product 28). Cool aqua
 * in hydrosoothing.css, because this is the one product in the range with a
 * measured cooling result.
 *
 * THE PAGE LEADS WITH THE TWO MEASURED FINDINGS, which were sitting unused in the
 * DTS MG homecare deck: skin temperature down about 1 C twenty minutes after
 * application, and hydration up 12% after four weeks. The Intertek dossier says
 * "Other Tests: None presented", so reading the dossier alone would have led to
 * deleting a supported claim. The deck is the source. Every presentation of these
 * numbers is followed by the caveat that the deck names no laboratory, no subject
 * count, no method and no instrument.
 *
 * THEN BETAINE AT 5%, which is the actual engine and appeared nowhere in our copy,
 * inside a 21.7% humectant load with butylene glycol and glycerin.
 *
 * THEN THE PREMIX EXPLANATION. Our old description named snail secretion filtrate
 * second because the supplier sheet lists the pre-diluted material at 0.100% and
 * that material is only 1% filtrate, so the finished cream holds 10 ppm. The page
 * shows the sheet figure against the real figure for seven ingredients and
 * explains the mechanism, because it is the honest reason for the error and it
 * generalises across the industry.
 *
 * Do not add: any mechanism from the snail filtrate (the deck credits it with
 * accelerating cell regeneration and stimulating collagen; it is at 10 ppm), any
 * anti-ageing or regeneration claim of the kind the drifted Russian panel makes,
 * beta-glucan as a barrier film former (4 ppm), benzene (an internal carbomer
 * question for DTS MG, not customer copy), anything implying use on broken skin,
 * the contract manufacturer, or the lot codes.
 */

import '../cerabarrier/cerabarrier.css'
import PageBreadcrumb from '@/components/PageBreadcrumb'
import './hydrosoothing.css'
...
export default function HydroSoothingProductPage({ product, unitsSold = 0, routineProducts = [] }: Props) {
  ...
  const addToCart = useCallback(
    async (qty: number, size: string = selectedSize) => {
      ...
      try {
        ...
        errorLog('HydroSoothing: add to cart failed', error)
        throw error
      }
    },
  ...
  return (
    <div className={`cera-page hsc-page min-h-[100dvh]`} dir={dir}>
      {/* ───────────────────────────── Hero ─────────────────────────────── */}
      ...
      {/* ─────────────── The two measured findings, first ───────────────── */}
      <section className="mx-auto max-w-[1040px] px-4 pt-14 sm:px-6 lg:pt-20">
        <CeraSectionHeader eyebrow={copy.clinical.eyebrow} title={copy.clinical.title} intro={copy.clinical.intro} />

        <div className="mt-10 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:mt-12 lg:gap-6">
          {copy.clinical.findings.map((f, i) => (
            <CeraReveal key={f.label} delay={i * 90} as="article" className="hsc-finding p-7 md:p-8">
              <p dir="ltr" className="cera-serif cera-numeral hsc-finding__value text-[46px] leading-none text-[var(--cera-rose)] sm:text-[56px]">
                {f.value}
              </p>
              <p className="mt-3 text-[13px] font-semibold uppercase tracking-[0.07em] text-[var(--cera-rose-ink)]">
                {f.label}
              </p>
              <p className="mt-3.5 text-[15px] leading-relaxed text-[var(--cera-body)]">{f.body}</p>
            </CeraReveal>
          ))}
        </div>

        <CeraReveal>
          <p className="hsc-caveat mt-8 flex items-start gap-2.5 pt-6 text-[14px] leading-relaxed">
            <Info className="mt-[2px] h-4 w-4 flex-none" aria-hidden="true" />
            <span>{copy.clinical.caveat}</span>
          </p>
        </CeraReveal>
      </section>

      {/* ───────────────────── Why cooling matters here ─────────────────── */}
      <section className="mx-auto max-w-[900px] px-4 py-14 sm:px-6 lg:py-16">
        <CeraReveal className="hsc-note p-6 md:p-9">
          <p className="cera-eyebrow">{copy.climate.eyebrow}</p>
          <h2 className="cera-serif mt-3 text-[26px] leading-tight sm:text-[32px]">{copy.climate.title}</h2>
          <p className="mt-5 text-[16px] leading-relaxed text-[var(--cera-body)]">{copy.climate.body}</p>
        </CeraReveal>
      </section>
          ...
          <div className="mt-10 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:mt-14 lg:grid-cols-3 lg:gap-6">
            {copy.working.items.map((item, i) => (
              <CeraReveal key={item.name} delay={i * 70} as="article" className="cera-card cera-card-hover flex flex-col p-6">
                ...
                <p dir="ltr" className="cera-serif cera-numeral hsc-figure mt-2 text-[25px] leading-none text-[var(--cera-rose)]">
                  {item.dose}
                </p>
      ...
      {/* ──────────── The premix trap, explained honestly ───────────────── */}
      <section className="mx-auto max-w-[1040px] px-4 py-16 sm:px-6 lg:py-24">
        ...
        <CeraReveal className="cera-card mt-10 overflow-hidden lg:mt-14">
          <div className="overflow-x-auto">
            <table className="hsc-table w-full border-collapse text-start">
              ...
              <tbody>
                {copy.reorder.rows.map(row => (
                  ...
                  >
                    ...
                    <td dir="ltr" className="hsc-figure whitespace-nowrap px-4 py-4 text-start align-top text-[14px] text-[var(--cera-muted)]">
                      {row.sheet}
                    </td>
                    <td dir="ltr" className="hsc-figure whitespace-nowrap px-4 py-4 text-start align-top text-[14px] text-[var(--cera-ink)]">
                      {row.actual}
                    </td>
                  </tr>
                ))}
        ...
        <CeraReveal>
          <p className="mt-7 text-[16px] leading-relaxed text-[var(--cera-body)]">{copy.reorder.body}</p>
        </CeraReveal>
      </section>

      {/* ───────────── Quality, then preservation and vegan note ────────── */}
      <section className="bg-white py-16 lg:py-24">
        <div className="mx-auto max-w-[1200px] px-4 sm:px-6">
          ...
          <CeraReveal delay={90} className="hsc-note mx-auto mt-8 max-w-[900px] p-6 md:p-7">
            <p className="cera-eyebrow">{copy.preservation.eyebrow}</p>
            <h3 className="cera-serif mt-2 text-[22px] leading-tight sm:text-[26px]">{copy.preservation.title}</h3>
            <p className="mt-3 text-[15px] leading-relaxed text-[var(--cera-body)]">{copy.preservation.body}</p>
          </CeraReveal>
        </div>
      </section>
