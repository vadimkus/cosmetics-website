...
/**
 * TVC Beauty Salon — paymentin @ shipment 06723 / invoice 04961.
 * 300 AED. Order GENCardM260821TVC → Доставлен.
 *
 *   node --import dotenv/config scripts/moysklad-create-tvc-paymentin-04961-20260821.js
 *   node --import dotenv/config scripts/moysklad-create-tvc-paymentin-04961-20260821.js --commit
 */
...
const MARKER = 'TVC-PAYMENTIN-04961-2026-08-21'

const PAYMENT = {
  amountMinor: 30000,
  invoiceName: '04961',
  invoiceId: 'f7fc8c22-9d68-11f1-0a80-0cc800334db6',
  shipmentName: '06723',
  shipmentId: 'f8ae6a08-9d68-11f1-0a80-09020032075e',
  orderName: 'GENCardM260821TVC',
  orderId: 'f7b8fb32-9d68-11f1-0a80-0e3800336f7d',
}
...
async function main() {
  console.log('====================================================================')
  console.log('  TVC — paymentin @ 04961 / 06723')
  console.log('====================================================================')
  ...
  if (!/t\s*v\s*c/i.test(order.agent?.name || '')) throw new Error(`Unexpected agent: ${order.agent?.name}`)
  ...
  const created = await api('POST', '/entity/paymentin', {
    ...
    description: [
      ...
      'Clinic order paid — Hydro Cool 1kg @300, delivery free.',
    ].join(' | '),
    operations: [
      {
        meta: {
