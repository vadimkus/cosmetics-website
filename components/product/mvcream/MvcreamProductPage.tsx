...
import { getMvcreamCopy } from './mvcreamCopy'
...
const PROOF_IMAGE = '/images/radiance/s3.jpeg'
...
export default function MvcreamProductPage({
  product,
  unitsSold = 0,
  routineProducts = [],
}: Props) {
  ...
  const isRtl = dir === 'rtl'
  const copy = getMvcreamCopy(locale)

  // Two tubes, 50g and 250g, so a size travels with every cart call.
  const sizeOptions = useMemo(() => getProductSizeOptions('31', product), [product])
  ...
  const addToCart = useCallback(
    async (qty: number, size: string = selectedSize) => {
      ...
      try {
        ...
        errorLog('MvCream: add to cart failed', error)
        throw error
      }
    },
  ...
  const handleAddRoutineProduct = useCallback(
    async (item: Product, selection?: ProductOptionSelection, quantity = 1) => {
      ...
      try {
        ...
        errorLog('MvCream: routine add to cart failed', error)
      } finally {
  ...
  return (
    <div className={`cera-page mvcream-page ${ceraSerif.variable} min-h-[100dvh]`} dir={dir}>
      {/* ───────────────────────────── Hero ─────────────────────────────── */}
        ...
        <div>
          ...
          {product.videoUrl ? (
            <CeraReveal className="mt-10">
              ...
              <div className="mvcream-video relative mt-4 mx-auto aspect-[9/16] w-full max-w-[340px] overflow-hidden rounded-[28px]">
                <video
      ...
      <section className="mx-auto max-w-[1000px] px-4 py-16 sm:px-6 lg:py-24">
        ...
        <div className="mt-10 grid grid-cols-1 gap-4 lg:mt-14 lg:grid-cols-2 lg:gap-6">
          ...
          <CeraReveal delay={90} className="cera-card mvcream-not p-6 lg:p-7">
            <h3 className="cera-serif text-[21px] leading-tight text-[var(--cera-ink)]">
