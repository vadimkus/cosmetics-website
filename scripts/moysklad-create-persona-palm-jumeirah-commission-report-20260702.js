...
/**
 * Persona Palm Jumeirah — consignment sales report only (agreement 00078).
 * No demand/shipment.
 *
 *   node --import dotenv/config scripts/moysklad-create-persona-palm-jumeirah-commission-report-20260702.js
 *   node --import dotenv/config scripts/moysklad-create-persona-palm-jumeirah-commission-report-20260702.js --commit
 */
...
const MARKER = `Persona Palm Jumeirah consignment sold June 2026 ${uaeToday()}`

/** [code, qty, label] */
const LINES = [
  ['00063', 6, 'Intensive Repair Collagen Mask 23g'],
  ['00051', 3, 'HR³ Matrix Hair Tonic 70ml'],
  ['00052', 2, 'HR³ Matrix Scalp & Hair Shampoo 300ml'],
  ['54457', 2, 'Ultra Shield Sun Cream SPF50/PA++++ 50g'],
  ['00012', 6, 'Peptide Gel Mask 39g'],
  ['00140', 6, 'Soothing Bomb Sea Algae Mask 23g'],
...
async function exportCommissionSalesPdf(reportId, reportName) {
  ...
  const out = path.join(ORDERS_DIR, `GENOSYS_Persona_Palm_Consignment_Sales_${reportName}.pdf`)
...
async function main() {
  console.log('====================================================================')
  console.log('  Persona Palm Jumeirah — consignment sales report only (00078)')
  console.log('====================================================================')
  ...
  const report = await api('POST', '/entity/commissionreportin', {
    ...
    description: [
      ...
      'June 2026 consignment sold list (14 lines, 37 pcs).',
    ].join('\n'),
