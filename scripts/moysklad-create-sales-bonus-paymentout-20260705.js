...
/**
 * Sales bonus payout to Vadim Sagatdinov — Wio 1,339.40 AED (2026-07-05).
 * Ref: LN82276115992176
 *
 *   node --import dotenv/config scripts/moysklad-create-sales-bonus-paymentout-20260705.js
 *   node --import dotenv/config scripts/moysklad-create-sales-bonus-paymentout-20260705.js --commit
 */
...
const SUM_MINOR = 133940 // 1,339.40 AED
const WIO_REF = 'LN82276115992176'
const MARKER = `SALES-BONUS-VADIM-${WIO_REF}-${uaeToday()}`
...
async function ensureNoDuplicate() {
  const search = await api('GET', `/entity/paymentout?search=${encodeURIComponent(WIO_REF)}&limit=5`)
  if (search.rows?.length) {
    throw new Error(`Duplicate Wio ref: ${search.rows[0].name} (${search.rows[0].id})`)
  }
...
async function main() {
  console.log('====================================================================')
  console.log('  Sales bonus → Vadim Sagatdinov (Wio payout)')
  console.log('====================================================================')
  console.log(`  Mode: ${COMMIT ? 'COMMIT' : 'DRY RUN'}`)
  console.log(`  Sum : ${money(SUM_MINOR)} AED`)
  console.log(`  Ref : ${WIO_REF}`)
  ...
  const created = await api('POST', '/entity/paymentout', {
    ...
    sum: SUM_MINOR,
    paymentPurpose: `Sales bonus ${money(SUM_MINOR)} AED — Wio ${WIO_REF} [${MARKER}]`,
    description: `Sales bonus>>July 2026 | Wio local transfer ${WIO_REF} debited ${uaeToday()} | ${MARKER}`,
  })
