...
/**
 * Bespoke product page for MULTI SUN CREAM [SPF40 / PA++] (product 40).
 *
...
export default function MultiSunProductPage({ product, unitsSold = 0, routineProducts = [] }: Props) {
  ...
  const isRtl = dir === 'rtl'
  const copy = getMultiSunCopy(locale)
  ...
  const addToCart = useCallback(
    async (qty: number) => {
      ...
      try {
        ...
        errorLog('MultiSun: add to cart failed', error)
        throw error
      }
    },
  ...
  return (
    <div className={`cera-page msc-page ${ceraSerif.variable} min-h-[100dvh]`} dir={dir}>
      {/* ───────────────────────────── Hero ─────────────────────────────── */}
                ...
                <button
                  ...
                  className={`inline-flex h-[54px] min-w-[220px] flex-1 items-center justify-center gap-2.5 rounded-full px-8 text-[15px] font-semibold tracking-wide transition-all duration-300 disabled:cursor-not-allowed disabled:opacity-45 ${
                    justAdded
                      ...
                      : 'bg-[var(--cera-cta)] text-white hover:bg-[var(--cera-rose-ink)] hover:shadow-[0_18px_38px_-20px_rgba(30,24,19,0.8)]'
                  }`}
                >
        ...
        <div className="grid grid-cols-1 gap-10 lg:grid-cols-[minmax(0,0.72fr)_minmax(0,1fr)] lg:gap-16">
          <div>
            <CeraReveal>
              ...
              <p className="mt-6 max-w-[46ch] text-[13.5px] leading-relaxed text-[var(--cera-muted)]">
                {copy.filters.total}
              </p>
            </CeraReveal>
          </div>
                  ...
                  <tr className="border-b border-[var(--cera-line)]">
                    {[copy.filters.columns.name, copy.filters.columns.amount, copy.filters.columns.role, copy.filters.columns.cap].map(head => (
                      <th
                        key={head}
                        scope="col"
                        className="px-3.5 py-3.5 text-start text-[11px] font-semibold uppercase tracking-[0.08em] text-[var(--cera-muted)]"
                      >
                        {head}
                      </th>
                    ))}
                  ...
                  {copy.filters.rows.map(row => (
                    <tr key={row.name} className="border-b border-[var(--cera-line)] last:border-b-0">
                      <th
                        scope="row"
                        dir="ltr"
                        className="px-3.5 py-4 text-start align-top text-[13px] font-medium leading-snug text-[var(--cera-ink)]"
                      >
                        {row.name}
                      </th>
                      <td dir="ltr" className="msc-figure whitespace-nowrap px-3.5 py-4 text-start align-top text-[13.5px] font-semibold text-[var(--cera-rose-ink)]">
                        {row.amount}
                      </td>
      ...
      <section className="bg-white py-16 lg:py-24">
        <div className="mx-auto max-w-[900px] px-4 sm:px-6">
          <CeraReveal>
            ...
            <p className="mt-5 text-[15.5px] leading-relaxed text-[var(--cera-body)]">{copy.grade.body}</p>
      ...
      <section className="mx-auto max-w-[900px] px-4 py-16 sm:px-6 lg:py-20">
        <CeraReveal className="msc-note p-6 md:p-9">
          ...
          <p className="mt-5 text-[15.5px] leading-relaxed text-[var(--cera-body)]">{copy.octinoxate.body}</p>
          ...
          <p className="mt-6 rounded-2xl bg-[var(--cera-blush)] p-5 text-[14.5px] font-medium leading-relaxed text-[var(--cera-rose-ink)]">
      ...
      <section className="bg-white py-16 lg:py-24">
        <div className="mx-auto max-w-[1000px] px-4 sm:px-6">
          ...
          <CeraReveal>
            <p className="mt-5 flex items-start gap-2.5 text-[13.5px] leading-relaxed text-[var(--cera-muted)]">
      ...
      <section className="mx-auto max-w-[900px] px-4 py-16 sm:px-6 lg:py-20">
        ...
        <CeraReveal delay={90} className="msc-note mt-6 p-6 md:p-9">
          ...
          <p className="mt-4 text-[14.5px] leading-relaxed text-[var(--cera-body)]">{copy.fragrance.body}</p>
      ...
      <section className="bg-white py-16 lg:py-24">
        <div className="mx-auto max-w-[1000px] px-4 sm:px-6">
          <CeraSectionHeader eyebrow={copy.pick.eyebrow} title={copy.pick.title} intro={copy.pick.intro} />
          <div className="mt-10 grid grid-cols-1 gap-5 lg:mt-14 lg:grid-cols-2">
            {[copy.pick.thisOne, copy.pick.otherOne].map((col, idx) => (
              <CeraReveal
                key={col.title}
                delay={idx * 90}
                as="article"
                className={`cera-card p-6 lg:p-7 ${idx === 1 ? 'msc-pick--other' : ''}`}
              >
                ...
                <ul className="mt-4 space-y-2.5">
                  {col.items.map(item => (
                    <li key={item} className={`flex gap-3 ${isRtl ? 'flex-row-reverse text-right' : ''}`}>
                      <Check className="mt-[3px] h-[15px] w-[15px] flex-none text-[var(--cera-rose)]" strokeWidth={2.5} aria-hidden="true" />
                ...
                <tbody>
                  {copy.lab.rows.map(row => (
                    <tr key={row.label} className="border-b border-[var(--cera-line)] last:border-b-0">
                      <th
                        scope="row"
                        className="w-[36%] px-5 py-4 text-start align-top text-[12.5px] font-semibold uppercase tracking-[0.07em] text-[var(--cera-muted)]"
                      >
                        {row.label}
                      </th>
