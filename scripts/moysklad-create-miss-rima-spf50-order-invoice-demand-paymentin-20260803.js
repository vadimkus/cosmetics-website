...
/**
 * Miss Rima — paid retail: Ultra Shield SPF50 ×1 @ 250 + delivery Dubai @ 45.
 * SO → invoice → shipment → paymentin. Invoice PDF → ~/Desktop/orders/
 *
 *   Phone: +971-55-253-9911
 *   Ship: Al Jadaf, Azizi Farhad, app 103, Dubai
 *
 *   node --import dotenv/config scripts/moysklad-create-miss-rima-spf50-order-invoice-demand-paymentin-20260803.js
 *   node --import dotenv/config scripts/moysklad-create-miss-rima-spf50-order-invoice-demand-paymentin-20260803.js --commit
 */
...
const AGENT_ID = '7fd3cebd-023d-11f1-0a80-098500017887' // Miss Rima

const CUSTOMER = {
  name: 'Miss Rima',
  phone: '+971-55-253-9911',
  city: 'Dubai',
  street: 'Al Jadaf, Azizi Farhad, app 103',
}

const ORDER = {
  name: `GENCardM${uaeShortDate()}9911`,
  moment: uaeMomentNow(),
  marker: `MISS-RIMA-SPF50-PAID-${uaeToday()}`,
}

/** [code, qty, retailAed, discountPct] */
const PRODUCT_LINES = [['54457', 1, 250, 0]]
...
async function exportInvoicePdf(invoiceId, invoiceName) {
  ...
  const outPath = path.join(ORDERS_DIR, `GENOSYS_Miss_Rima_${safe}.pdf`)
...
async function main() {
  console.log('====================================================================')
  console.log('  Miss Rima — SPF50 + delivery paid')
  console.log('====================================================================')
  ...
  for (const p of positions) {
    if (p._name) {
      console.log(`    54457 ${p._name.slice(0, 50)} x1 @ 250 (avail ${p._avail})`)
    } else {
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      '54457 Ultra Shield SPF50 x1 @250 retail; delivery Dubai 45.',
