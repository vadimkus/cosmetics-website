 ...
 *
 * Lines:
 *   00144  Cushion #2 Beige ×4
 *   54475  BIO-MESO PDRN Homecare Ampoule 5000 ×2
 *   54484  CERABARRIER Biome Gel Cleanser 200ml ×2
 *
 *   node --import dotenv/config scripts/moysklad-create-melanta-replenishment-demand-20260703.js
 *   node --import dotenv/config scripts/moysklad-create-melanta-replenishment-demand-20260703.js --commit
 */
...
const MARKER = `Melanta replenishment ${uaeToday()} beige/meso/cerrabarrier`

const LINES = [
  ['00144', 4, 'Skin Caring Blemish Balm Cushion #2 Beige'],
...
async function main() {
  ...
  const demand = await api('POST', '/entity/demand', {
    ...
    description: [
      ...
      'Contract: 14',
      'Replenishment: Cushion Beige x4, Bio-Meso Homecare 5000 x2, CERABARRIER 200ml x2.',
    ].join('\n'),
  ...
  if (process.platform === 'darwin') {
    try {
      execFileSync('open', [pdf.out], { stdio: 'inherit' })
    } catch {
      /* ignore */
    }
  }
