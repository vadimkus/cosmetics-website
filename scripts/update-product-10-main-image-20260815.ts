/**
 * Point product 10 at the cleaned Snow O₂ packshot.
 *
 * Live /images/cleanser/Main.jpg has generator garbage on both labels and is
 * served immutable for a year, so the clean shot ships as main_clean.jpeg.
 * The gallery stays S1–S6 exactly. Main.jpg stays on disk for emails already
 * sent; new orders pick up the new path from the product row.
 *
 * The database is shared with production, so this script HEADs the live asset
 * and refuses to write while it 404s.
 *
 *   npx tsx --env-file=.env.local scripts/update-product-10-main-image-20260815.ts
 *   npx tsx --env-file=.env.local scripts/update-product-10-main-image-20260815.ts --apply
 */
import { prisma } from '../lib/prisma'

const PRODUCT_ID = '10'
const OLD_MAIN = '/images/cleanser/Main.jpg'
const NEW_MAIN = '/images/cleanser/main_clean.jpeg'
...
async function main() {
  ...
  if (product.image !== OLD_MAIN && product.image !== NEW_MAIN) {
    throw new Error(`unexpected product 10 main image: ${product.image}`)
  }
  ...
  if (updated.images !== galleryBefore) {
    throw new Error('product 10 gallery changed unexpectedly')
  }
