...
/**
 * Olya Shipilova — new customer — full retail cycle, paid:
 *   order → invoice → shipment → paymentin → retail PDF
 *
 *   00013 Hydro Cool Modeling Mask 1kg x1 @ 300 AED (retail)
 *   Excellent Delivery Dubai x1 @ 45 AED
 *   Total: 345 AED
 *
 *   Ship: Seline Clinic Dubai, 623 Jumeira St, Umm Suqeim 1, Dubai
 *
 *   node --import dotenv/config scripts/moysklad-create-olya-shipilova-hydrocool-order-invoice-demand-paymentin-20260707.js
 *   node --import dotenv/config scripts/moysklad-create-olya-shipilova-hydrocool-order-invoice-demand-paymentin-20260707.js --commit
 */
...
const CUSTOMER = {
  ...
  phone: '+971523946737',
  city: 'Dubai',
  street: 'Seline Clinic, 623 Jumeira St, Umm Suqeim 1',
}

const ORDER = {
  name: `GENCardM${uaeShortDate()}6737`,
  moment: uaeMomentNow(),
  marker: `OLYA-SHIPILOVA-HYDROCOOL-RETAIL-${uaeToday()}`,
}
...
async function exportInvoicePdf(invoiceId, invoiceName) {
  ...
  const outPath = path.join(ordersDir, `GENOSYS_Olya_Shipilova_${safe}.pdf`)
...
async function main() {
  console.log('====================================================================')
  console.log('  Olya Shipilova — SO → invoice → shipment → paymentin (paid)')
  console.log('====================================================================')
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      'Hydro Cool Modeling Mask 00013 x1 @ 300 retail; delivery Dubai 45 AED. Paid — paymentin posted.',
