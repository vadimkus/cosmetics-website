...
export const LEGACY_HAIRGEN_BOOSTER_AR: HairGenBoosterCopy = {
  ...
  faq: {
    ...
    items: [
      ...
      {
        q: 'هل يؤلم؟',
        a: 'تصفه الشركة بإحساس تدليك لا وخز، بلا ألم أثناء الاستعمال. خذي ذلك على أنه كلام الشركة. وما سيُسجَّل أقوى هو مقشّر الفروة إن استعملته أولاً — فهو ثلثه كحول مُمَوَّه مع 0.9% منثول على بشرة أُزيل دهنها للتوّ.',
      },
      {
...
export const LEGACY_HAIRGEN_BOOSTER_RU: HairGenBoosterCopy = {
  ...
  faq: {
    ...
    items: [
      ...
      {
        q: 'Это больно?',
        a: 'Производитель описывает это как ощущение массажа, а не укола, без боли во время процедуры. Воспринимайте это как слова производителя. Сильнее отзовётся пилинг для кожи головы, если вы используете его перед этим: он на треть денатурированный спирт с 0,9% ментола и попадает на только что обезжиренную кожу.',
      },
      {
