...
/**
 * Miss Alena Zhakhovlska — paid retail order.
 *
 *   00022 Snow Booster Toner 200ml ×1 @ 260 −15%
 *   00040 Intensive Blemish Balm Cream 50g ×1 @ 250 −15%
 *   Excellent Delivery Dubai ×1 @ 45, no discount
 *   Total: 478.50 AED
 *
 *   Ship: Platinum Residence 1, DSO, apartment 411, Dubai
 *   Already paid → paymentin linked to shipment
 *
 *   node --import dotenv/config scripts/moysklad-create-miss-alena-zhakhovlska-order-invoice-demand-paymentin-20260811.js
...
const AGENT_ID = 'debfa6ef-1595-11ed-0a80-010b001b4dc3' // Miss Alena Zhahlovska

const CUSTOMER = {
  name: 'Miss Alena Zhakhovlska',
  phone: '+971582830294',
  city: 'Dubai',
  street: 'Platinum Residence 1, DSO, apartment 411',
}

const ORDER = {
  name: `GENCardM${uaeShortDate()}0294`,
  moment: uaeMomentNow(),
  marker: `MISS-ALENA-ZHAKHOVLSKA-PAID-${uaeToday()}`,
}
...
// 260*0.85 + 250*0.85 + 45 = 221 + 212.50 + 45 = 478.50
const EXPECTED_SUM_MINOR = 47850
...
async function ensureCustomer() {
  ...
  console.log(`  Customer was: ${agent.name} | ${agent.phone || CUSTOMER.phone}`)
  console.log(`  Old address: ${agent.actualAddress || '(none)'}`)
  if (COMMIT) {
    ...
    console.log(`  Updated → ${CUSTOMER.name} | ${CUSTOMER.street}, Dubai`)
  }
  return agent
}
...
async function exportInvoicePdf(invoiceId, invoiceName) {
  ...
  const outPath = path.join(ORDERS_DIR, `GENOSYS_Miss_Alena_Zhakhovlska_${safe}.pdf`)
...
async function main() {
  console.log('====================================================================')
  console.log('  Miss Alena Zhakhovlska — Snow Booster + BB cream (paid)')
  console.log('====================================================================')
  ...
  for (const p of positions) {
    if (p._name) {
      const net = (p._retail * (100 - p._disc)) / 100
      console.log(
        `    ${p._code} ${p._name.slice(0, 48)} x1 @ ${p._retail} −${p._disc}% = ${net.toFixed(2)} (avail ${p._avail})`,
      )
    } else {
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      '00022 Snow Booster 200ml x1 @260 −15%; 00040 Intensive BB Cream 50g x1 @250 −15%; delivery Dubai 45 no discount.',
      `Ship: ${CUSTOMER.street}, Dubai. Phone ${CUSTOMER.phone}. Paid.`,
    ].join(' | '),
