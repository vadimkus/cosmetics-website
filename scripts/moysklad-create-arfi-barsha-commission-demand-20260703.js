 ...
 * June 2026 sold list (26 pcs / 15 lines) from clinic table 2026-07-03.
 *
 *   node --import dotenv/config scripts/moysklad-create-arfi-barsha-commission-demand-20260703.js
 *   node --import dotenv/config scripts/moysklad-create-arfi-barsha-commission-demand-20260703.js --commit
 */
...
const AGENT_ID = '39a1aa83-a5a6-11f0-0a80-1cbc00050fea' // ARFI NAILS BEAUTY SALON
...
const MARKER = `ARFI Nails Barsha consignment sales June 2026 ${uaeToday()}`
...
const LINES = [
  ...
  ['54461', 3, 'Skin Defender Lip & Eye Makeup Remover 200ml'],
  ['00053', 5, 'EyeCell Eye Peptide Gel Patch (box)'],
]

const EXPECTED_UNITS = 26
...
async function ensureNoDuplicate(entityType, marker) {
  ...
  const dup = docs.find((d) => (d.description || '').includes('ARFI Nails Barsha consignment sales June 2026'))
...
async function main() {
  console.log('====================================================================')
  console.log('  ARFI Nails Barsha — report + demand (agreement 25, June 2026)')
  console.log('====================================================================')
  ...
  const report = await api('POST', '/entity/commissionreportin', {
    ...
    description: [
      MARKER,
      'ARFI NAILS BEAUTY SALON (Barsha) | Agreement 25 | June 2026 sold (26 pcs).',
      'Eye patch = 00053 box. Collagen / sea algae masks = 00063 23g.',
    ].join('\n'),
