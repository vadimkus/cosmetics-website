...
/**
 * Clinic customer GENCardM260708CLINIC — delivery 30 → 15 AED.
 *   Total 180 → 165; paymentin adjusted; invoice PDF re-exported.
 *
 *   node --import dotenv/config scripts/moysklad-fix-clinic-customer-delivery-15-04788-20260708.js
 *   node --import dotenv/config scripts/moysklad-fix-clinic-customer-delivery-15-04788-20260708.js --commit
 */
...
const ORDER_ID = 'af575d9a-7adb-11f1-0a80-0eda001ff442'
const INVOICE_ID = 'af93e86c-7adb-11f1-0a80-0da50020e5a9'
const DEMAND_ID = 'b03efbf0-7adb-11f1-0a80-1e2a00208d05'
const PAYMENT_ID = 'b09d3c03-7adb-11f1-0a80-08e8001fbbb9'
...
const OLD_DELIVERY_MINOR = 3000
const NEW_DELIVERY_MINOR = 1500
const EXPECTED_SUM_MINOR = 16500
...
function findDeliveryPosition(rows) {
  return rows.find((p) => {
    const href = p.assortment?.meta?.href || ''
    return href.includes(DELIVERY_SERVICE_ID) || p.assortment?.name?.includes('Delivery')
  })
}
...
async function patchDoc(entityType, docId, label) {
  ...
  const pos = findDeliveryPosition(rows)
  if (!pos) throw new Error(`${label}: delivery line not found`)
  if (pos.price === NEW_DELIVERY_MINOR) {
    console.log(`  ${label}: delivery already ${money(NEW_DELIVERY_MINOR)} AED`)
    return
  }
  if (pos.price !== OLD_DELIVERY_MINOR) {
    throw new Error(`${label}: unexpected delivery price ${money(pos.price)}`)
  }
  console.log(`  ${label}: delivery ${money(pos.price)} → ${money(NEW_DELIVERY_MINOR)} AED`)
  if (COMMIT) {
    await api('PUT', `/entity/${entityType}/${docId}/positions/${pos.id}`, positionPayload(pos, NEW_DELIVERY_MINOR))
  }
}
...
async function main() {
  console.log('====================================================================')
  console.log('  Clinic customer — delivery 30 → 15 AED (04788)')
  console.log('====================================================================')
  console.log(`  Mode: ${COMMIT ? 'COMMIT' : 'DRY RUN'}`)
  console.log(`  Delivery: ${money(OLD_DELIVERY_MINOR)} → ${money(NEW_DELIVERY_MINOR)} | total ${money(EXPECTED_SUM_MINOR)} AED`)
