...
/**
 * Love My Body (LOVE MY BODY LADIES SPA CLUB L.L.C) — agreement 27:
 *
 * 1. Received commissioner report (receivedCommission)
 * 2. Matching shipment (demand) under the same contract with the same lines
 *
 * Line mapping (user wording → MoySklad):
 * - Biege cushion x2        → 00144 Skin Caring Blemish Balm Cushion #2 Biege
 * - Green mask x2           → 00140 Soothing Bomb Sea Algae Mask 23g (not CO₂ box)
 * - Peptide mask 13 pcs     → 00012 Peptide Gel Mask 39g (pieces, not full 5-pc kits)
 * - Snow O2 180ml x1        → 00021 Snow O₂ Cleanser 180ml
 * - Serum for eye x1        → 00054 EyeCell Eye Contour Serum 10ml
 * - Mask PDRN x1            → 54467 Skin Reboot PDRN mask Pack
 * - SPF 40 x2               → 00041 Multi Sun Cream SPF40/PA++ 40g
 * - Hyaluron serum x1       → 00195 Moisture Replenishing Hyaluron Serum 30ml
 * - Collagen mask x1        → 00063 Intensive Repair Collagen Mask 23g
 *
 * Dry-run:
 *   set -a; source .env; set +a
 *   node scripts/moysklad-create-love-my-body-sales-20260508.js
 *
 * Commit:
 *   node scripts/moysklad-create-love-my-body-sales-20260508.js --commit
 */
...
const REPORT = {
  moment: '2026-05-08 14:00:00',
  stateNotPaidId: '3203736c-c43b-11eb-0a80-093a002b59a6',
  marker: 'Love My Body consignment sales 2026-05-08 Sea Algae mask',
}

const DEMAND = {
  moment: '2026-05-08 14:05:00',
  stateShippedId: '50d70717-4582-11ea-0a80-05e3001273a2',
  marker: 'Love My Body shipment 2026-05-08 Sea Algae mask',
}
...
async function createReport(resolved) {
  const payload = {
    ...
    description: [
      ...
      'Agreement: 27',
      'Peptide line: 13 pcs mask sheets (00012), not full kits.',
      'Green mask: Soothing Bomb Sea Algae Mask 23g (00140), not CO₂.',
    ].join('\n'),
...
async function createDemand(resolved, report) {
  const payload = {
    ...
    description: [
      ...
      'Same items and quantities as the received commissioner report.',
    ].join('\n'),
...
async function main() {
  console.log('====================================================================')
  console.log('  MoySklad Love My Body — commissioner report + shipment (agr. 27)')
  console.log('====================================================================')
