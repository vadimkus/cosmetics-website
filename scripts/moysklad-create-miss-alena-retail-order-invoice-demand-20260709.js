...
/**
 * Miss Alena — retail SO → invoice → shipment + PDF.
 *
 *   00144 Cushion #2 Beige ×1 @ 300 retail −15%
 *   00021 Snow O₂ Cleanser 180ml ×1 @ 330 −15%
 *   54458 Hyaluron Cream 50g ×1 @ 290 −15%
 *   00195 Hyaluron Serum 30ml ×1 @ 330 −15%
 *   Excellent Delivery Dubai ×1 @ 45, 100% off
 *   Total: 1,062.50 AED
 *
 *   Ship: DIFC, Damac Park Towers A, apt 908
 *
 *   node --import dotenv/config scripts/moysklad-create-miss-alena-retail-order-invoice-demand-20260709.js
...
const CUSTOMER = {
  ...
  phone: '+971561110511',
  city: 'Dubai',
  street: 'DIFC, Damac Park Towers A, apt 908',
}

const ORDER = {
  name: `GENCardM${uaeShortDate()}0511`,
  moment: uaeMomentNow(),
  marker: `MISS-ALENA-RETAIL-${uaeToday()}`,
}
...
const EXPECTED_SUM_MINOR = 106250
...
async function main() {
  console.log('====================================================================')
  console.log('  Miss Alena — SO → invoice → shipment (retail −15%)')
  console.log('====================================================================')
  ...
  for (const [code, qty, retailAed, discountPct] of PRODUCT_LINES) {
    ...
    console.log(
      `    ${code} ${item.name.slice(0, 48)} x${qty} @ ${retailAed.toFixed(2)} −${discountPct}% → ${net.toFixed(2)} AED`
    )
  }
  console.log(`    Excellent Delivery Dubai x1 @ ${DELIVERY_AED.toFixed(2)} AED, ${DELIVERY_DISCOUNT}% off`)
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      '00144 cushion beige, 00021 Snow O2 180ml, 54458 hyaluron cream 50g, 00195 hyaluron serum @ retail −15%; delivery free.',
