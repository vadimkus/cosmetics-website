/**
 * Send Olha Kravchuk the customer emails for manually imported order
 * MSK-OLHA-110726 (see scripts/import-olha-order-20260711.js):
 *   1. Order delivered / thank-you email (with items + total)
 *   2. GENOSYS Rewards points-earned email (+558 pts, balance 1,003)
 *
 * Run: npx tsx --env-file=.env.local scripts/send-olha-order-emails-20260711.ts
 */
...
async function main() {
  const order = await prisma.order.findUnique({
    where: { orderNumber: 'MSK-OLHA-110726' },
    include: { items: true },
  })
