...
/**
 * Persona Palm Jumeirah — consignment Отгрузка matching report 01365 + 10 peptide masks.
 *
 *   node --import dotenv/config scripts/moysklad-create-persona-palm-jumeirah-consignment-demand-20260601.js
 *   node --import dotenv/config scripts/moysklad-create-persona-palm-jumeirah-consignment-demand-20260601.js --commit
 */
...
const DEMAND = {
  ...
  moment: uaeMomentNow(),
  date: uaeToday(),
  marker: `Persona Palm Jumeirah shipment report 01365 + peptide mask ${uaeToday()}`,
}

/** Same as commission report 01365 + replenishment peptide masks */
const LINES = [
  ['00063', 11],
  ['00140', 10],
  ['00129', 1],
  ...
  ['54467', 1],
  ['00022', 2],
  ['00012', 10], // extra replenishment — peptide gel mask 39g
]
...
async function main() {
  console.log('====================================================================')
  console.log('  Persona Palm Jumeirah — отгрузка (00078) + 10 peptide masks')
  console.log('====================================================================')
  ...
  console.log(`  Total: ${money(totalMinor)} AED | ${resolved.reduce((s, l) => s + l.qty, 0)} units | ${resolved.length} lines`)
  console.log(`  (Report 01365 total 7,608 + peptide mask 10×38 = 380 → expected ${money(760800 + 38000)})`)
  ...
  const created = await api('POST', '/entity/demand', {
    ...
    description: [
      ...
      'Matches commissioner report 01365 (7,608 AED) + replenishment Peptide Gel Mask 00012 ×10.',
    ].join('\n'),
