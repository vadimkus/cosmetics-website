...
/**
 * Bianco Beauty Salon SPA (Dubai Hills) — SO → invoice → shipment + legal tax PDF.
 *
 *   00129 EPI Turnover Boosting Peeling Gel 100g x1
 *   00024 Snow O₂ Cleanser 500ml x1
 *   00067 Power Solution CVS x10 vials
 *   00071 Power Solution HES x10 vials
 *   00020 Power Solution SWS x10 vials
 *   00011 EZ CO₂ MASK Professional Box x5
 *   00013 Hydro Cool Modeling Mask 1kg x1
 *   00001 Standard Detachable Manual Roller 0.25mm x5
 *
 *   node --import dotenv/config scripts/moysklad-create-bianco-dubai-hills-order-invoice-demand-20260707.js
 *   node --import dotenv/config scripts/moysklad-create-bianco-dubai-hills-order-invoice-demand-20260707.js --commit
 */
...
const CONTRACT_ID = '83eaec1b-2946-11ef-0a80-08f00030f7f3' // Contract 00079
...
const ORDER = {
  name: `GENCardM${uaeShortDate()}8791`,
  moment: uaeMomentNow(),
  marker: `BIANCO-DUBAI-HILLS-PRO-ORDER-${uaeToday()}`,
}
...
function buildShipmentAddress(agent) {
  ...
  const street = addInfo || 'Dubai Hills, Dubai'
...
async function exportInvoicePdf(invoiceId, invoiceName) {
  ...
  const outPath = path.join(ordersDir, `GENOSYS_Bianco_Dubai_Hills_${safe}.pdf`)
...
async function main() {
  console.log('====================================================================')
  console.log('  Bianco Dubai Hills — SO + invoice + shipment')
  console.log('====================================================================')
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      'EPI peeling x1, Snow cleanser 500ml x1, CVS/HES/SWS x10 each, EZ CO2 x5, Hydro Cool 1kg x1, Roller 0.25 x5.',
    ].join(' | '),
