 ...
 * gallery, the sticky bar, the routine, the reviews and the spec table, so it
 * read as a different site to the product next to it.
 *
 * Where it diverges from its closest sibling, product 63 Revita Glow:
 *   - Three shades rather than two, and none defaulted. A base in the wrong
 *     depth is a return, so the CTA stays locked until the shopper picks.
 *   - A puff section. The applicator is quadruple-layered with a waterproof
 *     film and waterdrop tip, which is a real engineering detail and the one
 *     thing nothing else in the range has.
 *   - A wear section instead of a mechanism figure: the three fixing polymers
 *     are what get the finish through a Gulf day.
 *   - Laboratory specification instead of clinical percentages. No efficacy
 *     study exists for this product, so no percentage-improvement figure
 *     appears anywhere on the page.
 *
 * Every figure comes from bbCushionCopy.ts, which traces to the dossier audit.
 * Do not print "60% moisture essence", Volufiline as a volumiser, glutathione
 * as a tyrosinase blocker, the peptides as an engine, or a sixth UV filter.
 */
...
export default function BbCushionProductPage({
  product,
  unitsSold = 0,
  routineProducts = [],
}: Props) {
  ...
  const isRtl = dir === 'rtl'
  const copy = getBbCushionCopy(locale)

  const [quantity, setQuantity] = useState(1)
  const [isAdding, setIsAdding] = useState(false)
  const [justAdded, setJustAdded] = useState(false)

  // Three shades, none defaulted, for the same reason as product 63: the wrong
  // depth is a return rather than an inconvenience.
  const [shade, setShade] = useState<BbCushionShade['value'] | ''>('')
  ...
  /* The INCI is not its own column: it lives inside the ingredients JSON as an
     entry named "Full INCI", the same place every other bespoke page reads it
     from. An earlier draft looked for a `fullIngredients` field, which does not
     exist, and the section silently never rendered. */
  const fullInci = parsedIngredients.find(i => i.name === 'Full INCI')?.description?.trim() || null
  ...
  const routineSteps = useMemo(() => {
    const routine = PRODUCT_ROUTINES['41']
    if (!routine) return []
  ...
  // ── Cart actions ──────────────────────────────────────────────────────
  const selectShade = useCallback((value: BbCushionShade['value']) => {
    setShade(value)
  ...
  const addToCart = useCallback(
    async (qty: number) => {
      ...
      try {
        // The shade is a colour, not a size: the three share one price and one
        // pack size, so the size argument stays empty.
  ...
  const handleAddRoutineProduct = useCallback(
    async (item: Product, selection?: ProductOptionSelection, qty = 1) => {
      if (!user) {
        router.push(getLocalizedPath('/login', locale))
        return
      }
      ...
      try {
        await addItem(item, qty, selection?.selectedColor, selection?.selectedSize)
        try {
          trackAddToCart({
            id: item.id,
            name: item.name,
        ...
        errorLog('BbCushion: routine add to cart failed', error)
      } finally {
   ...
   *  away, so the routine can be filled without leaving the page. */
  ...
  const handleConfirmRoutineOptions = useCallback(
    async (item: Product, selection: ProductOptionSelection, qty: number) => {
      setOptionStep(null)
      await handleAddRoutineProduct(item, selection, qty)
    },
        ...
        {copy.shades.map(option => {
          ...
          return (
            <button
              ...
              className={`bbc-shade-button flex items-center gap-3.5 rounded-2xl border bg-white p-3.5 text-start ${
                selected
            ...
            >
              <span
                aria-hidden="true"
                className="bbc-swatch h-11 w-11 flex-none rounded-full"
  ...
  return (
    <div className={`cera-page bbc-page min-h-[100dvh]`} dir={dir}>
                ...
                <button
                  ...
                  className={`inline-flex h-[54px] min-w-[220px] flex-1 items-center justify-center gap-2.5 rounded-full px-8 text-[15px] font-semibold tracking-wide transition-all duration-300 disabled:cursor-not-allowed disabled:opacity-45 ${
                    justAdded
                      ...
                      : 'bg-[var(--cera-cta)] text-white hover:bg-[var(--cera-rose-ink)] hover:shadow-[0_18px_38px_-20px_rgba(26,22,19,0.8)]'
                  }`}
                >
      ...
      <section className="mx-auto max-w-[1200px] px-4 pb-16 sm:px-6 lg:pb-24">
        ...
        <ol className="mx-auto mt-10 grid max-w-[1040px] grid-cols-1 gap-4 lg:mt-14 lg:grid-cols-3 lg:gap-6">
          {copy.wear.steps.map((step, i) => (
            ...
            >
              <span className="cera-serif cera-numeral text-[26px] text-[var(--cera-rose)] sm:text-[30px]" aria-hidden="true">
                {step.step}
              </span>
        ...
        <CeraReveal delay={210}>
          <p className="mx-auto mt-8 max-w-[70ch] text-[13px] leading-relaxed text-[var(--cera-muted)]">
            {copy.wear.note}
          </p>
        </CeraReveal>
      </section>
          ...
          <div className="grid grid-cols-1 gap-10 lg:grid-cols-[minmax(0,0.85fr)_minmax(0,1fr)] lg:gap-16">
            ...
            <CeraReveal className="cera-card overflow-hidden">
              <div className="overflow-x-auto">
                <table className="bbc-table w-full border-collapse text-start">
                  ...
                  <tbody>
                    {copy.filters.rows.map(row => (
                      <tr key={row.name} className="border-b border-[var(--cera-line)] last:border-b-0">
                        ...
                        <td
                          dir="ltr"
                          className="whitespace-nowrap px-4 py-4 text-start align-top text-[14px] tabular-nums text-[var(--cera-rose-ink)]"
                        >
                          {row.amount}
                        </td>
      ...
      <section className="mx-auto max-w-[1200px] px-4 py-16 sm:px-6 lg:py-24">
          ...
          {copy.shades.map((option, i) => {
            ...
            return (
              <CeraReveal key={option.value} delay={i * 80} as="article" className="h-full">
                <div className={`cera-card flex h-full flex-col p-6 lg:p-7 ${selected ? 'border-[var(--cera-rose)]' : ''}`}>
                  <span
                    aria-hidden="true"
                  ...
                  <p className="mt-2.5 flex-1 text-[15px] leading-relaxed text-[var(--cera-body)]">{option.body}</p>
                  <button
                    type="button"
                    onClick={() => selectShade(option.value)}
        ...
        <CeraReveal>
          <p className="mx-auto mt-10 max-w-[70ch] border-s-2 border-[var(--cera-blush-deep)] ps-5 text-[15px] italic leading-relaxed text-[var(--cera-muted)]">
      ...
      <section className="bg-white py-16 lg:py-24">
        <div className="mx-auto max-w-[1200px] px-4 sm:px-6">
          <div className="grid grid-cols-1 gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(0,0.95fr)] lg:items-center lg:gap-16">
            <CeraReveal>
              <div className="relative aspect-square overflow-hidden rounded-[28px] border border-[var(--cera-line)] bg-white">
                <Image
                  src={PUFF_FIGURE}
                  alt={copy.puff.figureAlt}
                  fill
            ...
            <div>
              ...
              <ul className="mt-8 space-y-3">
                {copy.puff.points.map((point, i) => (
                  <CeraReveal key={point.title} as="li" delay={i * 80}>
                    <div className="cera-card p-5 lg:p-6">
          ...
          <div className="lg:sticky lg:top-24 lg:self-start">
            ...
            {product.videoUrl ? (
              <CeraReveal delay={90}>
                ...
                <p className="mt-3 text-[13px] leading-relaxed text-[var(--cera-muted)]">{copy.video.body}</p>
              </CeraReveal>
            ) : null}
          </div>
      ...
      {routineSteps.length > 0 && (
        <section className="mx-auto max-w-[1200px] px-4 py-16 sm:px-6 lg:py-24">
          ...
          />
          <ol className="mx-auto mt-10 grid max-w-[1080px] grid-cols-2 gap-3 lg:mt-14 lg:grid-cols-4 lg:gap-4">
            {routineSteps.map((step, i) => {
              ...
              const inBag = item
                ...
                : 0
              const itemPricing = item ? getPricingDisplay({ ...item, price: step.lowestPrice }, user) : null
              const href = step.productId ? getLocalizedPath(`/products/${step.productId}`, locale) : '#'
            ...
            <p className="min-w-0 max-w-[52%] shrink flex overflow-hidden items-center gap-1.5 text-[11px] text-[var(--cera-muted)] md:hidden">
              {selectedShade ? (
                <>
                  <span
                    aria-hidden="true"
                    className="bbc-swatch inline-block h-3 w-3 shrink-0 rounded-full"
