...
// Main analysis
console.log('🔍 Analyzing dependencies (refined)...\n');
...
for (const dep of allDependencies) {
  ...
  if (isUsed) {
    usedDeps.push(dep);
  } else if (essentialDeps.includes(dep)) {
...
console.log('\n⚠️  Potentially unused dependencies (safe to remove):');
...
console.log(`  Potentially unused: ${potentiallyUnusedDeps.length}`);

if (potentiallyUnusedDeps.length > 0) {
  console.log('\n💡 Consider removing unused dependencies to reduce bundle size:');
  console.log(`npm uninstall ${potentiallyUnusedDeps.join(' ')}`);
}
