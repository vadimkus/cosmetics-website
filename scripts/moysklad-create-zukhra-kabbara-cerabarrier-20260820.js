...
/**
 * Miss Zukhra Kabbara — clinic SO → invoice → shipment (unpaid).
 *
 *   54484 CERABARRIER Biome Gel Cleanser 200ml ×1 @ 190
 *   Delivery Dubai ×1 @ 45, 100% off
 *   Total: 190 AED
 *
 *   node --import dotenv/config scripts/moysklad-create-zukhra-kabbara-cerabarrier-20260820.js
 *   node --import dotenv/config scripts/moysklad-create-zukhra-kabbara-cerabarrier-20260820.js --commit
 */
...
const CUSTOMER = {
  name: 'Miss Zukhra Kabbara',
  phone: '+971 50 560 3505',
  city: 'Dubai',
  street: 'Al Salam Tower, office 36th floor, Metropolitan',
}

const ORDER = {
  name: `GENCardM${uaeShortDate()}3505`,
  moment: uaeMomentNow(),
  marker: `ZUKHRA-KABBARA-CERABARRIER-200-CLINIC-${uaeToday()}`,
}

const PRODUCT_LINES = [['54484', 1, 190]]
const DELIVERY_AED = 45
const EXPECTED_SUM_MINOR = 19000
...
function customerPayload() {
  return {
    ...
    description: `Clinic — ${CUSTOMER.street}, Dubai. Phone ${CUSTOMER.phone}.`,
  }
}

function isThisZukhra(r) {
  const digits = phoneDigits(r.phone)
  const name = (r.name || '').toLowerCase()
  return digits.endsWith('505603505') || name.includes('zukhra kabbara') || name.includes('zuhra kabbara')
}

async function findOrCreateCustomer() {
  for (const q of ['505603505', 'Zukhra Kabbara', 'Zuhra Kabbara', 'Kabbara']) {
    const d = await api('GET', `/entity/counterparty?search=${encodeURIComponent(q)}&limit=25`)
    const hit = (d.rows || []).find(isThisZukhra)
    if (hit) {
...
async function main() {
  console.log('====================================================================')
  console.log('  Miss Zukhra Kabbara — CERABARRIER 200ml clinic, delivery free')
  console.log('====================================================================')
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      '54484 CERABARRIER 200ml x1 @190; delivery free. Clinic list. Unpaid.',
