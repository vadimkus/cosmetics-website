...
/**
 * Bianco Spa FZCO (Cedre Center / shopping mall) — unpaid clinic SO → invoice → shipment.
 *
 *   00020 SWS vial ×10 @ 29
 *   00067 CVS vial ×10 @ 29
 *   00071 HES vial ×10 @ 29
 *   00015 SRS vial ×20 @ 40.5
 *   54467 Skin Reboot PDRN mask Pack ×1 @ 200
 *   Delivery Dubai ×1 @ 45, 100% off
 *   Total: 1,880 AED
 *
 *   node --import dotenv/config scripts/moysklad-create-bianco-cedre-ampoules-order-20260818.js
 *   node --import dotenv/config scripts/moysklad-create-bianco-cedre-ampoules-order-20260818.js --commit
 */
...
const ORDER = {
  name: `GENCardM${uaeShortDate()}CEDRE`,
  moment: uaeMomentNow(),
  marker: `BIANCO-CEDRE-SWS-CVS-HES-SRS-PDRN-${uaeToday()}`,
}
...
const EXPECTED_SUM_MINOR = 188000
...
function shipmentAddress(agent) {
  ...
  return {
    ...
    city: 'Dubai',
    street: 'Cedre Center, Dubai Silicon Oasis',
    addInfo: 'Cedre Center, Dubai Silicon Oasis, Dubai, United Arab Emirates',
  }
}
...
async function main() {
  console.log('====================================================================')
  console.log('  Bianco Cedre — SWS/CVS/HES/SRS/PDRN + free delivery')
  console.log('====================================================================')
  ...
  for (const p of positions) {
    console.log(
      `    ${p._code} ${String(p._name).slice(0, 55)} x${p.quantity} @ ${p._clinic}` +
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      'SWS x10, CVS x10, HES x10, SRS x20, PDRN mask pack x1. Delivery FREE.',
      'Clinic list. Invoice sale — not consignment. Unpaid.',
    ].join('\n'),
