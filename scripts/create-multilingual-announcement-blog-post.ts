...
async function createMultilingualAnnouncementBlogPost() {
  ...
  <div class="feature-section mt-8 text-center">
    ...
    const tags = ['announcement', 'multilingual', 'website-update', 'community', 'languages']

    // Try to check if post exists, but if it fails due to schema issues, just create
    ...
    try {
      ...
      debugLog('Could not check for existing post (schema may need sync), will attempt to create:')
      debugLog(`   Error: ${checkError.message}`)
    }
    ...
    try {
      ...
      debugLog('')
      debugLog('📝 Note: Translation fields (Arabic & Russian) are available in the schema.')
      debugLog('   To add translations, sync database with: npx prisma db push')
      debugLog('   Then update through admin interface at /admin')
      debugLog('')
      debugLog('📄 Translation content is ready in the script file for future use.')
    } catch (createError: any) {
      errorLog('❌ Failed to create/update blog post:', createError)
      errorLog(`   Error code: ${createError.code}`)
