...
/**
 * Miss Julia — clinic SO → invoice → shipment → paymentin + landscape print.
 * Ship to Five Cosmetic Poly Clinic, Jumeirah.
 *
 *   00011 EZ CO₂ Mask Professional Box ×1 @ 230 AED clinic
 *   00063 Collagen Mask 23g ×4 @ 18 AED clinic
 *   Excellent Delivery Dubai ×1 @ 45 AED
 *
 *   node --import dotenv/config scripts/moysklad-create-miss-julia-ez-mask-order-paymentin-20260712.js --commit
 */
...
const AGENT_ID = 'abe23bd1-8ef7-11f0-0a80-0828000b81c3' // Miss Julia

const SHIPMENT = {
  city: 'Dubai',
  street: 'Five Cosmetic Poly Clinic, 199 Jumeira Street, Villa 5, Jumeirah 1',
}

const ORDER = {
  name: `GENCardM${uaeShortDate()}0036`,
  moment: uaeMomentNow(),
  marker: `MISS-JULIA-EZ-MASK-COLLAGEN-FIVE-CLINIC-${uaeToday()}`,
}
...
const EXPECTED_SUM_MINOR = 34700
...
async function updateCustomerAddress(agent) {
  if (!COMMIT) {
    console.log('  DRY RUN: would update Miss Julia address → Five Cosmetic Poly Clinic, Jumeirah')
    return agent
  }
  const addr = shipmentAddress()
  const updated = await api('PUT', `/entity/counterparty/${agent.id}`, {
    meta: agent.meta,
    actualAddressFull: addr,
    legalAddressFull: addr,
    actualAddress: `${SHIPMENT.street}, ${SHIPMENT.city}`,
    description: [
      agent.description || '',
      `Delivery updated ${uaeToday()}: ship to Five Cosmetic Poly Clinic, Jumeirah.`,
    ]
      .filter(Boolean)
      .join(' | '),
  })
  console.log(`  Updated counterparty address: ${updated.actualAddress || SHIPMENT.street}`)
  return updated
}
...
function ordersPdfPath(invoiceName) {
  ...
  return path.join(ordersDir, `GENOSYS_Miss_Julia_${safe}.pdf`)
}

async function main() {
  console.log('====================================================================')
  console.log('  Miss Julia — EZ mask + collagen clinic order + paymentin')
  console.log('====================================================================')
  console.log(`  Mode: ${COMMIT ? 'COMMIT' : 'DRY RUN'}`)

  let agent = await api('GET', `/entity/counterparty/${AGENT_ID}`)
  console.log(`  Customer: ${agent.name} (${agent.phone || '—'})`)
  agent = await updateCustomerAddress(agent)
  ...
  for (const [code, qty, clinicAed] of PRODUCT_LINES) {
    const item = stock.get(code)
    console.log(`    ${code} ${item.name.slice(0, 50)} x${qty} @ ${clinicAed.toFixed(2)} clinic`)
  }
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      '00011 EZ mask box x1 @ 230; 00063 collagen mask x4 @ 18; delivery 45.',
      'Deliver: Five Cosmetic Poly Clinic, 199 Jumeira St, Villa 5, Jumeirah 1.',
      'Chain: invoice → shipment → paymentin.',
    ].join(' | '),
  ...
  const paymentIn = await api('POST', '/entity/paymentin', {
    ...
    description: `Incoming payment for ${demand.name} / ${ORDER.name} | ${ORDER.marker}`,
    sum: sumMinor,
    operations: [
      {
        meta: {
