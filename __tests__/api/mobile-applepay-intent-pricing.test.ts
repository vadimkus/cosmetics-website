import { POST } from '@/app/api/mobile/payments/applepay/intent/route'
...
jest.mock('@/lib/prisma', () => ({
  prisma: {
    order: {
      ...
      create: jest.fn(async () => ({ id: 'order-applepay-id' })),
      update: jest.fn(async () => ({ id: 'order-applepay-id' })),
    },
    orderItem: {
...
describe('mobile Apple Pay intent pricing', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  it('recomputes item pricing through server product data and ignores submitted client price', async () => {
    ...
    expect(body.meta.validatedTotals.discountAmount).toBe(40)
