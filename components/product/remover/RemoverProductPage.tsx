...
/**
 * Bespoke product page for SKIN DEFENDER LIP & EYE MAKEUP REMOVER
 * (product 11).
 *
 * Shares the editorial design system built for product 66 - the primitives,
 * the gallery and the structural CSS all come from ../cerabarrier - and
 * layers a cream / dark-gold palette on top via remover.css, taken from
 * the yellow oil layer and the carton.
 *
 * Section order:
 *
 *   effects  Shake · Hold · Wipe
 *   engine   the oil layer, ~50% solvents
 *   howTo    shake, cotton pad, hold, wipe, plus the product video
 ...
 *   routine  remover → cleanse → mist → cream
 *
 * There is a current DTS MG formula and carton, and no quantified clinical
 * trial, so this page has no proof chart. See removerCopy.ts for the
 * sourcing rules. In particular: do not rebuild the page around the
 * vitamins or the two peptides at cosmetic trace.
 */
...
import { getRemoverCopy } from './removerCopy'
...
/** Bottle packshot for the engine section. Gallery S1b-S6b still carry
 *  leftover slide copy (peptides, waterproof, no sting), so the engine
 *  figure stays on the main. */
...
export default function RemoverProductPage({
  product,
  unitsSold = 0,
  routineProducts = [],
}: Props) {
  ...
  const isRtl = dir === 'rtl'
  const copy = getRemoverCopy(locale)
  const Chevron = isRtl ? ChevronLeft : ChevronRight

  // Single SKU - one 200 ml bottle - so no size is ever passed to the cart.
  ...
  const routineSteps = useMemo(() => {
    const routine = PRODUCT_ROUTINES['11']
    if (!routine) return []
  ...
  const addToCart = useCallback(
    async (qty: number) => {
      ...
      try {
        ...
        errorLog('Remover: add to cart failed', error)
        throw error
      }
    },
  ...
  const handleAddRoutineProduct = useCallback(
    async (item: Product, selection?: ProductOptionSelection, quantity = 1) => {
      ...
      try {
        ...
        errorLog('Remover: routine add to cart failed', error)
      } finally {
  ...
  return (
    <div className={`cera-page dmr-page ${ceraSerif.variable} min-h-[100dvh]`} dir={dir}>
      {/* ───────────────────────────── Hero ─────────────────────────────── */}
        ...
        <div>
          ...
          {product.videoUrl ? (
            <CeraReveal className="mt-10">
              ...
              <div className="dmr-video relative mt-4 aspect-square overflow-hidden rounded-[28px] sm:aspect-video">
                <video
      ...
      <section className="mx-auto max-w-[1000px] px-4 py-16 sm:px-6 lg:py-24">
        ...
        <div className="mt-10 grid grid-cols-1 gap-4 lg:mt-14 lg:grid-cols-2 lg:gap-6">
          ...
          <CeraReveal delay={90} className="cera-card dmr-not p-6 lg:p-7">
            <h3 className="cera-serif text-[21px] leading-tight text-[var(--cera-ink)]">
