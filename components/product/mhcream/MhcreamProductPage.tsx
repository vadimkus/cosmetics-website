...
export default function MhcreamProductPage({
  product,
  unitsSold = 0,
  routineProducts = [],
}: Props) {
  ...
  const sizeOptions = useMemo(() => getProductSizeOptions('29', product), [product])
  ...
  const addToCart = useCallback(
    async (qty: number, size: string = selectedSize) => {
      ...
      try {
        ...
        errorLog('MhCream: add to cart failed', error)
        throw error
      }
    },
  ...
  const handleAddRoutineProduct = useCallback(
    async (item: Product, selection?: ProductOptionSelection, quantity = 1) => {
      ...
      try {
        ...
        errorLog('MhCream: routine add to cart failed', error)
      } finally {
    ...
    <div className={`cera-page mhcream-page ${ceraSerif.variable} min-h-[100dvh]`} dir={dir}>
        ...
        <div>
          ...
          {product.videoUrl ? (
            <CeraReveal className="mt-10">
              ...
              <div className="mhcream-video relative mt-4 aspect-square overflow-hidden rounded-[28px] sm:aspect-video">
                <video
      ...
      <section className="mx-auto max-w-[1000px] px-4 py-16 sm:px-6 lg:py-24">
        ...
        <div className="mt-10 grid grid-cols-1 gap-4 lg:mt-14 lg:grid-cols-2 lg:gap-6">
          ...
          <CeraReveal delay={90} className="cera-card mhcream-not p-6 lg:p-7">
            <h3 className="cera-serif text-[21px] leading-tight text-[var(--cera-ink)]">
