...
/**
 * BIANCO LAYAN BEAUTY SALON L.L.C — unpaid clinic SO → invoice → shipment.
 * Spreadsheet 25/08/2026; EZ qty overridden to 2 boxes (not 10).
 *
 *   54466 Bio-Ferment Age Defying Powder Mask 300g ×1 @ 125
 *   00011 EZ CO₂ MASK Professional Box ×2 @ 230
 *   00069 Power Solution CTS vial ×10 @ 29
 *   00071 Power Solution HES vial ×10 @ 29
 *   00015 SRS vial ×10 @ 40.5
 *   Total: 1,570.00 AED
 *
 *   node --import dotenv/config scripts/moysklad-create-bianco-layan-pro-order-20260825.js
 *   node --import dotenv/config scripts/moysklad-create-bianco-layan-pro-order-20260825.js --commit
 */
...
const ORDER = {
  name: `GENCardM${uaeShortDate()}2487`,
  moment: uaeMomentNow(),
  marker: `BIANCO-LAYAN-PRO-BIO-EZ-CTS-HES-SRS-${uaeToday()}`,
}
...
const EXPECTED_SUM_MINOR = 157000
...
function shipmentAddress() {
  return {
    ...
    city: 'Dubai',
    street: 'Layan Community, Wadi Al Safa 7, Dubai Land',
    addInfo: '',
  }
}
...
async function main() {
  console.log('====================================================================')
  console.log('  Bianco Layan — pro order 25 Aug (EZ ×2 boxes)')
  console.log('====================================================================')
  console.log(`  Mode: ${COMMIT ? 'COMMIT' : 'DRY RUN'}`)

  const agent = await api('GET', `/entity/counterparty/${AGENT_ID}`)
  console.log(`  Customer: ${agent.name}`)
  if (agent.name !== 'BIANCO LAYAN BEAUTY SALON L.L.C') throw new Error(`Unexpected agent: ${agent.name}`)
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      'Spreadsheet 25/08/2026. EZ ×2 boxes (not 10). Bio-Ferment ×1, CTS ×10, HES ×10, SRS ×10.',
      'Clinic list. Chain: invoice → shipment. Unpaid.',
    ].join(' | '),
  ...
  const outPath = path.join(ORDERS_DIR, `GENOSYS_Bianco_Layan_${String(invoice.name).replace(/[^\w.-]+/g, '_')}.pdf`)
