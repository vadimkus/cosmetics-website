...
/**
 * Miss Elmina Daminova — new customer + clinic-price order chain:
 *   HR³ Shampoo (00052) ×1 @ 170 AED clinic
 *   Excellent Delivery Dubai ×1 @ 35 AED
 *   order → invoice → shipment → paymentin (205 AED paid)
 *
 *   node --import dotenv/config scripts/moysklad-create-elmina-daminova-shampoo-order-paymentin-20260710.js --commit
 */
...
const CUSTOMER = {
  name: 'Miss Elmina Daminova',
  phone: '+971502201770',
  city: 'Dubai',
  street: 'Jumeirah Park, District 9, Villa S80',
}

const ORDER = {
  name: `GENCardM${uaeShortDate()}1770`,
  moment: uaeMomentNow(),
  marker: `ELMINA-DAMINOVA-SHAMPOO-CLINIC-${uaeToday()}`,
}

/** [code, qty, clinicAed VAT incl.] */
const PRODUCT_LINES = [['00052', 1, 170]]
const DELIVERY_AED = 35
...
async function findExistingCounterparty() {
  ...
  const bySearch = await api(
    'GET',
    `/entity/counterparty?search=${encodeURIComponent('502201770')}&limit=10`
  )
  return (bySearch?.rows || []).find((r) => (r.phone || '').includes('502201770')) || null
}
...
async function findOrCreateCounterparty() {
  ...
  const created = await api('POST', '/entity/counterparty', {
    ...
    description: `Clinic-price retail customer Jumeirah Park — created with order ${ORDER.name}`,
...
function ordersPdfPath(invoiceName) {
  ...
  return path.join(ordersDir, `GENOSYS_Elmina_Daminova_${safe}.pdf`)
}

async function main() {
  console.log('====================================================================')
  console.log('  Miss Elmina Daminova — shampoo clinic order + paymentin')
  console.log('====================================================================')
  ...
  for (const [code, qty, clinicAed] of PRODUCT_LINES) {
    const item = stock.get(code)
    console.log(`    ${code} ${item.name} x${qty} @ ${clinicAed.toFixed(2)} AED clinic`)
  }
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      'HR³ Shampoo 00052 x1 @ 170 AED clinic list; delivery 35 AED.',
      'Deliver: Jumeirah Park, District 9, Villa S80.',
      'Chain: invoice → shipment → paymentin (paid in full).',
    ].join(' | '),
