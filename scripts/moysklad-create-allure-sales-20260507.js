...
/**
 * Allure — commission report (sold items) + отгрузка (different replenishment lines).
 *
 * Counterparty: Allure (MoySklad name)
 * Agreement: 00045 (Commission)
 *
 * Dry-run:
 *   set -a; source .env; set +a
 *   node scripts/moysklad-create-allure-sales-20260507.js
 *
 * Commit:
 *   node scripts/moysklad-create-allure-sales-20260507.js --commit
 */
...
const REPORT = {
  moment: '2026-05-07 10:30:00',
  stateNotPaidId: '3203736c-c43b-11eb-0a80-093a002b59a6',
  marker: 'Allure consignment sales 2026-05-07',
}

const DEMAND = {
  moment: '2026-05-07 10:35:00',
  stateShippedId: '50d70717-4582-11ea-0a80-05e3001273a2',
  marker: 'Allure replenishment shipment 2026-05-07',
}

// Полученный отчет комиссионера — проданные позиции
const REPORT_LINES = [
  ['00144', 1], // Cushion #2 Biege
  ['00022', 4], // Snow Booster Toner 200ml
...
// Отгрузка — отдельный набор (пополнение)
const DEMAND_LINES = [
  ['00122', 4], // Multi-Vita Radiance Cream 50g
  ['00021', 2], // Snow O₂ Cleanser 180ml
...
function maxQtyByCode(lineGroups) {
  const m = new Map()
...
async function createReport(resolved) {
  const payload = {
    ...
    description: [
      REPORT.marker,
      'Customer: Allure',
      'Agreement: 00045 — sold items per user list.',
    ].join('\n'),
...
async function createDemand(resolved, report) {
  const payload = {
    ...
    description: [
      ...
      'Customer: Allure',
      'Agreement: 00045',
      `Replenishment отгрузка (not identical to report ${report.name}): Radiance cream 50g x4, Cleanser 180ml x2, Radiance serum x2.`,
    ].join('\n'),
...
async function main() {
  console.log('====================================================================')
  console.log('  MoySklad Allure — commission report + replenishment shipment')
  console.log('====================================================================')
  ...
  const maxNeeded = maxQtyByCode([REPORT_LINES, DEMAND_LINES])
  for (const [code, needQty] of maxNeeded) {
    const item = stock.get(code)
    if (!item) throw new Error(`Product code not found: ${code}`)
    if (item.available < needQty) {
      throw new Error(`Insufficient stock for ${code}: need ${needQty} (report+shipment), available ${item.available}`)
    }
  }

  const reportResolved = resolveLines(stock, REPORT_LINES, { checkStock: false })
  const demandResolved = resolveLines(stock, DEMAND_LINES, { checkStock: false })

  printLines('  Commission report (sold items)', reportResolved)
  printLines('  Shipment (replenishment only)', demandResolved)
  ...
  const report = await createReport(reportResolved)
  ...
  const demand = await createDemand(demandResolved, report)
