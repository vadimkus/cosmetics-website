...
/**
 * Yula Beauty Salon LLC — consignment sales report + demand (agreement 12).
 * EyeCell Eye Zone Care Kit (00059) ×1 @ clinic price.
 *
 *   node --import dotenv/config scripts/moysklad-create-yula-beauty-consignment-eye-kit-20260716.js
 *   node --import dotenv/config scripts/moysklad-create-yula-beauty-consignment-eye-kit-20260716.js --commit
 */
...
const MARKER = `Yula Beauty Salon consignment eye kit ${uaeToday()}`
const EXPECTED_SUM_MINOR = 49000

/** [code, qty, label] */
const LINES = [['00059', 1, 'EyeCell Eye Zone Care Kit (box)']]
...
async function main() {
  console.log('====================================================================')
  console.log('  Yula Beauty Salon LLC — consignment report + demand (eye kit)')
  console.log('====================================================================')
  ...
  const totalMinor = reportResolved.reduce((s, l) => s + l.price * l.qty, 0)
  ...
  const report = await api('POST', '/entity/commissionreportin', {
    ...
    commissionPeriodStart: `${moment.slice(0, 10)} 00:00:00`,
    commissionPeriodEnd: `${moment.slice(0, 10)} 23:59:59`,
    rewardType: 'PercentOfSales',
    rewardPercent: 0,
    description: [
      MARKER,
      'Yula Beauty Salon LLC | Agreement 12 | EyeCell Eye Zone Care Kit (00059) ×1 sold.',
      'Report + demand same line.',
    ].join('\n'),
  ...
  const demand = await api('POST', '/entity/demand', {
    ...
    description: [
      ...
      'Consignment replenishment — EyeCell kit 00059 ×1.',
    ].join('\n'),
  ...
  const salesOut = path.join(
    ORDERS_DIR,
    `GENOSYS_Yula_Beauty_Consignment_Sales_${report.name}.pdf`
  )
  const stockOut = path.join(
    ORDERS_DIR,
    `GENOSYS_Yula_Beauty_Consignment_Stock_Note_${demand.name}.pdf`
  )
