...
/**
 * SHINE MEDICAL CENTER LLC OPC (Dibba) — unpaid clinic SO → invoice → shipment.
 *
 *   00020 SWS vial ×20 @ 29
 *   00065 PCS vial ×10 @ 29
 *   00024 Snow O₂ Cleanser 500ml ×1 @ 255
 *   00183 Problem Control Toner 500ml ×1 @ 245
 *   00129 EPI Peeling Gel 100g ×1 @ 125
 *   Total: 1,495.00 AED
 *
 *   node --import dotenv/config scripts/moysklad-create-shine-dibba-sws-pcs-snow-20260822.js
 *   node --import dotenv/config scripts/moysklad-create-shine-dibba-sws-pcs-snow-20260822.js --commit
 */
...
const ORDER = {
  name: `GENCardM${uaeShortDate()}SHND`,
  marker: `SHINE-DIBBA-SWS20-PCS-SNOW500-TONER-EPI-${uaeToday()}`,
}
...
const EXPECTED_SUM_MINOR = 149500

function shipmentAddress() {
  return {
    country: href('country', COUNTRY_UAE_ID),
    city: 'Dibba Al Fujairah',
    street: 'Al Rifaa, Aspin Commercial Tower, office 3801, near Dibba Hospital',
    addInfo: '',
  }
}
...
async function exportInvoicePdf(invoiceId, invoiceName) {
  ...
  const out = path.join(ORDERS_DIR, `GENOSYS_Shine_Medical_Center_${safe}.pdf`)
...
async function main() {
  console.log('====================================================================')
  console.log('  Shine Dibba — SWS×20 + PCS + Snow 500 + toner 500 + EPI')
  console.log('====================================================================')
  console.log(`  Mode: ${COMMIT ? 'COMMIT' : 'DRY RUN'}`)

  const agent = await api('GET', `/entity/counterparty/${AGENT_ID}`)
  if (!/shine medical/i.test(agent.name)) throw new Error(`Unexpected agent: ${agent.name}`)
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      '00020 SWS x20 @29; 00065 PCS x10 @29; 00024 Snow 500 x1 @255; 00183 PC toner 500 x1 @245; 00129 EPI x1 @125. Clinic list. Unpaid 30 days net.',
  ...
  console.log(`  INVOICE_NAME=${invoice.name}`)
