 ...
 * other pages: 52 needles, three speeds, a ten-minute cutoff, 14 LEDs, 24-month warranty.
 * Same principle as product 49's dosimetry table.
 *
 ...
 * circulation, anagen/telogen/catagen, 5α-reductase, DHT, VEGF as an active, wound
 * healing, collagen production, a needle depth, or any efficacy claim for the LEDs.
 */
...
import { COMPANION_PRODUCT_IDS, getHairGenBoosterCopy } from './hairGenBoosterCopy'
...
export default function HairGenBoosterProductPage({ product, unitsSold = 0, routineProducts = [] }: Props) {
  ...
  const isRtl = dir === 'rtl'
  const copy = getHairGenBoosterCopy(locale)
  ...
  const addToCart = useCallback(
    async (qty: number) => {
      ...
      try {
        ...
        errorLog('HairGenBooster: add to cart failed', error)
        throw error
      }
    },
