...
/**
 * Miss Nina (existing) — full retail cycle, paid:
 *   order → invoice → shipment → paymentin → invoice PDF
 *
 *   54457 Ultra Shield Sun Cream SPF50 50g  x1 @ 250 AED retail
 *   00195 Moisture Replenishing Hyaluron Serum 30ml x1 @ 330 AED retail
 *   Excellent Delivery Dubai x1 @ 45 AED
 *
 * Counterparty: ee4ece95-206b-11f1-0a80-052e0023c84e
 * Address: Socio Tower 2, app 304, Dubai Hills · 0522142249
 *
 *   node --import dotenv/config scripts/moysklad-create-miss-nina-order-invoice-demand-paymentin-20260628.js
 *   node --import dotenv/config scripts/moysklad-create-miss-nina-order-invoice-demand-paymentin-20260628.js --commit
 */
...
const AGENT_ID = 'ee4ece95-206b-11f1-0a80-052e0023c84e' // Miss Nina
...
const PHONE = '0522142249'
const SHIPMENT = {
  city: 'Dubai',
  street: 'Socio Tower 2, app 304, Dubai Hills',
}

const ORDER = {
  name: `GENCardM${uaeShortDate()}2249`,
  moment: uaeMomentNow(),
  marker: `MISS-NINA-SPF-HYALURON-RETAIL-${uaeToday()}`,
}
...
function ordersPdfPath(invoiceName) {
  ...
  return path.join(ordersDir, `GENOSYS_Miss_Nina_${safe}.pdf`)
}

async function main() {
  console.log('====================================================================')
  console.log('  Miss Nina — SO → invoice → shipment → paymentin (paid)')
  console.log('====================================================================')
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      'SPF50 54457 + Hyaluron serum 00195; delivery 45. Paid — paymentin posted.',
