...
/**
 * BIANCO JGE GENTS SALON L.L.C — SO → invoice → shipment + Legal TAX PDF.
 *
 *   00063 Intensive Repair Collagen Mask ×10
 *   00140 Soothing Bomb Sea Algae Mask ×10
 *
 *   node --import dotenv/config scripts/moysklad-create-bianco-jge-gents-collagen-sea-order-20260810.js
 *   node --import dotenv/config scripts/moysklad-create-bianco-jge-gents-collagen-sea-order-20260810.js --commit
 */
...
const AGENT_ID = 'f7d1afa3-9f84-11f0-0a80-1567000e3f47' // BIANCO JGE GENTS SALON L.L.C
const CONTRACT_ID = '8686d39b-9f88-11f0-0a80-1880000e698c' // Contract 23
...
const ORDER = {
  ...
  moment: uaeMomentNow(),
  marker: `BIANCO-JGE-GENTS-COLLAGEN10-SEA10-${uaeToday()}`,
}
...
function buildShipmentAddress(agent) {
  const addInfo = agent.actualAddressFull?.addInfo || agent.actualAddress || ''
  const street =
    (typeof addInfo === 'string' ? addInfo : addInfo?.addInfo) ||
    'shop no 2, Jumeirah Golf Estate, Dubai'
...
async function exportInvoicePdf(invoiceId, invoiceName) {
  ...
  const outPath = path.join(ordersDir, `GENOSYS_Bianco_JGE_Gents_${safe}.pdf`)
...
async function main() {
  console.log('====================================================================')
  console.log('  Bianco JGE Gents — collagen×10 + sea algae×10')
  console.log('====================================================================')
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      'Collagen mask 00063 x10, Sea Algae mask 00140 x10. Agreement 23.',
    ].join(' | '),
