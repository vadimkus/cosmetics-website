...
/**
 * Miss Oshba Al Falasi — new customer, retail SO → invoice → shipment + PDF.
 *
 *   00194 Multi Vita Radiance Serum 30ml ×1 @ 330 AED retail
 *   00195 Moisture Replenishing Hyaluron Serum 30ml ×1 @ 330 AED retail
 *   Excellent Delivery Dubai ×1 @ 45 AED
 *
 *   node --import dotenv/config scripts/moysklad-create-miss-oshba-al-falasi-retail-order-invoice-demand-20260713.js
 *   node --import dotenv/config scripts/moysklad-create-miss-oshba-al-falasi-retail-order-invoice-demand-20260713.js --commit
 */
...
const CUSTOMER = {
  name: 'Miss Oshba Al Falasi',
  phone: '+971567170007',
  city: 'Dubai',
  street: 'Umm Suqeim 2, Brayw street, villa 143',
}

const ORDER = {
  name: `GENCardM${uaeShortDate()}0007`,
  moment: uaeMomentNow(),
  marker: `MISS-OSHBA-AL-FALASI-RETAIL-${uaeToday()}`,
}
...
async function findExistingCounterparty() {
  for (const ph of [CUSTOMER.phone, '+971 56 717 0007', '0567170007']) {
    const d = await api('GET', `/entity/counterparty?filter=phone=${encodeURIComponent(ph)}&limit=5`)
    if (d?.rows?.length) return d.rows[0]
  }
  const d = await api('GET', `/entity/counterparty?search=${encodeURIComponent('Oshba')}&limit=10`)
  return (d?.rows || []).find((r) => /oshba/i.test(r.name || '')) || null
}
...
async function findOrCreateCounterparty() {
  ...
  const created = await api('POST', '/entity/counterparty', {
    ...
    description: 'Retail customer — Umm Suqeim 2, villa 143.',
...
function ordersPdfPath(invoiceName) {
  ...
  return path.join(ordersDir, `GENOSYS_Miss_Oshba_Al_Falasi_${safe}.pdf`)
}

async function main() {
  console.log('====================================================================')
  console.log('  Miss Oshba Al Falasi — retail SO + invoice + shipment')
  console.log('====================================================================')
  ...
  for (const [code, qty, retailAed] of PRODUCT_LINES) {
    const item = stock.get(code)
    console.log(`    ${code} ${item.name.slice(0, 52)} x${qty} @ ${retailAed.toFixed(2)} retail`)
  }
  ...
  const orderDesc = [
    ORDER.marker,
    '00194 Radiance serum x1 @ 330; 00195 Hyaluron serum x1 @ 330; Dubai delivery 45.',
