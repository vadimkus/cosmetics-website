...
/**
 * Bespoke product page for EyeCell EYE PEPTIDE GEL PATCH (product 33).
 *
 ...
 * burgundy / wine palette on top via eyepatch.css, taken from the black
 * jar and the red EyeCell collar.
 *
 * Section order:
 *
 *   effects  Calm · Moisturize · Brighter look · Smoother lines
 *   engine   Niacinamide 2% + Adenosine 0.04%, the distinctive documented fact
 *   howTo    20-40 minutes, then remove, serum 17 and cream 24 after
 ...
 * clinical trial of this finished mask, so this page has no proof chart.
 * See eyepatchCopy.ts for the sourcing rules. In particular: Niacinamide 2%
 * leads, not the peptide name; no 10 Years Back; no Botox; take-off, not leave-on.
 */
...
import { getEyePatchCopy } from './eyepatchCopy'
...
export default function EyePatchProductPage({
  product,
  unitsSold = 0,
  routineProducts = [],
}: Props) {
  ...
  const routineSteps = useMemo(() => {
    const routine = PRODUCT_ROUTINES['33']
    if (!routine) return []
  ...
  const addToCart = useCallback(
    async (qty: number) => {
      ...
      try {
        ...
        errorLog('Eye patch: add to cart failed', error)
        throw error
      }
    },
  ...
  const handleAddRoutineProduct = useCallback(
    async (item: Product, selection?: ProductOptionSelection, quantity = 1) => {
      ...
      try {
        ...
        errorLog('Eye patch: routine add to cart failed', error)
      } finally {
  ...
  return (
    <div className={`cera-page egp-page min-h-[100dvh]`} dir={dir}>
      {/* ───────────────────────────── Hero ─────────────────────────────── */}
        ...
        <div>
          ...
          {product.videoUrl ? (
            <CeraReveal className="mt-10">
              ...
              <div className="egp-video relative mt-4 mx-auto aspect-[9/16] w-full max-w-[340px] overflow-hidden rounded-[28px]">
                <video
      ...
      <section className="mx-auto max-w-[1000px] px-4 py-16 sm:px-6 lg:py-24">
        ...
        <div className="mt-10 grid grid-cols-1 gap-4 lg:mt-14 lg:grid-cols-2 lg:gap-6">
          ...
          <CeraReveal delay={90} className="cera-card egp-not p-6 lg:p-7">
            <h3 className="cera-serif text-[21px] leading-tight text-[var(--cera-ink)]">
