...
/**
 * Lips for Kiss Clinic — unpaid clinic SO → invoice → shipment.
 *
 *   00021 Snow O₂ Cleanser 180ml ×5 @ 165
 *   00041 Multi Sun Cream SPF40 ×6 @ 105
 *   00122 Multi-Vita Radiance Cream 50g ×6 @ 145
 *   54458 Hyaluron Cream 50g ×6 @ 145
 *   00054 Eye Contour Serum ×5 @ 185
 *   Delivery Dubai ×1 @ 45, 100% off
 *   Total: 4,120 AED
 *
 *   node --import dotenv/config scripts/moysklad-create-lips-for-kiss-order-20260819.js
 *   node --import dotenv/config scripts/moysklad-create-lips-for-kiss-order-20260819.js --commit
 */
...
const ORDER = {
  ...
  moment: uaeMomentNow(),
  marker: `LIPS-FOR-KISS-SNOW-SPF-CREAMS-EYE-${uaeToday()}`,
}
...
const EXPECTED_SUM_MINOR = 412000
...
function shipmentAddress(agent) {
  ...
  return {
    ...
    city: 'Dubai',
    street: full?.addInfo || agent.actualAddress || 'Al Saqr Business Tower, DIFC',
  }
}
...
async function exportInvoicePdf(invoiceId, invoiceName) {
  ...
  const out = path.join(ORDERS_DIR, `GENOSYS_Lips_for_Kiss_${safe}.pdf`)
...
async function main() {
  console.log('====================================================================')
  console.log('  Lips for Kiss Clinic — snow + SPF + creams + eye serum')
  console.log('====================================================================')
  console.log(`  Mode: ${COMMIT ? 'COMMIT' : 'DRY RUN'}`)

  const agent = await api('GET', `/entity/counterparty/${AGENT_ID}`)
  console.log(`  Customer: ${agent.name} | ${agent.phone || '—'}`)
  if (agent.name !== 'Lips for Kiss Clinic') throw new Error(`Unexpected agent: ${agent.name}`)
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      '00021 x5 @165; 00041 x6 @105; 00122 x6 @145; 54458 x6 @145; 00054 x5 @185; delivery free. Clinic list. Unpaid.',
