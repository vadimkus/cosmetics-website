...
/**
 * Bespoke product page for EGF REPAIR OXYMASK CREAM (product 26). Cool oxygen
 * blue in oxymask.css, because nothing else in the range foams on the face.
 *
 * THE USAGE RULES COME FIRST, above the formula and above any selling. Dry skin,
 * do not rub, three to five pumps, do not rinse. On a product whose entire
 * mechanism is a foaming reaction, getting these wrong means it does not work,
 * and none of them were on our site. They are functional information, not
 * nice-to-have, which is why they get numbered cards rather than a note.
 *
 * THEN THE ENGINE: methyl perfluoroisobutyl ether at 5%, the second ingredient
 * after water, which carries and releases the oxygen; and decyl glucoside at
 * 2.75%, which is why it can foam at all. Neither was in our key-ingredient list.
 *
 * THEN THE CARTON'S OWN DISCLOSURE. The Korean panel prints "(0.1ppm)" beside
 * sh-Oligopeptide-1 — the EGF the product is named for. No other GENOSYS carton
 * audited this week declares a concentration inside the ingredient list, so this
 * page credits the manufacturer for it and repeats it rather than talking around
 * it. That framing is the point: we are agreeing with the box, not correcting it.
 *
 * ON THE CLINICAL STUDY: the deck names it — "Clinical study on skin soothing
 * effect against external stimulus (physical stimulus)" — and shows the result as
 * a chart with no readable value, unlike the other products in the same deck. The
 * page says the study exists and that we hold no figure. Neither "proven" nor
 * "unsupported".
 *
 * Do not add: any EGF mechanism from the deck (keratinocyte proliferation, wound
 * healing, "attracts cells to the wound site") — 0.1 ppm and drug claims; any
 * regeneration story from madecassoside (1 ppm) or copper tripeptide-1 (0.05 ppm);
 * the deck's "oxygen improves cellular metabolism / accelerates healing /
 * anti-inflammatory"; that sh-oligopeptide-1 is a recombinant E. coli peptide;
 * eucalyptol toxicology (internal only); the contract manufacturer; the lot codes.
 */
...
import { COMPANION_PRODUCT_IDS, getOxymaskCopy } from './oxymaskCopy'
...
export default function OxymaskProductPage({ product, unitsSold = 0, routineProducts = [] }: Props) {
  ...
  const isRtl = dir === 'rtl'
  const copy = getOxymaskCopy(locale)
  ...
  const addToCart = useCallback(
    async (qty: number) => {
      ...
      try {
        ...
        errorLog('Oxymask: add to cart failed', error)
        throw error
      }
    },
  ...
  return (
    <div className={`cera-page eoc-page ${ceraSerif.variable} min-h-[100dvh]`} dir={dir}>
      {/* ───────────────────────────── Hero ─────────────────────────────── */}
      ...
      {/* ──────────── The four usage rules, before anything else ────────── */}
      <section className="mx-auto max-w-[1040px] px-4 pt-14 sm:px-6 lg:pt-20">
        <CeraSectionHeader eyebrow={copy.rules.eyebrow} title={copy.rules.title} intro={copy.rules.body} />
        <div className="mt-10 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:mt-12 lg:gap-5">
          {copy.rules.items.map((r, i) => (
            <CeraReveal key={r.do} delay={i * 70} as="article" className="eoc-rule flex gap-5 p-6">
              <span className="eoc-rule__n cera-numeral flex-none text-[30px] leading-none">{i + 1}</span>
              <div className="min-w-0 flex-1">
                <p className="cera-serif text-[19px] leading-tight text-[var(--cera-ink)]">{r.do}</p>
                <p className="mt-2 text-[14.5px] leading-relaxed text-[var(--cera-body)]">{r.body}</p>
              </div>
            </CeraReveal>
          ))}
        </div>
      ...
      <section className="bg-white py-16 lg:py-24">
        <div className="mx-auto max-w-[1040px] px-4 sm:px-6">
          <CeraSectionHeader eyebrow={copy.engine.eyebrow} title={copy.engine.title} intro={copy.engine.intro} />
          <div className="mt-10 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:mt-14 lg:grid-cols-3 lg:gap-6">
            {copy.engine.items.map((item, i) => (
              <CeraReveal key={item.name} delay={i * 70} as="article" className="cera-card cera-card-hover flex flex-col p-6">
                ...
                <p dir="ltr" className="cera-numeral eoc-figure mt-2 text-[25px] leading-none text-[var(--cera-rose)]">
                  {item.dose}
                </p>
      ...
      {/* ─────────── The carton prints the dose: credit and table ───────── */}
      <section className="mx-auto max-w-[1040px] px-4 py-16 sm:px-6 lg:py-24">
        <CeraSectionHeader eyebrow={copy.carton.eyebrow} title={copy.carton.title} intro={copy.carton.body} />

        <CeraReveal className="cera-card mt-10 overflow-hidden lg:mt-14">
          <table className="eoc-table w-full border-collapse text-start">
            <caption className="sr-only">{copy.carton.title}</caption>
            <tbody>
              {copy.carton.rows.map(row => (
                <tr
                  key={row.name}
                  data-real={row.real ? 'true' : undefined}
                  className="border-b border-[var(--cera-line)] last:border-b-0"
                >
                  <th scope="row" dir="ltr" className="px-5 py-4 text-start align-top text-[14px] font-medium leading-snug text-[var(--cera-ink)]">
                    {row.name}
                  </th>
                  <td dir="ltr" className="eoc-figure whitespace-nowrap px-5 py-4 text-start align-top text-[15px] font-semibold text-[var(--cera-rose)]">
                    {row.value}
                  </td>
        ...
        <CeraReveal>
          <p className="mt-7 text-[15.5px] leading-relaxed text-[var(--cera-body)]">{copy.carton.footer}</p>
        </CeraReveal>

        <CeraReveal delay={90} className="eoc-note mt-6 p-6 md:p-7">
          <p className="cera-eyebrow">{copy.study.eyebrow}</p>
          <h3 className="cera-serif mt-2 text-[22px] leading-tight sm:text-[26px]">{copy.study.title}</h3>
          <p className="mt-3 text-[14.5px] leading-relaxed text-[var(--cera-body)]">{copy.study.body}</p>
        </CeraReveal>
      </section>

      {/* ──────────────── This or the postcream ─────────────────────────── */}
      <section className="bg-white py-16 lg:py-24">
        <div className="mx-auto max-w-[1040px] px-4 sm:px-6">
          <CeraSectionHeader eyebrow={copy.versus.eyebrow} title={copy.versus.title} intro={copy.versus.intro} />

          <div className="mt-10 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:mt-12 lg:gap-6">
            <CeraReveal className="eoc-versus p-6 md:p-7">
              <p className="cera-eyebrow">{copy.versus.columns.post}</p>
              <p className="mt-3 text-[14.5px] leading-relaxed text-[var(--cera-body)]">{copy.versus.postBody}</p>
            </CeraReveal>
            <CeraReveal delay={90} className="eoc-versus eoc-versus--self p-6 md:p-7">
              <p className="cera-eyebrow">{copy.versus.columns.oxy}</p>
              <p className="mt-3 text-[14.5px] leading-relaxed text-[var(--cera-ink)]">{copy.versus.oxyBody}</p>
            </CeraReveal>
          </div>

          <CeraReveal>
            <p className="mt-7 text-[15.5px] leading-relaxed text-[var(--cera-body)]">{copy.versus.body}</p>
          </CeraReveal>
        </div>
      </section>

      {/* ───────────── Quality, then fragrance and vegan note ───────────── */}
      <section className="py-16 lg:py-24">
        <div className="mx-auto max-w-[1200px] px-4 sm:px-6">
          ...
          <CeraReveal delay={90} className="eoc-note mx-auto mt-8 max-w-[900px] p-6 md:p-7">
