...
/**
 * Persona Palm Jumeirah — August 2026 commissioner report + matching demand.
 * Agreement 00078. Clinic list. Unpaid. Same 13 lines on both docs.
 *
 *   node --import dotenv/config scripts/moysklad-create-persona-palm-aug-consignment-20260903.js
 *   node --import dotenv/config scripts/moysklad-create-persona-palm-aug-consignment-20260903.js --commit
 */
...
const MARKER = `PERSONA-PALM-AUG-2026-CONS-${uaeToday()}`
...
const LINES = [
  ...
  ['00048', 1, 370, 'HR³ Matrix Hair Solution Professional Box'],
...
async function main() {
  console.log('====================================================================')
  console.log('  Persona Palm — August consignment sales + matching demand')
  console.log('====================================================================')
  ...
  if (contract.name !== '00078') throw new Error(`Expected agreement 00078, got ${contract.name}`)
  ...
  const report = await api('POST', '/entity/commissionreportin', {
    ...
    description: [
      MARKER,
      'Persona Palm Jumeirah | Agreement 00078 | August sold (18 pcs).',
      'Clinic list. Matching demand 1:1.',
    ].join('\n'),
  ...
  const demand = await api('POST', '/entity/demand', {
    ...
    description: [
      ...
      `Replenishment 1:1 — same ${LINES.length} SKU / ${EXPECTED_QTY} pcs / ${money(EXPECTED_SUM_MINOR)} AED.`,
      'Agreement 00078. No SO / invoice / payment.',
    ].join('\n'),
  ...
  const salesPdf = await exportPdf(
    ...
    path.join(ORDERS_DIR, `GENOSYS_Persona_Palm_Consignment_Sales_${report.name}.pdf`),
  )
  const stockPdf = await exportPdf(
    'demand',
    demand.id,
    STOCK_NOTE_TEMPLATE_ID,
    path.join(ORDERS_DIR, `GENOSYS_Persona_Palm_Consignment_Stock_Note_${demand.name}.pdf`),
  )
