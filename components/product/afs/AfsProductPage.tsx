...
/**
 * Bespoke product page for ALL FOR SENSITIVE SERUM (product 19).
 *
 ...
 * lime/charcoal palette on top via afs.css, taken from the pack.
 *
 * Section order:
 *
 *   effects  Relieve · Protect · Moisturize, the artwork sentence
 *   engine   MultiEx BSASM® Plus at 1%, the distinctive documented fact
 *   clean    five no-additions, without calling the serum fragrance-free
 *   howTo    AM & PM from the pack, plus the product video
 ...
 *   routine  the sensitive-skin leave-on steps
 *
 * There is no efficacy study and no dedicated marketing deck. Laboratory
 * figures live in the details table (pH, appearance, shelf life) rather than
 * in a proof section that would have to invent percentages.
 *
 * See afsCopy.ts for the sourcing rules and the list of claims that must not
 * come back without a document.
 */
...
import { getAfsCopy } from './afsCopy'
...
const ENGINE_IMAGE = '/images/sensitive_serum/s4.jpeg'
const HOWTO_IMAGE = '/images/sensitive_serum/s5.jpeg'
const EFFECTS_IMAGE = '/images/sensitive_serum/s2.jpeg'
const PROOF_IMAGE = '/images/sensitive_serum/s1.jpeg'
...
export default function AfsProductPage({
  product,
  unitsSold = 0,
  routineProducts = [],
}: Props) {
  ...
  const addToCart = useCallback(
    async (qty: number) => {
      ...
      try {
        ...
        errorLog('AfsSerum: add to cart failed', error)
        throw error
      }
    },
  ...
  const handleAddRoutineProduct = useCallback(
    async (item: Product, selection?: ProductOptionSelection, quantity = 1) => {
      ...
      try {
        ...
        errorLog('AfsSerum: routine add to cart failed', error)
      } finally {
  ...
  return (
    <div className={`cera-page afs-page ${ceraSerif.variable} min-h-[100dvh]`} dir={dir}>
      {/* ───────────────────────────── Hero ─────────────────────────────── */}
        ...
        <div>
          ...
          {product.videoUrl ? (
            <CeraReveal className="mt-10">
              ...
              <div className="afs-video relative mt-4 mx-auto aspect-[9/16] w-full max-w-[340px] overflow-hidden rounded-[28px]">
                <video
      ...
      <section className="bg-white py-16 lg:py-24">
        <div className="mx-auto max-w-[1200px] px-4 sm:px-6">
          ...
          <CeraSectionHeader
            ...
            intro={copy.clean.intro}
          />
          <ul className="mt-10 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:mt-12">
            {copy.clean.items.map((item, i) => (
              <CeraReveal key={item} as="li" delay={i * 50}>
                <div className="cera-card flex items-center gap-3 px-5 py-4">
      ...
      <section className="mx-auto max-w-[1000px] px-4 py-16 sm:px-6 lg:py-24">
        ...
        <div className="mt-10 grid grid-cols-1 gap-4 lg:mt-14 lg:grid-cols-2 lg:gap-6">
          ...
          <CeraReveal delay={90} className="cera-card afs-not p-6 lg:p-7">
            <h3 className="cera-serif text-[21px] leading-tight text-[var(--cera-ink)]">
