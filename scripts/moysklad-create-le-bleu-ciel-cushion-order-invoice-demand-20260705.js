...
/**
 * Le Bleu Ciel Ladies Beauty Salon — SO → invoice → shipment + Legal_TAX PDF (landscape).
 *
 *   00144 Cushion #2 Beige ×3 @ 150 AED
 *   00143 Cushion #1 Ivory ×2 @ 150 AED
 *
 *   node --import dotenv/config scripts/moysklad-create-le-bleu-ciel-cushion-order-invoice-demand-20260705.js
 *   node --import dotenv/config scripts/moysklad-create-le-bleu-ciel-cushion-order-invoice-demand-20260705.js --commit
 */
...
const AGENT_ID = 'd28b9ecf-44c0-11ef-0a80-0379001bda44' // Le Bleu Ciel Ladies Beauty Salon
...
const ORDER = {
  name: `GENCardM${uaeShortDate()}7775`,
  moment: uaeMomentNow(),
  marker: `Le Bleu Ciel cushion order ${uaeToday()}`,
}
...
const EXPECTED_SUM_MINOR = 75000 // 450 + 300
...
async function exportInvoicePdf(invoiceId, invoiceName) {
  ...
  const outPath = path.join(ORDERS_DIR, `GENOSYS_Le_Bleu_Ciel_${safe}.pdf`)
...
async function main() {
  console.log('====================================================================')
  console.log('  Le Bleu Ciel — SO + invoice + shipment (cushions)')
  console.log('====================================================================')
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      '00144 Beige cushion x3 @150; 00143 Ivory cushion x2 @150 (clinic list).',
    ].join(' | '),
