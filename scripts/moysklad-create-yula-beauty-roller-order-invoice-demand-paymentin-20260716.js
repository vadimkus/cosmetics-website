...
/**
 * Yula Beauty Salon LLC — SO → invoice → shipment → paymentin.
 * Standard Detachable Manual Roller 0.25mm (00001) ×1 @ clinic price.
 *
 *   node --import dotenv/config scripts/moysklad-create-yula-beauty-roller-order-invoice-demand-paymentin-20260716.js
 *   node --import dotenv/config scripts/moysklad-create-yula-beauty-roller-order-invoice-demand-paymentin-20260716.js --commit
 */
...
const ORDER = {
  name: `GENCardM${uaeShortDate()}7103`,
  marker: `Yula Beauty Salon standard roller 0.25mm ${uaeToday()}`,
}

/** [code, qty] */
const PRODUCT_LINES = [['00001', 1]]
const EXPECTED_SUM_MINOR = 11500
...
function buildShipmentAddress(agent) {
  ...
  return { country: countryHref(), city: 'Dubai', street: addInfo || 'UAE — Yula Beauty Salon LLC' }
}
...
async function exportInvoicePdf(invoiceId, invoiceName) {
  ...
  const outPath = path.join(ordersDir, `GENOSYS_Yula_Beauty_Salon_${safe}.pdf`)
...
async function main() {
  console.log('====================================================================')
  console.log('  Yula Beauty Salon LLC — SO → invoice → shipment → paymentin')
  console.log('====================================================================')
  ...
  const t3 = uaeMomentAddMinutes(10)
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      '00001 Standard Detachable Manual Roller 0.25mm x1 @ clinic price. Paid.',
    ].join(' | '),
  ...
  const paymentIn = await api('POST', '/entity/paymentin', {
    ...
    description: [
      ORDER.marker,
      `Invoice ${invoice.name} / shipment ${demand.name}`,
      `Order ${order.name}`,
      'Standard roller 0.25mm 00001 x1 — paid.',
    ].join(' | '),
    operations: [
      {
        meta: {
  ...
  console.log(`  4) Paymentin: ${paymentIn.name} | ${money(paymentIn.sum)} AED`)
