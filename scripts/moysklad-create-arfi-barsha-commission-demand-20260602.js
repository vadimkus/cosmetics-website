...
/**
 * ARFI Nails Barsha — commissioner report + consignment Отгрузка (contract 25).
 *
 *   node --import dotenv/config scripts/moysklad-create-arfi-barsha-commission-demand-20260602.js
...
const COMMON = {
  ...
  agentId: '39a1aa83-a5a6-11f0-0a80-1cbc00050fea', // ARFI NAILS BEAUTY SALON
  contractId: '739936aa-a809-11f0-0a80-07ba002a8e67', // Contract 25
  date: uaeToday(),
...
const REPORT = {
  ...
  marker: `ARFI Nails Barsha consignment sales ${uaeToday()}`,
}

const DEMAND = {
  moment: uaeMomentAddMinutes(3),
  stateShippedId: '50d70717-4582-11ea-0a80-05e3001273a2',
  marker: `ARFI Nails Barsha shipment ${uaeToday()}`,
}
...
async function main() {
  console.log('====================================================================')
  console.log('  ARFI Nails Barsha — report + отгрузка (contract 25)')
  console.log('====================================================================')
  ...
  const report = await api('POST', '/entity/commissionreportin', {
    ...
    description: [
      REPORT.marker,
      'ARFI NAILS BEAUTY SALON (Barsha) | Contract 25 | May 2026 sold items.',
      'Eye peptide gel patch = 00053 box.',
    ].join('\n'),
  ...
  const demand = await api('POST', '/entity/demand', {
    ...
    description: [
      DEMAND.marker,
      'ARFI NAILS BEAUTY SALON (Barsha) | Contract 25.',
