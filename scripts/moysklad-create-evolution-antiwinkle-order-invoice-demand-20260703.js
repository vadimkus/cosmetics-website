...
/**
 * Evolution Aesthetics Clinic — SO → invoice → shipment.
 *   00190 Multi Functional Anti-Wrinkle Cream 50g ×3 @ clinic list (VAT incl.)
 *
 *   node --import dotenv/config scripts/moysklad-create-evolution-antiwinkle-order-invoice-demand-20260703.js
 *   node --import dotenv/config scripts/moysklad-create-evolution-antiwinkle-order-invoice-demand-20260703.js --commit
 */
...
const ORDER = {
  name: `GENCardM${uaeShortDate()}EVOL`,
  moment: uaeMomentNow(),
  marker: `Evolution anti-wrinkle cream 00190 x3 ${uaeToday()}`,
}

const PRODUCT_LINES = [['00190', 3]] // Multi Functional Anti-Wrinkle Cream 50g
const EXPECTED_SUM_MINOR = 43500 // 3 × 145 AED VAT incl.
...
function buildShipmentAddress(agent) {
  ...
  return { country: countryHref(), city: 'Dubai', street: '49 Umm Al Sheif Rd, Jumeirah 3, Dubai' }
}
...
async function ensureNoDuplicateToday(agentId) {
  ...
  const dup = docs.find((d) => (d.description || '').includes('Evolution anti-wrinkle cream 00190 x3'))
...
async function exportInvoicePdf(invoiceId, invoiceName) {
  ...
  const outPath = path.join(ORDERS_DIR, `GENOSYS_Evolution_${safe}.pdf`)
...
async function main() {
  console.log('====================================================================')
  console.log('  Evolution Aesthetics Clinic — SO + invoice + shipment')
  console.log('====================================================================')
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    moment: t0,
    shared: true,
    description: [ORDER.marker, '00190 Multi Functional Anti-Wrinkle Cream 50g x3 @ clinic list.'].join(' | '),
