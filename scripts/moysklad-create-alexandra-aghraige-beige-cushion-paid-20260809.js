...
/**
 * Miss Alexandra Aghraige — paid retail SO → invoice → shipment → paymentin
 * + Legal TAX PDF landscape print (beige cushion 300 + delivery 30).
 *
 *   00144 Skin Caring Blemish Balm Cushion #2 Beige ×1 @ 300
 *   Delivery Dubai ×1 @ 30
 *   Total: 330 AED
 *   Address: Burj Royale 3808, Dubai
 *
 *   node --import dotenv/config scripts/moysklad-create-alexandra-aghraige-beige-cushion-paid-20260809.js
 *   node --import dotenv/config scripts/moysklad-create-alexandra-aghraige-beige-cushion-paid-20260809.js --commit
 *   node --import dotenv/config scripts/moysklad-create-alexandra-aghraige-beige-cushion-paid-20260809.js --commit --no-print
 */
...
const AGENT_ID = '43dad6d4-dbf3-11ec-0a80-01d000046060' // Miss Alexandra Aghraige
...
const CUSTOMER = {
  name: 'Miss Alexandra Aghraige',
  phone: '+971 50 109 4505',
  city: 'Dubai',
  street: 'Burj Royale 3808',
}

const ORDER = {
  name: `GENCardM${uaeShortDate()}4505`,
  moment: uaeMomentNow(),
  marker: `ALEXANDRA-AGHRAIGE-BEIGE-DELIVERY30-${uaeToday()}`,
}

/** [code, qty, retailAed, discountPct] */
const PRODUCT_LINES = [['00144', 1, 300, 0]]
const DELIVERY_AED = 30
const DELIVERY_DISCOUNT = 0
const EXPECTED_SUM_MINOR = 33000
...
async function updateCustomerAddress() {
  ...
  await api('PUT', `/entity/counterparty/${AGENT_ID}`, {
    ...
    actualAddress: `${CUSTOMER.street}, Dubai, UAE`,
...
async function exportInvoicePdf(invoiceId, invoiceName) {
  ...
  const outPath = path.join(ORDERS_DIR, `GENOSYS_Miss_Alexandra_Aghraige_${invoiceName}.pdf`)
...
async function main() {
  console.log('====================================================================')
  console.log('  Alexandra Aghraige — beige 300 + delivery 30 (paid) + landscape')
  console.log('====================================================================')
  console.log(`  Mode: ${COMMIT ? 'COMMIT' : 'DRY RUN'}`)

  const agent = await api('GET', `/entity/counterparty/${AGENT_ID}`)
  console.log(`  Customer: ${agent.name} | ${agent.phone || CUSTOMER.phone}`)
  console.log(`  Ship (new): ${CUSTOMER.street}, Dubai`)
  console.log(`  Ship (old): ${agent.actualAddress || '—'}`)
  ...
  for (const [code, qty, retailAed, discountPct] of PRODUCT_LINES) {
    ...
    console.log(
      `    ${code} ${item.name.slice(0, 52)} x${qty} @ ${retailAed} → ${net.toFixed(2)} (avail ${item.available})`,
    )
  }
  ...
  console.log('  Counterparty address updated → Burj Royale 3808')
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      '00144 beige cushion x1 @300 retail; delivery Dubai @30.',
      `Ship: ${CUSTOMER.street}, Dubai. Phone ${CUSTOMER.phone}.`,
      'Paid in full — delivered. Address updated from Binghatti Gardenia.',
    ].join('\n'),
