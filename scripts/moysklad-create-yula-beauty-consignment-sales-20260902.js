...
/**
 * Yula Beauty Salon LLC — commissioner report + matching demand.
 * Agreement 12. Clinic list. Unpaid.
 *
 *   node --import dotenv/config scripts/moysklad-create-yula-beauty-consignment-sales-20260902.js
 *   node --import dotenv/config scripts/moysklad-create-yula-beauty-consignment-sales-20260902.js --commit
 */
...
const MARKER = `YULA-BEAUTY-CONS-SALES-${uaeToday()}`
...
async function main() {
  console.log('====================================================================')
  console.log('  Yula Beauty — consignment sales + matching demand')
  console.log('====================================================================')
  ...
  if (contract.name !== '12') throw new Error(`Expected agreement 12, got ${contract.name}`)
  console.log(`  Customer: ${agent.name}`)
  console.log(`  Agreement: ${contract.name}`)
  console.log('  Period: 2026-07-17 → 2026-09-02')
  ...
  const report = await api('POST', '/entity/commissionreportin', {
    ...
    commissionPeriodStart: '2026-07-17 00:00:00',
    commissionPeriodEnd: `${uaeToday()} 23:59:59`,
    rewardType: 'PercentOfSales',
    rewardPercent: 0,
    description: [
      MARKER,
      'Yula Beauty Salon LLC | Agreement 12 | consignment sold (2 pcs).',
  ...
  const demand = await api('POST', '/entity/demand', {
    ...
    description: [
      ...
      'Agreement 12. No SO / invoice / payment.',
    ].join('\n'),
  ...
  const salesPdf = await exportPdf(
    ...
    path.join(ORDERS_DIR, `GENOSYS_Yula_Beauty_Consignment_Sales_${report.name}.pdf`),
  )
  const stockPdf = await exportPdf(
    'demand',
    demand.id,
    STOCK_NOTE_TEMPLATE_ID,
    path.join(ORDERS_DIR, `GENOSYS_Yula_Beauty_Consignment_Stock_Note_${demand.name}.pdf`),
  )
