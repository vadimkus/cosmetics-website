...
/**
 * Family Class INV 05027 — add Intensive Blemish Balm Cream 00040 ×1 @ 125.
 * Re-export Legal_TAX → ~/Desktop/orders/
 *
 *   node --import dotenv/config scripts/moysklad-amend-family-class-05027-blemish-20260905.js
 *   node --import dotenv/config scripts/moysklad-amend-family-class-05027-blemish-20260905.js --commit
 */
...
const ADD_CODE = '00040'
const ADD_QTY = 1
const ADD_AED = 125
const CURRENT_SUM_MINOR = 100000
const EXPECTED_SUM_MINOR = 112500
const MARKER = 'FAMILY-CLASS-05027-ADD-BLEMISH-2026-09-05'
...
async function amendDoc(type, id, label) {
  ...
  await api('POST', `/entity/${type}/${id}/positions`, {
    ...
    discount: 0,
    assortment: href('product', amendDoc.productId),
    vat: 5,
    vatEnabled: true,
  })
...
async function main() {
  console.log('====================================================================')
  console.log('  Family Class 05027 — add blemish balm 50g ×1 @ 125')
  console.log('====================================================================')
  ...
  await api('PUT', `/entity/customerorder/${ORDER_ID}`, {
    meta: so.meta,
    description: [so.description || '', MARKER, 'Added 00040 Blemish Balm Cream x1 @125. Total 1125 unpaid.'].join(
      '\n',
    ),
  })
