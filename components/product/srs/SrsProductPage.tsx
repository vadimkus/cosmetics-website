...
/**
 * Bespoke product page for SKIN RENEWAL PEELING SYSTEM (SRS)
 * (product 13).
 *
 ...
 * layers a cool-bone / steel palette on top via srs.css, taken from the
 * cold-water rinse. The box is Pantone 187 C, already the Eye Kit accent,
 * so this page does not take that crimson.
 *
 * Section order:
 *
 *   effects  Apply · Sit · Cold rinse
 *   engine   glycolic 15%, lactic 13.5%, mandelic 2%
 *   howTo    patch test, apply, 15-20 min, cold rinse
 ...
 *
 * No home retail routine. SRS is a professional clinic peel. Epi is the
 * weekly cellulose roll. See srsCopy.ts for the sourcing rules. In
 * particular: do not rebuild the page around sh-Polypeptide-7 at 0.1 ppb,
 * a neutralize step, or microneedling as the purpose.
 */
...
import { getSrsCopy } from './srsCopy'

interface Props {
  product: Product
  unitsSold?: number
  /** Kept so the route can pass the same props as other bespoke pages.
   *  SRS has no home routine and does not render companions. */
...
const ENGINE_IMAGE = '/images/srs_2_new/s3.jpg'

/** The apply / sit / cold-rinse slide, beside the protocol steps. */
const HOWTO_IMAGE = '/images/srs_2_new/s7.jpg'
...
export default function SrsProductPage({
  product,
  unitsSold = 0,
}: Props) {
  ...
  const addToCart = useCallback(
    async (qty: number) => {
      ...
      try {
        ...
        errorLog('SRS: add to cart failed', error)
        throw error
      }
    },
    ...
    <div className={`cera-page srs-page ${ceraSerif.variable} min-h-[100dvh]`} dir={dir}>
        ...
        <div>
          ...
          {product.videoUrl ? (
            <CeraReveal className="mt-10">
              ...
              <div className="srs-video relative mt-4 mx-auto aspect-[9/16] w-full max-w-[340px] overflow-hidden rounded-[28px]">
                <video
      ...
      <section className="mx-auto max-w-[1000px] px-4 py-16 sm:px-6 lg:py-24">
        ...
        <div className="mt-10 grid grid-cols-1 gap-4 lg:mt-14 lg:grid-cols-2 lg:gap-6">
          ...
          <CeraReveal delay={90} className="cera-card srs-not p-6 lg:p-7">
            <h3 className="cera-serif text-[21px] leading-tight text-[var(--cera-ink)]">
