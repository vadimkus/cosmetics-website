...
/**
 * AMERICAN MEDICAL CENTER DMCC — SO → invoice → shipment + Legal_TAX PDF → Desktop/orders.
 *
 *   00050 HR³ Matrix Scalp Peeling 100ml ×2 @ 145
 *   00051 HR³ Matrix Hair Tonic 70ml ×2 @ 145
 *   00121 Shampoo tester 30ml ×5 @ 25 (same as inv 04224)
 *   Total: 705.00 AED
 *
 *   node --import dotenv/config scripts/moysklad-create-american-medical-center-hair-testers-order-20260806.js
 *   node --import dotenv/config scripts/moysklad-create-american-medical-center-hair-testers-order-20260806.js --commit
 */
...
const ORDER = {
  name: `GENCardM${uaeShortDate()}AMC`,
  moment: uaeMomentNow(),
  marker: `AMC-SCALP-TONIC-SHAMPOO-TESTERS-${uaeToday()}`,
}

/** [code, qty, clinicAed] */
const PRODUCT_LINES = [
  ['00050', 2, 145], // Scalp Peeling 100ml
  ['00051', 2, 145], // Hair Tonic 70ml
  ['00121', 5, 25], // Shampoo tester 30ml (prev inv 04224 @ 25)
]
const EXPECTED_SUM_MINOR = 70500 // 290 + 290 + 125
...
function buildShipmentAddress(agent) {
  ...
  return {
    ...
    city: 'Dubai',
    street:
      'Jumeirah Lakes Towers 101, First Floor Red Diamond Building, Opposite to Cluster Y',
  }
}
...
async function main() {
  console.log('====================================================================')
  console.log('  AMERICAN MEDICAL CENTER — peeling×2 + tonic×2 + shampoo testers×5')
  console.log('====================================================================')
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      '00050 Scalp Peeling x2 @145; 00051 Hair Tonic x2 @145; 00121 shampoo testers x5 @25 (as inv 04224).',
    ].join(' | '),
