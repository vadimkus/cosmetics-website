/**
 * Update product 24 (EyeCell EYE CONTOUR CREAM) to the
 * public/images/eye_cream/ set (main + s1–s6).
 *
 * Dry run:
 *   npx tsx --env-file=.env.local scripts/update-product-24-images.ts
 * Apply:
 *   npx tsx --env-file=.env.local scripts/update-product-24-images.ts --apply
 */
...
const REPLACEMENTS: Array<[string, string]> = [
  ['/images/EC.jpg', NEW_MAIN],
  ['/images/Second/Eye_Cream_2.jpg', NEW_GALLERY[0]],
]
...
async function main() {
  const product = await prisma.product.findFirst({
    where: { OR: [{ id: '24' }, { productNumber: '24' }] },
