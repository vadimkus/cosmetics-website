...
/**
 * Bespoke product page for SNOW BOOSTER (product 16).
 *
 ...
 * cyan / slate palette on top via booster.css, taken from the SNOW BOOSTER
 * name bar (PANTONE 306 / 432).
 *
 * Section order:
 *
 *   effects  Moisture · Soothe · Refine after the cleanse
 *   engine   Betaine 3%
 *   howTo    AM/PM apply or spray, over makeup, plus video
 ...
 *   routine  cleanse → booster → Multi Vita serum → Multi Vita cream → SPF
 *
 * See boosterCopy.ts. Do not rebuild the page around Phytolex SC as a 0.5%
 * finished active, a brightening claim, or a 5-10 minute cotton-pad mask.
 */
...
import { getBoosterCopy } from './boosterCopy'
...
export default function BoosterProductPage({
  product,
  unitsSold = 0,
  routineProducts = [],
}: Props) {
  ...
  const sizeOptions = useMemo(() => getProductSizeOptions('16', product), [product])
  const [selectedSize, setSelectedSize] = useState(sizeOptions[0]?.value || product.size || '200ml')


  const isRtl = dir === 'rtl'
  const copy = getBoosterCopy(locale)
  const Chevron = isRtl ? ChevronLeft : ChevronRight

  // Two sizes, 200 ml and 1000 ml. The selected size is passed to the cart.
  ...
  const routineSteps = useMemo(() => {
    const routine = PRODUCT_ROUTINES['16']
    if (!routine) return []
  ...
  const addToCart = useCallback(
    async (qty: number) => {
      ...
      try {
        ...
        errorLog('Booster: add to cart failed', error)
        throw error
      }
    },
  ...
  const handleAddRoutineProduct = useCallback(
    async (item: Product, selection?: ProductOptionSelection, quantity = 1) => {
      ...
      try {
        ...
        errorLog('Booster: routine add to cart failed', error)
      } finally {
  ...
  return (
    <div className={`cera-page booster-page ${ceraSerif.variable} min-h-[100dvh]`} dir={dir}>
      {/* ───────────────────────────── Hero ─────────────────────────────── */}
        ...
        <div>
          ...
          {product.videoUrl ? (
            <CeraReveal className="mt-10">
              ...
              <div className="booster-video relative mt-4 aspect-square overflow-hidden rounded-[28px] sm:aspect-video">
                <video
      ...
      <section className="mx-auto max-w-[1000px] px-4 py-16 sm:px-6 lg:py-24">
        ...
        <div className="mt-10 grid grid-cols-1 gap-4 lg:mt-14 lg:grid-cols-2 lg:gap-6">
          ...
          <CeraReveal delay={90} className="cera-card booster-not p-6 lg:p-7">
            <h3 className="cera-serif text-[21px] leading-tight text-[var(--cera-ink)]">
