 ...
 *
 * Callers supply their own outer <section>: the homepage wraps this in its
 * reveal-on-view band, /products wraps it in a cera-page shell because the
 * block is rendered on the server, outside the products client component.
 */
...
export interface ConcernShowcaseProps {
  locale: Locale
  dir: 'ltr' | 'rtl'
  concernCounts?: Record<string, number> | undefined
  /** The homepage labels its own <section>; /products labels this heading. */
  headingId?: string
  /** /products keeps the CTA; set false anywhere it would be redundant. */
  showCta?: boolean
}

export default function ConcernShowcase({
  locale,
  dir,
  concernCounts,
  headingId = 'skin-concern-heading',
  showCta = true,
}: ConcernShowcaseProps) {
  const isRtl = dir === 'rtl'
  const copy = SECTION_COPY[locale]

  return (
    <div className="mx-auto max-w-[1200px]" data-testid="concern-showcase">
      <div
