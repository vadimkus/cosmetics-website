 ...
 *   54458 Hyaluron Cream 50g x5
 *   00041 Multi Sun Cream SPF40 x5
 *   00122 Multi-Vita Radiance Cream 50g x2
 *   00021 Snow O₂ Cleanser 180ml x5
 *   54457 Ultra Shield SPF50 x5
 *
 *   node --import dotenv/config scripts/moysklad-create-lips-for-kiss-order-invoice-demand-20260703.js
 *   node --import dotenv/config scripts/moysklad-create-lips-for-kiss-order-invoice-demand-20260703.js --commit
 */
...
const ORDER = {
  ...
  moment: uaeMomentNow(),
  marker: `Lips for Kiss creams cleanser SPF ${uaeToday()}`,
}
...
const EXPECTED_SUM_MINOR = 299000 // 2,990 AED @ clinic list
...
async function ensureNoDuplicateToday(agentId) {
  ...
  const dup = docs.find((d) => (d.description || '').includes('Lips for Kiss creams cleanser SPF'))
...
async function main() {
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      '54458 x5, 00041 x5, 00122 x2, 00021 x5, 54457 x5 @ clinic list.',
    ].join(' | '),
