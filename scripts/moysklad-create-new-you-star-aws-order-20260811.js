...
/**
 * NEW YOU STAR BEAUTY HEALTH CLINIC L.L.C — clinic SO → invoice → shipment
 * + Legal TAX PDF → ~/Desktop/orders/
 *
 *   00018 Power Solution AWS 1 Vial 2ml ×10 @ clinic salePrice (29)
 *   Total: 290 AED
 *
 *   node --import dotenv/config scripts/moysklad-create-new-you-star-aws-order-20260811.js
 *   node --import dotenv/config scripts/moysklad-create-new-you-star-aws-order-20260811.js --commit
 */
...
const AGENT_ID = '69e1db3e-7fa4-11f1-0a80-0283002585b0' // NEW YOU STAR BEAUTY HEALTH CLINIC L.L.C
...
const ORDER = {
  name: `GENCardM${uaeShortDate()}NYSAWS`,
  moment: uaeMomentNow(),
  marker: `NEW-YOU-STAR-AWS-X10-${uaeToday()}`,
}

/** [code, qty] — clinic salePrice from MoySklad */
const LINES = [['00018', 10]]
const EXPECTED_SUM_MINOR = 29000 // 10 × 29
...
function shipmentAddress() {
  return {
    ...
    city: 'Dubai',
    street: 'The Mall, Shop 21, Umm Suqeim Third',
  }
}
...
function buildPositions(stock) {
  ...
  for (const [code, qty] of LINES) {
    ...
    console.log(
      `    ${code} ${item.name.slice(0, 52)} x${qty} @ ${money(item.price)} → ${money(lineMinor)} AED (avail ${item.available})`,
    )
  }
...
async function exportInvoicePdf(invoiceId, invoiceName) {
  ...
  const outPath = path.join(ordersDir, `GENOSYS_New_You_Star_AWS_${safe}.pdf`)
...
async function main() {
  console.log('====================================================================')
  console.log('  NEW YOU STAR — AWS vials ×10 (clinic sale + Legal TAX PDF)')
  console.log('====================================================================')
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      'Power Solution AWS 00018 x10 @ clinic. Invoice sale (not consignment).',
      `Ship: ${ship.street}, Dubai. Phone ${agent.phone || '+971503359777'}.`,
    ].join(' | '),
