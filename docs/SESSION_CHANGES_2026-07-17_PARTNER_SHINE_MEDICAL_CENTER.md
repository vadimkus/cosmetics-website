# Partner added — Shine Medical Center (Dibba Al Fujairah) — 2026-07-17

## Added

**SHINE MEDICAL CENTER, DIBBA AL FUJAIRAH** (`مركز شاين الطبي`)

| Field | Value |
|-------|--------|
| Location | H7P9+2WM — Al Rifa‘ — Dibba Al Fujairah — Fujairah |
| Phone | +971 50 374 5371 |
| Instagram | https://www.instagram.com/shine.medical.center/ |
| Directions | https://www.google.com/maps/dir/?api=1&destination=H7P9%2B2WM+Al+Rifa+Dibba+Al+Fujairah |
| Logo | `/images/partners/shine.png` |
| Theme | emerald |

Also noted from Google listing: ~4,800+ reviews; opens around 13:00 (hours vary).

## Also

- Partners area filter: new **Northern Emirates** pill (Fujairah / Dibba / Sharjah / etc.) so Fujairah is not lumped under “Other Dubai”.
- Chatbot partner directory + `CHATBOT_KNOWLEDGE.md` updated.
