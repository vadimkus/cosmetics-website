 ...
 *   routine  cleanse → mask → mist → cream
 *
 * There is a dedicated DTS MG deck and a clinical trial, so this page has a
 * proof section the way product 52 does. There is no safety assessment on
 * file. See bioFermentCopy.ts for the sourcing rules and the list of claims
 * that must not come back without a document. In particular: do not rebuild
 * the page around the six peptides at 1 ppb.
 */

import '../cerabarrier/cerabarrier.css'
import './bioferment.css'
...
import { getBioFermentCopy } from './bioFermentCopy'
...
export default function BioFermentProductPage({
  product,
  unitsSold = 0,
  routineProducts = [],
}: Props) {
  ...
  const routineSteps = useMemo(() => {
    const routine = PRODUCT_ROUTINES['51']
    if (!routine) return []
  ...
  const addToCart = useCallback(
    async (qty: number) => {
      ...
      try {
        ...
        errorLog('BioFerment: add to cart failed', error)
        throw error
      }
    },
  ...
  const handleAddRoutineProduct = useCallback(
    async (item: Product, selection?: ProductOptionSelection, quantity = 1) => {
      ...
      try {
        ...
        errorLog('BioFerment: routine add to cart failed', error)
      } finally {
  ...
  return (
    <div className={`cera-page bf-page ${ceraSerif.variable} min-h-[100dvh]`} dir={dir}>
      {/* ───────────────────────────── Hero ─────────────────────────────── */}
      ...
      {locale === 'en' && <section className="bg-white py-16 lg:py-24">
        <div className="mx-auto max-w-[1200px] px-4 sm:px-6">
          ...
          <div className="mt-10 grid grid-cols-1 gap-6 lg:mt-14 lg:grid-cols-[minmax(0,1.15fr)_minmax(0,1fr)] lg:gap-8">
            <CeraReveal className="cera-card p-6 lg:p-8">
              ...
              <div className="bf-chart mt-8" aria-hidden="true">
                <div className="bf-bar-wrap">
                  <div
                    className="bf-bar bf-bar-before"
                    style={{ height: `${(HYDRATION.before / HYDRATION.after) * 200}px` }}
                  >
                    <span className="bf-bar-value">{HYDRATION.before}</span>
                  </div>
                  <p className="text-center text-[12.5px] text-[var(--cera-muted)]">
                    {copy.proof.beforeLabel}
                  </p>
                </div>
                <div className="bf-bar-wrap">
                  <div className="bf-bar bf-bar-after" style={{ height: '200px' }}>
                    <span className="bf-bar-value">{HYDRATION.after}</span>
                  </div>
                  <p className="text-center text-[12.5px] text-[var(--cera-muted)]">
                    {copy.proof.afterLabel}
                  </p>
                </div>
              </div>
            </CeraReveal>
            ...
            <div className="space-y-4">
              {[copy.proof.headline, copy.proof.cooling].map((claim, i) => (
                <CeraReveal key={claim.label} delay={i * 90} className="cera-card p-6 lg:p-7">
                  <p className="cera-serif cera-numeral text-[34px] leading-none text-[var(--cera-rose-ink)] sm:text-[42px]">
                    {claim.value}
                  </p>
      ...
      <section className="mx-auto max-w-[1000px] px-4 py-16 sm:px-6 lg:py-24">
        ...
        <div className="mt-10 grid grid-cols-1 gap-4 lg:mt-14 lg:grid-cols-2 lg:gap-6">
          ...
          <CeraReveal delay={90} className="cera-card bf-not p-6 lg:p-7">
            <h3 className="cera-serif text-[21px] leading-tight text-[var(--cera-ink)]">
