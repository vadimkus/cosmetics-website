 ...
 *
 *   Peptide Gel Mask 39g single (00012) ×20
 *
 *   node --import dotenv/config scripts/moysklad-create-brau-ladies-peptide-order-invoice-demand-20260708.js
 *   node --import dotenv/config scripts/moysklad-create-brau-ladies-peptide-order-invoice-demand-20260708.js --commit
 */
...
const ORDER = {
  name: `GENCardM${uaeShortDate()}BRAUP20`,
  moment: uaeMomentNow(),
  marker: `BRAU-PEPTIDE-P20-${uaeToday()}`,
}
...
function buildShipmentAddress(agent) {
  ...
  return { country: countryHref(), city: 'Dubai', street: addInfo || 'Brau Ladies Salon LLC — UAE' }
}
...
async function main() {
  console.log('====================================================================')
  console.log('  Brau Ladies Salon LLC — peptide gel mask x20')
  console.log('====================================================================')
  ...
  const order = await api('POST', '/entity/customerorder', {
    name: ORDER.name,
    moment: t0,
    description: [ORDER.marker, 'Peptide Gel Mask 00012 x20.', 'Chain: invoice → shipment.'].join(' | '),
  ...
  if (pdfBuf) {
    ...
    const outPath = path.join(ordersDir, `GENOSYS_Brau_Ladies_${safeInv}.pdf`)
