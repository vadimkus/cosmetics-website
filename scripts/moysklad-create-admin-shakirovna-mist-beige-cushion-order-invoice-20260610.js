...
/**
 * Admin Shakirovna Salon — retail: order → invoice → shipment + landscape PDF.
 *
 * Lines (genosys.ae retail, VAT incl.):
 *   Microbiome Mist (00188) ×1 @ 80 AED (genosys.ae retail)
 *   Beige BB cushion (00144) ×1 @ 150 AED
 *   Excellent Delivery Dubai ×1 @ 10 AED
 *
...
const ORDER = {
  name: `GENCardM${uaeShortDate()}SHK`,
  moment: uaeMomentNow(),
  marker: `ADMIN-SHAKIROVNA-MIST-BEIGE-CUSHION-${uaeToday()}`,
}

/** [code, qty, retailAed] */
const PRODUCT_LINES = [
  ['00188', 1, 80], // Microbiome Energy Infusing Mist 80ml — retail 80 AED
  ['00144', 1, 150], // Skin Caring Blemish Balm Cushion #2 Beige
]
const DELIVERY_AED = 10

/** Posted 2026-06-10 with mist @ 160 (wrong) — amend via --fix-existing-prices */
const EXISTING_DOCS = {
  customerOrderId: '31d33ad7-648e-11f1-0a80-1ccc000fe713',
  invoiceOutId: '3210093a-648e-11f1-0a80-026e000f6a1d',
  demandId: '32febb34-648e-11f1-0a80-1ba8000f86f8',
}
...
async function fixExistingRetailPrices() {
  ...
  console.log('  Amending GENCardM260610SHK / invoice 04648 — mist 160 → 80 AED...')
...
function ordersPdfPath(invoiceName) {
  ...
  return path.join(ordersDir, `GENOSYS_Admin_Shakirovna_${safe}.pdf`)
}

async function main() {
  if (FIX_EXISTING_PRICES) {
    console.log('====================================================================')
    console.log('  Admin Shakirovna — fix mist retail price (160 → 80 AED)')
    console.log('====================================================================')
  ...
  console.log('====================================================================')
  console.log('  Admin Shakirovna — order → invoice → shipment + retail PDF')
  console.log('====================================================================')
  ...
  for (const [code, qty, retailAed] of PRODUCT_LINES) {
    const item = stock.get(code)
    console.log(
      `    ${code} ${item.name.slice(0, 55)} x${qty} @ ${retailAed.toFixed(2)} → ${(retailAed * qty).toFixed(2)} AED`
    )
  }
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      'Mist 00188 x1, Beige cushion 00144 x1, delivery 10 AED. Chain: invoice → shipment (from invoice).',
    ].join(' | '),
  ...
  console.log(`    Shipment sum: ${money(demandRead.sum)} AED`)
