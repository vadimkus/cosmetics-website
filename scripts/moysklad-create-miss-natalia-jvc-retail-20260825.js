...
/**
 * Miss Natalia (JVC) — new retail customer, SO → invoice → shipment (unpaid).
 *
 *   00022 Snow Booster Toner 200ml ×1 @ 260 −10%
 *   54473 Revita Glow BB #02 Natural ×1 @ 290 −10%
 *   00144 Cushion #2 Beige ×1 @ 330 −10%
 *   00044 ND Cell Anti-Wrinkle Cream ×1 @ 380 −10%
 *   Excellent Delivery Dubai ×1 @ 45 (no discount)
 *   Total: 1,179.00 AED
 *
 *   node --import dotenv/config scripts/moysklad-create-miss-natalia-jvc-retail-20260825.js
 *   node --import dotenv/config scripts/moysklad-create-miss-natalia-jvc-retail-20260825.js --commit
 */
...
const CUSTOMER = {
  ...
  phone: '+971507646784',
  city: 'Dubai',
  street: 'Herad Tower, Apt 1604, JVC',
}

const ORDER = {
  name: `GENCardM${uaeShortDate()}6784`,
  moment: uaeMomentNow(),
  marker: `MISS-NATALIA-JVC-RETAIL-10-${uaeToday()}`,
}
...
const EXPECTED_SUM_MINOR = 117900
...
async function findExistingCounterparty() {
  for (const ph of [CUSTOMER.phone, '+971 50 764 6784', '0507646784', '507646784']) {
...
async function exportInvoicePdf(invoiceId, invoiceName) {
  ...
  const out = path.join(ORDERS_DIR, `GENOSYS_Miss_Natalia_JVC_${safe}.pdf`)
...
async function main() {
  console.log('====================================================================')
  console.log('  Miss Natalia JVC — retail −10% + delivery 45, unpaid')
  console.log('====================================================================')
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      '00022 Snow Booster 200 x1 @260 −10%; 54473 Revita Natural x1 @290 −10%; 00144 Beige x1 @330 −10%; 00044 ND Cell x1 @380 −10%; delivery Dubai 45 no discount.',
      `Total 1179 AED unpaid. Ship: ${CUSTOMER.street}, ${CUSTOMER.city}. Phone ${CUSTOMER.phone}.`,
    ].join('\n'),
