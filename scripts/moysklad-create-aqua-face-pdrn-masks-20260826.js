...
/**
 * Aqua Face Facialbar Ltd — unpaid clinic SO → invoice → shipment.
 *
 *   54467 Skin Reboot PDRN mask Pack ×2 @ 200
 *   Excellent Delivery Dubai ×1 @ 45
 *   Total: 445.00 AED
 *
 *   node --import dotenv/config scripts/moysklad-create-aqua-face-pdrn-masks-20260826.js
 *   node --import dotenv/config scripts/moysklad-create-aqua-face-pdrn-masks-20260826.js --commit
 */
...
const AGENT_ID = '1404a0d3-3a3a-11f1-0a80-07a600082120'
...
const ORDER = {
  name: `GENCardM${uaeShortDate()}6594`,
  marker: `AQUA-FACE-PDRN-MASK-X2-DEL45-${uaeToday()}`,
}

const PRODUCT_LINES = [['54467', 2, 200]]
...
async function exportInvoicePdf(invoiceId, invoiceName) {
  ...
  const outPath = path.join(ORDERS_DIR, `GENOSYS_Aqua_Face_${safe}.pdf`)
...
async function main() {
  console.log('====================================================================')
  console.log('  Aqua Face — PDRN masks ×2 + delivery 45 (clinic, unpaid)')
  console.log('====================================================================')
  console.log(`  Mode: ${COMMIT ? 'COMMIT' : 'DRY RUN'}`)

  const agent = await api('GET', `/entity/counterparty/${AGENT_ID}`)
  if (agent.name !== 'Aqua Face Facialbar Ltd') throw new Error(`Unexpected agent: ${agent.name}`)
  ...
  console.log(`    DELIV Excellent Delivery Dubai x1 @ ${DELIVERY_AED} = ${money(deliveryMinor)}`)
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      '54467 PDRN mask pack x2 @200 clinic; delivery Dubai 45. Unpaid.',
    ].join(' | '),
