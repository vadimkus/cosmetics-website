 ...
 * Spreadsheet 27 Aug 2026 (Товар / Количество), clinic list, agr. 00030.
 *
 *   00190 Anti-Wrinkle Cream 50g ×1 @ 145
 *   00194 Radiance Serum 30ml ×1 @ 165
 *   00189 Overnight Cream Mask 100g ×2 @ 170
 *   00144 Beige cushion ×1 @ 150
 *   54467 PDRN Mask Pack ×1 @ 200
 *   00140 Sea Algae Mask ×4 @ 18
 *   Total: 1,072 AED / 10 pcs
 *
 *   node --import dotenv/config scripts/moysklad-create-shakirovna-marina-consignment-sales-20260827.js
 *   node --import dotenv/config scripts/moysklad-create-shakirovna-marina-consignment-sales-20260827.js --commit
 */
...
const MARKER = `SHAKIROVNA-MARINA-CONS-SALES-SHEET-10PCS-${uaeToday()}`
const PERIOD_START = `${uaeToday()} 00:00:00`
...
const EXPECTED_SUM_MINOR = 107200
...
async function exportSalesPdf(reportId, reportName) {
  ...
  const out = path.join(ORDERS_DIR, `GENOSYS_Shakirovna_Marina_Consignment_Sales_${reportName}.pdf`)
...
async function main() {
  console.log('====================================================================')
  console.log('  Shakirovna Marina — consignment sales 27 Aug sheet')
  console.log('====================================================================')
  ...
  console.log(`  Period  : ${PERIOD_START.slice(0, 10)}`)
  ...
  const report = await api('POST', '/entity/commissionreportin', {
    ...
    description: [
      MARKER,
      'Shakirovna Ladies Beauty Saloon | Marina | Agreement 00030.',
      'Consignment sold from 27 Aug spreadsheet (Товар / Количество). Clinic prices.',
