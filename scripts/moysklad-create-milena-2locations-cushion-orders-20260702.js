...
/**
 * Milena — 2 locations, separate SO + invoice + shipment each (clinic prices @ 150):
 *   Location 1 — Milena AESTHETIC CLINIC LLC (Al Wasl): Beige x5, Camel x5 → 1,500 AED
 *   Location 2 — Milena JBR Branch (Rimal): Beige x3, Camel x3 → 900 AED
 *
 *   node --import dotenv/config scripts/moysklad-create-milena-2locations-cushion-orders-20260702.js
 *   node --import dotenv/config scripts/moysklad-create-milena-2locations-cushion-orders-20260702.js --commit
 */
...
const ORDERS = [
  {
    agentId: '10d5bed0-c15e-11f0-0a80-1aae00102afc',
    label: 'Wasl',
    location: 'Location 1 — Al Wasl, Jumeirah',
    nameSuffix: 'MILW',
    marker: `MILENA-WASL-CUSHIONS-${uaeToday()}`,
    lines: [
      ['00144', 5],
      ['54464', 5],
    ],
    expectedSumMinor: 150000,
  },
  {
    agentId: 'b16bd870-da6d-11f0-0a80-1902000d2f93',
    label: 'JBR',
    location: 'Location 2 — Rimal, JBR',
    nameSuffix: 'MILJ',
    marker: `MILENA-JBR-CUSHIONS-${uaeToday()}`,
    lines: [
      ['00144', 3],
      ['54464', 3],
    ],
    expectedSumMinor: 90000,
  },
]
...
function buildShipmentAddress(agent) {
  ...
  const street = full?.addInfo || agent.actualAddress || `${agent.name} — Dubai`
...
async function runCycle(orderDef, idx, stock) {
  const agent = await api('GET', `/entity/counterparty/${orderDef.agentId}`)
  ...
  if (!COMMIT) return { name, sumMinor, label: orderDef.label }

  await ensureNoDuplicate(orderDef.marker, orderDef.agentId)
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      orderDef.marker,
      orderDef.location,
      'Clinic cushion prices @ 150 AED. Chain: invoice → shipment.',
    ].join(' | '),
  ...
  if (pdfBuf) {
    ...
    const outPath = path.join(ORDERS_DIR, `GENOSYS_Milena_${orderDef.label}_${safeInv}.pdf`)
...
async function main() {
  console.log('====================================================================')
  console.log('  Milena — 2 locations, cushion orders (clinic @ 150 AED)')
  console.log('====================================================================')
  ...
  for (let i = 0; i < ORDERS.length; i++) {
    results.push(await runCycle(ORDERS[i], i, stock))
  }
  ...
  console.log('\n  ✓ Both locations posted.')
  for (const r of results) {
    console.log(`    ${r.label}: SO ${r.order} | inv ${r.invoice} | ship ${r.demand} | ${r.sum} AED`)
  }
}
