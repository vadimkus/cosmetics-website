...
/**
 * Create MoySklad counterparty: MEDYUMED MEDICAL CLINIC L.L.C
 * Field layout matches FACE ROOM / CEIA:
 *   email + fax = trade license #
 *   legalAddressFull.comment = VAT TRN
 *
 * Source docs:
 *   ~/Desktop/Drive/Genosys/Contract_Customers/MedUmed/
 *   - MEDYMED MAIN LICENSE UPDATED (1).pdf
 *   - VAT Registration Certificate.pdf
 *
 *   node --import dotenv/config scripts/moysklad-create-medumed-customer-20260810.js
 *   node --import dotenv/config scripts/moysklad-create-medumed-customer-20260810.js --commit
 */
...
const ADDRESS = 'R-10, Blue Waters Residential, Bluewaters Island, Dubai, United Arab Emirates'

const CUSTOMER = {
  name: 'MEDYUMED MEDICAL CLINIC L.L.C',
  phone: '+971554772214',
  licenseNo: '1119644', // Dubai Economy and Tourism (DET)
...
async function findExisting() {
  ...
  for (const q of ['MEDYUMED', 'MedUmed', 'Medyumed', 'Yulia Shchekleina']) {
    const bySearch = await api('GET', `/entity/counterparty?search=${encodeURIComponent(q)}&limit=15`)
    const hit = (bySearch?.rows || []).find((r) => /MEDYUMED|MedUmed|Medyumed/i.test(r.name || ''))
...
async function main() {
  console.log('====================================================================')
  console.log('  MoySklad — create counterparty MEDYUMED MEDICAL CLINIC L.L.C')
  console.log('====================================================================')
  console.log(`  Mode    : ${COMMIT ? 'COMMIT' : 'DRY RUN'}`)
  console.log(`  Name    : ${CUSTOMER.name}`)
  console.log(`  Phone   : ${CUSTOMER.phone}`)
  console.log(`  Owner   : ${CUSTOMER.owner}`)
  ...
  const payload = {
    ...
    description: [
      'Clinic customer — MEDYUMED MEDICAL CLINIC L.L.C',
      `Owner/Manager ${CUSTOMER.owner}`,
      `DET license ${CUSTOMER.licenseNo}`,
      `VAT TRN ${CUSTOMER.trn} (effective 01/01/2023)`,
      'Agreement No. 39 (consignment)',
      'Sources: Contract_Customers/MedUmed trade license + VAT PDFs',
