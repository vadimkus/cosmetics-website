...
/**
 * Viktoriia Klymenko — unpaid clinic SO → invoice → shipment.
 * WhatsApp: Viktoriya Cosmetologist Genosys +971 52 545 1403
 *
 *   00143 Ivory cushion ×1 @ 150
 *   54475 PDRN Homecare 5000 ×1 @ 150
 *   00188 Microbiome mist ×5 @ 80
 *   Total: 700.00 AED
 *
 *   node --import dotenv/config scripts/moysklad-create-viktoriia-klymenko-clinic-20260827.js
 *   node --import dotenv/config scripts/moysklad-create-viktoriia-klymenko-clinic-20260827.js --commit
 */
...
const ORDER = {
  name: `GENCardM${uaeShortDate()}VIKT`,
  marker: `VIKTORIIA-KLYMENKO-IVORY-PDRN5000-MIST-X5-${uaeToday()}`,
}
...
const EXPECTED_SUM_MINOR = 70000
...
function shipmentAddress() {
  return {
    ...
    city: 'Dubai',
    street: 'Tamani Arts offices building, office 2013',
    addInfo: '',
  }
}
...
async function exportInvoicePdf(invoiceId, invoiceName) {
  ...
  const outPath = path.join(ORDERS_DIR, `GENOSYS_Viktoriia_Klymenko_${safe}.pdf`)
...
async function main() {
  console.log('====================================================================')
  console.log('  Viktoriia Klymenko — ivory + PDRN 5000 + mist ×5 (clinic, unpaid)')
  console.log('====================================================================')
  console.log(`  Mode: ${COMMIT ? 'COMMIT' : 'DRY RUN'}`)

  const agent = await api('GET', `/entity/counterparty/${AGENT_ID}`)
  if (agent.name !== 'Viktoriia Klymenko') throw new Error(`Unexpected agent: ${agent.name}`)
  if (!String(agent.phone || '').includes('525451403')) throw new Error(`Unexpected phone: ${agent.phone}`)
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      '00143 Ivory x1 @150; 54475 PDRN 5000 x1 @150; 00188 mist x5 @80 clinic. Unpaid.',
    ].join(' | '),
