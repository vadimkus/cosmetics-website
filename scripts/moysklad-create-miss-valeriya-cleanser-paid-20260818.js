...
/**
 * Miss Valeriya (Royal Atlantis, apt 1201) — paid retail SO → invoice → shipment → paymentin.
 *
 *   00021 Snow O₂ Cleanser 180ml ×1 @ 330
 *   Delivery Dubai ×1 @ 45
 *   Total: 375 AED
 *
 *   Agent: 74932015-eaca-11f0-0a80-08de007a1ccf (The Royal Atlantis Residence, app 1201)
 *
 *   node --import dotenv/config scripts/moysklad-create-miss-valeriya-cleanser-paid-20260818.js
 *   node --import dotenv/config scripts/moysklad-create-miss-valeriya-cleanser-paid-20260818.js --commit --print
 */
...
const AGENT_ID = '74932015-eaca-11f0-0a80-08de007a1ccf'
...
const ORDER = {
  ...
  moment: uaeMomentNow(),
  marker: `MISS-VALERIYA-CLEANSER-180-RETAIL-${uaeToday()}`,
}

const PRODUCT_LINES = [['00021', 1, 330]]
const DELIVERY_AED = 45
const EXPECTED_SUM_MINOR = 37500
...
async function exportInvoicePdf(invoiceId, invoiceName) {
  ...
  const outPath = path.join(ORDERS_DIR, `GENOSYS_Miss_Valeriya_${safe}.pdf`)
...
async function main() {
  console.log('====================================================================')
  console.log('  Miss Valeriya — cleanser 180 + delivery, paid + print')
  console.log('====================================================================')
  console.log(`  Mode: ${COMMIT ? 'COMMIT' : 'DRY RUN'}${DO_PRINT ? ' + PRINT' : ''}`)

  const agent = await api('GET', `/entity/counterparty/${AGENT_ID}`)
  console.log(`  Customer: ${agent.name} | ${agent.phone || '—'}`)
  console.log(`  Address: ${agent.actualAddress || SHIPMENT.street}`)
  if (agent.name !== 'Miss Valeriya') throw new Error(`Unexpected agent: ${agent.name}`)
  if (!/atlantis/i.test(agent.actualAddress || '') || !/1201/.test(agent.actualAddress || '')) {
    throw new Error(`Address mismatch: ${agent.actualAddress}`)
  }
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      '00021 Snow O2 Cleanser 180ml x1 @ 330; delivery Dubai 45. Paid.',
