...
/**
 * Salary payout to Vadim Sagatdinov — 23,000.00 AED (July 2026).
 *
 *   node --import dotenv/config scripts/moysklad-create-salary-paymentout-july-23000-20260718.js
 *   node --import dotenv/config scripts/moysklad-create-salary-paymentout-july-23000-20260718.js --commit
 */
...
const SUM_MINOR = 2300000 // 23,000.00 AED
const MARKER = `SALARY-SAGATDINOV-JULY-23000-${uaeToday()}`
...
async function ensureNoDuplicate() {
  ...
  const july = await api(
    'GET',
    `/entity/paymentout?search=${encodeURIComponent('Salary July 2026')}&limit=10&expand=agent,expenseItem`
  )
  const hit = (july.rows || []).find(
    (r) =>
...
async function main() {
  console.log('====================================================================')
  console.log('  Salary July 2026 → Vadim Sagatdinov')
  console.log('====================================================================')
  ...
  const created = await api('POST', '/entity/paymentout', {
    ...
    sum: SUM_MINOR,
    paymentPurpose: `Salary July 2026 ${money(SUM_MINOR)} AED — Sagatdinov [${MARKER}]`,
    description: `Salary July 2026 — Sagatdinov | ${MARKER}`,
  })
