 ...
 * Section order:
 *
 *   effects  Cool · Soothe · Peel
 *   engine   diatomaceous earth 65% + algin set + peppermint cool
 *   howTo    pack mix 30g at 1 : 0.8, peel, plus the product video
 ...
 *   routine  cleanse → mist → mask 35 → hyaluron cream
 *
 * There is no safety assessment and no DTS MG clinical deck under this SKU
 * name. See hydroCoolCopy.ts. In particular: do not rebuild the page around
 * hyaluronic acid, ceramide or centella at 0.01%. This is the cooling clinic
 * kilo. Bio-Ferment (51) is the mask that holds moisture.
 */

import '../cerabarrier/cerabarrier.css'
import PageBreadcrumb from '@/components/PageBreadcrumb'
import './hydrocool.css'
...
import { getHydroCoolCopy } from './hydroCoolCopy'
...
export default function HydroCoolProductPage({
  product,
  unitsSold = 0,
  routineProducts = [],
}: Props) {
  ...
  const isRtl = dir === 'rtl'
  const copy = getHydroCoolCopy(locale)
  ...
  const routineSteps = useMemo(() => {
    const routine = PRODUCT_ROUTINES['35']
    if (!routine) return []
  ...
  const addToCart = useCallback(
    async (qty: number) => {
      ...
      try {
        ...
        errorLog('HydroCool: add to cart failed', error)
        throw error
      }
    },
  ...
  const handleAddRoutineProduct = useCallback(
    async (item: Product, selection?: ProductOptionSelection, quantity = 1) => {
      ...
      try {
        ...
        errorLog('HydroCool: routine add to cart failed', error)
      } finally {
    ...
    <div className={`cera-page hc-page ${ceraSerif.variable} min-h-[100dvh]`} dir={dir}>
      ...
      <section className="mx-auto max-w-[1000px] px-4 py-16 sm:px-6 lg:py-24">
        ...
        <div className="mt-10 grid grid-cols-1 gap-4 lg:mt-14 lg:grid-cols-2 lg:gap-6">
          ...
          <CeraReveal delay={90} className="cera-card hc-not p-6 lg:p-7">
            <h3 className="cera-serif text-[21px] leading-tight text-[var(--cera-ink)]">
