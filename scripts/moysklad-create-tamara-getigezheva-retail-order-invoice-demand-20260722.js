...
/**
 * Miss Tamara Getigezheva — new customer, retail SO → invoice → shipment + PDF.
 *
 *   54467 Skin Reboot PDRN mask Pack ×1 @ 400 AED retail
 *   Excellent Delivery Dubai ×1 @ 45 AED
 *   Total: 445 AED
 *
 *   Ship: Apt 1004, The Royal Atlantis Resorts and Residences, Palm Jumeirah, Dubai
 *
 *   node --import dotenv/config scripts/moysklad-create-tamara-getigezheva-retail-order-invoice-demand-20260722.js
 *   node --import dotenv/config scripts/moysklad-create-tamara-getigezheva-retail-order-invoice-demand-20260722.js --commit
 */
...
const CUSTOMER = {
  name: 'Miss Tamara Getigezheva',
  phone: '+971557839148',
  city: 'Dubai',
  street: 'Apartment 1004, The Royal Atlantis Resorts and Residences, Palm Jumeirah',
}

const ORDER = {
  name: `GENCardM${uaeShortDate()}9148`,
  moment: uaeMomentNow(),
  marker: `TAMARA-GETIGEZHEVA-PDRN-MASK-${uaeToday()}`,
}

/** [code, qty, retailAed VAT incl.] */
const PRODUCT_LINES = [['54467', 1, 400]]
const DELIVERY_AED = 45
const EXPECTED_SUM_MINOR = 44500
...
async function findExistingCounterparty() {
  for (const ph of [CUSTOMER.phone, '+971 55 783 9148', '0557839148', '557839148']) {
    const d = await api('GET', `/entity/counterparty?filter=phone=${encodeURIComponent(ph)}&limit=5`)
    if (d?.rows?.length) return d.rows[0]
  }
  const d = await api('GET', `/entity/counterparty?search=${encodeURIComponent('Getigezheva')}&limit=10`)
  return (d?.rows || []).find((r) => /getigezheva/i.test(r.name || '')) || null
}
...
async function findOrCreateCounterparty() {
  ...
  const created = await api('POST', '/entity/counterparty', {
    ...
    description: 'Retail customer — Royal Atlantis, Palm Jumeirah, apt 1004.',
...
function ordersPdfPath(invoiceName) {
  ...
  return path.join(ordersDir, `GENOSYS_Tamara_Getigezheva_${safe}.pdf`)
}

async function main() {
  console.log('====================================================================')
  console.log('  Miss Tamara Getigezheva — retail SO + invoice + shipment')
  console.log('====================================================================')
  ...
  for (const [code, qty, retailAed] of PRODUCT_LINES) {
    const item = stock.get(code)
    console.log(`    ${code} ${item.name.slice(0, 60)} x${qty} @ ${retailAed.toFixed(2)} retail`)
  }
  ...
  const orderDesc = [
    ORDER.marker,
    '54467 Skin Reboot PDRN mask Pack x1 @ 400; Dubai delivery 45.',
