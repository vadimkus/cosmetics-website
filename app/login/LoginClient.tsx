...
export default function LoginClient() {
    ...
    return (
      <div className={`min-h-[100dvh] bg-white flex flex-col ${isRTL ? 'rtl' : 'ltr'}`} dir={dir}>
        ...
        <div className="flex-1 px-6 pb-8">
          {/* Social + App buttons — only on Sign In. The Create Account flow
              is deliberately focused on the manual form (no auth shortcuts,
              no app store distractions), so users commit to one clear path. */}
          {isLoginMode && (
            <>
              ...
              <a
                ...
                className={`w-full flex items-center justify-center gap-2 py-3 px-4 mb-3 bg-black text-white rounded-xl hover:bg-black/90 transition-colors ${isRTL ? 'flex-row-reverse' : ''}`}
              >
                <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
                  <path d="M17.05 20.28c-.98.95-2.05.8-3.08.35-1.09-.46-2.09-.48-3.24 0-1.44.62-2.2.44-3.06-.35C2.79 15.25 3.51 7.59 9.05 7.31c1.35.07 2.29.74 3.08.8 1.18-.24 2.31-.93 3.57-.84 1.51.12 2.65.72 3.4 1.8-3.12 1.87-2.38 5.98.48 7.13-.57 1.5-1.31 2.99-2.54 4.09l.01-.01zM12.03 7.25c-.15-2.23 1.66-4.07 3.74-4.25.29 2.58-2.34 4.5-3.74 4.25z"/>
                </svg>
                <span className="font-medium text-sm">{t('login.downloadAppApple')}</span>
              </a>
              ...
              >
                ...
                <span className="font-medium text-sm">{t('login.downloadAppGoogle')}</span>
              </a>
          ...
          {/* Create Account – subtle value-prop strip (visible only in signup mode) */}
          {!isLoginMode && (
            <div className={`mb-5 rounded-xl border border-primary-100 bg-primary-50/60 px-4 py-3 ${isRTL ? 'text-right' : ''}`}>
  ...
  return (
    <div
      className={`${dir === 'rtl' ? 'text-right' : ''} lg:grid lg:grid-cols-2 lg:min-h-[calc(100vh-64px)] lg:bg-gray-50`}
      dir={dir}
    >
      {/* ─── Brand panel (desktop-only at lg+) ─────────────────────────────
          Replaces the bare "form floating on white" desktop treatment with a
          split-screen layout. On <lg we hide this panel entirely and fall back
          to the single-column layout the page has always had. */}
      <aside
        className="hidden lg:flex relative flex-col justify-between overflow-hidden bg-gradient-to-br from-gray-900 via-gray-900 to-primary-900 text-white p-12 xl:p-16"
        aria-hidden="true"
      >
        {/* Decorative gradient blobs — purely visual */}
        <div className="pointer-events-none absolute -top-24 -right-24 w-96 h-96 rounded-full bg-primary-500/20 blur-3xl" />
        <div className="pointer-events-none absolute -bottom-32 -left-20 w-[28rem] h-[28rem] rounded-full bg-primary-700/20 blur-3xl" />
        ...
        <div className="relative z-10">
          ...
          />
          <p className="mt-4 text-[11px] tracking-[0.18em] font-semibold text-primary-300 uppercase">
            {locale === 'ar'
        ...
        <div className="relative z-10 max-w-md">
          <h2 className="text-4xl xl:text-[44px] xl:leading-[1.1] font-bold font-display tracking-tight">
            {locale === 'ar'
          ...
          <ul className="mt-8 space-y-3 text-sm">
            {[
              ...
              <li key={i} className="flex items-start gap-3">
                <span className="mt-0.5 flex h-5 w-5 flex-shrink-0 items-center justify-center rounded-full bg-primary-500/25 ring-1 ring-primary-300/40">
                  <svg className="h-3 w-3 text-primary-200" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                  </svg>
                </span>
                <span className="text-gray-200 leading-snug">{line}</span>
              </li>
            ))}
          </ul>
        </div>

        <div className="relative z-10 flex items-center gap-6 text-xs text-gray-400">
      ...
      {/* ─── Form panel ──────────────────────────────────────────────────── */}
      <div className={`container mx-auto px-3 md:px-4 py-4 md:py-8 lg:py-12 lg:px-8 lg:max-w-none lg:mx-0 lg:flex lg:flex-col lg:justify-center lg:bg-gray-50 min-h-0`}>
      {/* Navigation Breadcrumb — keep for <lg, hide at lg+ (brand panel covers it) */}
      <nav className={`text-xs md:text-base text-gray-600 mb-2 md:mb-4 lg:hidden ${dir === 'rtl' ? 'text-right' : ''}`} aria-label="Breadcrumb">
      ...
      {/* Back to Home — hidden on lg+ */}
      <Link
        href={getLocalizedPath('/', locale)}
        className={`inline-flex items-center gap-1 text-xs md:text-sm text-primary-600 hover:text-primary-700 mb-3 md:mb-6 lg:hidden ${dir === 'rtl' ? 'flex-row-reverse' : ''}`}
      >
        <ArrowLeft className="h-3 w-3 md:h-4 md:w-4" />
        <span>{t('login.backToHome')}</span>
      </Link>

      <div className="max-w-md mx-auto lg:mx-auto lg:w-full">
        ...
        <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-4 md:p-6">
          ...
          <div className="space-y-3 md:space-y-4">
            ...
            {/* Value-prop strip for Create Account — keeps the desktop form
                clean and brand-led while users fill the form. */}
            {!isLoginMode && (
            ...
            {/* Social + app buttons — only on Sign In. Create Account is
                intentionally form-only so users commit to one clear path. */}
            {isLoginMode && (
              <>
                ...
                <button
                  ...
                  disabled={isLoading}
                  className={`w-full flex items-center justify-center gap-2 md:gap-3 bg-white border-2 border-gray-300 text-gray-700 py-2.5 md:py-3 rounded-lg font-semibold hover:bg-gray-50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed shadow-sm ${dir === 'rtl' ? 'flex-row-reverse' : ''}`}
                >
                ...
                <button
                  ...
                  disabled={isLoading}
                  className={`w-full flex items-center justify-center gap-2 md:gap-3 bg-black text-white py-2.5 md:py-3 rounded-lg font-semibold hover:bg-black/90 transition-colors disabled:opacity-50 disabled:cursor-not-allowed shadow-sm ${dir === 'rtl' ? 'flex-row-reverse' : ''}`}
                >
            ...
            <form onSubmit={handleSubmit} className="space-y-3 md:space-y-4">
              ...
              {!isLoginMode && (
                <>
                  ...
                  <select
                    ...
                    value={formData.emirate}
                    onChange={handleInputChange}
                    className={`w-full px-3 py-2.5 md:py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 text-xs md:text-sm bg-white ${formData.emirate ? 'text-gray-900' : 'text-gray-400'} ${dir === 'rtl' ? 'text-right' : ''}`}
                    required={!isLoginMode}
                  >
                    <option value="">{t('login.selectEmirate')}</option>
                    {EMIRATES.map(e => (
                  ...
                  <div>
                    <label className={`block text-[11px] md:text-xs font-medium text-gray-600 mb-1 ${dir === 'rtl' ? 'text-right' : ''}`}>
                      {t('login.birthday')}
                    </label>
                    <input
                      type="date"
              ...
              <div className="relative">
                <input
                  ...
                  value={formData.password}
                  onChange={handleInputChange}
                  className={`w-full px-3 py-2.5 md:py-2 ${dir === 'rtl' ? 'pl-10' : 'pr-10'} border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 text-xs md:text-sm bg-white text-gray-900 ${dir === 'rtl' ? 'text-right' : ''}`}
                  required
                />
                <button
                  type="button"
                ...
                <div className="space-y-2.5">
                  <div className="bg-gray-50 border border-gray-200 rounded-lg p-3 overflow-hidden">
                    <div className={`flex items-center justify-between ${dir === 'rtl' ? 'flex-row-reverse' : ''}`}>
                      <h4 className={`font-semibold text-gray-800 text-xs md:text-sm ${dir === 'rtl' ? 'text-right' : ''}`}>{t('login.privacyPolicy')}</h4>
                      <button
                        type="button"
                        onClick={() => setShowPrivacyPolicy(!showPrivacyPolicy)}
                        className="text-primary-600 hover:text-primary-700 text-[11px] md:text-xs font-medium underline flex-shrink-0"
                      >
                        {showPrivacyPolicy ? t('login.hideDetails') : t('login.viewDetails')}
                      </button>
                    </div>
                    {showPrivacyPolicy && (
                      <div className="mt-2.5 pt-2.5 border-t border-gray-200 max-h-40 overflow-y-auto">
                        <div className={`text-[11px] text-gray-600 leading-relaxed space-y-2 pr-1 ${dir === 'rtl' ? 'text-right pl-1 pr-0' : ''}`}>
                  ...
                  <div className={`flex items-start gap-2 ${dir === 'rtl' ? 'flex-row-reverse' : ''}`}>
                    <input
                      ...
                      required
                    />
                    <label htmlFor="privacy-consent" className={`text-[11px] md:text-xs text-gray-600 leading-snug ${dir === 'rtl' ? 'text-right' : ''}`}>
              ...
              <button
                type="submit"
                disabled={isLoading}
                className="w-full bg-primary-600 text-white py-2.5 md:py-3 text-sm md:text-base rounded-lg font-semibold hover:bg-primary-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed shadow-sm"
              >
