...
/**
 * HORTMAN CLINICS 2 L.L.C — SO → invoice → shipment.
 *
 * Lines: Peptide Gel Mask 39g (00012) ×100 @ clinic list (38 AED ex-VAT).
 *
 ...
 * PDF: Genosys_Invoice_Legal_TAX (not RETAIL_PRINT).
 *
 *   node --import dotenv/config scripts/moysklad-create-hortman-clinics2-peptide-order-invoice-demand-20260701.js
 *   node --import dotenv/config scripts/moysklad-create-hortman-clinics2-peptide-order-invoice-demand-20260701.js --commit
 */
...
const HORTMAN_VAT_INCLUDED = false // clinic list ex-VAT; 5% added on top
...
function buildPositions(stock) {
  ...
  if (!stock.price) throw new Error('No salePrice for 00012')
  const positions = [
    {
      quantity: QTY,
      price: stock.price,
...
async function main() {
  console.log('====================================================================')
  console.log('  HORTMAN CLINICS 2 L.L.C — peptide mask SO + invoice + shipment')
  console.log('====================================================================')
  ...
  console.log(`\n  Order: ${ORDER.name}`)
  console.log(`    00012 ${stock.name} x${QTY} @ ${money(stock.price)} AED (ex-VAT line)`)
  console.log(`    Line base: ${money(sumMinor)} AED → VAT incl. ~${money(Math.round(sumMinor * 1.05))} AED`)
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.paymentNote,
      ORDER.marker,
      'Peptide Gel Mask 39g (00012) x100.',
