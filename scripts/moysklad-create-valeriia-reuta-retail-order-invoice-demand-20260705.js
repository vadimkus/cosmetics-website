...
/**
 * Miss Valeriia Reuta — retail SO → invoice → shipment + PDF.
 *
 *   00021 Snow O₂ Cleanser 180ml ×1 @ 330 −10%
 *   00022 Snow Booster Toner 200ml ×1 @ 260 −10%
 *   54458 Hyaluron Cream 50g ×1 @ 290 −10%
 *   54470 Bio Meso PDRN Expert 60000 ×1 @ 600 −10%
 *   Delivery Dubai ×1 @ 45
 *   Total: 1,377 AED
 *
 *   node --import dotenv/config scripts/moysklad-create-valeriia-reuta-retail-order-invoice-demand-20260705.js
 *   node --import dotenv/config scripts/moysklad-create-valeriia-reuta-retail-order-invoice-demand-20260705.js --commit
 */
...
const ORDER = {
  name: `GENCardM${uaeShortDate()}8965`,
  moment: uaeMomentNow(),
  marker: `VALERIIA-REUTA-RETAIL-${uaeToday()}`,
}

/** [code, qty, retailAed VAT incl., discountPercent] */
const PRODUCT_LINES = [
  ['00021', 1, 330, 10],
  ['00022', 1, 260, 10],
  ['54458', 1, 290, 10],
...
const EXPECTED_SUM_MINOR = 137700
...
async function findExistingCounterparty() {
  const bySearch = await api('GET', `/entity/counterparty?search=${encodeURIComponent(CUSTOMER.phone)}&limit=10`)
  return (
    (bySearch?.rows || []).find((r) => r.phone === CUSTOMER.phone || r.name === CUSTOMER.name) || null
  )
}
...
async function findOrCreateCounterparty() {
  ...
  if (existing) {
    console.log(`  Customer (existing): ${existing.name} (${existing.id})`)
    return existing
  }
  if (!COMMIT) {
  ...
  console.log(`  Customer (created): ${created.name} (${created.id})`)
  return created
}
...
function buildPositions(stock) {
  ...
  let sumMinor = 0
  console.log('\n  Lines:')
  for (const [code, qty, retailAed, discount] of PRODUCT_LINES) {
    ...
    const lineMinor = Math.round(priceMinor * qty * (1 - discount / 100))
    sumMinor += lineMinor
    positions.push({
      quantity: qty,
      price: priceMinor,
    ...
    console.log(
      `    ${code} ${item.name.slice(0, 42)} x${qty} @ ${retailAed.toFixed(2)} −${discount}% → ${money(lineMinor)}`
    )
  }
  ...
  console.log(`    delivery Dubai x1 @ ${DELIVERY_AED.toFixed(2)} → ${money(deliveryMinor)}`)
...
async function exportInvoicePdf(invoiceId, invoiceName) {
  ...
  const outPath = path.join(ordersDir, `GENOSYS_Valieriia_Reuta_${safe}.pdf`)
...
async function main() {
  console.log('====================================================================')
  console.log('  Miss Valeriia Reuta — retail SO + invoice + shipment')
  console.log('====================================================================')
  ...
  const shipmentAddress = shipmentAddressFull()
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      'Retail 10% off: Snow cleanser, Snow Booster, Hyaluron cream, Bio Meso PDRN 60000.',
      'Delivery Dubai 45 AED. Total 1,377 AED.',
    ].join(' | '),
  ...
  if (pdfPath) console.log(`    Saved: ${pdfPath}`)
}
