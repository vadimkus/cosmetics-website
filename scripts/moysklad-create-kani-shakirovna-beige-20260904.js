...
/**
 * Kanyshai Shakirovna (Kani) — new retail customer. SO → invoice → shipment → paymentin.
 *
 *   00144 Beige cushion ×4 @ 150
 *   Excellent Delivery Dubai ×1 @ 15
 *   Total: 615.00 AED paid
 *
 *   node --import dotenv/config scripts/moysklad-create-kani-shakirovna-beige-20260904.js
 *   node --import dotenv/config scripts/moysklad-create-kani-shakirovna-beige-20260904.js --commit
 */
...
const CUSTOMER = {
  name: 'Kanyshai Shakirovna',
  phone: '0585752633',
  city: 'Dubai',
  street: 'Marina Wharf 1, apartment 2202',
}

const ORDER = {
  name: `GENCardM${uaeShortDate()}KANI`,
  moment: uaeMomentNow(),
  marker: `KANI-SHAKIROVNA-BEIGE4-PAID-${uaeToday()}`,
}

const PRODUCT_LINES = [['00144', 4, 150]]
const DELIVERY_AED = 15
const EXPECTED_SUM_MINOR = 61500
...
function isThisKani(row) {
  const digits = phoneDigits(row.phone)
  const name = (row.name || '').toLowerCase()
  return digits.endsWith('585752633') || name.includes('kanyshai') || name === 'kani shakirovna'
}
...
async function findOrCreateCounterparty() {
  for (const q of ['Kanyshai', 'Kani Shakirovna', '0585752633', '585752633', '058 575 2633']) {
    const d = await api('GET', `/entity/counterparty?search=${encodeURIComponent(q)}&limit=25`)
    const hit = (d.rows || []).find(isThisKani)
    if (hit) {
  ...
  const created = await api('POST', '/entity/counterparty', {
    ...
    description: `Retail customer Kani — Marina Wharf 1 apt 2202. Created with ${ORDER.name}`,
...
async function exportInvoicePdf(invoiceId, invoiceName) {
  ...
  const out = path.join(ORDERS_DIR, `GENOSYS_Kani_Shakirovna_${safe}.pdf`)
...
async function main() {
  console.log('====================================================================')
  console.log('  Kani Shakirovna — beige ×4 + delivery 15, paid')
  console.log('====================================================================')
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      '00144 Beige x4 @150; delivery Dubai 15. Paid.',
