...
export default async function ConcernPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <div className={`cera-page genosys-page ${ceraSerif.variable} min-h-screen`}>
            ...
            <details className="group">
              ...
              <div className="mt-2">
                <a
                  ...
                  className="ed-doc group/link block rounded-2xl border border-[var(--cera-line)] bg-white p-5 shadow-sm transition-all duration-200 hover:border-[var(--cera-blush-deep)] hover:shadow-md sm:p-6"
                >
                  <div className="flex items-center gap-4 sm:gap-5">
                    <div className="flex-shrink-0 w-10 h-10 sm:w-12 sm:h-12 flex items-center justify-center rounded-xl bg-[var(--cera-blush)] transition-colors group-hover/link:bg-[var(--cera-blush-deep)]">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-0.5">
                        <h3 className="text-sm sm:text-base font-semibold text-[var(--cera-ink)] group-hover/link:text-[var(--cera-rose-ink)] transition-colors">
                          {concern.protocolPdf.title.en}
                        </h3>
                        <span className="inline-flex items-center px-1.5 py-0.5 rounded-full text-[10px] font-medium bg-[var(--cera-blush)] text-[var(--cera-rose-ink)]">
                          PDF
                        </span>
                      </div>
                      <p className="text-[var(--cera-muted)] text-xs sm:text-sm leading-relaxed line-clamp-1 sm:line-clamp-none">
                        {concern.protocolPdf.description.en}
                      </p>
                    </div>
                    <div className="flex-shrink-0 hidden sm:flex items-center gap-1.5 text-sm font-medium text-[var(--cera-rose-ink)] group-hover/link:gap-2 transition-all">
                      Download
                  ...
                  {section.steps.map((step) => (
                    <details key={step.step} className="bg-white rounded-xl border border-[var(--cera-line)] overflow-hidden group open:border-[var(--cera-blush-deep)] open:ring-1 open:ring-[var(--cera-blush)] transition-all">
                      <summary className="px-5 py-4 cursor-pointer list-none flex items-center gap-3 hover:bg-[var(--cera-cream-deep)] group-open:bg-[var(--cera-blush)]/40 transition-colors">
                        <span className="flex-shrink-0 w-7 h-7 rounded-full bg-[var(--cera-ink)] group-open:bg-[var(--cera-rose)] text-white text-xs font-bold flex items-center justify-center transition-colors">
                          {step.step}
                        </span>
                        <div className="flex-1 min-w-0">
                          <span className="font-medium text-[var(--cera-ink)] group-open:text-[var(--cera-rose-ink)] transition-colors">{step.title}</span>
                          <span className="text-[var(--cera-muted)] text-sm ml-2">({step.duration})</span>
                        </div>
                        <span className="text-xs text-[var(--cera-muted)] hidden sm:block mr-2 group-open:hidden">{step.summary.slice(0, 60)}…</span>
            ...
            <div className="space-y-4">
              {faq.map((item, i) => (
                <details key={i} className="bg-white rounded-lg border border-[var(--cera-line)] overflow-hidden group">
                  ...
                  <div className="px-6 pb-4 text-[var(--cera-body)] leading-relaxed">
                    {item.answer}
                  </div>
                </details>
              ))}
      ...
      <RelatedConcernCards currentSlug={slug} locale="en" />
      <ConcernStickyBar locale="en" />
    </div>
  )
}
