/**
 * Script to add Russian fields to blog_posts table and translate all blog posts
 * This script can be run directly: npx tsx scripts/setup-russian-blog.ts
 */
...
async function setupRussianBlog() {
  try {
    console.log('🚀 Starting Russian blog setup...\n')

    // Step 1: Add Russian fields to database
    console.log('📝 Step 1: Adding Russian fields to blog_posts table...')
    ...
    try {
      ...
      `
      fieldsExist = true
      console.log('   ✅ Russian fields already exist')
    } catch (error: any) {
      if (error.message?.includes('column') || error.message?.includes('does not exist')) {
        console.log('   📝 Adding Russian fields...')
        ...
        console.log('   ✅ Successfully added Russian fields')
    ...
    for (const post of posts) {
      ...
      if (post.titleRu && post.excerptRu && post.contentRu) {
