...
/**
 * Warehouse write-off @ buyPrice — expired / stock adjustment (2026-06-28).
 *
 *   node --import dotenv/config scripts/moysklad-create-inventory-writeoff-20260628.js
...
const MARKER = `INVENTORY-WRITE-OFF-EXPIRED-${uaeToday()}`
...
const LINES = [
  ...
  ['00020', 20, 'Power Solution SWS 1 Vial 2ml (expired)'],
  ['00015', 20, 'Skin Renewal Peeling System (SRS) 1 Vial 2ml (expired)'],
...
async function main() {
  console.log('====================================================================')
  console.log('  Inventory write-off @ buyPrice (warehouse)')
  console.log('====================================================================')
  ...
  console.log(`\n  Total buy cost: ${money(totalMinor)} AED | ${LINES.reduce((s, [, q]) => s + q, 0)} pcs`)
  ...
  const created = await api('POST', '/entity/loss', {
    ...
    description: [
      ...
      'SPF40 00041 x2, bio ferment 54466 x2, collagen 00063 x10, sea algae 00140 x10, mist 00188 x3, SWS 00020 x20 expired, SRS 00015 x20 expired, Hyaluron cream 50g 54458 x2, Multifunctional cream 50g 00190 x2, eye patch 00053 x1.',
    ].join(' | '),
