...
/**
 * Iryna Solodka (Miss Irina Solodkaya) — retail order + invoice.
 *
 * Lines: Microbiome Mist (00188) ×6, Eye Peptide Gel Patch box (00053) ×2,
 *        Collagen mask 23g (00063) ×10, Sea Algae mask (00140) ×5,
 *        Peptide Gel Mask 39g (00012) ×5.
 * Prices: MoySklad list (salePrice).
 *
 *   node --import dotenv/config scripts/moysklad-create-iryna-solodka-order-invoice-20260528.js
 *   node --import dotenv/config scripts/moysklad-create-iryna-solodka-order-invoice-20260528.js --commit
 */
...
const AGENT_ID = 'fe9bcf0c-ab69-11ed-0a80-0cad00322ed0' // Miss Irina Solodkaya

const ORDER_SUFFIX = '5770' // +380 67 380 5770

const ORDER = {
  name: `GENCardM${uaeShortDate()}${ORDER_SUFFIX}`,
  moment: uaeMomentNow(),
  marker: `Iryna Solodka mist patches masks ${uaeToday()}`,
}
...
async function main() {
  console.log('====================================================================')
  console.log('  Iryna Solodka — order + invoice')
  console.log('====================================================================')
  ...
  const orderPayload = {
    ...
    description: [
      ORDER.marker,
      'Mist x6, Eye patch box x2, Collagen x10, Sea Algae x5, Peptide gel mask x5, Dubai delivery 45 AED',
    ].join(' | '),
