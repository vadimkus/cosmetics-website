...
/**
 * Admin Shakirovna Salon — unpaid clinic SO → invoice → shipment.
 *
 *   00144 Cushion #2 Beige ×1 @ 150
 *   00194 Multi Vita Radiance Serum 30ml ×3 @ 165
 *   Delivery Dubai ×1 @ 45, 100% off
 *   Total: 645.00 AED
 *
 *   node --import dotenv/config scripts/moysklad-create-admin-shakirovna-beige-radiance-20260822.js
 *   node --import dotenv/config scripts/moysklad-create-admin-shakirovna-beige-radiance-20260822.js --commit
 */
...
const AGENT_ID = '8619c8a7-eb46-11ed-0a80-00cb00846a48'
...
const ORDER = {
  name: `GENCardM${uaeShortDate()}SHKBRS`,
  marker: `ADMIN-SHAKIROVNA-BEIGE-RADIANCE-SERUM-${uaeToday()}`,
}
...
const EXPECTED_SUM_MINOR = 64500
...
async function exportInvoicePdf(invoiceId, invoiceName) {
  ...
  const out = path.join(ORDERS_DIR, `GENOSYS_Admin_Shakirovna_${safe}.pdf`)
...
async function main() {
  console.log('====================================================================')
  console.log('  Admin Shakirovna — beige ×1 + Radiance Serum ×3, delivery free')
  console.log('====================================================================')
  console.log(`  Mode: ${COMMIT ? 'COMMIT' : 'DRY RUN'}`)

  const agent = await api('GET', `/entity/counterparty/${AGENT_ID}`)
  if (!/admin shakirovna/i.test(agent.name)) throw new Error(`Unexpected agent: ${agent.name}`)
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      '00144 Beige x1 @150; 00194 Radiance Serum x3 @165; delivery free. Clinic list. Unpaid.',
  ...
  try {
    ...
    console.log(`  Print failed, opened PDF: ${e.message}`)
  }
