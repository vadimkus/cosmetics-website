...
/**
 * Salon 971 — consignment sold recon (agreement 35).
 * Book stock − shelf count 2026-08-05 → sold lines.
 *
 *   Shelf: PDRN 1, patches 2, cleanser 2, camel 2, beige 3
 *   Sold:  patches ×1, camel ×1, PDRN ×1 = 540 AED
 *
 *   node --import dotenv/config scripts/moysklad-create-salon971-commission-demand-20260805.js
 *   node --import dotenv/config scripts/moysklad-create-salon971-commission-demand-20260805.js --commit
 */
...
const MARKER = `Salon 971 consignment sold book-minus-shelf ${uaeToday()}`

/** Sold = book − shelf (only positive deltas) */
const LINES = [
...
const EXPECTED_UNITS = 3
const EXPECTED_SUM_MINOR = 54000 // 190+150+200
...
async function main() {
  console.log('====================================================================')
  console.log('  Salon 971 — report + demand (agreement 35, shelf recon)')
  console.log('====================================================================')
  ...
  console.log('  Period: 2026-06-30 → 2026-08-05')
  console.log('  Shelf: PDRN1 patches2 cleanser2 camel2 beige3')
  console.log('  Delta sold: patches1 + camel1 + PDRN1')
  ...
  const report = await api('POST', '/entity/commissionreportin', {
    ...
    commissionPeriodStart: '2026-06-30 00:00:00',
    commissionPeriodEnd: '2026-08-05 23:59:59',
    rewardType: 'PercentOfSales',
    rewardPercent: 0,
    description: [
      MARKER,
      'Salon 971 | Agreement 35 | shelf recon 2026-08-05.',
      'Book − shelf: patches x1, camel x1, PDRN x1. Cleanser/beige unchanged.',
    ].join('\n'),
  ...
  if (EXPORT_PDF) {
    const salesOut = path.join(ORDERS_DIR, `GENOSYS_Salon971_Consignment_Sales_${report.name}.pdf`)
    const stockOut = path.join(ORDERS_DIR, `GENOSYS_Salon971_Consignment_Stock_Note_${demand.name}.pdf`)
