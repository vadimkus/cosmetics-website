...
/**
 * Persona Marina — SO → invoice → shipment (clinic prices, unpaid).
 *
 *   00024 Snow O₂ Cleanser 500ml ×1 @ 255
 *   00036 Intensive Problem Control Cream 250g ×1 @ 210
 *   Delivery Dubai ×1 @ 45 FREE
 *   Total: 465 AED
 *
 *   node --import dotenv/config scripts/moysklad-create-persona-marina-cleanser-problem-cream-order-20260803.js
 *   node --import dotenv/config scripts/moysklad-create-persona-marina-cleanser-problem-cream-order-20260803.js --commit
 */
...
const CUSTOMER = {
  city: 'Dubai',
  street: 'The Residences at Marina Gate 1, Mezzanine Floor, R-113, Dubai Marina',
}

const ORDER = {
  name: `GENCardM${uaeShortDate()}PMMG`,
  moment: uaeMomentNow(),
  marker: `PERSONA-MARINA-CLEANSER500-PROBLEM250-${uaeToday()}`,
}
...
const EXPECTED_SUM_MINOR = 46500
...
function buildPositions(stock) {
  ...
  for (const [code, qty, clinicAed, discountPct] of PRODUCT_LINES) {
    ...
    if (item.price && item.price !== priceMinor) {
      console.warn(`  WARN ${code}: stock salePrice ${money(item.price)} vs expected ${clinicAed}`)
    }
...
async function exportInvoicePdf(invoiceId, invoiceName) {
  ...
  const out = path.join(ORDERS_DIR, `GENOSYS_Persona_Marina_${safe}.pdf`)
...
async function main() {
  console.log('====================================================================')
  console.log('  Persona Marina — cleanser 500 + problem cream 250 (clinic)')
  console.log('====================================================================')
  ...
  console.log(`  Ship: ${CUSTOMER.street}`)
  for (const p of positions) {
    if (p._code) {
      console.log(
        `    ${p._code} ${p._name.slice(0, 48)} x1 @ ${p._clinic} clinic (avail ${p._avail})`,
      )
    } else {
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      'Clinic: 00024 cleanser 500 ×1 @255; 00036 problem cream 250g ×1 @210; delivery FREE.',
      `Ship: ${CUSTOMER.street}. Unpaid — no paymentin.`,
    ].join('\n'),
