...
/**
 * Miss Lavine Kawani — retail SO → invoice → shipment + landscape print.
 * (No paymentin — COD / pay on delivery.)
 *
 *   00048 HR³ Matrix Hair Solution Box (8pcs) ×1 @ 740 AED retail
 *   Excellent Delivery Dubai ×1 @ 45 AED
 *
 *   node --import dotenv/config scripts/moysklad-create-lavine-kawani-hair-solution-order-invoice-demand-20260711.js --commit
 */
...
const CUSTOMER = {
  name: 'Miss Lavine Kawani',
  phone: '+447487272798',
  city: 'Dubai',
  street:
    'Creek Crescent, Floor 4 Room 407, Dubai Creek Harbour, Al Kheeran First',
}

const ORDER = {
  name: `GENCardM${uaeShortDate()}2798`,
  moment: uaeMomentNow(),
  marker: `LAVINE-KAWANI-HAIR-SOLUTION-RETAIL-${uaeToday()}`,
}

/** [code, qty, retailAed VAT incl.] */
const PRODUCT_LINES = [['00048', 1, 740]]
const DELIVERY_AED = 45
const EXPECTED_SUM_MINOR = 78500
...
async function findExistingCounterparty() {
  ...
  const bySearch = await api(
    'GET',
    `/entity/counterparty?search=${encodeURIComponent('7487272798')}&limit=10`
  )
  return (bySearch?.rows || []).find((r) => (r.phone || '').includes('7487272798')) || null
}
...
async function findOrCreateCounterparty() {
  ...
  const created = await api('POST', '/entity/counterparty', {
    ...
    description: `Retail customer Dubai Creek Harbour — created with order ${ORDER.name}`,
...
function ordersPdfPath(invoiceName) {
  ...
  return path.join(ordersDir, `GENOSYS_Lavine_Kawani_${safe}.pdf`)
}

async function main() {
  console.log('====================================================================')
  console.log('  Miss Lavine Kawani — hair solution retail + invoice + shipment')
  console.log('====================================================================')
  ...
  for (const [code, qty, retailAed] of PRODUCT_LINES) {
    const item = stock.get(code)
    console.log(`    ${code} ${item.name.slice(0, 55)} x${qty} @ ${retailAed.toFixed(2)} retail`)
  }
  console.log(`    Excellent Delivery Dubai x1 @ ${DELIVERY_AED.toFixed(2)} AED`)
  console.log(`  Total: ${money(sumMinor)} AED`)
  console.log('  Courier note: call customer 5 minutes before arriving.')
  ...
  const orderDesc = [
    ORDER.marker,
    '00048 HR³ Matrix Hair Solution Box x1 @ 740 AED retail; delivery 45 AED.',
    'Recipient: Lavine Kawani | UK phone +447487272798.',
    'Driver: call 5 min before arrival — customer will come down.',
