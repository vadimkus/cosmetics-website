...
/**
 * Bespoke product page for PEPTIDE GEL MASK (product 37).
 *
 ...
 * royal-blue palette on top via peptidegel.css, taken from the
 * white-and-blue pouch.
 *
 * Section order:
 *
 *   effects  Moisturize · Soothe · Cool · After a procedure
 *   engine   Glycerin 20% is the figure on the card; the peptide sits at 0.05 ppm
 *   howTo    20-40 minutes, take the sheet off, massage the leftover in
 ...
 *   routine  cleanse → mist → mask 37 → postcream 25
 *
 * There is a safety assessment and no quantified clinical trial of this
 * finished mask, so this page has no proof chart. See peptideGelCopy.ts.
 * In particular: Glycerin 20% leads, not the peptide name; no patented
 * transdermal story; no Botox; the sheet comes off.
 */

import '../cerabarrier/cerabarrier.css'
import PageBreadcrumb from '@/components/PageBreadcrumb'
import './peptidegel.css'
...
import { getPeptideGelCopy } from './peptideGelCopy'
...
export default function PeptideGelProductPage({
  product,
  unitsSold = 0,
  routineProducts = [],
}: Props) {
  ...
  const isRtl = dir === 'rtl'
  const copy = getPeptideGelCopy(locale)

  // Single SKU - one 38g x 5 box - so no size is ever passed to the cart.
  ...
  const routineSteps = useMemo(() => {
    const routine = PRODUCT_ROUTINES['37']
    if (!routine) return []
  ...
  const addToCart = useCallback(
    async (qty: number) => {
      ...
      try {
        ...
        errorLog('Peptide gel mask: add to cart failed', error)
        throw error
      }
    },
  ...
  const handleAddRoutineProduct = useCallback(
    async (item: Product, selection?: ProductOptionSelection, quantity = 1) => {
      ...
      try {
        ...
        errorLog('Peptide gel mask: routine add to cart failed', error)
      } finally {
  ...
  return (
    <div className={`cera-page pgm-page ${ceraSerif.variable} min-h-[100dvh]`} dir={dir}>
      {/* ───────────────────────────── Hero ─────────────────────────────── */}
        ...
        <div>
          ...
          {product.videoUrl ? (
            <CeraReveal className="mt-10">
              ...
              <div className="pgm-video relative mt-4 mx-auto aspect-[9/16] w-full max-w-[340px] overflow-hidden rounded-[28px]">
                <video
      ...
      <section className="mx-auto max-w-[1000px] px-4 py-16 sm:px-6 lg:py-24">
        ...
        <div className="mt-10 grid grid-cols-1 gap-4 lg:mt-14 lg:grid-cols-2 lg:gap-6">
          ...
          <CeraReveal delay={90} className="cera-card pgm-not p-6 lg:p-7">
            <h3 className="cera-serif text-[21px] leading-tight text-[var(--cera-ink)]">
