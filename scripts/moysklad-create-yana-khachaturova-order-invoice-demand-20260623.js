...
/**
 * Miss Khachaturova Yana — existing customer — SO → invoice → shipment @ clinic salePrice.
 *
 *   Multi-Vita Radiance Cream 50g (00122) ×1
 *   Multi Vita Radiance Serum 30ml (00194) ×2
 *   Skin Barrier Protecting Cream 100g (00037) ×2
 *   Skin Reboot PDRN mask Pack (54467) ×2
 *   Snow Booster Toner 200ml (00022) ×2
 *
 *   node --import dotenv/config scripts/moysklad-create-yana-khachaturova-order-invoice-demand-20260623.js
 *   node --import dotenv/config scripts/moysklad-create-yana-khachaturova-order-invoice-demand-20260623.js --commit
 */
...
const AGENT_ID = '5636a558-3cef-11ee-0a80-01b4000c32f0' // Miss Khachaturova Yana

const ORDER = {
  name: `GENCardM${uaeShortDate()}YANA`,
  moment: uaeMomentNow(),
  marker: `Yana Khachaturova retail order ${uaeToday()}`,
}
...
const PRODUCT_LINES = [
  ...
  ['54467', 2], // Skin Reboot PDRN mask Pack (30 sheets)
...
function buildShipmentAddress(agent) {
  ...
  return { country: countryHref(), city: 'Dubai', street: addInfo || 'Miss Khachaturova Yana — UAE' }
}
...
async function exportInvoicePdf(invoiceId, invoiceName) {
  ...
  const outPath = path.join(ordersDir, `GENOSYS_Yana_Khachaturova_${safe}.pdf`)
...
async function main() {
  console.log('====================================================================')
  console.log('  Miss Khachaturova Yana — SO → invoice → shipment')
  console.log('====================================================================')
  console.log(`  Mode: ${COMMIT ? 'COMMIT' : 'DRY RUN'}`)

  const agent = await api('GET', `/entity/counterparty/${AGENT_ID}`)
  console.log(`  Counterparty: ${agent.name} (${agent.phone || 'no phone'})`)
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      '00122 x1, 00194 x2, 00037 x2, 54467 x2, 00022 x2.',
