...
/**
 * DIFFERENT AESTHETIC CLINIC (Abu Dhabi) — unpaid clinic SO → invoice → shipment.
 *
 *   00140 Sea algae ×10 @ 18
 *   00063 Collagen ×10 @ 18
 *   00021 Snow O₂ 180ml ×2 @ 165
 *   00190 Anti-Wrinkle Cream 50g ×1 @ 145
 *   00122 Radiance Cream 50g ×1 @ 145
 *   00040 Blemish Balm Cream 50g ×1 @ 125
 *   00041 Multi Sun Cream SPF40 ×2 @ 105
 *   00188 Microbiome Mist 80ml ×2 @ 80
 *   00053 EyeCell patches ×1 @ 190
 *   54461 Skin Defender 200ml ×2 @ 145
 *   00013 Hydro Cool 1kg ×1 @ 300
 *   Total: 2,255.00 AED
 *
 *   node --import dotenv/config scripts/moysklad-create-different-clinic-order-20260903.js
 *   node --import dotenv/config scripts/moysklad-create-different-clinic-order-20260903.js --commit
 */
...
const CUSTOMER = {
  ...
  phone: '0585192533',
  city: 'Abu Dhabi',
  street: 'St Regis Residences Block 5',
}

const ORDER = {
  name: `GENCardM${uaeShortDate()}DIFF`,
  marker: `DIFFERENT-CLINIC-MIX-2255-${uaeToday()}`,
}
...
const EXPECTED_SUM_MINOR = 225500
...
function isDifferentClinic(row) {
  const digits = phoneDigits(row.phone)
  const name = (row.name || '').toLowerCase()
  return (
    digits.endsWith('585192533') ||
    name.includes('different aesthetic') ||
    name === 'different aesthetic clinic'
  )
}
...
function customerPayload() {
  return {
    ...
    description: [
      'DIFFERENT AESTHETIC CLINIC, St Regis Residences Block 5, Abu Dhabi.',
      `Phone ${CUSTOMER.phone}.`,
    ].join(' '),
  }
}
...
async function findOrCreateCustomer() {
  for (const q of ['DIFFERENT AESTHETIC', 'Different Aesthetic', '0585192533', '585192533']) {
    const d = await api('GET', `/entity/counterparty?search=${encodeURIComponent(q)}&limit=50`)
    const hit = (d.rows || []).find(isDifferentClinic)
    if (hit) {
...
async function exportInvoicePdf(invoiceId, invoiceName) {
  ...
  const out = path.join(ORDERS_DIR, `GENOSYS_Different_Clinic_${safe}.pdf`)
...
async function main() {
  console.log('====================================================================')
  console.log('  DIFFERENT AESTHETIC CLINIC — mix 2,255 AED unpaid')
  console.log('====================================================================')
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      'Clinic list mix 2255 AED unpaid. No delivery.',
