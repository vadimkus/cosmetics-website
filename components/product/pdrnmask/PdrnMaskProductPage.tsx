...
/**
 * Bespoke product page for the SKIN REBOOT PDRN MASK PACK (product 52).
 *
 ...
 * clay palette on top via pdrnmask.css, sampled off the tub.
 *
 * Structurally it starts from the product 53 layout, the other mask in the
 * range, and then diverges in the two places where this product has something
 * 53 does not. Section order:
 *
 *   licence  the Korean dual-function registration, which is the strongest
 *            verifiable thing this product has and therefore leads
 *   proof    the P&K barrier study, charted from the raw TEWL readings
 *   sheet    the lyocell carrier
 *   howTo    six steps from the carton's own directions
 *   actives  ingredient cards, plus the full INCI
 *   suited   who it is for and who should buy something else
 *   routine  where a mask sits between the leave-on steps
 *
 * Where it deliberately diverges from product 53:
 *
 *   - It HAS a proof section, because it has a real study: P&K Skin Research
 *     Center, May 2025, 20 women, TEWL measured against an untreated control.
 *     Product 53 has no study and correctly shows no proof block. Do not copy
 *     this section back to 53.
 *
 *   - The actives cards are read from the copy module rather than from
 *     product.ingredients. Every other bespoke layout reads the DB field, which
 *     means the Arabic and Russian pages render English ingredient text, since
 *     the route hands the layout the untranslated row. Holding the cards in
 *     pdrnMaskCopy.ts is what makes this page read in Arabic and Russian. The DB
 *     cards are still maintained in all three languages for the shared PDP and
 *     the mobile app.
 *
 * See pdrnMaskCopy.ts for the sourcing rules, the full quantitative formula and
 * the list of claims that must not come back without a document. In particular:
 * there are no peptides in this product, and the page must never say there are.
 */
...
import { FULL_INCI, getPdrnMaskCopy, TEWL_READINGS } from './pdrnMaskCopy'
...
/** The two shots used inline, taken from the 2026 campaign set so the page and
 *  the gallery tell one story. These are finished compositions on their own
 *  near-white ground rather than packshots on pure white, so they are covered
 *  full-bleed and carry .pdrnmask-slide to opt out of the multiply rule; see
 *  the note above it in pdrnmask.css.
 *
 *  Nothing from the older set goes back on this page: four of the seven images
 *  this product had on file are AI renders with mangled pack text, two of them
 *  spelling "PDRN" as "PORN". See the image note in pdrnMaskCopy.ts. */
const LICENCE_IMAGE = '/images/pdrn_mask_new/S3.jpeg'
const SHEET_IMAGE = '/images/pdrn_mask_new/S5.jpeg'

/** Bars are drawn as a share of the highest reading in the study, so the chart
 *  is the data rather than an illustration of it. */
const TEWL_MAX = Math.max(...TEWL_READINGS.flatMap(r => [r.control, r.treated]))
...
export default function PdrnMaskProductPage({
  product,
  unitsSold = 0,
  routineProducts = [],
}: Props) {
  ...
  const isRtl = dir === 'rtl'
  const copy = getPdrnMaskCopy(locale)
  ...
  const routineSteps = useMemo(() => {
    const routine = PRODUCT_ROUTINES['52']
    if (!routine) return []
  ...
  const addToCart = useCallback(
    async (qty: number) => {
      ...
      try {
        ...
        errorLog('PdrnMask: add to cart failed', error)
        throw error
      }
    },
  ...
  const handleAddRoutineProduct = useCallback(
    async (item: Product, selection?: ProductOptionSelection, quantity = 1) => {
      ...
      try {
        ...
        errorLog('PdrnMask: routine add to cart failed', error)
      } finally {
    ...
    <div className={`cera-page pdrnmask-page ${ceraSerif.variable} min-h-[100dvh]`} dir={dir}>
      ...
      <section className="mx-auto max-w-[1200px] px-4 pt-4 sm:px-6 md:pt-8 lg:pt-12">
        ...
        <div className="mt-5 grid grid-cols-1 gap-8 lg:mt-9 lg:grid-cols-[minmax(0,1.45fr)_minmax(0,1fr)] lg:gap-12 xl:gap-16">
          ...
          <div>
            ...
            {/* Single format, so the pack is stated rather than selected. The
      ...
      <section className="mt-14 border-y border-[var(--cera-line)] bg-white md:mt-20">
        <div className="mx-auto grid max-w-[1200px] grid-cols-2 gap-y-8 px-4 py-10 sm:px-6 lg:grid-cols-4 lg:py-12">
          {copy.stats.map((stat, i) => (
            ...
            >
              {/* Not cera-numeral: these values are words, not figures, because
                  the manufacturer specifies no counts, depths or dimensions. */}
        ...
        <div className="grid grid-cols-1 gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(0,0.9fr)] lg:gap-16">
          <div>
            ...
            <div className="mt-8 space-y-4">
              {copy.licence.points.map((point, i) => (
                <CeraReveal
                  key={point.title}
                  delay={i * 80}
                  as="article"
          ...
          {/* Full-bleed: the slide is composed square with its own margins, so
              padding it inside the stage would frame an already-framed image. */}
          <CeraReveal className="lg:sticky lg:top-24 lg:self-start">
            <div className="cera-stage pdrnmask-slide relative aspect-square overflow-hidden rounded-[28px]">
              <Image
                src={LICENCE_IMAGE}
      ...
      <section className="bg-white py-16 lg:py-24">
        <div className="mx-auto max-w-[1200px] px-4 sm:px-6">
          ...
          <div className="mt-10 grid grid-cols-1 gap-6 lg:mt-14 lg:grid-cols-[minmax(0,1.25fr)_minmax(0,1fr)] lg:gap-8">
            {/* The chart. Decorative reinforcement of the table beside it, so it
            ...
            <CeraReveal className="cera-card p-6 lg:p-8">
              <p className="text-[13px] font-semibold uppercase tracking-[0.12em] text-[var(--cera-rose-ink)]">
                {copy.proof.chartTitle}
              </p>

              <div className="mt-8 pdrn-chart" aria-hidden="true">
                {TEWL_READINGS.map((reading, i) => (
                  <div key={copy.proof.stages[i]} className="pdrn-chart-group">
                    <span
                      className="pdrn-bar pdrn-bar-control"
                      style={{ height: `${(reading.control / TEWL_MAX) * 100}%` }}
                    >
                      <span className="pdrn-bar-value">{reading.control.toFixed(2)}</span>
                    </span>
                    <span
                      className="pdrn-bar pdrn-bar-treated"
                      style={{ height: `${(reading.treated / TEWL_MAX) * 100}%` }}
                    >
                      <span className="pdrn-bar-value">{reading.treated.toFixed(2)}</span>
                    </span>
                  </div>
                ))}
              </div>
              ...
              <ul className="mt-5 flex flex-wrap gap-x-5 gap-y-2 text-[12.5px] text-[var(--cera-muted)]">
                ...
                <li className="flex items-center gap-2">
                  <span className="h-2.5 w-2.5 rounded-sm bg-[var(--cera-rose)]" />
                  {copy.proof.treatedLabel}
                </li>
              </ul>
            </CeraReveal>
      ...
      <section className="mx-auto max-w-[1000px] px-4 py-16 sm:px-6 lg:py-24">
        ...
        <div className="mt-10 grid grid-cols-1 gap-4 lg:mt-14 lg:grid-cols-2 lg:gap-6">
          ...
          <CeraReveal delay={90} className="cera-card pdrnmask-not p-6 lg:p-7">
            <h3 className="cera-serif text-[21px] leading-tight text-[var(--cera-ink)]">
      ...
      {/* ────────────────────────── Closing band ───────────────────────── */}
      {/* The pack shot is on pure white, so it multiplies cleanly into the band
          and the tub sits on the tint with no panel around it. */}
      <CeraClosingCta
        image={product.image}
        imageFit="blend"
        name={product.name}
