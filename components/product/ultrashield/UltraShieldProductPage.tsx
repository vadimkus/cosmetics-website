...
/**
 * Bespoke product page for ULTRA SHIELD SUN CREAM [SPF50+ / PA++++] (product 39).
 *
 * Built on the shared editorial system, with a violet palette in
 * ultrashield.css taken from the carton keyline.
 *
 * This page is the inverse of products 36 and 41. There the headline claim was
 * thinner than it looked; here it is stronger. The EU safety assessment records
 * an in-vivo SPF of 65.9 and a UVA protection factor of 23.13 and 24.3 against
 * a required 22.0, and none of that was anywhere on the old page. So the
 * measured numbers lead, then the six filters with their percentages, then
 * niacinamide 2% and adenosine 0.04%.
 *
 * The page also carries three things a sunscreen page normally hides: the
 * homosalate regulatory history, the absence of a water-resistance test, and
 * the fact that the marketed recovery complex runs from 0.1 ppb to 1 ppm.
 *
 * Do not add: a seventh filter (butyloctyl salicylate is a photostabiliser),
 * "reef-safe", any swimming or water-resistance claim, barrier repair from
 * Ceramide NP, the deck's "improves rosacea and acne", the contract
 * manufacturer, or the lot code. See the source audit.
 */

import '../cerabarrier/cerabarrier.css'
import PageBreadcrumb from '@/components/PageBreadcrumb'
import './ultrashield.css'
...
export default function UltraShieldProductPage({ product, unitsSold = 0, routineProducts = [] }: Props) {
  ...
  const isRtl = dir === 'rtl'
  const copy = getUltraShieldCopy(locale)
      ...
      try {
        ...
        try {
          trackAddToCart({
            id: product.id,
            name: product.name,
            category: product.category || 'Sun',
            price: product.price,
            quantity: qty,
          })
        } catch { /* analytics is best-effort */ }
      } catch (error) {
        errorLog('UltraShield: add to cart failed', error)
        throw error
      }
    },
  ...
  return (
    <div className={`cera-page usc-page min-h-[100dvh]`} dir={dir}>
                ...
                <button
                  ...
                  className={`inline-flex h-[54px] min-w-[220px] flex-1 items-center justify-center gap-2.5 rounded-full px-8 text-[15px] font-semibold tracking-wide transition-all duration-300 disabled:cursor-not-allowed disabled:opacity-45 ${
                    justAdded
                      ...
                      : 'bg-[var(--cera-cta)] text-white hover:bg-[var(--cera-rose-ink)] hover:shadow-[0_18px_38px_-20px_rgba(27,23,35,0.8)]'
                  }`}
                >
      ...
      <section className="mx-auto max-w-[1000px] px-4 py-16 sm:px-6 lg:py-24">
        ...
        <CeraReveal className="usc-measure mt-10 p-6 md:p-9 lg:mt-14">
          <dl className="divide-y divide-[var(--cera-blush-deep)]">
            {copy.measured.rows.map(row => (
              <div key={row.label} className="grid grid-cols-1 gap-2 py-5 first:pt-0 last:pb-0 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-baseline sm:gap-6">
                <div className="sm:order-1">
                  <dt className="cera-serif text-[19px] leading-tight text-[var(--cera-ink)] sm:text-[21px]">
                    {row.label}
                  </dt>
                  <dd className="mt-1.5 max-w-[62ch] text-[14px] leading-relaxed text-[var(--cera-body)]">
                    {row.note}
                  </dd>
                </div>
                <p
                  dir="ltr"
                  className="cera-serif cera-numeral usc-figure text-[26px] leading-none text-[var(--cera-rose-ink)] sm:order-2 sm:text-[30px]"
                >
                  {row.value}
                </p>
              </div>
        ...
        <CeraReveal>
          <p className="mt-5 flex items-start gap-2.5 text-[13px] leading-relaxed text-[var(--cera-muted)]">
            <Info className="mt-[2px] h-4 w-4 flex-none" aria-hidden="true" />
            <span>{copy.measured.footnote}</span>
          </p>
        </CeraReveal>
      </section>
      ...
      <section className="bg-white py-16 lg:py-24">
        <div className="mx-auto max-w-[1200px] px-4 sm:px-6">
          <div className="grid grid-cols-1 gap-10 lg:grid-cols-[minmax(0,0.72fr)_minmax(0,1fr)] lg:gap-16">
            <div>
              <CeraReveal>
                  ...
                  <tbody>
                    {copy.filters.rows.map(row => (
                      <tr key={row.name} className="border-b border-[var(--cera-line)] last:border-b-0">
                        ...
                        <td
                          dir="ltr"
                          className="usc-figure whitespace-nowrap px-4 py-4 text-start align-top text-[14px] text-[var(--cera-rose-ink)]"
                        >
                          {row.amount}
                        </td>
        ...
        <div className="mt-10 grid grid-cols-1 gap-4 md:grid-cols-3 lg:mt-14 lg:gap-6">
          {copy.actives.items.map((item, i) => (
            <CeraReveal key={item.name} delay={i * 80} as="article" className="cera-card cera-card-hover flex flex-col p-6 lg:p-7">
              ...
              <p dir="ltr" className="cera-serif cera-numeral usc-figure mt-2 text-[30px] leading-none text-[var(--cera-rose)]">
                {item.dose}
              </p>
      ...
      <section className="bg-white py-16 lg:py-24">
        <div className="mx-auto max-w-[900px] px-4 sm:px-6">
          <CeraSectionHeader eyebrow={copy.uae.eyebrow} title={copy.uae.title} intro={copy.uae.intro} />
