...
/**
 * Cosmiden / Lilyne — consignment replenishment into agreement 15.
 *   00012 Peptide Gel Mask 39g ×10 @ 38
 *   Total: 380 AED
 *
 *   node --import dotenv/config scripts/moysklad-create-cosmiden-peptide-demand-20260903.js
 *   node --import dotenv/config scripts/moysklad-create-cosmiden-peptide-demand-20260903.js --commit
 */
...
const MARKER = `COSMIDEN-LILYNE-PEPTIDE-DEMAND-${uaeToday()}`

const LINES = [['00012', 10, 38, 'Peptide Gel Mask 39g']]
...
async function exportPdf(demandId, demandName) {
  ...
  const out = path.join(ORDERS_DIR, `GENOSYS_Cosmiden_Lilyne_Consignment_Stock_Note_${demandName}.pdf`)
...
async function main() {
  console.log('====================================================================')
  console.log('  Cosmiden / Lilyne — peptide masks ×10 into agr. 15')
  console.log('====================================================================')
  ...
  if (contract.name !== '15') throw new Error(`Expected agreement 15, got ${contract.name}`)
  ...
  const demand = await api('POST', '/entity/demand', {
    ...
    description: [
      MARKER,
      'COSMIDEN MEDICAL CENTER L.L.C / Lilyne | Agreement 15.',
      'Replenishment: 00012 Peptide Gel Mask x10 @38.',
