...
/**
 * NEW YOU STAR BEAUTY HEALTH CLINIC L.L.C — consignment stock note (agreement 37).
 *   00052 HR³ Matrix Scalp & Hair Shampoo 300ml ×4 @ clinic list
 *
 *   node --import dotenv/config scripts/moysklad-create-new-you-star-shampoo-consignment-20260725.js
 *   node --import dotenv/config scripts/moysklad-create-new-you-star-shampoo-consignment-20260725.js --commit
 */
...
const COMMON = {
  ...
  agentId: '69e1db3e-7fa4-11f1-0a80-0283002585b0',
  contractId: '6a2aabf3-7fa4-11f1-0a80-0283002585c9', // Agreement 37
}

const CUSTOMER_STREET = 'The Mall, Shop 21, Umm Suqeim Third, Jumeirah St, Dubai'
const MARKER = `NEW-YOU-STAR-CONSIGNMENT-SHAMPOO-x4-${uaeToday()}`

/** [code, qty, clinicAed, label] */
const LINES = [['00052', 4, 170, 'HR³ Matrix Scalp & Hair Shampoo 300ml']]
const EXPECTED_SUM_MINOR = 4 * 170 * 100 // 68000
...
async function main() {
  console.log('====================================================================')
  console.log('  NEW YOU STAR — consignment shampoo ×4 + stock note')
  console.log('====================================================================')
  ...
  console.log('\n  Lines:')
  for (const [code, qty, clinicAed, label] of LINES) {
    ...
    resolved.push({ ...row, qty, priceMinor, label })
    console.log(
      `    ${code} x${qty} @ ${money(priceMinor)} → ${money(lineMinor)} | avail ${row.available} | ${label}`,
    )
  }
  ...
  const demand = await api('POST', '/entity/demand', {
    ...
    description: [
      MARKER,
      'Consignment replenishment — HR3 Scalp & Hair Shampoo 300ml x4 @ clinic list.',
