...
/**
 * Bespoke product page for HR³ MATRIX SCALP PEELING α (product 46), fourth of the
 * five-product scalp line. Shares hr3.css with the rest of the line.
 *
 * WHAT THIS PRODUCT IS, which took reading the whole dossier to work out: a scalp prep
 * solution, not an exfoliating treatment. A third of the bottle is denatured alcohol
 * (33.600%) with propylene glycol at 11.994%, applied on a cotton swab, rubbed in
 * firmly, and NOT rinsed off. Its job is to strip oil and build-up so the Hair Solution
 * ampoule that follows is needled into clean skin. Understood that way it is a good
 * product; described as a gentle peel it is a misleading one.
 *
 * THE AMBER BLOCK LEADS, and it is unusual in that it corrects our own copy: the
 * previous record called this "gentle" three times. A solution that is a third alcohol,
 * carries 1.7% cooling agents and is rubbed in with a swab is not gentle. The page says
 * so before it sells anything, and lists what to expect: cold and sharp, stings broken
 * skin, flammable, and NOT a disinfectant.
 *
 * MUST NEVER BE ADDED:
 *   - "Gentle", in any form.
 *   - "Disinfects the treatment area" (DTS MG deck). Alcohol needs roughly 60-70% to
 *     act as an antiseptic; this is 33.6%. The page states the opposite explicitly,
 *     because someone needling at home might otherwise rely on it.
 ...
 *   - Anything attributed to the carton panel: the registered artwork PDF has no text
 *     layer, so the safety section says so and points the reader at their own box.
 *
 * THE COOLING TABLE is the other reason this page exists. Totals across the line:
 * peeling 1.700%, shampoo 1.200%, tonic 0.380%, ampoule 0.200%. The shampoo has more
 * menthol specifically (1.120% vs 0.900%), so its copy was corrected the same day to
 * claim "most menthol" rather than "hardest-cooling". Keep the two consistent.
 */
...
import { COMPANION_PRODUCT_IDS, getScalpPeelingCopy } from './scalpPeelingCopy'
...
export default function ScalpPeelingProductPage({ product, unitsSold = 0, routineProducts = [] }: Props) {
  ...
  const isRtl = dir === 'rtl'
  const copy = getScalpPeelingCopy(locale)
  ...
  const addToCart = useCallback(
    async (qty: number) => {
      ...
      try {
        ...
        errorLog('ScalpPeeling: add to cart failed', error)
        throw error
      }
    },
  ...
  return (
    <div className={`cera-page hr3-page ${ceraSerif.variable} min-h-[100dvh]`} dir={dir}>
      ...
      {/* ──────────── It is not gentle, said before anything else ─────────── */}
      <section className="mx-auto max-w-[900px] px-4 pt-14 sm:px-6 lg:pt-20">
        <CeraReveal className="hr3-alert p-6 md:p-9">
          <p className="cera-eyebrow hr3-alert__title">{copy.notGentle.eyebrow}</p>
          <h2 className="cera-serif hr3-alert__title mt-3 text-[26px] leading-tight sm:text-[33px]">
            {copy.notGentle.title}
          </h2>
          <p className="mt-5 text-[15.5px] leading-relaxed text-[var(--cera-body)]">{copy.notGentle.body}</p>
          <ul className="mt-5 space-y-2.5">
            {copy.notGentle.items.map(item => (
          ...
          <p className="mt-5 text-[14.5px] leading-relaxed text-[var(--cera-body)]">{copy.notGentle.detail}</p>
        </CeraReveal>
      </section>
        ...
        <div className="mx-auto max-w-[1000px] px-4 sm:px-6">
          ...
          <CeraReveal className="cera-card mt-9 overflow-hidden">
            <table className="hr3-table w-full border-collapse text-start">
              <caption className="sr-only">{copy.cooling.title}</caption>
              <tbody>
                {copy.cooling.rows.map(row => (
                  <tr
                    key={row.product}
                  ...
                  >
                    ...
                    <td dir="ltr" className="hidden whitespace-nowrap px-5 py-4 text-end align-middle text-[12.5px] text-[var(--cera-muted)] sm:table-cell">
                      {row.note}
                    </td>
                  </tr>
                ))}
          ...
          <CeraReveal>
            <p className="mt-7 text-[15.5px] leading-relaxed text-[var(--cera-body)]">{copy.cooling.body}</p>
          </CeraReveal>
        </div>
      </section>

      {/* ───────────── The two ingredients the label leads on ───────────── */}
      <section className="py-16 lg:py-24">
        <div className="mx-auto max-w-[1040px] px-4 sm:px-6">
          <CeraReveal>
            <p className="cera-eyebrow">{copy.labelClaims.eyebrow}</p>
            <h2 className="cera-serif mt-3 text-[28px] leading-[1.14] sm:text-[38px]">{copy.labelClaims.title}</h2>
            <p className="mt-4 max-w-[62ch] text-[15.5px] leading-relaxed text-[var(--cera-body)]">
              {copy.labelClaims.body}
            </p>
          </CeraReveal>

          <div className="mt-9 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4 lg:gap-4">
            {copy.labelClaims.items.map((item, i) => (
          ...
          <CeraReveal>
            <p className="mt-7 text-[14.5px] leading-relaxed text-[var(--cera-body)]">{copy.labelClaims.footnote}</p>
          </CeraReveal>
        </div>
      </section>
