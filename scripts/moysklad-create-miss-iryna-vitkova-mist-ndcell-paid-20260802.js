...
/**
 * Miss Iryna Vitkova — paid clinic order (already paid & delivered).
 *
 *   00188 Microbiome Mist 80ml ×1 @ clinic
 *   00044 ND Cell Anti-Wrinkle Cream 50ml ×1 @ clinic
 *   Total: 265 AED (no delivery)
 *
 *   node --import dotenv/config scripts/moysklad-create-miss-iryna-vitkova-mist-ndcell-paid-20260802.js
 *   node --import dotenv/config scripts/moysklad-create-miss-iryna-vitkova-mist-ndcell-paid-20260802.js --commit
 */
...
const AGENT_ID = '5a1304aa-8d07-11f1-0a80-1b8100249b98' // Miss Iryna Vitkova
...
const ORDER = {
  name: `GENCardM${uaeShortDate()}7420`,
  moment: uaeMomentNow(),
  marker: `MISS-IRYNA-MIST-NDCELL-PAID-${uaeToday()}`,
}

/** [code, qty] — clinic salePrice from stock */
const PRODUCT_LINES = [
  ['00188', 1], // Microbiome Mist 80ml @ 80
  ['00044', 1], // ND Cell Anti-Wrinkle Cream 50ml @ 185
]
const EXPECTED_SUM_MINOR = 26500 // 80 + 185
...
async function exportInvoicePdf(invoiceId, invoiceName) {
  ...
  const outPath = path.join(ORDERS_DIR, `GENOSYS_Miss_Iryna_Vitkova_${invoiceName}.pdf`)
...
async function main() {
  console.log('====================================================================')
  console.log('  Miss Iryna Vitkova — mist + ND Cell (clinic, paid/delivered)')
  console.log('====================================================================')
  ...
  for (const p of positions) {
    console.log(
      `    ${p.assortment.meta.href.split('/').pop()} ${p._name.slice(0, 48)} x${p.quantity} @ ${money(p.price)} (avail ${p._avail})`,
    )
  }
  // prettier log with codes
  for (const [code, qty] of PRODUCT_LINES) {
    const item = stock.get(code)
    console.log(`    ${code} ${item.name.slice(0, 48)} x${qty} @ ${money(item.price)}`)
  }
  console.log(`  Total: ${money(sumMinor)} AED (paid + delivered)`)
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      'Clinic: mist 00188 x1 + ND Cell 00044 x1. No delivery.',
