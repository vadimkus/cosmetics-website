  ...
  return (
    <>
      ...
      >
        {/* Expanded panel */}
        {expanded && (
          <div className="bg-white border-t border-[var(--cera-line)] shadow-lg px-4 pt-3 pb-2 max-h-[50vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-3">
              <span className="text-sm font-semibold text-[var(--cera-ink)]">
                {totalItems} {totalItems === 1 ? t.item : t.items}
              </span>
              <button
                type="button"
                onClick={() => setExpanded(false)}
                className="p-1 text-[var(--cera-muted)] hover:text-[var(--cera-body)]"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
              ...
              {items.map((item, idx) => {
                ...
                return (
                  <div key={`${item.product.id}-${idx}`} className="flex items-center justify-between text-sm">
                    <span className="text-[var(--cera-body)] truncate max-w-[60%]">
                      {item.product.name} {item.quantity > 1 && <span className="text-[var(--cera-muted)]">x{item.quantity}</span>}
                    </span>
                    {pricing && (
                      <span className="flex items-center gap-1.5 flex-shrink-0">
                        {pricing.hasDiscount ? (
                          <>
                            <span className="font-semibold text-[var(--cera-rose-ink)]">
                          ...
                          <span className="font-medium text-[var(--cera-ink)]">
            ...
            {canSee && discountTotal < 200 && (
              <p className="text-[10px] text-[var(--cera-muted)] mt-1">{t.freeShipping}</p>
            )}
          </div>
        )}

        {/* Collapsed bar */}
        <div className="bg-white/95 backdrop-blur-sm border-t border-[var(--cera-line)] shadow-[0_-2px_8px_rgba(0,0,0,0.1)]">
          <div className="flex items-center gap-3 px-4 py-3">
            ...
            >
              <ShoppingBag className="w-6 h-6 text-[var(--cera-body)]" />
              <span className="absolute -top-1.5 -right-1.5 bg-[var(--cera-rose)] text-white text-[10px] font-bold rounded-full min-w-[18px] h-[18px] flex items-center justify-center">
              ...
              <div className="flex-1 min-w-0">
                {savings > 0 ? (
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-bold text-[var(--cera-ink)]">
            ...
            {!canSee && (
              <div className="flex-1 min-w-0">
                <span className="text-sm text-[var(--cera-body)]">
                  {totalItems} {totalItems === 1 ? t.item : t.items}
                </span>
              </div>
            )}
            ...
            <button
              type="button"
              onClick={() => setExpanded(!expanded)}
              className="p-1 text-[var(--cera-muted)] flex-shrink-0"
            >
