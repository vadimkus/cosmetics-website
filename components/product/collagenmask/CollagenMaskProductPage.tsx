...
/**
 * Bespoke product page for the INTENSIVE REPAIR COLLAGEN MASK (product 53).
 *
 ...
 * crimson palette on top via collagenmask.css, taken from the sachet itself.
 *
 * Structurally this is the product 61 layout (single SKU, no size selector, one
 * inline figure) with the ingredients section from product 66 added back, since
 * a sheet mask has a formula and a silicone brush does not. Section order:
 *
 *   effects  the four claims the English sachet actually makes - firm, soothe,
 *            hydrate, protect
 *   engine   the humectant base, which is the strongest true argument this
 *            product has and therefore leads the body of the page
 *   howTo    five steps from the pack's own directions
 ...
 *   - No clinical or proof section. No study exists for this product: no deck,
 *     no Safety Assessment report, and nothing in either COA. A proof block here
 *     could only be padding, and padding is how the old copy ended up claiming
 *     "clinically tested ... with proven results" for a mask that has never been
 *     clinically tested.
 *   - No brochure link, for the same reason. There is no marketing deck.
 *   - The `engine` section leads on percentages rather than on the ingredient
 *     the product is named after. That is a deliberate ordering decision and the
 *     reasoning is set out at length in collagenMaskCopy.ts. Read it before
 *     reordering this page.
 *
 * See collagenMaskCopy.ts for the sourcing rules and the list of claims that
 * must not come back without a document.
 */

import '../cerabarrier/cerabarrier.css'
import './collagenmask.css'
...
import { getCollagenMaskCopy } from './collagenMaskCopy'
...
export default function CollagenMaskProductPage({
  product,
  unitsSold = 0,
  routineProducts = [],
}: Props) {
  ...
  const isRtl = dir === 'rtl'
  const copy = getCollagenMaskCopy(locale)
  ...
  const routineSteps = useMemo(() => {
    const routine = PRODUCT_ROUTINES['53']
    if (!routine) return []
  ...
  const addToCart = useCallback(
    async (qty: number) => {
      ...
      try {
        ...
        errorLog('CollagenMask: add to cart failed', error)
        throw error
      }
    },
  ...
  const handleAddRoutineProduct = useCallback(
    async (item: Product, selection?: ProductOptionSelection, quantity = 1) => {
      ...
      try {
        ...
        errorLog('CollagenMask: routine add to cart failed', error)
      } finally {
  ...
  return (
    <div className={`cera-page collagenmask-page ${ceraSerif.variable} min-h-[100dvh]`} dir={dir}>
      {/* ───────────────────────────── Hero ─────────────────────────────── */}
      ...
      <section className="mx-auto max-w-[1000px] px-4 py-16 sm:px-6 lg:py-24">
        ...
        <div className="mt-10 grid grid-cols-1 gap-4 lg:mt-14 lg:grid-cols-2 lg:gap-6">
          ...
          <CeraReveal delay={90} className="cera-card collagenmask-not p-6 lg:p-7">
            <h3 className="cera-serif text-[21px] leading-tight text-[var(--cera-ink)]">
