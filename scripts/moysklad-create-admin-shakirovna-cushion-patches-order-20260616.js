...
/**
 * Admin Shakirovna Salon — clinic prices: SO → invoice → shipment + PDF.
 *
 *   Beige cushion (00144) ×2 @ 150 AED
 *   Eye peptide gel patch box (00053) ×1 @ 190 AED
 *   Excellent Delivery Dubai ×1 @ 10 AED
 *
 *   node --import dotenv/config scripts/moysklad-create-admin-shakirovna-cushion-patches-order-20260616.js
 *   node --import dotenv/config scripts/moysklad-create-admin-shakirovna-cushion-patches-order-20260616.js --commit
 */
...
const ORDER = {
  name: `GENCardM${uaeShortDate()}SHKP`,
  moment: uaeMomentNow(),
  marker: `ADMIN-SHAKIROVNA-CUSHION-PATCHES-${uaeToday()}`,
}

/** [code, qty, clinicAed] — MoySklad salePrice */
const PRODUCT_LINES = [
  ['00144', 2, 150], // Beige BB cushion
...
async function main() {
  console.log('====================================================================')
  console.log('  Admin Shakirovna — cushion + patches (clinic prices)')
  console.log('====================================================================')
  ...
  for (const [code, qty, clinicAed] of PRODUCT_LINES) {
    const item = stock.get(code)
    console.log(
      `    ${code} ${item.name.slice(0, 55)} x${qty} @ ${clinicAed.toFixed(2)} → ${(clinicAed * qty).toFixed(2)} AED`
    )
  }
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      'Clinic prices: Beige cushion 00144 x2 @ 150, Eye patch box 00053 x1 @ 190, delivery 10 AED.',
    ].join(' | '),
