...
export const PRODUCT_QUICK_FACTS_CATALOG: Record<string, LocalizedQuickFact[]> = {
  ...
  '11': [
    t('Shake, hold, wipe', 'Biphasic lip and eye makeup remover. Shake, cotton pad, a few seconds, wipe.', 'Встряхнуть, подержать, стереть', 'Двухфазный ремувер для губ и глаз. Встряхнуть, диск, несколько секунд, стереть.', 'رجّي، انتظري، امسحي', 'مزيل ثنائي الطور للشفاه والعين. رجّي، قطنة، ثوانٍ، امسحي.'),
    t('Oil layer 49.8%', 'Cetyl ethylhexanoate 27.8%, disiloxane 13%, isohexadecane 9%. That is the remover.', 'Масляный слой 49,8%', 'Цетилэтилгексаноат 27,8%, дисилоксан 13%, изогексадекан 9%. Это ремувер.', 'طبقة الزيت 49.8%', 'سيتيل إيثيل هكسانوات 27.8%، ديسيلوكسان 13%، إيزوهكسا ديكان 9%. هذا هو المزيل.'),
    t('Fresh, not greasy', 'Disiloxane flashes off. The carton calls it a fresh, non-greasy wipe.', 'Свежий, не жирный', 'Дисилоксан улетучивается. Коробка пишет: свежее, нежирное стирание.', 'منعش لا دهني', 'الديسيلوكسان يتطاير. العلبة تسميه مسحة منعشة غير دهنية.'),
    t('Lip and eye only', 'Not a face wash. Cleanse after with Snow O₂ or Cerabarrier.', 'Только губы и глаза', 'Не умывание. Затем Snow O₂ или Cerabarrier.', 'الشفاه والعين فقط', 'ليس غسول وجه. اغسلي بعدها بـ Snow O₂ أو Cerabarrier.'),
    t('Peptides at trace', 'Palmitoyl Tripeptide-5 0.65 ppb. Acetyl Tetrapeptide-5 0.5 ppb. Not the engine.', 'Пептиды в следе', 'Пальмитоил трипептид-5 0,65 ppb. Ацетил тетрапептид-5 0,5 ppb. Не двигатель.', 'الببتيدات عند أثر', 'بالميتويل تراي ببتيد-5 0.65 جزء في البليون. أسيتيل تترا ببتيد-5 0.5. ليست المحرك.'),
    t('200 ml, 12 months', 'Dermatologically tested. Use within 12 months of opening.', '200 мл, 12 месяцев', 'Дерматологически протестировано. 12 месяцев после вскрытия.', '200 مل، 12 شهراً', 'مختبر جلدياً. يُستخدم خلال 12 شهراً من الفتح.'),
  ],
  '12': [
  ...
  '14': [
    t('Shake, then spray', 'Opaque emulsion. Shake well, spray at 10-20 cm with eyes closed, through the day.', 'Встряхните, потом спрей', 'Непрозрачная эмульсия. Встряхните, распылите с 10-20 см, глаза закрыты, в течение дня.', 'رجّي ثم رشي', 'مستحلب معتم. رجّي جيداً، رشي على بعد 10-20 سم والعينان مغلقتان، طوال اليوم.'),
    t('Shea 1.2%', 'The largest named oil after the solvents. That is why the mist is opaque and why you shake.', 'Ши 1,2%', 'Самое крупное названное масло после растворителей. Поэтому мист непрозрачный и поэтому встряхивание.', 'زبدة الشيا 1.2%', 'أكبر زيت مسمّى بعد المذيبات. لهذا الرذاذ معتم ولهذا ترجّين.'),
    t('Moisturizing, nourishing', 'The carton function. Not a water toner. Not Snow Booster.', 'Увлажнение и питание', 'Функция на коробке. Не водный тоник. Не Snow Booster.', 'ترطيب وتغذية', 'وظيفة العلبة. ليس تونر ماء. ليس Snow Booster.'),
    t('Over makeup', 'The carton says it can be sprayed over make-up. Before makeup it is the glow pass.', 'Поверх макияжа', 'Коробка говорит, что можно поверх макияжа. До макияжа это проход на сияние.', 'فوق المكياج', 'العلبة تقول يمكن رشه فوق المكياج. قبل المكياج هو تمريرة التوهج.'),
    t('Peptide at 0.000001%', 'Acetyl Heptapeptide-4 is on the INCI. It is not the mist.', 'Пептид 0,000001%', 'Acetyl Heptapeptide-4 в INCI. Он не мист.', 'الببتيد عند 0.000001%', 'أسيتيل هيبتاببتيد-4 على القائمة. ليس الرذاذ.'),
    t('80 ml', 'pH 5.48, inside 5.00 to 6.00. 12 months after opening.', '80 мл', 'pH 5,48, в пределах 5,00-6,00. 12 месяцев после вскрытия.', '80 مل', 'الأس الهيدروجيني 5.48، داخل 5.00 إلى 6.00. 12 شهراً بعد الفتح.'),
  ],
  '35': [
  ...
  '41': [
    ...
    t('Licensed for three things', 'Korea registers it for UV protection, whitening and wrinkle improvement at once.', 'Лицензия сразу на три', 'Корея регистрирует его сразу как защиту от UV, осветление и коррекцию морщин.', 'مرخّص لثلاث وظائف', 'ترخّصه كوريا للحماية من الأشعة والتفتيح وتحسين التجاعيد معاً.'),
  ...
  '21': [
    ...
    t('Niacinamide 20,000 ppm', 'Two percent, printed on the carton, and the registered whitening active in Korea.', 'Ниацинамид 20 000 ppm', 'Два процента, напечатано на упаковке, зарегистрированный в Корее осветляющий актив.', 'نياسيناميد ٢٠٬٠٠٠ ppm', 'اثنان بالمئة، مطبوعة على العلبة، والفعّال المسجّل للتفتيح في كوريا.'),
    ...
    t('It may sting at first', 'The maker says so: start with a small amount and build up. Stop if it continues.', 'Поначалу может пощипывать', 'Производитель предупреждает: начните с малого и наращивайте. Не проходит — прекратите.', 'قد يلسع في البداية', 'المصنّع يقول ذلك: ابدئي بكمية صغيرة وتدرّجي. وإن استمر، توقّفي.'),
  ],
  '27': [
  ...
  '18': [
    ...
    t('Apply and pat AM/PM', 'On the face, fingers, morning and evening. The carton stops here.', 'Нанеси и похлопай', 'На лицо, пальцами, утро и вечер. Коробка останавливается здесь.', 'ضعي وربّتي ص/م', 'على الوجه، بالأصابع، صباحاً ومساءً. العلبة تتوقف هنا.'),
    t('Hyaluronan 11 complex', 'Brand name. Eight hyaluronate INCIs. One of them is the 2,000 ppm.', 'Комплекс Hyaluronan 11', 'Имя бренда. Восемь гиалуронатов. Один из них - 2 000 ppm.', 'مركّب Hyaluronan 11', 'اسم العلامة. ثمانية أسماء هيالورونات. واحد منها هو الـ 2,000.'),
    t('PENTAVITIN 0.615%', 'Saccharide isomerate at a real dose. The deck calls it a moisture magnet.', 'PENTAVITIN 0,615%', 'Сахарид изомерат в реальной дозе. В презентации - магнит влаги.', 'PENTAVITIN 0.615%', 'سكاريد أيزوميرات بجرعة حقيقية. العرض يسمّيه مغناطيس رطوبة.'),
    t('Coconut water 0.80%', 'The carton calls it a coconut-water serum. Aqua is still the water. Not 78%.', 'Кокосовая вода 0,80%', 'Коробка называет это сывороткой на кокосовой воде. Вода всё равно Aqua. Не 78%.', 'ماء جوز الهند 0.80%', 'العلبة تسمّيه سيروم ماء جوز الهند. الماء ما زال Aqua. ليست 78%.'),
    t('Immediate, not +52%', 'Inner hydration moved after one use. The leftover 52% was a misread of 52.238.', 'Сразу, не +52%', 'Внутренняя гидратация сдвинулась после одного нанесения. Старые 52% - неверное чтение 52,238.', 'فوراً، لا +52%', 'الترطيب الداخلي تحرّك بعد استخدام واحد. الـ 52% القديمة قراءة خاطئة لـ 52.238.'),
  ],
  '15': [
    t('Zinc PCA 0.5%', 'The named active at a real dose. This is the oil-control toner, not a water toner.', 'Zinc PCA 0,5%', 'Названный актив в реальной дозе. Тоник для контроля жира, не водный тоник.', 'زنك PCA 0.5%', 'المكوّن المسمّى بجرعة حقيقية. تونر تحكم بالدهون، لا تونر ماء.'),
    t('Apply or spray AM/PM', 'After cleansing, enough to cover the face. Morning and evening.', 'Нанеси или распыли утро/вечер', 'После умывания, достаточно чтобы покрыть лицо. Утром и вечером.', 'ضعي أو رشي ص/م', 'بعد التنظيف، بما يكفي لتغطية الوجه. صباحاً ومساءً.'),
    t('Oil control, then water', 'Takes excess oil and sebum, then puts water back. Carton function is oil control.', 'Сначала жир, потом вода', 'Снимает лишний жир и себум, затем возвращает воду. Функция на коробке - контроль жира.', 'الدهون ثم الماء', 'يزيل الزيت والزهم الزائد ثم يعيد الماء. وظيفة العلبة التحكم بالدهون.'),
    t('200 ml 360°', 'The home bottle sprays upside down for the back and the neck.', '200 мл 360°', 'Домашний флакон распыляет вверх дном для спины и шеи.', '200 مل 360°', 'زجاجة المنزل ترش بالمقلوب للظهر والرقبة.'),
