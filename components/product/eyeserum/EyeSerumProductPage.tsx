...
/**
 * Bespoke product page for EyeCell EYE CONTOUR SERUM (product 17).
 *
 ...
 * cool silver / lilac palette on top via eyeserum.css, taken from the black
 * syringe applicator and the silver collar.
 *
 * Section order:
 *
 *   effects  Deep wrinkles · Dark circles · Eye puffs, the artwork sentence
 *   engine   Arbutin 2% + Adenosine 0.04%, the distinctive documented fact
 *   howTo    AM & PM, pat the serum, cream 24 seals
 ...
 *   routine  cleanse → patch 33 → serum 17 → cream 24
 *
 * There is a dedicated DTS MG deck and a safety assessment, and no quantified
 * clinical trial of this finished serum, so this page has no proof chart.
 * See eyeserumCopy.ts for the sourcing rules. In particular: Arbutin 2%
 * leads, not the peptide premix; no 10 Years Back; no Botox.
 */
...
import { getEyeSerumCopy } from './eyeserumCopy'
...
/** Formula slide for the engine section. Main / s1 / s7 still print
 *  10 Years Back or Turn Years Back on the bottle. s3 and s4 are the same
 *  file. The engine figure is s6, which prints Arbutin 2% and Adenosine
 *  0.04%. The "Intertek formula" bake-in on s6 is a later image job. */
...
export default function EyeSerumProductPage({
  product,
  unitsSold = 0,
  routineProducts = [],
}: Props) {
  ...
  const isRtl = dir === 'rtl'
  const copy = getEyeSerumCopy(locale)
  const Chevron = isRtl ? ChevronLeft : ChevronRight

  // Single SKU - one 10ml serum - so no size is ever passed to the cart.
  ...
  const routineSteps = useMemo(() => {
    const routine = PRODUCT_ROUTINES['17']
    if (!routine) return []
  ...
  const addToCart = useCallback(
    async (qty: number) => {
      ...
      try {
        ...
        errorLog('Eye serum: add to cart failed', error)
        throw error
      }
    },
  ...
  const handleAddRoutineProduct = useCallback(
    async (item: Product, selection?: ProductOptionSelection, quantity = 1) => {
      ...
      try {
        ...
        errorLog('Eye serum: routine add to cart failed', error)
      } finally {
  ...
  return (
    <div className={`cera-page eys-page ${ceraSerif.variable} min-h-[100dvh]`} dir={dir}>
      {/* ───────────────────────────── Hero ─────────────────────────────── */}
      ...
      <section className="mx-auto max-w-[1000px] px-4 py-16 sm:px-6 lg:py-24">
        ...
        <div className="mt-10 grid grid-cols-1 gap-4 lg:mt-14 lg:grid-cols-2 lg:gap-6">
          ...
          <CeraReveal delay={90} className="cera-card eys-not p-6 lg:p-7">
            <h3 className="cera-serif text-[21px] leading-tight text-[var(--cera-ink)]">
