...
import { sendWelcomeEmail, sendOrderConfirmationEmail, sendAdminNewUserNotification, sendAdminNewOrderNotification, sendOrderStatusUpdate, sendEmail, generateCODOrderHTML, sendDiscountAssignmentEmail } from '@/lib/email'
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    switch (type) {
      ...
      default:
        return NextResponse.json(
          { error: 'Invalid email type. Use: welcome, order, admin-user, admin-order, order-status, cod, or discount-assigned' },
          { status: 400 }
        )
    }
