...
/**
 * Miss Anna — new customer + order → invoice → shipment.
 *
 *   Beige cushion (00144) ×1 @ 300 AED retail
 *   Excellent Delivery Dubai ×1 @ 45 AED
 *
 *   node --import dotenv/config scripts/moysklad-create-miss-anna-order-invoice-demand-20260613.js
 *   node --import dotenv/config scripts/moysklad-create-miss-anna-order-invoice-demand-20260613.js --commit
 */
...
const ORDER = {
  name: `GENCardM${uaeShortDate()}3623`,
  moment: uaeMomentNow(),
  marker: `MISS-ANNA-BEIGE-CUSHION-RETAIL-${uaeToday()}`,
}
...
async function findExistingCounterparty() {
  ...
  const bySearch = await api(
    'GET',
    `/entity/counterparty?search=${encodeURIComponent('589863623')}&limit=10`
  )
  return (bySearch?.rows || []).find((r) => (r.phone || '').includes('589863623')) || null
}
...
function ordersPdfPath(invoiceName) {
  ...
  return path.join(ordersDir, `GENOSYS_Miss_Anna_${safe}.pdf`)
}

async function main() {
  console.log('====================================================================')
  console.log('  Miss Anna — customer + order → invoice → shipment')
  console.log('====================================================================')
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      'Beige cushion 00144 x1 @ 300 AED retail; delivery 45 AED.',
