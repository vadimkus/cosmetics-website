...
/**
 * Admin Shakirovna Salon — full cycle: order → invoice → shipment → paymentin + PDF.
 *
 *   Multi-Vita Radiance Cream 50g (00122) ×1 @ 145 AED
 *   Ultra Shield SPF50 50g (54457) ×1 @ 125 AED
 *   Excellent Delivery Dubai ×1 @ 15 AED
 *
 *   node --import dotenv/config scripts/moysklad-create-admin-shakirovna-radiance-spf-order-invoice-demand-paymentin-20260625.js
 *   node --import dotenv/config scripts/moysklad-create-admin-shakirovna-radiance-spf-order-invoice-demand-paymentin-20260625.js --commit
 */
...
const ORDER = {
  name: `GENCardM${uaeShortDate()}SHKR`,
  moment: uaeMomentNow(),
  marker: `ADMIN-SHAKIROVNA-RADIANCE-SPF-${uaeToday()}`,
}

/** [code, qty, unitAed VAT incl.] */
const PRODUCT_LINES = [
  ['00122', 1, 145],
  ['54457', 1, 125],
]
const DELIVERY_AED = 15
const EXPECTED_SUM_MINOR = 28500
...
function orgAccountHref(accountId) {
  return {
    meta: {
      href: `${API}/entity/account/${accountId}`,
      type: 'account',
...
function buildShipmentAddress(agent) {
  ...
  return { country: countryHref(), city: 'Dubai', street: 'Admin Shakirovna Salon — UAE' }
}
...
async function main() {
  console.log('====================================================================')
  console.log('  Admin Shakirovna — radiance cream + SPF50 + delivery')
  console.log('====================================================================')
  ...
  for (const [code, qty, unitAed] of PRODUCT_LINES) {
    const item = stock.get(code)
    console.log(`    ${code} ${item.name} x${qty} @ ${unitAed.toFixed(2)} AED`)
  }
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      '00122 x1 @ 145; 54457 x1 @ 125; delivery 15 AED.',
