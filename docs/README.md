...
| File | Description |
|------|-------------|
| [SESSION_CHANGES_2026-04-18.md](./SESSION_CHANGES_2026-04-18.md) | **NEW** Three items. (1) **Sentry wired up, LogRocket dropped** — real prod error tracking via `@sentry/nextjs` with Next 16 instrumentation (`instrumentation.ts`, `instrumentation-client.ts`, `sentry.{server,edge}.config.ts`), `app/global-error.tsx` now a plain-HTML last-resort boundary, `lib/monitoring.ts` simplified, ~150 KB client bundle freed. (2) **Page caching strategy (ISR + `revalidateTag`)** — `/products/[id]` drops `force-dynamic` → `revalidate = 300` with `unstable_cache` layer tagged `products`; FAQ pages (EN/AR/RU) share new `lib/faqDb.ts` + tag `faq`; AR/RU blog slugs now `revalidate = 60` to match EN; admin product/FAQ routes call `revalidateTag('…', 'max')` on write. Repeat-visit LCP comes from cache; admin edits propagate instantly. (3) **CLI observability tooling** — Sentry + Vercel CLIs installed globally, new `scripts/sentry-errors.js` wrapper over Sentry REST API, three npm scripts: `sentry:errors`, `vercel:logs`, `vercel:logs:follow`. `NEXT_PUBLIC_SENTRY_DSN` wired in Vercel (`cosmetics-website2` project, all environments), empty-commit redeploy triggered ingestion; verified end-to-end with `sentry-cli send-event` (release tag matches `VERCEL_GIT_COMMIT_SHA`). `.env.example` updated with real org slug (`genosys-middle-east-fz-llc`) and `SENTRY_AUTH_TOKEN` guidance (PAT with `project:read` + `event:read`). |
