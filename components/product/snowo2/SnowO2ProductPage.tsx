...
/**
 * Bespoke product page for SNOW O2 CLEANSER (product 10).
 *
 ...
 * ice-white / ember-orange palette on top via snowo2.css, taken from the pump.
 *
 * Section order:
 *
 *   effects  Apply dry · Bubbles · Rinse
 *   engine   ether 8%, the bubbles on dry skin
 *   howTo    dry face, bubbles, massage, rinse, plus the product video
 ...
 *   routine  cleanse → booster → Multi Vita serum → cream → SPF
 *
 * There is a safety assessment and no quantified clinical trial, so this page
 * has no proof chart. See snowo2Copy.ts for the sourcing rules. In particular:
 * do not rebuild the page around Phytolex at 0.003% or MultiEx at 0.001%, and
 * do not sell oxygen therapy.
 */
...
import { getSnowO2Copy } from './snowo2Copy'
...
export default function SnowO2ProductPage({
  product,
  unitsSold = 0,
  routineProducts = [],
}: Props) {
  ...
  const sizeOptions = useMemo(() => getProductSizeOptions('10', product), [product])
  const [selectedSize, setSelectedSize] = useState(sizeOptions[0]?.value || '180ml')


  const isRtl = dir === 'rtl'
  const copy = getSnowO2Copy(locale)
  const Chevron = isRtl ? ChevronLeft : ChevronRight

  // Two sizes, 180 ml and 500 ml. The selected size is passed to the cart.
  ...
  const routineSteps = useMemo(() => {
    const routine = PRODUCT_ROUTINES['10']
    if (!routine) return []
  ...
  const addToCart = useCallback(
    async (qty: number) => {
      ...
      try {
        ...
        errorLog('SnowO2: add to cart failed', error)
        throw error
      }
    },
  ...
  const handleAddRoutineProduct = useCallback(
    async (item: Product, selection?: ProductOptionSelection, quantity = 1) => {
      ...
      try {
        ...
        errorLog('SnowO2: routine add to cart failed', error)
      } finally {
  ...
  return (
    <div className={`cera-page snowo2-page ${ceraSerif.variable} min-h-[100dvh]`} dir={dir}>
      {/* ───────────────────────────── Hero ─────────────────────────────── */}
        ...
        <div>
          ...
          {product.videoUrl ? (
            <CeraReveal className="mt-10">
              ...
              <div className="snowo2-video relative mt-4 aspect-square overflow-hidden rounded-[28px] sm:aspect-video">
                <video
      ...
      <section className="mx-auto max-w-[1000px] px-4 py-16 sm:px-6 lg:py-24">
        ...
        <div className="mt-10 grid grid-cols-1 gap-4 lg:mt-14 lg:grid-cols-2 lg:gap-6">
          ...
          <CeraReveal delay={90} className="cera-card snowo2-not p-6 lg:p-7">
            <h3 className="cera-serif text-[21px] leading-tight text-[var(--cera-ink)]">
