...
/**
 * Persona Nakheel Mall — paymentin @ shipment 06793 / invoice 05013.
 * 500 AED. Order GENCardM260902PERSNK → Доставлен.
 *
 *   node --import dotenv/config scripts/moysklad-create-persona-nakheel-paymentin-05013-20260903.js
 *   node --import dotenv/config scripts/moysklad-create-persona-nakheel-paymentin-05013-20260903.js --commit
 */
...
const MARKER = `PERSONA-NAKHEEL-PAYMENTIN-05013-${uaeToday()}`

const PAYMENT = {
  amountMinor: 50000,
  invoiceName: '05013',
  invoiceId: '50221f24-a6de-11f1-0a80-17c5003c6432',
  shipmentName: '06793',
  shipmentId: '50e76755-a6de-11f1-0a80-08a9003f4d9e',
  orderName: 'GENCardM260902PERSNK',
  orderId: '4fcd7f37-a6de-11f1-0a80-105a004028ac',
}
...
async function main() {
  console.log('====================================================================')
  console.log('  Persona Nakheel — paymentin @ 05013 / 06793')
  console.log('====================================================================')
  ...
  if (!/first person ladies salon llc \(palm/i.test(order.agent?.name || '')) {
  ...
  const created = await api('POST', '/entity/paymentin', {
    ...
    description: [
      ...
      `Order ${PAYMENT.orderName} — 500 AED.`,
    ].join(' | '),
    operations: [
      {
        meta: {
