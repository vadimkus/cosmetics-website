...
/**
 * Lips for Kiss Clinic — SO → invoice → shipment (order 2, 2026-08-05).
 *
 *   00021 Snow O₂ Cleanser 180ml ×3
 *   54458 Hyaluron Cream 50g ×3
 *   00129 EPI Peeling Gel ×1
 *   00143 Cushion Ivory ×1
 *   00041 Multi Sun Cream SPF40/PA++ ×3
 *
 * Clinic list (salePrice). Invoice PDF → ~/Desktop/orders/
 *
 *   node --import dotenv/config scripts/moysklad-create-lips-for-kiss-order2-invoice-demand-20260805.js
 *   node --import dotenv/config scripts/moysklad-create-lips-for-kiss-order2-invoice-demand-20260805.js --commit
 */
...
const ORDER = {
  name: `GENCardM${uaeShortDate()}LFK2`,
  moment: uaeMomentNow(),
  marker: `Lips for Kiss order2 snow hyaluron EPI ivory SPF ${uaeToday()}`,
}
...
const PRODUCT_LINES = [
  ...
  ['00041', 3], // Multi Sun Cream SPF40/PA++ 40g
]

const EXPECTED_SUM_MINOR = 152000 // 1,520 AED @ clinic list
...
async function main() {
  console.log('====================================================================')
  console.log('  Lips for Kiss Clinic — order 2 SO + invoice + shipment')
  console.log('====================================================================')
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      '00021 x3, 54458 x3, 00129 x1, 00143 x1, 00041 x3 @ clinic list.',
    ].join(' | '),
