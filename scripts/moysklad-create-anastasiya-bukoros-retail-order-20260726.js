...
/**
 * Miss Anastasiya Bukoros (0585301121) — retail SO → invoice → shipment → paymentin + landscape print.
 *
 *   Update address → Westwood Grande II by Imtiaz, apartment 501 (tower 2), Dubai
 *   54464 Camel cushion ×1 @ 300 retail
 *   00063 Collagen mask ×2 @ 36, 100% off (free)
 *   00140 Sea Algae mask ×2 @ 36, 100% off (free)
 *   Delivery Dubai ×1 @ 45
 *   Total: 345 AED
 *
 *   node --import dotenv/config scripts/moysklad-create-anastasiya-bukoros-retail-order-20260726.js
 *   node --import dotenv/config scripts/moysklad-create-anastasiya-bukoros-retail-order-20260726.js --commit
 *   node --import dotenv/config scripts/moysklad-create-anastasiya-bukoros-retail-order-20260726.js --commit --no-print
 */
...
const AGENT_ID = '52db9889-45e8-11f0-0a80-169e000d8e56' // Miss Anastasiya Bukoros
...
const CUSTOMER = {
  name: 'Miss Anastasiya Bukoros',
  phone: '0585301121',
  city: 'Dubai',
  street: 'Westwood Grande II by Imtiaz, apartment 501 (tower 2)',
}

const ORDER = {
  name: `GENCardM${uaeShortDate()}1121`,
  moment: uaeMomentNow(),
  marker: `ANASTASIYA-BUKOROS-CAMEL-MASKS-FREE-${uaeToday()}`,
}
...
async function updateCounterpartyAddress() {
  ...
  console.log(`  Address updated: ${CUSTOMER.street}`)
  return updated
}
...
async function exportInvoicePdf(invoiceId, invoiceName) {
  ...
  const outPath = path.join(ORDERS_DIR, `GENOSYS_Miss_Anastasiya_Bukoros_${safe}.pdf`)
...
function printPdfLandscape(pdfPath) {
  ...
  if (whichLp.status === 0 && whichLp.stdout.trim()) {
    try {
      ...
      console.log('  Sent to printer (lp, landscape).')
      return
    } catch (e) {
...
async function main() {
  console.log('====================================================================')
  console.log('  Miss Anastasiya Bukoros — retail paid order + print')
  console.log('====================================================================')
  console.log(`  Mode: ${COMMIT ? 'COMMIT' : 'DRY RUN'}`)

  const agent = await api('GET', `/entity/counterparty/${AGENT_ID}`)
  console.log(`  Customer: ${agent.name} | ${agent.phone || CUSTOMER.phone}`)
  console.log(`  Old addr: ${agent.actualAddress || agent.actualAddressFull?.street || '(none)'}`)

  await ensureOrderNameFree()
  if (COMMIT) {
    await ensureNoDuplicateToday(agent.id)
    await updateCounterpartyAddress()
  } else {
    console.log(`  Would update address → ${CUSTOMER.street}`)
  }
  ...
  for (const [code, qty, retailAed, discountPct] of PRODUCT_LINES) {
    ...
    const disc = discountPct === 100 ? 'FREE' : 'retail'
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      '54464 camel cushion x1 @300 retail; collagen x2 FREE; sea algae x2 FREE; delivery 45.',
