 ...
 *   00042 EGF Repair Oxymask Cream 50ml ×1 @ 145
 *   00188 Microbiome Energy Infusing Mist 80ml ×1 @ 80
 *   Total 225 AED on agreement 00025.
 *
 *   node --import dotenv/config scripts/moysklad-create-aniskina-egf-mist-sales-paymentin-20260829.js
 *   node --import dotenv/config scripts/moysklad-create-aniskina-egf-mist-sales-paymentin-20260829.js --commit
 */
...
const MARKER = `ANISKINA-EGF-MIST-SALES-${uaeToday()}`
const LINES = [
  ['00042', 1, 145],
  ['00188', 1, 80],
]
const EXPECTED_SUM_MINOR = 22500
...
async function main() {
  console.log('====================================================================')
  console.log('  Aniskina — consignment sales EGF + mist + paymentin')
  console.log('====================================================================')
  ...
  const report = await api('POST', '/entity/commissionreportin', {
    ...
    description: [
      ...
      '00042 EGF Oxymask x1 @145; 00188 mist x1 @80.',
    ].join('\n'),
  ...
  const paymentIn = await api('POST', '/entity/paymentin', {
    ...
    description: [
      ...
      'Tatiana Aniskina Nail Master — EGF + mist — paid in full.',
    ].join(' | '),
    operations: [
      {
        meta: {
