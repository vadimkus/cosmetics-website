...
/**
 * Miss Indira Urazova — new customer + order → invoice → shipment + landscape PDF.
 *
 ...
 *   Sea Algae Mask 23g (00140) ×1
 *   Collagen Mask 23g (00063) ×1
 *   Skin Barrier Protecting Cream (00037) ×1
 *
...
const CUSTOMER = {
  name: 'Miss Indira Urazova',
  phone: '+971585922075',
  city: 'Dubai',
  street: 'Arabian Ranches, Mirador St 1, Villa 75',
}

const ORDER = {
  name: `GENCardM${uaeShortDate()}2075`,
  moment: uaeMomentNow(),
  marker: `MISS-INDIRA-URAZOVA-CLINIC-${uaeToday()}`,
}
...
function ordersPdfPath(invoiceName) {
  ...
  return path.join(ordersDir, `GENOSYS_Miss_Indira_Urazova_${safe}.pdf`)
}

async function main() {
  console.log('====================================================================')
  console.log('  Miss Indira Urazova — customer + order → invoice → shipment')
  console.log('====================================================================')
  ...
  console.log(`\n  Order: ${ORDER.name} (clinic list, vatIncluded: true)`)
  for (const [code, qty] of PRODUCT_LINES) {
    const item = stock.get(code)
    const unit = item.price / 100
    console.log(
      `    ${code} ${item.name.slice(0, 55)} x${qty} @ ${unit.toFixed(2)} → ${(unit * qty).toFixed(2)} AED`
    )
  }
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      'Beige cushion x2, PDRN pack, SPF50, mist, sea algae + collagen masks, barrier cream x1 each @ clinic.',
