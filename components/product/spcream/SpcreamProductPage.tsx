...
import { getSpcreamCopy } from './spcreamCopy'
...
const PROOF_IMAGE = '/images/skin_barr/s3.jpeg'
...
export default function SpcreamProductPage({
  product,
  unitsSold = 0,
  routineProducts = [],
}: Props) {
  ...
  const isRtl = dir === 'rtl'
  const copy = getSpcreamCopy(locale)
  ...
  const addToCart = useCallback(
    async (qty: number) => {
      ...
      try {
        ...
        errorLog('SpCream: add to cart failed', error)
        throw error
      }
    },
  ...
  const handleAddRoutineProduct = useCallback(
    async (item: Product, selection?: ProductOptionSelection, quantity = 1) => {
      ...
      try {
        ...
        errorLog('SpCream: routine add to cart failed', error)
      } finally {
  ...
  return (
    <div className={`cera-page spcream-page ${ceraSerif.variable} min-h-[100dvh]`} dir={dir}>
      {/* ───────────────────────────── Hero ─────────────────────────────── */}
        ...
        <div>
          ...
          {product.videoUrl ? (
            <CeraReveal className="mt-10">
              ...
              <div className="spcream-video relative mt-4 mx-auto aspect-[9/16] w-full max-w-[340px] overflow-hidden rounded-[28px]">
                <video
      ...
      <section className="mx-auto max-w-[1000px] px-4 py-16 sm:px-6 lg:py-24">
        ...
        <div className="mt-10 grid grid-cols-1 gap-4 lg:mt-14 lg:grid-cols-2 lg:gap-6">
          ...
          <CeraReveal delay={90} className="cera-card spcream-not p-6 lg:p-7">
            <h3 className="cera-serif text-[21px] leading-tight text-[var(--cera-ink)]">
