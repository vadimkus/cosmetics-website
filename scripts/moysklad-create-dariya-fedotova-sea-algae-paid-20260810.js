...
/**
 * Miss Dariya Fedotova — new retail customer + paid order.
 * Sea Algae Mask ×5 @ 36 + delivery Dubai @ 45 = 225 AED.
 * SO → invoice → shipment → paymentin + Legal TAX PDF landscape print.
 *
 *   node --import dotenv/config scripts/moysklad-create-dariya-fedotova-sea-algae-paid-20260810.js
 *   node --import dotenv/config scripts/moysklad-create-dariya-fedotova-sea-algae-paid-20260810.js --commit
 *   node --import dotenv/config scripts/moysklad-create-dariya-fedotova-sea-algae-paid-20260810.js --commit --no-print
 */
...
const CUSTOMER = {
  name: 'Miss Dariya Fedotova',
  email: 'dariyafedotova888@gmail.com',
  phone: '+971 54 747 4655',
  city: 'Dubai',
  street: 'Al Barari, Ashjar E2, apartment 213',
}

const ORDER = {
  name: `GENCardM${uaeShortDate()}4655`,
  moment: uaeMomentNow(),
  marker: `DARIYA-FEDOTOVA-SEA-ALGAE-PAID-${uaeToday()}`,
}

/** [code, qty, retailAed, discountPct] */
const PRODUCT_LINES = [['00140', 5, 36, 0]]
const DELIVERY_AED = 45
const DELIVERY_DISCOUNT = 0
const EXPECTED_SUM_MINOR = 22500 // 5×36 + 45
...
function customerPayload() {
  return {
    ...
    description: `Retail — ${CUSTOMER.street}, Dubai. Phone ${CUSTOMER.phone}. Email ${CUSTOMER.email}.`,
  }
}
...
function isDariyaMatch(r) {
  const phoneDigits = (r.phone || '').replace(/\D/g, '')
  const email = (r.email || '').toLowerCase()
  const name = r.name || ''
  return (
    email === CUSTOMER.email.toLowerCase() ||
    phoneDigits.endsWith('547474655') ||
    /^miss\s+dariya\s+fedotova$/i.test(name.trim())
  )
}

async function findOrCreateCustomer() {
  // Prefer known id (created 2026-08-10 after Iryna Fedotova false-match fix)
  const KNOWN_ID = 'e3472149-94cd-11f1-0a80-1029008dd68b'
  try {
    const known = await api('GET', `/entity/counterparty/${KNOWN_ID}`)
    if (isDariyaMatch(known)) {
      console.log(`  Customer: ${known.name} (${known.id})`)
      if (COMMIT) await api('PUT', `/entity/counterparty/${known.id}`, customerPayload())
      return known
    }
  } catch (_) {
    /* create below */
  ...
  for (const q of ['Dariya Fedotova', 'dariyafedotova888', '547474655']) {
    const d = await api('GET', `/entity/counterparty?search=${encodeURIComponent(q)}&limit=20`)
    const hit = (d.rows || []).find(isDariyaMatch)
    if (hit) {
...
async function exportInvoicePdf(invoiceId, invoiceName) {
  ...
  const outPath = path.join(ORDERS_DIR, `GENOSYS_Miss_Dariya_Fedotova_${safe}.pdf`)
...
async function main() {
  console.log('====================================================================')
  console.log('  Miss Dariya Fedotova — sea algae ×5 @36 + delivery 45 (paid)')
  console.log('====================================================================')
  ...
  for (const p of positions) {
    if (p._name) {
      console.log(
        `    ${p._code} ${p._name.slice(0, 50)} x${p.quantity} @ ${p._retail} (avail ${p._avail})`,
      )
    } else {
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      '00140 Sea Algae Mask x5 @36 retail; delivery Dubai @45.',
      `Ship: ${CUSTOMER.street}, Dubai. Phone ${CUSTOMER.phone}. Email ${CUSTOMER.email}.`,
      'Paid in full — delivered.',
    ].join('\n'),
