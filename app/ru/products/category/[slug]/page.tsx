...
export async function generateMetadata(
  ...
  return {
    ...
    openGraph: {
      ...
      description: seo.description,
      type: 'website',
      url: `${baseUrl}/ru/products/category/${slug}`,
      siteName: 'GENOSYS',
      locale: 'ru_AE',
      images: [{ url: `${baseUrl}/images/genosys-products.jpg`, width: 1200, height: 630, alt: seo.h1 }],
    },
    alternates: {
      canonical: `${baseUrl}/ru/products/category/${slug}`,
      languages: {
...
export default async function RussianCategoryPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params
  const category = getCategoryBySlug(slug)
    ...
    <div className={`cera-page genosys-page ${ceraSerif.variable} min-h-screen`}>
      <BreadcrumbSchema
        items={[
          ...
          { name: seo.h1, url: `/ru/products/category/${slug}` },
        ]}
      />
      {products.length > 0 && (
        <CollectionPageSchema
          name={seo.h1}
          description={seo.description}
          url={`https://genosys.ae/ru/products/category/${slug}`}
          products={products}
        />
      )}
      ...
      <section className="px-4 py-12 sm:py-16">
        <div className="max-w-6xl mx-auto text-center">
          <nav className="text-sm text-[var(--cera-muted)] mb-4">
            <Link href="/ru" className="hover:text-[var(--cera-body)]">Главная</Link>
            <span className="mx-2">/</span>
            <Link href="/ru/products" className="hover:text-[var(--cera-body)]">Продукция</Link>
            <span className="mx-2">/</span>
      ...
      <section className="py-6 px-4">
        <div className="max-w-6xl mx-auto">
          <Link href="/ru/products" className="inline-flex items-center text-blue-600 hover:text-blue-800 font-medium">
