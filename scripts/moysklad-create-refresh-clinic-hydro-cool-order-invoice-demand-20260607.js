...
/**
 * Refresh Clinic — customer order + invoice + shipment.
 *
 * Line:
 *   Hydro Cool Modeling Mask 1kg (00013) x1 @ MoySklad salePrice
 *
 *   node --import dotenv/config scripts/moysklad-create-refresh-clinic-hydro-cool-order-invoice-demand-20260607.js
 *   node --import dotenv/config scripts/moysklad-create-refresh-clinic-hydro-cool-order-invoice-demand-20260607.js --commit
 */
...
const ORDER = {
  name: `GENCardM${uaeShortDate()}REFH`,
  moment: uaeMomentNow(),
  marker: `Refresh Clinic Hydro Cool Modeling Mask order invoice shipment ${uaeToday()}`,
}
...
async function main() {
  console.log('====================================================================')
  console.log('  Refresh Clinic — SO + invoice + shipment (Hydro Cool)')
  console.log('====================================================================')
  ...
  const order = await api('POST', '/entity/customerorder', {
    name: ORDER.name,
    moment: t0,
    description: [ORDER.marker, 'Hydro Cool Modeling Mask 1kg 00013 x1. Chain: invoice → shipment.'].join(' | '),
