...
/**
 * Clinic customer — full cycle (paid):
 *   order → invoice → shipment → paymentin → invoice PDF
 *
 *   00144 Cushion #2 Beige ×1 @ 150 AED
 *   Excellent Delivery Dubai ×1 @ 15 AED
 *
 *   node --import dotenv/config scripts/moysklad-create-clinic-customer-cushion-order-full-cycle-20260701.js
 *   node --import dotenv/config scripts/moysklad-create-clinic-customer-cushion-order-full-cycle-20260701.js --commit
 */
...
const CUSHION_AED = 150
const DELIVERY_AED = 15
const EXPECTED_TOTAL_AED = 165

const ORDER = {
  name: `GENCardM${uaeShortDate()}CLINIC`,
  moment: uaeMomentNow(),
  marker: `Clinic customer cushion Beige delivery ${uaeToday()}`,
}
...
async function fetchStock00144() {
  const rows = await fetchAll('/report/stock/all?stockMode=all')
  const row = rows.find((r) => r.code === '00144')
  if (!row) throw new Error('Stock row not found for 00144')
  return {
...
function buildPositions(productId) {
  const cushionMinor = Math.round(CUSHION_AED * 100)
  ...
  return { positions, sumMinor: cushionMinor + deliveryMinor }
}
...
function ordersPdfPath(invoiceName) {
  ...
  return path.join(ordersDir, `GENOSYS_Clinic_Customer_${safe}.pdf`)
}

async function main() {
  console.log('====================================================================')
  console.log('  Clinic customer — full cycle (cushion Beige + delivery 15)')
  console.log('====================================================================')
  ...
  const stock = await fetchStock00144()
  if (stock.available < 1) {
    throw new Error(`Insufficient stock 00144: have ${stock.available}`)
  }

  const { positions, sumMinor } = buildPositions(stock.id)
  const shipment = shipmentAddress()

  console.log(`\n  Order: ${ORDER.name}`)
  console.log(`    00144 ${stock.name} x1 @ ${CUSHION_AED.toFixed(2)} AED`)
  ...
  if (Math.abs(sumMinor - EXPECTED_TOTAL_AED * 100) > 1) {
    throw new Error(`Total mismatch: ${money(sumMinor)} vs ${EXPECTED_TOTAL_AED}`)
  }
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      'Cushion Beige 00144 x1 @ 150; delivery 15 AED. Paid — paymentin posted.',
    ].join(' | '),
