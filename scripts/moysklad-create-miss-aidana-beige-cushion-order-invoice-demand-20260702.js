...
/**
 * Miss Aidana Yerkegaliyeva (existing) — full retail cycle:
 *   order → invoice → shipment + invoice PDF (retail print template)
 *
 * Same items as invoice 04223 (2026-03-05):
 *   Beige BB cushion (00144) ×2 @ 300 AED retail
 *   Excellent Delivery Dubai ×1 @ 40 AED
 *
 * Counterparty: e62a4998-8e9f-11ef-0a80-157e000dd8e4
 * Phone: +971 58 120 3838
 * Ship: Picadilly Green Villa 45, Damac Hills, Dubai
 *
 *   node --import dotenv/config scripts/moysklad-create-miss-aidana-beige-cushion-order-invoice-demand-20260702.js
 *   node --import dotenv/config scripts/moysklad-create-miss-aidana-beige-cushion-order-invoice-demand-20260702.js --commit
 */
...
const AGENT_ID = 'e62a4998-8e9f-11ef-0a80-157e000dd8e4' // Miss Aidana Yerkegaliyeva
...
const PHONE = '+971 58 120 3838'
const SHIPMENT = {
  city: 'Dubai',
  street: 'Picadilly Green Villa 45, Damac Hills',
}

const ORDER = {
  name: `GENCardM${uaeShortDate()}3838`,
  moment: uaeMomentNow(),
  marker: `MISS-AIDANA-BEIGE-CUSHION-RETAIL-${uaeToday()}`,
}

/** [code, qty, retailAed VAT incl.] */
const PRODUCT_LINES = [['00144', 2, 300]]
const DELIVERY_AED = 40
...
async function main() {
  console.log('====================================================================')
  console.log('  Miss Aidana Yerkegaliyeva — SO → invoice → shipment')
  console.log('====================================================================')
  console.log(`  Mode: ${COMMIT ? 'COMMIT' : 'DRY RUN'}`)

  const agent = await api('GET', `/entity/counterparty/${AGENT_ID}`)
  console.log(`  Counterparty: ${agent.name} | ${agent.phone || PHONE}`)
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      'Beige cushion 00144 x2 @ 300 AED retail; delivery 40 AED.',
      `Ship to: ${shipment.street}, ${shipment.city}.`,
      'Same items as prior invoice 04223.',
