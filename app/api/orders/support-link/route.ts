...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    debugLog('🎟️ SUPPORT-LINK DISCOUNT DEBUG:', JSON.stringify({
      orderNumber,
      customerEmail,
      userFound: !!user,
      userId: user?.id,
    ...
    debugLog('🎟️ SUPPORT-LINK DISCOUNT CALCULATED:', JSON.stringify({
      orderNumber,
    ...
    const orderHTMLData: OrderHTMLData = {
      ...
      items: items.map((item: { id?: string; name: string; quantity: number; price: number; total?: number; image?: string; size?: string; color?: string; bundleDiscount?: number }): OrderHTMLItem => {
        const itemName = item.name || 'Product'
