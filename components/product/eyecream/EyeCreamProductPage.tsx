 ...
 * Section order:
 *
 *   effects  Wrinkles · Dark circles · Puffiness, the artwork sentence
 *   engine   Arbutin 2% + Adenosine 0.04%, the distinctive documented fact
 *   howTo    AM & PM, serum first, cream seals
 ...
 * clinical trial of this finished cream, so this page has no proof chart.
 * See eyecreamCopy.ts for the sourcing rules. In particular: Arbutin 2%
...
import { getEyeCreamCopy } from './eyecreamCopy'
...
/** Formula slide for the engine section. Main / s1 / s6 still print 10 Years Back
 *  on the bottle, so the engine figure is s4, which prints Arbutin 2% and
 *  Adenosine 0.04%. The "Intertek formula" bake-in on s4 is a later image job. */
...
export default function EyeCreamProductPage({
  product,
  unitsSold = 0,
  routineProducts = [],
}: Props) {
  ...
  const isRtl = dir === 'rtl'
  const copy = getEyeCreamCopy(locale)
  const Chevron = isRtl ? ChevronLeft : ChevronRight

  // Single SKU - one five-treatment kit - so no size is ever passed to the cart.
  ...
  const routineSteps = useMemo(() => {
    const routine = PRODUCT_ROUTINES['24']
    if (!routine) return []
  ...
  const addToCart = useCallback(
    async (qty: number) => {
      ...
      try {
        ...
        errorLog('Eye cream: add to cart failed', error)
        throw error
      }
    },
  ...
  const handleAddRoutineProduct = useCallback(
    async (item: Product, selection?: ProductOptionSelection, quantity = 1) => {
      ...
      try {
        ...
        errorLog('Eye cream: routine add to cart failed', error)
      } finally {
  ...
  return (
    <div className={`cera-page eye-page ${ceraSerif.variable} min-h-[100dvh]`} dir={dir}>
      {/* ───────────────────────────── Hero ─────────────────────────────── */}
        ...
        <div>
          ...
          {product.videoUrl ? (
            <CeraReveal className="mt-10">
              ...
              <div className="eye-video relative mt-4 aspect-square overflow-hidden rounded-[28px] sm:aspect-video">
                <video
      ...
      <section className="mx-auto max-w-[1000px] px-4 py-16 sm:px-6 lg:py-24">
        ...
        <div className="mt-10 grid grid-cols-1 gap-4 lg:mt-14 lg:grid-cols-2 lg:gap-6">
          ...
          <CeraReveal delay={90} className="cera-card eye-not p-6 lg:p-7">
            <h3 className="cera-serif text-[21px] leading-tight text-[var(--cera-ink)]">
