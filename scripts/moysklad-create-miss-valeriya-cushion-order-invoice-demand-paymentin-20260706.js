...
/**
 * Miss Valeriya (existing, +971585207755) — full retail cycle, paid:
 *   order → invoice → shipment → paymentin → retail PDF
 *
 *   00144 Cushion #2 Beige x1 @ 300 AED
 *   Excellent Delivery Dubai x1 @ 45 AED
 *   Total: 345 AED
 *
 * Counterparty: 74932015-eaca-11f0-0a80-08de007a1ccf (Royal Atlantis; duplicate card 9efda634 ignored)
 *
 *   node --import dotenv/config scripts/moysklad-create-miss-valeriya-cushion-order-invoice-demand-paymentin-20260706.js
 *   node --import dotenv/config scripts/moysklad-create-miss-valeriya-cushion-order-invoice-demand-paymentin-20260706.js --commit
 */
...
const AGENT_ID = '74932015-eaca-11f0-0a80-08de007a1ccf' // Miss Valeriya
...
const PHONE = '+971585207755'
const SHIPMENT = {
  city: 'Dubai',
  street: 'The Royal Atlantis Residence, app 1201',
}

const ORDER = {
  name: `GENCardM${uaeShortDate()}7755`,
  moment: uaeMomentNow(),
  marker: `MISS-VALERIYA-CUSHION-RETAIL-${uaeToday()}`,
}
...
async function loadAndUpdateCounterparty() {
  ...
  console.log(`  Counterparty: ${agent.name} | was ${agent.phone || '—'}`)
  console.log(`  Address: ${SHIPMENT.street}, ${SHIPMENT.city}`)
  if (!COMMIT) return agent
...
function ordersPdfPath(invoiceName) {
  ...
  return path.join(ordersDir, `GENOSYS_Miss_Valeriya_${safe}.pdf`)
}

async function main() {
  console.log('====================================================================')
  console.log('  Miss Valeriya — SO → invoice → shipment → paymentin (paid)')
  console.log('====================================================================')
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      'Cushion Beige 00144 x1 @ 300; delivery Dubai 45. Paid — paymentin posted.',
