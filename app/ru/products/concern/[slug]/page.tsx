  ...
  return (
    <div className="min-h-screen bg-white">
              ...
              <div className="flex items-center gap-4 sm:gap-5">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-0.5">
                    <h3 className="text-sm sm:text-base font-semibold text-gray-900 group-hover:text-amber-800 transition-colors">{concern.protocolPdf.title.ru}</h3>
      ...
      {relatedConcerns.length > 0 && (
        <section className="py-8 sm:py-12 px-4">
          <div className="max-w-6xl mx-auto">
            <h2 className="text-xl sm:text-2xl font-semibold text-gray-900 mb-6">Связанные проблемы кожи</h2>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {relatedConcerns.map(related => related && (
                <Link key={related.slug} href={`/ru/products/concern/${related.slug}`}
                  className="block p-6 bg-gray-50 rounded-xl hover:bg-gray-100 transition-colors border border-gray-100">
                  <h3 className="font-semibold text-gray-900 mb-1">{related.seo.ru.h1}</h3>
                  <p className="text-sm text-gray-500 line-clamp-2">{related.seo.ru.description}</p>
                </Link>
              ))}
            </div>
          </div>
