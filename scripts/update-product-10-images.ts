/**
 * Update product 10 (SNOW O₂ CLEANSER) to the new
 * public/images/cleanser/ set (Main + S1–S6).
 *
 * Dry run:
 *   npx tsx --env-file=.env.local scripts/update-product-10-images.ts
 * Apply:
...
const NEW_MAIN = '/images/cleanser/Main.jpg'
const NEW_GALLERY = [
...
const OLD_IMAGES = [
  '/images/SNOW.jpg',
  'https://genosys.ae/images/SNOW.jpg',
  '/images/Second/cleanserboth.jpg',
  '/images/Second/cleanser_big.jpg',
]

async function main() {
  const p = await prisma.product.findFirst({
    where: { OR: [{ id: '10' }, { productNumber: '10' }] },
    select: { id: true, productNumber: true, name: true, image: true, images: true },
  })
  if (!p) throw new Error('Product 10 not found')
