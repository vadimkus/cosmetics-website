/**
 * Mirror paid MoySklad order GENCardM2608104655 into Dariya Fedotova's
 * website account, award GENOSYS Rewards, then optionally send one customer email.
 *
 * Read-only against MoySklad — does not recreate the SO/invoice/shipment/payment chain.
 *
 * Preview:
 *   npx tsx --env-file=.env --env-file=.env.local scripts/import-dariya-fedotova-paid-order-20260810.ts
 * Import + award:
 *   npx tsx --env-file=.env --env-file=.env.local scripts/import-dariya-fedotova-paid-order-20260810.ts --commit
 * One-off email (after import verified):
 *   npx tsx --env-file=.env --env-file=.env.local scripts/import-dariya-fedotova-paid-order-20260810.ts --send-email
 */
...
const EMAIL = 'dariyafedotova888@gmail.com'
const ORDER_NUMBER = 'GENCardM2608104655'
const INVOICE_NUMBER = '04911'
const DEMAND_NUMBER = '06660'
const PAYMENT_NUMBER = '06058'
const MOYSKLAD_CUSTOMER_ID = 'e3472149-94cd-11f1-0a80-1029008dd68b'
const PRODUCT_ID = '36'
const PRODUCT_QTY = 5
const UNIT_AED = 36
const PRODUCT_AED = UNIT_AED * PRODUCT_QTY
const SHIPPING_AED = 45
const TOTAL_AED = PRODUCT_AED + SHIPPING_AED
const ADDRESS = 'Al Barari, Ashjar E2, apartment 213'
const EMIRATE = 'Dubai'
const EMAIL_MARKER = 'dariyaPaidOrderAccountEmail'
...
function moySkladDate(value: string | undefined): Date {
  if (!value) return new Date('2026-08-10T15:00:00.000Z')
...
async function getAuthoritativeMoySkladData() {
  ...
  for (const entity of [invoice, demand]) {
...
async function loadPlan() {
  ...
  if (!product || !/sea algae/i.test(product.name) || Number(product.price) !== UNIT_AED) {
    throw new Error(`Website product ${PRODUCT_ID} is not the expected ${UNIT_AED} AED Sea Algae Mask.`)
  }
...
function printPlan(plan: Awaited<ReturnType<typeof loadPlan>>) {
  ...
  console.log(`=== DARIYA FEDOTOVA WEBSITE IMPORT (${COMMIT ? 'COMMIT' : SEND_EMAIL ? 'SEND EMAIL' : 'DRY RUN'}) ===`)
  ...
  console.log(`Website item: ${product.name} ×${PRODUCT_QTY} | product ${product.id} | image ${image}`)
  console.log(
    `Amounts: ${PRODUCT_QTY}×${UNIT_AED} = AED ${money(PRODUCT_AED)} + delivery AED ${money(SHIPPING_AED)} = AED ${money(TOTAL_AED)}`,
  )
  ...
  console.log(`Email subject: Welcome to GENOSYS — your order is in your account`)
...
function buildEmail(params: {
  ...
  const subject = 'Welcome to GENOSYS — your order is in your account'
    ...
    <table role="presentation" width="100%" cellspacing="0" cellpadding="0">
      <tr><td align="center" style="padding:40px 20px">
        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:580px">
          ...
          <tr><td align="center"><h1 style="margin:0 0 16px;font-size:28px">Welcome to GENOSYS</h1></td></tr>
          <tr><td style="font-size:16px;line-height:1.6;text-align:center;padding-bottom:24px">
            Hi ${firstName}, thank you for your order. We added it to your GENOSYS account
            and awarded your rewards points. You have not been charged again and no new shipment was created.
          </td></tr>
          <tr><td style="background:#fbfbfd;border-radius:16px;padding:22px">
            ...
            <table role="presentation" width="100%" cellspacing="0" cellpadding="6" style="font-size:15px">
              <tr><td>Soothing Bomb Sea Algae Mask × ${PRODUCT_QTY}</td><td align="right">AED ${money(PRODUCT_AED)}</td></tr>
          ...
          <tr><td align="center" style="padding:28px 16px">
            ...
            <div style="font-size:14px;color:#86868b;margin-top:8px">Balance: ${params.balance} points</div>
          ...
          <tr><td style="font-size:15px;line-height:1.55;text-align:center;padding-bottom:28px">
...
async function sendCustomerEmail() {
  ...
  const { subject, html } = buildEmail({
    firstName: plan.user.name.trim().split(/\s+/)[0] || 'Dariya',
