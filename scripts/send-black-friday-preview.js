...
async function sendPreview() {
  try {
    ...
    console.log('📧 Sending Black Friday email preview to:', PREVIEW_EMAIL)
    ...
    console.error('❌ Error sending preview email:', error.message)
    if (error.code === 'EAUTH') {
      console.error('❌ Authentication failed. Please check your Gmail credentials in .env.local')
    }
    process.exit(1)
  }
}
