...
/**
 * X BEAUTY CONSULTING - F.Z.C — unpaid clinic SO → invoice → shipment.
 *
 *   54461 Skin Defender Remover 200ml ×1 @ 145
 ...
 *   Total: 900.00 AED
 *
 *   node --import dotenv/config scripts/moysklad-create-x-beauty-consulting-clinic-20260822.js
 *   node --import dotenv/config scripts/moysklad-create-x-beauty-consulting-clinic-20260822.js --commit
 */
...
const ORDER = {
  name: `GENCardM${uaeShortDate()}XBC`,
  marker: `X-BEAUTY-CONSULTING-CLINIC-DEF-SNOW-MIST-RAD-ON-PDRN-${uaeToday()}`,
}
...
const EXPECTED_SUM_MINOR = 90000
...
function buildShipmentAddress(agent) {
  ...
  return {
    ...
    street: full.street || full.addInfo || 'Office - BLV - 6F - SF60836, Ajman Boulevard-A Building',
    addInfo: '',
  }
}
...
async function main() {
  console.log('====================================================================')
  console.log('  X Beauty Consulting — clinic basket + delivery 45, unpaid')
  console.log('====================================================================')
  console.log(`  Mode: ${COMMIT ? 'COMMIT' : 'DRY RUN'}`)

  const agent = await api('GET', `/entity/counterparty/${AGENT_ID}`)
  if (!/x beauty consulting/i.test(agent.name)) throw new Error(`Unexpected agent: ${agent.name}`)
  ...
  const ship = buildShipmentAddress(agent)
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      '54461 Defender x1 @145; 00021 Snow 180 x1 @165; 00188 Mist x1 @80; 00122 Radiance 50g x1 @145; 00189 Overnight x1 @170; 54475 PDRN 5000 x1 @150; delivery 45. Clinic list. Unpaid.',
    ].join('\n'),
