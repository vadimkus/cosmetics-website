...
/**
 * Bespoke product page for MULTI FUNCTIONAL ANTI-WRINKLE SERUM (product 22).
 *
 * Built on the shared editorial system, with a muted olive-gold palette in
 * antiwrinkle.css.
 *
 * Nothing in this product is fabricated. The problem was proportion: a quarter
 ...
 * but we hold no report), mechanisms for the six peptides or the ceramide
 * liposome, "mango seed butter" (deck slide 3 lists it; it is not in the
 * formula), the deck's "safe without side effects" and acne claims, the contract
...
import { COMPANION_PRODUCT_IDS, getAntiWrinkleCopy } from './antiWrinkleCopy'
...
export default function AntiWrinkleSerumProductPage({ product, unitsSold = 0, routineProducts = [] }: Props) {
  ...
  const isRtl = dir === 'rtl'
  const copy = getAntiWrinkleCopy(locale)
  ...
  const addToCart = useCallback(
    async (qty: number) => {
      ...
      try {
        ...
        errorLog('AntiWrinkleSerum: add to cart failed', error)
        throw error
      }
    },
  ...
  return (
    <div className={`cera-page aws-page ${ceraSerif.variable} min-h-[100dvh]`} dir={dir}>
      {/* ───────────────────────────── Hero ─────────────────────────────── */}
                ...
                <button
                  ...
                  className={`inline-flex h-[54px] min-w-[220px] flex-1 items-center justify-center gap-2.5 rounded-full px-8 text-[15px] font-semibold tracking-wide transition-all duration-300 disabled:cursor-not-allowed disabled:opacity-45 ${
                    justAdded
                      ...
                      : 'bg-[var(--cera-cta)] text-white hover:bg-[var(--cera-rose-ink)] hover:shadow-[0_18px_38px_-20px_rgba(28,26,19,0.8)]'
                  }`}
                >
      ...
      {/* ────────────────────── The glycerin headline ──────────────────── */}
      <section className="mx-auto max-w-[1000px] px-4 py-16 sm:px-6 lg:py-24">
        <CeraReveal className="aws-hero-figure p-6 md:p-10">
          <p className="cera-eyebrow">{copy.glycerin.eyebrow}</p>
          <h2 className="cera-serif mt-3 text-[30px] leading-[1.1] sm:text-[40px]">{copy.glycerin.title}</h2>
          <p className="mt-5 text-[15.5px] leading-relaxed text-[var(--cera-body)]">{copy.glycerin.body}</p>
      ...
      {/* ─────────────────── Bakuchiol, at the real dose ────────────────── */}
      <section className="bg-white py-16 lg:py-24">
        <div className="mx-auto max-w-[1000px] px-4 sm:px-6">
          <CeraSectionHeader eyebrow={copy.bakuchiol.eyebrow} title={copy.bakuchiol.title} intro={copy.bakuchiol.body} />

          <dl className="mt-10 grid grid-cols-1 gap-4 lg:mt-14 lg:grid-cols-3">
          ...
          <CeraReveal>
            <p className="aws-note mt-6 p-6 text-[15px] leading-relaxed text-[var(--cera-body)] md:p-7">
              {copy.bakuchiol.verdict}
            </p>
          </CeraReveal>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-[1040px] px-4 sm:px-6">
          ...
          <CeraReveal className="cera-card mt-10 overflow-hidden lg:mt-14">
            <div className="overflow-x-auto">
              <table className="aws-table w-full border-collapse text-start">
                <caption className="sr-only">{copy.peptides.title}</caption>
                <thead>
                  <tr className="border-b border-[var(--cera-line)]">
                    {[copy.peptides.columns.brand, copy.peptides.columns.supplier, copy.peptides.columns.peptide, copy.peptides.columns.amount].map(head => (
                      <th
                        key={head}
                        scope="col"
                ...
                <tbody>
                  {copy.peptides.rows.map((row, i) => (
                    <tr key={`${row.brand}-${row.peptide}-${i}`} className="border-b border-[var(--cera-line)] last:border-b-0">
                      <th scope="row" dir="ltr" className="px-4 py-4 text-start align-top text-[13.5px] font-medium leading-snug text-[var(--cera-ink)]">
                        {row.brand}
                      </th>
                      <td dir="ltr" className="px-4 py-4 text-start align-top text-[13px] text-[var(--cera-muted)]">
                        {row.supplier}
                      </td>
                      <td dir="ltr" className="px-4 py-4 text-start align-top text-[13px] leading-snug text-[var(--cera-body)]">
                        {row.peptide}
                      </td>
                      <td dir="ltr" className="aws-figure whitespace-nowrap px-4 py-4 text-start align-top text-[13.5px] font-semibold text-[var(--cera-rose-ink)]">
                        {row.amount}
                      </td>
                    </tr>
                  ))}
          ...
          <CeraReveal>
            <p className="aws-note mt-6 p-6 text-[14.5px] leading-relaxed text-[var(--cera-body)] md:p-7">
              {copy.peptides.note}
            </p>
          </CeraReveal>
        </div>
      ...
      <section className="mx-auto max-w-[1200px] px-4 py-16 sm:px-6 lg:py-24">
        ...
        <CeraReveal delay={90} className="aws-note mx-auto mt-8 max-w-[900px] p-6 md:p-7">
