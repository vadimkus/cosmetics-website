 ...
 *
 *   node --import dotenv/config scripts/moysklad-create-brau-ladies-hydro-peptide-order-invoice-demand-20260614.js
 *   node --import dotenv/config scripts/moysklad-create-brau-ladies-hydro-peptide-order-invoice-demand-20260614.js --commit
 */
...
const ORDER = {
  name: `GENCardM${uaeShortDate()}BRAU`,
  moment: uaeMomentNow(),
  marker: `Brau Ladies Hydro Cool + peptide gel mask ${uaeToday()}`,
}

const PRODUCT_LINES = [
  ['00013', 1], // Hydro Cool Modeling Mask 1kg
  ['00012', 20], // Peptide Gel Mask 39g (single pcs)
]
...
function ordersPdfPath(invoiceName) {
  ...
  return path.join(ordersDir, `GENOSYS_Brau_Ladies_${safe}.pdf`)
}

async function main() {
  console.log('====================================================================')
  console.log('  Brau Ladies Salon LLC — SO → invoice → shipment')
  console.log('====================================================================')
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      'Hydro Cool Modeling Mask 1kg 00013 x1; Peptide Gel Mask 39g 00012 x20.',
