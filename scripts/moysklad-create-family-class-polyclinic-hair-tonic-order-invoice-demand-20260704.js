...
/**
 * FAMILY CLASS POLY CLINIC L.L.C — SO → invoice → shipment + Legal_TAX PDF (landscape print).
 *
 *   00051 HR³ Matrix Hair Tonic 70ml ×1 @ 145 AED
 *   00048 HR³ Matrix Hair Solution Professional Box ×1 @ 370 AED
 *   Excellent Delivery Dubai ×1 @ 45 AED
 *
 *   node --import dotenv/config scripts/moysklad-create-family-class-polyclinic-hair-tonic-order-invoice-demand-20260704.js
 *   node --import dotenv/config scripts/moysklad-create-family-class-polyclinic-hair-tonic-order-invoice-demand-20260704.js --commit
 */
...
const ORDER = {
  ...
  moment: uaeMomentNow(),
  marker: `Family Class Polyclinic hair tonic solution delivery ${uaeToday()}`,
}
...
const EXPECTED_SUM_MINOR = 56000 // 145 + 370 + 45
...
function buildShipmentAddress(agent) {
  ...
  return { country: countryHref(), city: 'Dubai', street: addInfo || 'Triple 777 Center, Jumeirah 3, Dubai' }
}
...
async function exportInvoicePdf(invoiceId, invoiceName) {
  ...
  const outPath = path.join(ORDERS_DIR, `GENOSYS_Family_Class_Polyclinic_${safe}.pdf`)
...
async function main() {
  console.log('====================================================================')
  console.log('  FAMILY CLASS POLY CLINIC — SO + invoice + shipment (hair line)')
  console.log('====================================================================')
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      '00051 hair tonic x1 @145; 00048 hair solution pro box x1 @370; delivery 45 AED.',
    ].join(' | '),
