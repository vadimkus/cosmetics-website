...
/**
 * Rise UP — paymentin for commissioner report 01455 (745 AED) + mark Paid.
 *
 *   node --import dotenv/config scripts/moysklad-create-rise-up-paymentin-01455-20260902.js
 *   node --import dotenv/config scripts/moysklad-create-rise-up-paymentin-01455-20260902.js --commit
 */
...
const REPORT_ID = 'baf89e93-a6df-11f1-0a80-1fc6003dc001'
const REPORT_NAME = '01455'
const EXPECTED_SUM_MINOR = 74500
const STATE_REPORT_PAID_ID = 'fd15289c-c3c4-11eb-0a80-065200268290'
const MARKER = `RISE-UP-CONSIGNMENT-PAYMENT-${REPORT_NAME}-${uaeToday()}`
...
async function main() {
  console.log('====================================================================')
  console.log('  Rise UP — paymentin @ consignment report 01455 (745)')
  console.log('====================================================================')
  ...
  if (!/Rise UP/i.test(agent.name || '')) throw new Error(`Unexpected agent: ${agent.name}`)
  if (contract.name !== '34') throw new Error(`Unexpected contract: ${contract.name}`)
  ...
  const paymentIn = await api('POST', '/entity/paymentin', {
    ...
    description: [
      ...
      'Rise UP — consignment sales 745 AED — paid in full.',
    ].join(' | '),
    sum: openMinor,
    operations: [
      {
