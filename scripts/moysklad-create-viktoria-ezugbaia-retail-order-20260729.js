...
/**
 * Miss Viktoria Ezugbaia — retail SO → invoice → shipment → paymentin.
 *
 *   Ship: Marina Diamond 5, App 307, Dubai
 *   Phone: +971526091561
 *   54464 Camel cushion ×1 @ 300 retail
 *   00029 Problem Control Serum ×1 @ 330 retail
 *   Delivery Dubai ×1 @ 45
 *   00063 Collagen (red) mask ×1 FOC
 *   00140 Sea Algae (green) mask ×1 FOC
 *   Testers FOC: 00121 shampoo, 00116 Problem Control samples, 54476 Overnight samples
 *   Total paid: 675 AED
 *
 *   node --import dotenv/config scripts/moysklad-create-viktoria-ezugbaia-retail-order-20260729.js
 *   node --import dotenv/config scripts/moysklad-create-viktoria-ezugbaia-retail-order-20260729.js --commit
 */
...
const CUSTOMER = {
  name: 'Miss Viktoria Ezugbaia',
  phone: '+971526091561',
  city: 'Dubai',
  street: 'Marina Diamond 5, App 307',
}

const ORDER = {
  name: `GENCardM${uaeShortDate()}1561`,
  moment: uaeMomentNow(),
  marker: `VIKTORIA-EZUGBAIA-CAMEL-PCS-MASKS-TESTERS-${uaeToday()}`,
}

/** [code, qty, retailAed, discountPct] */
const PRODUCT_LINES = [
  ['54464', 1, 300, 0], // Camel cushion
  ['00029', 1, 330, 0], // Problem Control Serum
  ['00063', 1, 36, 100], // Red mask FOC
  ['00140', 1, 36, 100], // Green mask FOC
  ['00121', 1, 18, 100], // Shampoo tester FOC
  ['00116', 1, 18, 100], // Problem Control samples FOC
  ['54476', 1, 18, 100], // Overnight mask samples FOC
]
const DELIVERY_AED = 45
const EXPECTED_SUM_MINOR = 67500 // 300 + 330 + 45
...
async function findExistingCounterparty() {
  for (const ph of [CUSTOMER.phone, '+971 52 609 1561', '0526091561', '526091561']) {
    const d = await api('GET', `/entity/counterparty?filter=phone=${encodeURIComponent(ph)}&limit=5`)
    if (d?.rows?.length) return d.rows[0]
  }
  const d = await api('GET', `/entity/counterparty?search=${encodeURIComponent('Ezugbaia')}&limit=10`)
  return (d?.rows || []).find((r) => /ezugbaia|viktoria/i.test(r.name || '')) || null
}
...
async function exportInvoicePdf(invoiceId, invoiceName) {
  ...
  const outPath = path.join(ORDERS_DIR, `GENOSYS_Miss_Viktoria_Ezugbaia_${safe}.pdf`)
...
async function main() {
  console.log('====================================================================')
  console.log('  Miss Viktoria Ezugbaia — retail paid order')
  console.log('====================================================================')
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      '54464 camel x1 @300; 00029 PCS x1 @330; delivery 45; red+green masks FOC; testers FOC.',
