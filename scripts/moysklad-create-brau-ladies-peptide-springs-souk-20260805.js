...
/**
 * Brau Ladies Salon LLC — SO → invoice → shipment @ clinic salePrice.
 *
 *   Peptide Gel Mask 39g (00012) ×10
 *   Order comment: Springs Souk
 *   State: Доставлен - Ждем оплату
 *   Invoice Legal_TAX PDF → ~/Desktop/orders/
 *
 *   node --import dotenv/config scripts/moysklad-create-brau-ladies-peptide-sproings-souk-20260805.js
 *   node --import dotenv/config scripts/moysklad-create-brau-ladies-peptide-sproings-souk-20260805.js --commit
 */
...
const STATE_DELIVERED_AWAIT_PAY_ID = 'e1a0af19-33c5-11ea-0a80-043f000b2760' // Доставлен - Ждем оплату
...
const ORDER = {
  name: `GENCardM${uaeShortDate()}BRAUSP10`,
  moment: uaeMomentNow(),
  marker: `BRAU-PEPTIDE-SPRINGS-SOUK-P10-${uaeToday()}`,
  comment: 'Springs Souk',
}

const PRODUCT_LINES = [['00012', 10]]
const EXPECTED_SUM_MINOR = 38000 // 10 × 38 AED clinic
...
function buildShipmentAddress(agent) {
  ...
  return {
    country: countryHref(),
    city: 'Dubai',
    street: addInfo || 'Brau Ladies Salon LLC — Springs Souk',
  }
}
...
async function main() {
  console.log('====================================================================')
  console.log('  Brau Ladies — peptide x10 | Springs Souk | delivered awaiting pay')
  console.log('====================================================================')
  ...
  const invoice = await api('POST', '/entity/invoiceout', {
    ...
    rate: { currency: href('currency', CURRENCY_ID) },
    shipmentAddressFull,
    description: `Invoice for ${ORDER.name} | ${ORDER.comment} | ${ORDER.marker}`,
    positions,
  })
  ...
  const demand = await api('POST', '/entity/demand', {
    ...
    shipmentAddressFull,
    description: `Shipment for ${invoice.name} / ${ORDER.name} | ${ORDER.comment} | ${ORDER.marker}`,
  ...
  const outPath = path.join(ordersDir, `GENOSYS_Brau_Ladies_Springs_Souk_${safeInv}.pdf`)
