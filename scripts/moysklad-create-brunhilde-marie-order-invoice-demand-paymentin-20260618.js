...
/**
 * Brunhilde Marie — new customer + retail chain:
 *   order → invoice → shipment → paymentin
 *
 * Al Noor tower, Apartment 1902, Business Bay — Dubai
 * 0502911388
 *
 *   Collagen mask (00063) ×6 @ 36 AED retail
 *   Sea algae mask (00140) ×2 @ 36 AED retail
 *   Excellent Delivery Dubai ×1 @ 45 AED
 *
 *   node --import dotenv/config scripts/moysklad-create-brunhilde-marie-order-invoice-demand-paymentin-20260618.js
...
const CUSTOMER = {
  ...
  phone: '+971502911388',
  city: 'Dubai',
  street: 'Al Noor tower, Apartment 1902, Business Bay',
}

const ORDER = {
  name: `GENCardM${uaeShortDate()}1388`,
  moment: uaeMomentNow(),
  marker: `BRUNHILDE-MARIE-MASKS-RETAIL-${uaeToday()}`,
}

/** [code, qty, retailAed VAT incl.] */
const PRODUCT_LINES = [
  ['00063', 6, 36], // Intensive Repair Collagen Mask 23g
  ['00140', 2, 36], // Soothing Bomb Sea Algae Mask 23g
]
...
async function findExistingCounterparty() {
  ...
  const bySearch = await api(
    'GET',
    `/entity/counterparty?search=${encodeURIComponent('502911388')}&limit=10`
  )
  return (bySearch?.rows || []).find((r) => (r.phone || '').includes('502911388')) || null
}
...
async function findOrCreateCounterparty() {
  ...
  const created = await api('POST', '/entity/counterparty', {
    ...
    description: `Retail customer Business Bay — created with order ${ORDER.name}`,
...
function ordersPdfPath(invoiceName) {
  ...
  return path.join(ordersDir, `GENOSYS_Brunhilde_Marie_${safe}.pdf`)
}

async function main() {
  console.log('====================================================================')
  console.log('  Brunhilde Marie — customer + order → invoice → shipment → paymentin')
  console.log('====================================================================')
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      'Collagen 00063 x6, Sea algae 00140 x2 @ 36 AED retail; delivery 45 AED.',
      'Deliver: Al Noor tower, Apt 1902, Business Bay.',
