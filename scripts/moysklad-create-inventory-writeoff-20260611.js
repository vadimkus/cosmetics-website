...
/**
 * Warehouse write-off @ buyPrice — mist, masks, cleanser (user request 2026-06-11).
 *
 *   node --import dotenv/config scripts/moysklad-create-inventory-writeoff-20260611.js
...
const MARKER = `INVENTORY-WRITE-OFF-${uaeToday()}`
...
async function main() {
  ...
  const created = await api('POST', '/entity/loss', {
    ...
    description: [
      MARKER,
      'Списание со склада по buyPrice.',
      'Mist 00188 x2, collagen 00063 x10, sea algae 00140 x10, overnight 00189 x1, cleanser 00021 x6.',
    ].join(' | '),
