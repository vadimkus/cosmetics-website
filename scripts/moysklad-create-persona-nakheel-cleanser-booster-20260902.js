...
/**
 * Persona Nakheel Mall — unpaid clinic SO → invoice → shipment.
 *
 *   00024 Snow O₂ Cleanser 500ml ×1 @ 255
 *   00025 Snow Booster Toner 1000ml ×1 @ 245
 *   Total: 500.00 AED
 *
 *   node --import dotenv/config scripts/moysklad-create-persona-nakheel-cleanser-booster-20260902.js
 *   node --import dotenv/config scripts/moysklad-create-persona-nakheel-cleanser-booster-20260902.js --commit
 */
...
const ORDER = {
  name: `GENCardM${uaeShortDate()}PERSNK`,
  marker: `PERSONA-NAKHEEL-CLEANSER500-BOOSTER1000-${uaeToday()}`,
}
...
async function exportInvoicePdf(invoiceId, invoiceName) {
  ...
  const out = path.join(ORDERS_DIR, `GENOSYS_Persona_Nakheel_${safe}.pdf`)
...
async function main() {
  console.log('====================================================================')
  console.log('  Persona Nakheel — cleanser 500 + booster 1000, unpaid')
  console.log('====================================================================')
  console.log(`  Mode: ${COMMIT ? 'COMMIT' : 'DRY RUN'}`)

  const agent = await api('GET', `/entity/counterparty/${AGENT_ID}`)
  if (!/first person ladies salon/i.test(agent.name) || !/palm/i.test(agent.name)) {
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      '00024 Snow O2 500ml x1 @255; 00025 Snow Booster 1000ml x1 @245.',
      `Total 500 AED unpaid. Clinic list. Ship: ${shipment.street}, ${shipment.city}.`,
    ].join('\n'),
