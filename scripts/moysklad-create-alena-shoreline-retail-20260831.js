...
/**
 * Miss Alena (Shoreline) — new retail customer, SO → invoice → shipment (unpaid).
 *
 *   00144 Cushion #2 Beige ×1 @ 300 −10%
 *   54458 Hyaluron Cream 50g ×1 @ 290 −10%
 *   Excellent Delivery Dubai ×1 @ 45 (no discount)
 *   Total: 576.00 AED
 *
 *   Shoreline 16, PH07, Dubai. Alena WhatsApp / Stripe link. Not Dudareva DIFC.
 *
 *   node --import dotenv/config scripts/moysklad-create-alena-shoreline-retail-20260831.js
 *   node --import dotenv/config scripts/moysklad-create-alena-shoreline-retail-20260831.js --commit
 */
...
const CUSTOMER = {
  name: 'Miss Alena (Shoreline)',
  city: 'Dubai',
  street: 'Shoreline 16, PH07',
}

const ORDER = {
  name: `GENCardM${uaeShortDate()}SH16`,
  moment: uaeMomentNow(),
  marker: `ALENA-SHORELINE-RETAIL-10-${uaeToday()}`,
}
...
const EXPECTED_SUM_MINOR = 57600
...
function customerNote() {
  return 'Retail Alena client. Shoreline 16 PH07. Phone TBC. Stripe link sent 2026-08-31. Not Miss Alena Dudareva (DIFC).'
}
...
async function findOrCreateCounterparty() {
  const search = await api('GET', `/entity/counterparty?search=${encodeURIComponent(CUSTOMER.name)}&limit=10`)
  const existing = (search.rows || []).find((r) => r.name === CUSTOMER.name)
  if (existing) {
...
async function exportInvoicePdf(invoiceId, invoiceName) {
  ...
  const out = path.join(ORDERS_DIR, `GENOSYS_Miss_Alena_Shoreline_${safe}.pdf`)
...
async function main() {
  console.log('====================================================================')
  console.log('  Miss Alena (Shoreline) — retail −10% + delivery 45, unpaid')
  console.log('====================================================================')
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      '00144 Beige cushion x1 @300 −10%; 54458 Hyaluron Cream 50g x1 @290 −10%; delivery Dubai 45 no discount.',
      `Total 576 AED unpaid. Ship: ${CUSTOMER.street}, ${CUSTOMER.city}. Phone TBC. Stripe sent.`,
    ].join('\n'),
