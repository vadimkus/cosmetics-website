...
/**
 * Dominika Heidenreichova — SO → invoice → shipment (Problem Control set, retail).
 *
 ...
 *
 *   node --import dotenv/config scripts/moysklad-create-dominika-problem-control-order-invoice-demand-20260616.js
 *   node --import dotenv/config scripts/moysklad-create-dominika-problem-control-order-invoice-demand-20260616.js --commit
 */
...
const ORDER = {
  name: `GENCardM${uaeShortDate()}6866`,
  moment: uaeMomentNow(),
  marker: `DOMINIKA-PROBLEM-CONTROL-RETAIL-${uaeToday()}`,
}
...
function ordersPdfPath(invoiceName) {
  ...
  return path.join(ordersDir, `GENOSYS_Dominika_Heidenreichova_${safe}.pdf`)
}

async function main() {
  console.log('====================================================================')
  console.log('  Dominika Heidenreichova — SO → invoice → shipment')
  console.log('====================================================================')
  console.log(`  Mode: ${COMMIT ? 'COMMIT' : 'DRY RUN'}`)

  const agent = await api('GET', `/entity/counterparty/${AGENT_ID}`)
  console.log(`  Customer: ${agent.name} | ${agent.phone || '+971585576866'}`)
  ...
  console.log(`\n  Order: ${ORDER.name}`)
  console.log(`  Ship to: ${shipment.street}, ${shipment.city}`)
  for (const [code, qty, retailAed] of PRODUCT_LINES) {
    const item = stock.get(code)
    console.log(`    ${code} ${item.name.slice(0, 55)} x${qty} @ ${retailAed.toFixed(2)} AED`)
  }
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      'Problem Control set: toner 260 + serum 330 + cream 290 = 880 AED retail.',
