"""
Squares the POWER SOLUTION SWS hero shot by continuing its own studio sweep.

WHY. SWS.jpg is 956x662 on a light studio backdrop, same geometry as CVS.jpg.
The gallery stage is square and uses object-contain, so the shot would sit in
the middle of the stage tint with 147px of a different grey above and below it.
...
SRC = 'public/images/SWS.jpg'
OUT = sys.argv[1] if len(sys.argv) > 1 else 'public/images/sws-hero.jpg'
...
# Lift the sweep toward the warm paper the SWS page sits on, without chalking
# the orange SWS wordmark. Same split as the CVS script: blur holds the sweep,
...
print(f'  gap to page tint #faf6f1 at the corners: {gap}/255')
