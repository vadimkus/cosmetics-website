...
/**
 * Milena Al Wasl — unpaid clinic SO → invoice → shipment.
 *
 *   00144 Beige ×6 @ 150
 *   54464 Camel ×4 @ 150
 *   Total: 1,500.00 AED
 *
 *   node --import dotenv/config scripts/moysklad-create-milena-wasl-cushions-20260903.js
 *   node --import dotenv/config scripts/moysklad-create-milena-wasl-cushions-20260903.js --commit
 */
...
const ORDER = {
  name: `GENCardM${uaeShortDate()}MILW`,
  marker: `MILENA-WASL-CUSHIONS-BEIGE6-CAMEL4-${uaeToday()}`,
}
...
function shipmentAddress() {
  return {
    ...
    city: 'Dubai',
    street: 'Villa D01, 892 Al Wasl Rd, Umm Suqeim 1',
    addInfo: '',
  }
}
...
async function exportInvoicePdf(invoiceId, invoiceName) {
  ...
  const out = path.join(ORDERS_DIR, `GENOSYS_Milena_Wasl_${safe}.pdf`)
...
async function main() {
  console.log('====================================================================')
  console.log('  Milena Al Wasl — beige 6 + camel 4, unpaid')
  console.log('====================================================================')
  console.log(`  Mode: ${COMMIT ? 'COMMIT' : 'DRY RUN'}`)

  const agent = await api('GET', `/entity/counterparty/${AGENT_ID}`)
  if (!/milena/i.test(agent.name || '') || /jbr/i.test(agent.name || '')) {
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      '00144 Beige x6 @150; 54464 Camel x4 @150.',
      `Total 1500 AED unpaid. Clinic list. Ship: ${shipment.street}, ${shipment.city}.`,
    ].join('\n'),
