 ...
 *   routine  gentle cleanse, mist, this serum, hyaluron cream
 *
 * See hsserumCopy.ts. Do not rebuild the page around coconut water 78%,
 * a +52% hydration claim, aquaporin as the engine, or a 4-step mechanism.
 */
...
import { getHsserumCopy } from './hsserumCopy'
...
export default function HsserumProductPage({
  product,
  unitsSold = 0,
  routineProducts = [],
}: Props) {
  ...
  const isRtl = dir === 'rtl'
  const copy = getHsserumCopy(locale)

  // Single SKU - one 30 ml dropper - so no size is ever passed to the cart.
  ...
  const routineSteps = useMemo(() => {
    const routine = PRODUCT_ROUTINES['18']
    if (!routine) return []
  ...
  const addToCart = useCallback(
    async (qty: number) => {
      ...
      try {
        ...
        errorLog('Hsserum: add to cart failed', error)
        throw error
      }
    },
  ...
  const handleAddRoutineProduct = useCallback(
    async (item: Product, selection?: ProductOptionSelection, quantity = 1) => {
      ...
      try {
        ...
        errorLog('Hsserum: routine add to cart failed', error)
      } finally {
  ...
  return (
    <div className={`cera-page hsserum-page ${ceraSerif.variable} min-h-[100dvh]`} dir={dir}>
      {/* ───────────────────────────── Hero ─────────────────────────────── */}
        ...
        <div>
          ...
          {product.videoUrl ? (
            <CeraReveal className="mt-10">
              ...
              <div className="hsserum-video relative mt-4 mx-auto aspect-[9/16] w-full max-w-[340px] overflow-hidden rounded-[28px]">
                <video
      ...
      <section className="mx-auto max-w-[1000px] px-4 py-16 sm:px-6 lg:py-24">
        ...
        <div className="mt-10 grid grid-cols-1 gap-4 lg:mt-14 lg:grid-cols-2 lg:gap-6">
          ...
          <CeraReveal delay={90} className="cera-card hsserum-not p-6 lg:p-7">
            <h3 className="cera-serif text-[21px] leading-tight text-[var(--cera-ink)]">
