...
/**
 * Presents — warehouse write-off @ buyPrice (2026-08-08):
 *   00144 Beige cushion ×1
 *   54461 Skin Defender ×1
 *   54475 Meso PDRN 5000 ×3
 *   00013 Hydro Cool Modeling Mask 1kg ×1
 *   00015 SRS vial 2ml ×30
 *
 *   node --import dotenv/config scripts/moysklad-create-presents-writeoff-beige-defender-pdrn-hydro-srs-20260808.js
 *   node --import dotenv/config scripts/moysklad-create-presents-writeoff-beige-defender-pdrn-hydro-srs-20260808.js --commit
 */
...
const MARKER = `PRESENTS-BEIGE-DEFENDER-PDRN5000-HYDRO-SRS-${uaeToday()}`
...
const LINES = [
  ...
  ['00015', 30, 'Skin Renewal Peeling System (SRS) vial 2ml'],
]
...
async function fetchAll(pathStr) {
  ...
  while (true) {
    ...
    const data = await api(
      'GET',
      `${pathStr}${separator}limit=1000&offset=${offset}`
    )
    rows.push(...(data.rows || []))
...
async function ensureNoDuplicate() {
  const data = await api(
    'GET',
    `/entity/loss?search=${encodeURIComponent(MARKER)}&limit=10`
  )
...
async function main() {
  console.log('====================================================================')
  console.log('  Presents — Beige / Defender / PDRN 5000 / Hydro Cool / SRS')
  console.log('====================================================================')
  ...
  const created = await api('POST', '/entity/loss', {
    ...
    description: [
      ...
      'Beige cushion x1, Skin Defender x1, Meso PDRN 5000 x3, Hydro Cool Modeling Mask x1, SRS vials x30.',
    ].join(' | '),
