...
/**
 * Rahil Genosys — new customer, retail SO → invoice → shipment + landscape print.
 * (No paymentin — husband pays cash on delivery per WhatsApp.)
 *
 *   00140 Sea Algae Mask 23g ×3 @ 36 AED retail
 *   00035 Problem Control Cream 50g ×1 @ 290 AED retail
 *   Delivery Sharjah ×1 @ 70 AED
 *
 *   node --import dotenv/config scripts/moysklad-create-rahil-genosys-retail-order-invoice-demand-20260712.js --commit
 */
...
const DELIVERY_SHARJAH_SERVICE_ID = '52864050-59a7-11eb-0a80-022e00579624'
...
const CUSTOMER = {
  ...
  phone: '+971566794959',
  city: 'Sharjah',
  street: '10 8 Street, Industrial Area, Industrial Areas, Sharjah',
}

const ORDER = {
  name: `GENCardM${uaeShortDate()}4959`,
  moment: uaeMomentNow(),
  marker: `RAHIL-GENOSYS-RETAIL-SHARJAH-${uaeToday()}`,
}
...
const EXPECTED_SUM_MINOR = 46800
...
async function findExistingCounterparty() {
  for (const ph of [CUSTOMER.phone, '0566794959', '+971 56 679 4959']) {
    const d = await api('GET', `/entity/counterparty?filter=phone=${encodeURIComponent(ph)}&limit=5`)
    if (d?.rows?.length) return d.rows[0]
  }
  const d = await api('GET', `/entity/counterparty?search=${encodeURIComponent('566794959')}&limit=10`)
  return (d?.rows || []).find((r) => (r.phone || '').replace(/\D/g, '').includes('566794959')) || null
}
...
async function findOrCreateCounterparty() {
  ...
  const created = await api('POST', '/entity/counterparty', {
    ...
    description: `Retail customer Sharjah — WhatsApp Rahil Genosys. Husband pays cash on delivery.`,
...
function buildPositions(stock) {
  ...
  positions.push({
    ...
    price: deliveryMinor,
    discount: 0,
    assortment: href('service', DELIVERY_SHARJAH_SERVICE_ID),
    vat: 5,
    vatEnabled: true,
  })
...
function ordersPdfPath(invoiceName) {
  ...
  return path.join(ordersDir, `GENOSYS_Rahil_Genosys_${safe}.pdf`)
}

async function main() {
  console.log('====================================================================')
  console.log('  Rahil Genosys — new customer retail SO + invoice + shipment')
  console.log('====================================================================')
  ...
  for (const [code, qty, retailAed] of PRODUCT_LINES) {
    const item = stock.get(code)
    console.log(`    ${code} ${item.name.slice(0, 50)} x${qty} @ ${retailAed.toFixed(2)} retail`)
  }
  console.log(`    Delivery Sharjah x1 @ ${DELIVERY_AED.toFixed(2)} AED`)
  console.log(`  Total: ${money(sumMinor)} AED (COD — husband pays cash)`)
  ...
  const orderDesc = [
    ORDER.marker,
    '00140 Sea algae mask x3 @ 36; 00035 Problem Control Cream 50g x1 @ 290; Sharjah delivery 70.',
    'Contact 0566794959. Husband pays cash on delivery.',
