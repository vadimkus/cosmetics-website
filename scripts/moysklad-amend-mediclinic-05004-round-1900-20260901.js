...
/**
 * Mediclinic 05004 — round to last-invoice / PO+VAT: 50 × 38 VAT-included = 1,900.
 * (36.19 × 1.05 ≈ 38; same as 04614 / 04330)
 *
 *   node --import dotenv/config scripts/moysklad-amend-mediclinic-05004-round-1900-20260901.js
 *   node --import dotenv/config scripts/moysklad-amend-mediclinic-05004-round-1900-20260901.js --commit
 */
...
const PRICE_MINOR = 3800
const EXPECTED_SUM_MINOR = 190000
...
async function amendDoc(type, id, label) {
  ...
  if (after.vatIncluded !== true) throw new Error(`${label} vatIncluded still ${after.vatIncluded}`)
  console.log(`    → ${money(after.sum)} vat ${money(after.vatSum)} incl`)
}

async function main() {
  console.log('====================================================================')
  console.log('  Mediclinic 05004 — round to 1,900 (38 VAT-included)')
  console.log('====================================================================')
  ...
  await api('PUT', `/entity/customerorder/${ORDER_ID}`, {
    ...
    description: [
      so.description || '',
      '2026-09-01: rounded to 50 x 38 VAT-included = 1900 (same as 04614). PO 36.19 + 5% ≈ 38.',
    ].join(' | '),
  })
