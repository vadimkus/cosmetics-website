...
/**
 * Bespoke product page for INTENSIVE BLEMISH BALM CREAM [SPF30 / PA++]
 * (product 42).
 *
 * Built on the shared editorial system, with a clay-rose palette in
 * blemishbalm.css.
 *
 * Two things drive the structure.
 *
 * First, the carton says what this is for and our site never did: covering
 ...
 *
 * The page also carries the mandatory Korean arbutin precaution, which is
 * printed on the box and was on no page of ours, and states the single shade,
 * the beeswax, and the D5/D6 date.
 *
 * Do not add: eucalyptus oil, perilla seed oil or Rumex Crispus as actives (50,
 * 50 and 10 ppm), an unqualified sensitive-skin claim, any water-resistance
 * claim, 7.70% titanium dioxide read as 7.70% of protection, the contract
 * manufacturer, or the lot code. See the source audit.
 */

import '../cerabarrier/cerabarrier.css'
import PageBreadcrumb from '@/components/PageBreadcrumb'
import './blemishbalm.css'
...
export default function BlemishBalmProductPage({ product, unitsSold = 0, routineProducts = [] }: Props) {
  ...
  const isRtl = dir === 'rtl'
  const copy = getBlemishBalmCopy(locale)
      ...
      try {
        ...
        try {
          trackAddToCart({
            id: product.id,
            name: product.name,
            category: product.category || 'Cushion BB',
            price: product.price,
            quantity: qty,
          })
        } catch { /* analytics is best-effort */ }
      } catch (error) {
        errorLog('BlemishBalm: add to cart failed', error)
        throw error
      }
    },
  ...
  return (
    <div className={`cera-page bbc-page min-h-[100dvh]`} dir={dir}>
                ...
                <button
                  ...
                  className={`inline-flex h-[54px] min-w-[220px] flex-1 items-center justify-center gap-2.5 rounded-full px-8 text-[15px] font-semibold tracking-wide transition-all duration-300 disabled:cursor-not-allowed disabled:opacity-45 ${
                    justAdded
                      ...
                      : 'bg-[var(--cera-cta)] text-white hover:bg-[var(--cera-rose-ink)] hover:shadow-[0_18px_38px_-20px_rgba(31,24,21,0.8)]'
                  }`}
                >
      ...
      <section className="mx-auto max-w-[900px] px-4 py-16 sm:px-6 lg:py-24">
        <CeraSectionHeader eyebrow={copy.useCase.eyebrow} title={copy.useCase.title} intro={copy.useCase.body} />
      ...
      {/* ──────────────── The three-way filter comparison ──────────────── */}
      <section className="bg-white py-16 lg:py-24">
        <div className="mx-auto max-w-[1000px] px-4 sm:px-6">
          <CeraSectionHeader eyebrow={copy.paradox.eyebrow} title={copy.paradox.title} intro={copy.paradox.intro} />
          ...
          <CeraReveal className="cera-card mt-10 overflow-hidden lg:mt-14">
            <div className="overflow-x-auto">
              <table className="bbc-table w-full border-collapse text-start">
                <caption className="sr-only">{copy.paradox.title}</caption>
                <thead>
                  <tr className="border-b border-[var(--cera-line)]">
                    {[copy.paradox.columns.name, copy.paradox.columns.filters, copy.paradox.columns.load, copy.paradox.columns.grade].map(head => (
                      <th
                        key={head}
                        scope="col"
                ...
                <tbody>
                  {copy.paradox.rows.map(row => (
                    ...
                    >
                      ...
                      <td dir="ltr" className="bbc-figure px-4 py-4 text-start align-top text-[14px] text-[var(--cera-body)]">
                        {row.filters}
                      </td>
                      <td dir="ltr" className="bbc-figure whitespace-nowrap px-4 py-4 text-start align-top text-[14px] text-[var(--cera-body)]">
                        {row.load}
                      </td>
                      <td dir="ltr" className="bbc-figure whitespace-nowrap px-4 py-4 text-start align-top text-[14px] font-semibold text-[var(--cera-ink)]">
                        {row.grade}
                      </td>
                    </tr>
                  ))}
        ...
        <div className="mt-10 grid grid-cols-1 gap-4 md:grid-cols-3 lg:mt-14 lg:gap-6">
          {copy.actives.items.map((item, i) => (
            <CeraReveal key={item.name} delay={i * 80} as="article" className="cera-card cera-card-hover flex flex-col p-6 lg:p-7">
              ...
              <p dir="ltr" className="cera-serif cera-numeral bbc-figure mt-2 text-[30px] leading-none text-[var(--cera-rose)]">
                {item.dose}
              </p>
      ...
      {/* ───────────────────── The arbutin precaution ───────────────────── */}
      <section className="bg-white py-16 lg:py-20">
        <div className="mx-auto max-w-[900px] px-4 sm:px-6">
          <CeraReveal className="bbc-note p-6 md:p-9">
      ...
      <section className="mx-auto max-w-[1000px] px-4 py-16 sm:px-6 lg:py-24">
            ...
            <tbody>
              {copy.assay.rows.map(row => (
                <tr key={row.name} className="border-b border-[var(--cera-line)] last:border-b-0">
                  ...
                  <td dir="ltr" className="bbc-figure whitespace-nowrap px-4 py-4 text-start align-top text-[14px] font-semibold text-[var(--cera-rose-ink)]">
                    {row.found}
                  </td>
                </tr>
              ))}
        ...
        <CeraReveal>
          <p className="mt-6 flex items-start gap-2.5 text-[14px] leading-relaxed text-[var(--cera-body)]">
            <Info className="mt-[3px] h-4 w-4 flex-none text-[var(--cera-rose)]" aria-hidden="true" />
            <span>{copy.assay.purity}</span>
          </p>
        </CeraReveal>
      </section>

      {/* ──────────── Five no-additions, then the silicones ─────────────── */}
      <section className="bg-white py-16 lg:py-24">
        <div className="mx-auto max-w-[900px] px-4 sm:px-6">
          <CeraReveal>
      ...
      <section className="mx-auto max-w-[900px] px-4 py-16 sm:px-6 lg:py-20">
        <CeraSectionHeader eyebrow={copy.limits.eyebrow} title={copy.limits.title} intro={copy.limits.intro} />
        <ul className="mt-10 space-y-3">
          {copy.limits.items.map((item, i) => (
            <CeraReveal key={item} as="li" delay={i * 70} className="cera-card flex gap-4 p-5">
