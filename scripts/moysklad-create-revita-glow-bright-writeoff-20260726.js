...
/**
 * Warehouse write-off @ buyPrice — Revita Glow BB #01 Bright 50g ×1.
 *
 *   node --import dotenv/config scripts/moysklad-create-revita-glow-bright-writeoff-20260726.js
 *   node --import dotenv/config scripts/moysklad-create-revita-glow-bright-writeoff-20260726.js --commit
 */
...
const MARKER = `REVITA-GLOW-BRIGHT-WRITE-OFF-x1-${uaeToday()}`

const LINES = [['54472', 1, 'Revita Glow BB Cream #01 Bright 50g']]
...
async function main() {
  console.log('====================================================================')
  console.log('  Warehouse write-off — Revita Glow BB Bright 54472 ×1')
  console.log('====================================================================')
  ...
  for (const [code, qty, label] of LINES) {
    ...
    console.log(`    ${code} x${qty} @ ${money(buyMinor)} → ${money(buyMinor * qty)} | avail ${row.available} | ${label}`)
  }
  ...
  const created = await api('POST', '/entity/loss', {
    ...
    description: [
      ...
      '54472 Revita Glow BB Cream #01 Bright 50g x1.',
    ].join(' | '),
