...
/**
 * Ekaterina Petukhova — retail order + invoice.
 *
 * Lines: PDRN mask pack (54467) ×1 @ 200 AED,
 *        Radiance cream 50g (00122) ×1 @ 145 AED.
 *
 *   node --import dotenv/config scripts/moysklad-create-ekaterina-petukhova-order-invoice-20260528.js
 *   node --import dotenv/config scripts/moysklad-create-ekaterina-petukhova-order-invoice-20260528.js --commit
 */
...
const AGENT_ID = '189657a7-ee67-11ed-0a80-078b00ae97d6' // miss Ekaterina Petukhova

const ORDER_SUFFIX = '3433' // +971509213433

const ORDER = {
  name: `GENCardM${uaeShortDate()}${ORDER_SUFFIX}`,
  moment: uaeMomentNow(),
  marker: `Ekaterina Petukhova PDRN Radiance cream ${uaeToday()}`,
}

/** code, qty, unit AED */
const PRODUCT_LINES = [
  ['54467', 1, 200], // Skin Reboot PDRN mask Pack (30 sheets)
...
function buildShipmentAddress(agent) {
  ...
  return {
    country: countryHref(),
    city: 'Dubai',
    street: full?.addInfo || agent.actualAddress?.addInfo || 'UAE',
  }
}
...
async function main() {
  console.log('====================================================================')
  console.log('  Ekaterina Petukhova — order + invoice')
  console.log('====================================================================')
  ...
  for (const [code, qty, unitAed] of PRODUCT_LINES) {
    const item = stock.get(code)
    console.log(
      `    ${code} ${item.name.slice(0, 52)} x${qty} @ ${unitAed.toFixed(2)} AED`
    )
  }
  ...
  const orderPayload = {
    ...
    description: [
      ORDER.marker,
      'PDRN mask pack 54467 x1 @ 200, Radiance cream 50g 00122 x1 @ 145 AED',
    ].join(' | '),
