...
/**
 * BIANCO LAYAN BEAUTY SALON L.L.C — SO → invoice → shipment + landscape print.
 *
 *   00053 EyeCell Eye Peptide Gel Patch (box) x1
 *   54466 Bio-Ferment Age Defying Powder Mask 300g x1
 *   00011 EZ CO₂ MASK Professional Box x2
 *   00069 Power Solution CTS x10 vials
 *   00024 Snow O₂ Cleanser 500ml x1
 *
 *   node --import dotenv/config scripts/moysklad-create-bianco-layan-order-invoice-demand-20260707.js
 *   node --import dotenv/config scripts/moysklad-create-bianco-layan-order-invoice-demand-20260707.js --commit
 *   node --import dotenv/config scripts/moysklad-create-bianco-layan-order-invoice-demand-20260707.js --commit --print
 */
...
const AGENT_ID = '303f576b-bc51-11ef-0a80-18d900088ff1' // BIANCO LAYAN BEAUTY SALON L.L.C
...
const ORDER = {
  name: `GENCardM${uaeShortDate()}7589`,
  moment: uaeMomentNow(),
  marker: `BIANCO-LAYAN-PRO-ORDER-${uaeToday()}`,
}
...
const EXPECTED_SUM_MINOR = 132000 // 190+125+460+290+255
...
function buildShipmentAddress(agent) {
  ...
  const street = addInfo || 'Layan Community, Wadi Al Safa 7, Dubai Land'
...
function saveAndPrintPdf(pdfBuf, invoiceName) {
  ...
  const outPath = path.join(ordersDir, `GENOSYS_Bianco_Layan_${safe}.pdf`)
...
async function main() {
  console.log('====================================================================')
  console.log('  Bianco Layan — SO + invoice + shipment + landscape print')
  console.log('====================================================================')
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      'Eye patch x1, Bio-Ferment mask x1, EZ CO2 x2, CTS x10, Snow cleanser 500ml x1.',
