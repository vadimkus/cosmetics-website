...
/**
 * Bespoke product page for INTENSIVE PROBLEM CONTROL TONER (product 15).
 *
 ...
 * PCT-blue palette on top via pctToner.css, taken from the name bar.
 *
 * Section order:
 *
 *   effects  Apply or spray · Oil control · Quick hydration
 *   engine   Zinc PCA 0.5%
 *   howTo    AM/PM apply or spray, cotton wipe, 200 ml 360 back, plus video
 ...
 *   routine  cleanse → toner → bomb mask → problem serum → problem cream
 *
 * There is no safety assessment for this formula. The DTS MG deck carries
 * the 50% sebum / 4 weeks figure and the QACS non-comedogenic line. See
 * pctTonerCopy.ts. Do not rebuild the page around salicylic acid at 0.001%
 * or a copper peptide that is not in the registered INCI.
 */
...
import { getPctTonerCopy } from './pctTonerCopy'
...
export default function PctTonerProductPage({
  product,
  unitsSold = 0,
  routineProducts = [],
}: Props) {
  ...
  const sizeOptions = useMemo(() => getProductSizeOptions('15', product), [product])
  const [selectedSize, setSelectedSize] = useState(sizeOptions[0]?.value || product.size || '200ml')


  const isRtl = dir === 'rtl'
  const copy = getPctTonerCopy(locale)
  const Chevron = isRtl ? ChevronLeft : ChevronRight

  // Two sizes, 200 ml and 500 ml. The selected size is passed to the cart.
  ...
  const routineSteps = useMemo(() => {
    const routine = PRODUCT_ROUTINES['15']
    if (!routine) return []
  ...
  const addToCart = useCallback(
    async (qty: number) => {
      ...
      try {
        await addItem(product, qty, undefined, selectedSize)
        try {
          trackAddToCart({
            id: product.id,
            name: product.name,
        ...
        errorLog('PctToner: add to cart failed', error)
        throw error
      }
    },
  ...
  const handleAddRoutineProduct = useCallback(
    async (item: Product, selection?: ProductOptionSelection, quantity = 1) => {
      ...
      try {
        ...
        errorLog('PctToner: routine add to cart failed', error)
      } finally {
  ...
  return (
    <div className={`cera-page pct-page ${ceraSerif.variable} min-h-[100dvh]`} dir={dir}>
      {/* ───────────────────────────── Hero ─────────────────────────────── */}
      <section className="mx-auto max-w-[1200px] px-4 pt-4 sm:px-6 md:pt-8 lg:pt-12">
        ...
        <div className="mt-5 grid grid-cols-1 gap-8 lg:mt-9 lg:grid-cols-[minmax(0,1.45fr)_minmax(0,1fr)] lg:gap-12 xl:gap-16">
          ...
          <div>
                    ...
                    return (
                      <button
                        ...
                        className={`group relative overflow-hidden rounded-2xl border p-4 text-start transition-all duration-300 ${
                          isActive
                            ? 'border-[var(--cera-rose)] bg-white shadow-[0_16px_36px_-26px_rgba(180,80,20,0.45)]'
            ...
            <p className="mt-4 text-[13px] font-semibold text-[var(--cera-rose-ink)]">{copy.usageNote}</p>
            ...
            <div ref={ctaSentinel} className="mt-7">
              ...
              {inCartQty > 0 ? (
                <div className="mt-3 flex items-center gap-2 text-[13px] text-emerald-700">
                  ...
                  <span>{copy.inBag} · {inCartQty} × {selectedSize}</span>
                  <button
                    type="button"
        ...
        <div>
          ...
          {product.videoUrl ? (
            <CeraReveal className="mt-10">
              ...
              <div className="pct-video relative mt-4 aspect-square overflow-hidden rounded-[28px] sm:aspect-video">
                <video
      ...
      <section className="mx-auto max-w-[1000px] px-4 py-16 sm:px-6 lg:py-24">
        ...
        <div className="mt-10 grid grid-cols-1 gap-4 lg:mt-14 lg:grid-cols-2 lg:gap-6">
          ...
          <CeraReveal delay={90} className="cera-card pct-not p-6 lg:p-7">
            <h3 className="cera-serif text-[21px] leading-tight text-[var(--cera-ink)]">
      ...
      >
        <div className="mx-auto flex max-w-[1200px] items-center gap-3 px-4 pt-3 sm:gap-4 sm:px-6">
          ...
          <div className="min-w-0 flex-none">
            ...
            <p className="mt-1 text-[11px] text-[var(--cera-muted)] md:hidden">{selectedSize}</p>
          </div>
