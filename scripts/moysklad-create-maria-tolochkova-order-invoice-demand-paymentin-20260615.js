...
/**
 * Miss Maria Tolochkova (existing) — retail chain + address update:
 *   order → invoice → shipment → paymentin
 *
 * Delivery: Sobha Heartland, The Crest, Tower B, Apt B0208 — Dubai
 * +971562265577
 *
 *   Beige BB cushion (00144) ×1 @ 300 AED retail
 *   Excellent Delivery Dubai ×1 @ 56 AED
 *
 *   node --import dotenv/config scripts/moysklad-create-maria-tolochkova-order-invoice-demand-paymentin-20260615.js
 *   node --import dotenv/config scripts/moysklad-create-maria-tolochkova-order-invoice-demand-paymentin-20260615.js --commit
 */
...
const AGENT_ID = '4099cbea-15e2-11ef-0a80-0e86002abb64' // Miss Maria Tolochkova
...
const SHIPMENT = {
  city: 'Dubai',
  street: 'Sobha Heartland, The Crest, Tower B, Apt B0208',
}

const ORDER = {
  name: `GENCardM${uaeShortDate()}5577`,
  moment: uaeMomentNow(),
  marker: `MARIA-TOLOCHKOVA-BEIGE-CUSHION-RETAIL-${uaeToday()}`,
}

const PRODUCT_LINES = [['00144', 1, 300]]
const DELIVERY_AED = 56
...
async function loadAndUpdateCounterparty() {
  const agent = await api('GET', `/entity/counterparty/${AGENT_ID}`)
  console.log(`  Counterparty: ${agent.name} | ${agent.phone || '—'}`)

  const addr = shipmentAddress()
  const oldStreet =
    agent.actualAddressFull?.street || agent.actualAddress?.addInfo || '(none)'
  console.log(`  Address was: ${oldStreet}`)
  console.log(`  Address now: ${addr.street}, ${addr.city}`)

  if (!COMMIT) {
    console.log('  DRY RUN: would update counterparty delivery address')
    return agent
  }
  ...
  console.log('  Counterparty address updated.')
  return updated
}
...
function ordersPdfPath(invoiceName) {
  ...
  return path.join(ordersDir, `GENOSYS_Maria_Tolochkova_${safe}.pdf`)
}

async function main() {
  console.log('====================================================================')
  console.log('  Miss Maria Tolochkova — SO → invoice → shipment → paymentin')
  console.log('====================================================================')
  console.log(`  Mode: ${COMMIT ? 'COMMIT' : 'DRY RUN'}`)

  const agent = await loadAndUpdateCounterparty()
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      'Beige cushion 00144 x1 @ 300 AED retail; delivery 56 AED.',
