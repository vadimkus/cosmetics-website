...
/**
 * Bespoke product page for the HR³ MATRIX MESOPECIA KIT (product 47), last of the
 * five-product scalp line. Shares hr3.css with the rest of it.
 *
 ...
 * printed on the box in English and the buyer will be holding it.
 *
 * THE APPLICATOR IS A ROLLER. The carton says "GENOSYS STAMP(ROLLER)" in its contents
 * line and "roll (stamp)" in its instructions, our record picked stamp, and the product
 * photograph plainly shows a drum roller. The page says roller and rolls, not presses.
 *
 * THE ARITHMETIC SECTION is gated behind canSeePrices, like every other price on the
 * site, and it is not a saving. Peeling 290 + six vials pro-rata 555 = 845 against a
 * 1,100 kit, so the stamp is 255. But 290 + 740 = 1,030 buys both liquids with EIGHT
 * vials for less than the kit costs. Both halves of that have to stay on the page: the
 * second is the reason a customer wanting a full course should buy product 45 instead.
 ...
 *
 * MUST NEVER BE ADDED:
 *   - Any of the four claims listed above, including the carton's own English sentence.
 *   - "5 ml" vials. The carton contents line says 4 ml × 6.
 *   - "Stamp" as the applicator's name, or pressing as its action.
 *   - A session frequency. The carton gives the four steps, a 10-minute repeat and no
 *     interval between sessions; do not invent one.
 *   - Anything presenting the kit as a discount.
 */
...
import { COMPANION_PRODUCT_IDS, getMesopeciaKitCopy } from './mesopeciaKitCopy'
...
export default function MesopeciaKitProductPage({ product, unitsSold = 0, routineProducts = [] }: Props) {
  ...
  const isRtl = dir === 'rtl'
  const copy = getMesopeciaKitCopy(locale)
  ...
  const inciLists = useMemo(() => {
    ...
    const pick = (needle: string) =>
      list.find(i => i?.name?.startsWith('Full INCI') && i.name.includes(needle))?.description?.trim() || null
    return { peeling: pick('Peeling'), solution: pick('Solution') }
  }, [product.ingredients])

  // The arithmetic answer quotes list prices, so it follows the same visibility rule as
  // every other price on the site rather than becoming the one place they leak.
  const faqItems = useMemo(
    () => copy.faq.items.filter(item => !item.needsPrices || canSeePrices),
    [canSeePrices, copy.faq.items]
  )
  ...
  const addToCart = useCallback(
    async (qty: number) => {
      ...
      try {
        ...
        errorLog('MesopeciaKit: add to cart failed', error)
        throw error
      }
    },
  ...
  return (
    <div className={`cera-page hr3-page min-h-[100dvh]`} dir={dir}>
      ...
      {/* ──────────── It is two other products, said before anything else ──────────── */}
      <section className="mx-auto max-w-[900px] px-4 pt-14 sm:px-6 lg:pt-20">
        <CeraReveal className="hr3-alert p-6 md:p-9">
          ...
          <p className="mt-4 text-[15px] leading-relaxed text-[var(--cera-body)]">{copy.whatItIs.carton}</p>
        </CeraReveal>
      </section>
      ...
      <section className="py-16 lg:py-24">
        <div className="mx-auto max-w-[1200px] px-4 sm:px-6">
          <CeraSectionHeader eyebrow={copy.contents.eyebrow} title={copy.contents.title} intro={copy.contents.intro} />
          <div className="mt-10 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:mt-14 lg:grid-cols-3 lg:gap-6">
            {copy.contents.items.map((item, i) => (
              <CeraReveal
                key={item.name}
                delay={i * 60}
                as="article"
      ...
      {/* ───────── The arithmetic. Behind the same gate as every other price ───────── */}
      {canSeePrices ? (
        <section className="bg-white py-16 lg:py-24">
          <div className="mx-auto max-w-[1000px] px-4 sm:px-6">
            <CeraSectionHeader
              eyebrow={copy.arithmetic.eyebrow}
              title={copy.arithmetic.title}
              intro={copy.arithmetic.intro}
            />

            <CeraReveal className="cera-card mt-9 overflow-hidden">
              <table className="hr3-table w-full border-collapse text-start">
                <caption className="sr-only">{copy.arithmetic.title}</caption>
                <tbody>
                  {copy.arithmetic.rows.map(row => (
                    <tr
                      key={row.label}
            ...
            <CeraReveal>
              <p className="mt-7 text-[16px] leading-relaxed text-[var(--cera-body)]">{copy.arithmetic.body}</p>
            </CeraReveal>
          </div>
        </section>
      ) : null}

      {/* ───────────── The roller, its depth and its avoid-list ───────────── */}
      <section className="py-16 lg:py-24">
        <div className="mx-auto max-w-[1040px] px-4 sm:px-6">
          <CeraReveal>
            <p className="cera-eyebrow">{copy.roller.eyebrow}</p>
            <h2 className="cera-serif mt-3 text-[28px] leading-[1.14] sm:text-[38px]">{copy.roller.title}</h2>
            <p className="mt-4 max-w-[62ch] text-[16px] leading-relaxed text-[var(--cera-body)]">
              {copy.roller.body}
            </p>
          </CeraReveal>

          <div className="mt-9 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4 lg:gap-4">
            {copy.roller.items.map((item, i) => (
              <CeraReveal key={item.name} delay={i * 70} className="cera-card p-5 text-center">
                <p dir="ltr" className="cera-serif cera-numeral hr3-figure text-[24px] leading-none text-[var(--cera-rose)]">
                  {item.dose}
                </p>
                <p className="mt-2 text-[14px] font-semibold leading-snug text-[var(--cera-ink)]">{item.name}</p>
                <p className="mt-1.5 text-[12px] leading-snug text-[var(--cera-muted)]">{item.note}</p>
              </CeraReveal>
            ))}
          </div>

          <CeraReveal>
            <p className="mt-7 text-[15px] leading-relaxed text-[var(--cera-body)]">{copy.roller.footnote}</p>
          </CeraReveal>
        </div>
      </section>
        ...
        <section className="bg-white py-16 lg:py-20">
          <div className="mx-auto max-w-[900px] px-4 sm:px-6">
            ...
            <CeraReveal className="mt-8 border-t border-[var(--cera-line)]">
              {inciLists.peeling ? (
                <CeraAccordion title={copy.inci.peeling}>
              ...
              {inciLists.solution ? (
                <CeraAccordion title={copy.inci.solution}>
                  <p className="text-[15px] leading-[1.9] text-[var(--cera-body)]" dir="ltr">
                    {inciLists.solution}
                  </p>
                  <p className="mt-3 text-[13px] text-[var(--cera-muted)]">{copy.inci.fullInciNote}</p>
      ...
      {companions.length > 0 && (
        <section className="bg-white py-16 lg:py-20">
          <div className="mx-auto max-w-[1040px] px-4 sm:px-6">
            <CeraSectionHeader eyebrow={copy.inci.eyebrow} title={copy.companionsTitle} />
