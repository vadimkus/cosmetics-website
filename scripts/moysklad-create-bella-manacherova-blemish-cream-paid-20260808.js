...
/**
 * Miss Bella Manacherova — paid retail SO → invoice → shipment → paymentin
 * + Legal TAX PDF landscape print (same as inv 04430: BB cream 250 + delivery 45).
 *
 *   00040 Intensive Blemish Balm Cream 50g ×1 @ 250
 *   Delivery Dubai ×1 @ 45
 *   Total: 295 AED
 *
 *   node --import dotenv/config scripts/moysklad-create-bella-manacherova-blemish-cream-paid-20260808.js
 *   node --import dotenv/config scripts/moysklad-create-bella-manacherova-blemish-cream-paid-20260808.js --commit
 *   node --import dotenv/config scripts/moysklad-create-bella-manacherova-blemish-cream-paid-20260808.js --commit --no-print
 */
...
const AGENT_ID = '3d8c6c2c-425c-11f1-0a80-0d5b0084dbab' // Miss Bella Manacherova
...
const CUSTOMER = {
  name: 'Miss Bella Manacherova',
  phone: '0564371507',
  city: 'Dubai',
  street: 'La Cote 3, 305 app',
}

const ORDER = {
  name: `GENCardM${uaeShortDate()}1507`,
  moment: uaeMomentNow(),
  marker: `BELLA-MANACHEROVA-BLEMISH-CREAM-DELIVERY45-${uaeToday()}`,
}

/** [code, qty, retailAed, discountPct] */
const PRODUCT_LINES = [['00040', 1, 250, 0]]
const DELIVERY_AED = 45
const DELIVERY_DISCOUNT = 0
const EXPECTED_SUM_MINOR = 29500
...
async function exportInvoicePdf(invoiceId, invoiceName) {
  ...
  const outPath = path.join(ORDERS_DIR, `GENOSYS_Miss_Bella_Manacherova_${invoiceName}.pdf`)
...
async function main() {
  console.log('====================================================================')
  console.log('  Bella Manacherova — BB cream 250 + delivery 45 (paid) + landscape')
  console.log('====================================================================')
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      '00040 Intensive Blemish Balm Cream 50g x1 @250 retail; delivery Dubai @45.',
      `Ship: ${CUSTOMER.street}, Dubai. Phone ${CUSTOMER.phone}.`,
      'Same as prior inv 04430 — paid in full — delivered.',
    ].join('\n'),
