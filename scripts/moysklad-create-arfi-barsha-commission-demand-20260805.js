...
/**
 * ARFI Nails Barsha — commissioner report + consignment demand (agreement 25).
 * July 2026 sold list (13 pcs / 9 lines) from clinic MoySklad table 2026-08-05.
 *
 *   node --import dotenv/config scripts/moysklad-create-arfi-barsha-commission-demand-20260805.js
 *   node --import dotenv/config scripts/moysklad-create-arfi-barsha-commission-demand-20260805.js --commit
 */
...
const AGENT_ID = '39a1aa83-a5a6-11f0-0a80-1cbc00050fea' // ARFI NAILS BEAUTY SALON (Barsha)
const CONTRACT_ID = '739936aa-a809-11f0-0a80-07ba002a8e67' // Agreement 25
...
const MARKER = `ARFI Nails Barsha consignment sales July 2026 ${uaeToday()}`

/** [code, qty, label] — from Barsha WhatsApp table 2026-08-05 */
const LINES = [
  ...
  ['00063', 2, 'Intensive Repair Collagen Mask'],
  ['00053', 3, 'EyeCell Eye Peptide Gel Patch (box)'],
]

const EXPECTED_UNITS = 13
const EXPECTED_SUM_MINOR = 167400 // 1,674.00
...
async function main() {
  console.log('====================================================================')
  console.log('  ARFI Nails Barsha — report + demand (agreement 25, July 2026)')
  console.log('====================================================================')
  ...
  const report = await api('POST', '/entity/commissionreportin', {
    ...
    description: [
      MARKER,
      'ARFI NAILS BEAUTY SALON (Barsha) | Agreement 25 | July 2026 sold (13 pcs).',
      'Source: clinic MoySklad sold table WhatsApp 2026-08-05.',
    ].join('\n'),
  ...
  if (EXPORT_PDF) {
    const salesOut = path.join(
      ORDERS_DIR,
      `GENOSYS_ARFI_Nails_Barsha_Consignment_Sales_${report.name}.pdf`,
    )
    const stockOut = path.join(
      ORDERS_DIR,
      `GENOSYS_ARFI_Nails_Barsha_Consignment_Stock_Note_${demand.name}.pdf`,
    )
