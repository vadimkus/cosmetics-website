...
/**
 * SHAKIROVNA ESTHETIC CLINIC L.L.C — clinic SO → invoice → shipment
 * + Legal TAX PDF → ~/Desktop/orders/
 *
 *   00024 Snow O₂ Cleanser 500ml ×2 @ clinic salePrice (255)
 *   Total: 510 AED
 *
 *   node --import dotenv/config scripts/moysklad-create-shakirovna-clinic-cleanser500-20260811.js
 *   node --import dotenv/config scripts/moysklad-create-shakirovna-clinic-cleanser500-20260811.js --commit
 */
...
const ORDER = {
  name: `GENCardM${uaeShortDate()}SHKCLN`,
  moment: uaeMomentNow(),
  marker: `SHAKIROVNA-CLINIC-CLEANSER500-X2-${uaeToday()}`,
}

const LINES = [['00024', 2]]
const EXPECTED_SUM_MINOR = 51000 // 2 × 255
...
function shipmentAddress(agent) {
  ...
  if (full?.city && (full.street || full.addInfo)) {
    return {
      country: href('country', COUNTRY_UAE_ID),
      city: full.city || 'Dubai',
      street: full.street || full.addInfo || agent.actualAddress || '',
    }
  }
  return {
    country: href('country', COUNTRY_UAE_ID),
    city: 'Dubai',
    street: agent.actualAddress || 'One by Omniyat, Al Asayel Street - Business Bay',
  }
}
...
async function exportInvoicePdf(invoiceId, invoiceName) {
  ...
  const outPath = path.join(ordersDir, `GENOSYS_Shakirovna_Clinic_Cleanser500_${safe}.pdf`)
...
async function main() {
  console.log('====================================================================')
  console.log('  SHAKIROVNA CLINIC — Snow O₂ Cleanser 500ml ×2')
  console.log('====================================================================')
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      'Snow O2 Cleanser 500ml 00024 x2 @ clinic. Invoice sale.',
      `Ship: ${ship.street}, Dubai. Phone ${agent.phone || ''}.`,
    ].join(' | '),
