...
/**
 * Bespoke product page for MICROBIOME ENERGY INFUSING MIST (product 14).
 *
 ...
 * peach / cocoa palette on top via mist.css, taken from the bottle.
 *
 * Section order:
 *
 *   effects  Shake · Spray · Over makeup
 *   engine   shea 1.2%, the opaque emulsion
 *   howTo    shake, 10-20 cm, eyes closed, through the day, plus the product video
 ...
 *   routine  gentle cleanse → mist → hyaluron serum → hyaluron cream
 *
 * There is no safety assessment for this formula and no quantified clinical
 * trial, so this page has no proof chart. See mistCopy.ts for the sourcing
 * rules. In particular: do not rebuild the page around FENSEBIOME at
 * 0.000001% or the hyaluronan complex at ~0.001%, and do not sell a water
 * toner.
 */
...
import { getMistCopy } from './mistCopy'
...
export default function MistProductPage({
  ...
  const sizeOptions = useMemo(() => getProductSizeOptions('14', product), [product])
  const [selectedSize, setSelectedSize] = useState(sizeOptions[0]?.value || product.size || '80ml')


  const isRtl = dir === 'rtl'
  const copy = getMistCopy(locale)
  const Chevron = isRtl ? ChevronLeft : ChevronRight

  // Single 80 ml SKU. Size stays on the cart line for the same add path.
  ...
  const routineSteps = useMemo(() => {
    const routine = PRODUCT_ROUTINES['14']
    if (!routine) return []
  ...
  const addToCart = useCallback(
    async (qty: number) => {
      ...
      try {
        ...
        errorLog('Mist: add to cart failed', error)
        throw error
      }
    },
  ...
  const handleAddRoutineProduct = useCallback(
    async (item: Product, selection?: ProductOptionSelection, quantity = 1) => {
      ...
      try {
        ...
        errorLog('Mist: routine add to cart failed', error)
      } finally {
  ...
  return (
    <div className={`cera-page mist-page ${ceraSerif.variable} min-h-[100dvh]`} dir={dir}>
      {/* ───────────────────────────── Hero ─────────────────────────────── */}
        ...
        <div>
          ...
          {product.videoUrl ? (
            <CeraReveal className="mt-10">
              ...
              <div className="mist-video relative mt-4 aspect-square overflow-hidden rounded-[28px] sm:aspect-video">
                <video
      ...
      <section className="mx-auto max-w-[1000px] px-4 py-16 sm:px-6 lg:py-24">
        ...
        <div className="mt-10 grid grid-cols-1 gap-4 lg:mt-14 lg:grid-cols-2 lg:gap-6">
          ...
          <CeraReveal delay={90} className="cera-card mist-not p-6 lg:p-7">
            <h3 className="cera-serif text-[21px] leading-tight text-[var(--cera-ink)]">
