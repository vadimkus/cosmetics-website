...
/**
 * Dr. Valeria Borscheva — clinic SO → invoice → shipment + PDF.
 *
 *   00041 Multi Sun Cream SPF40/PA++ 40g ×1 @ 105
 *   00140 Soothing Bomb Sea Algae Mask 23g ×3 @ 18
 *   00063 Intensive Repair Collagen Mask 23g ×1 @ 18
 *   00144 Cushion + Refiller #02 Beige ×1 @ 150
 *   Delivery Dubai ×1 @ 45
 *   Total: 372 AED
 *
 *   node --import dotenv/config scripts/moysklad-create-valeriya-borscheva-order-invoice-demand-20260716.js
 *   node --import dotenv/config scripts/moysklad-create-valeriya-borscheva-order-invoice-demand-20260716.js --commit
 */
...
const ORDER = {
  name: `GENCardM${uaeShortDate()}9446`,
  marker: `Valeriya Borscheva SPF40 masks cushion beige ${uaeToday()}`,
}
...
const EXPECTED_SUM_MINOR = 37200
...
function buildShipmentAddress(agent) {
  ...
  return {
    country: countryHref(),
    city: 'Dubai',
    street: addInfo || 'UAE — Dr. Valeria Borscheva',
  }
}
...
async function ensureNoDuplicateToday(agentId) {
  const date = ORDER.moment?.slice(0, 10) || uaeToday()
  const filter = [
...
async function exportInvoicePdf(invoiceId, invoiceName) {
  ...
  const outPath = path.join(ordersDir, `GENOSYS_Valeriya_Borscheva_${safe}.pdf`)
...
async function main() {
  console.log('====================================================================')
  console.log('  Valeriya Borscheva — clinic SO → invoice → shipment + PDF')
  console.log('====================================================================')
  ...
  for (const [code, qty, clinicAed, discountPct] of PRODUCT_LINES) {
    const item = stock.get(code)
    const disc = discountPct ? `, ${discountPct}% off` : ''
    console.log(`    ${code} ${item.name} x${qty} @ ${clinicAed.toFixed(2)} AED${disc}`)
  }
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      '00041 SPF40 x1; 00140 sea algae mask x3; 00063 collagen mask x1; 00144 cushion+refiller beige x1 @ clinic list; delivery 45 AED.',
