...
/**
 * Bespoke product page for HR³ MATRIX MEDI SCALP SHAMPOO α (product 44), second of
 * the five-product scalp line. Shares hr3.css with the rest of the line.
 *
 ...
 * dandruff claim explicitly rather than staying quiet about it, because the claim is
 * on the carton the customer will be holding.
 *
 * THE HONEST STORY, and the reason this page leads where it does: caffeine at a real
 * 1.000%, which is ONE HUNDRED TIMES the 10 ppm in the hair tonic from the same
 * line. Both cartons name caffeine. A customer comparing ingredient lists could
 * never discover that, so the page opens with the two side by side. Backed by the
 * hardest cooling in the range (menthol 1.120% + menthyl lactate 0.080%), no SLS or
 * SLES, and pH 5.6.
 *
 ...
 * the caffeine gets no contact time at all.
 */
...
import { COMPANION_PRODUCT_IDS, getMediShampooCopy } from './mediShampooCopy'
...
export default function MediShampooProductPage({ product, unitsSold = 0, routineProducts = [] }: Props) {
  ...
  const addToCart = useCallback(
    async (qty: number) => {
      ...
      try {
        ...
        errorLog('MediShampoo: add to cart failed', error)
        throw error
      }
    },
  ...
  return (
    <div className={`cera-page hr3-page ${ceraSerif.variable} min-h-[100dvh]`} dir={dir}>
      ...
      {/* ─────────── Caffeine, and where it actually is ─────────────────── */}
      <section className="mx-auto max-w-[1000px] px-4 pt-14 sm:px-6 lg:pt-20">
        <CeraSectionHeader eyebrow={copy.caffeine.eyebrow} title={copy.caffeine.title} intro={copy.caffeine.intro} />

        <CeraReveal className="cera-card mt-9 overflow-hidden lg:mt-11">
          <table className="hr3-table w-full border-collapse text-start">
            <caption className="sr-only">{copy.caffeine.title}</caption>
            <tbody>
              {copy.caffeine.rows.map(row => (
                <tr
                  key={row.product}
        ...
        <CeraReveal>
          <p className="mt-7 text-[15.5px] leading-relaxed text-[var(--cera-body)]">{copy.caffeine.body}</p>
        </CeraReveal>
      </section>
        ...
        <div className="mx-auto max-w-[1200px] px-4 sm:px-6">
          ...
          <div className="mt-10 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:mt-14 lg:grid-cols-4 lg:gap-5">
            {copy.working.items.map((item, i) => (
              <CeraReveal key={item.name} delay={i * 60} as="article" className="cera-card cera-card-hover flex flex-col p-5 md:p-6">
      ...
      {/* ───────────── Proportion, then the dandruff refusal ────────────── */}
      <section className="bg-white py-16 lg:py-24">
        <div className="mx-auto max-w-[1040px] px-4 sm:px-6">
          <CeraReveal>
            <p className="cera-eyebrow">{copy.trace.eyebrow}</p>
            <h2 className="cera-serif mt-3 text-[28px] leading-[1.14] sm:text-[38px]">{copy.trace.title}</h2>
            <p className="mt-4 max-w-[62ch] text-[15.5px] leading-relaxed text-[var(--cera-body)]">{copy.trace.body}</p>
          </CeraReveal>
          ...
          <div className="mt-9 grid grid-cols-2 gap-3 sm:grid-cols-5 lg:gap-4">
            {copy.trace.items.map((item, i) => (
              <CeraReveal key={item.name} delay={i * 60} className="cera-card p-4 text-center md:p-5">
                ...
                <p className="mt-2 text-[12.5px] leading-snug text-[var(--cera-muted)]">{item.name}</p>
              </CeraReveal>
            ))}
          </div>

          <CeraReveal>
            <p className="mt-6 text-[14.5px] leading-relaxed text-[var(--cera-body)]">{copy.trace.footnote}</p>
          </CeraReveal>

          <CeraReveal delay={90} className="hr3-note mt-10 p-6 md:p-8">
            <p className="cera-eyebrow">{copy.dandruff.eyebrow}</p>
            <h3 className="cera-serif mt-2 text-[23px] leading-tight sm:text-[28px]">{copy.dandruff.title}</h3>
            <p className="mt-3 text-[15px] leading-relaxed text-[var(--cera-body)]">{copy.dandruff.body}</p>
          </CeraReveal>
        </div>
      </section>
