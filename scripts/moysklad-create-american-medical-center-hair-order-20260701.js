...
/**
 * AMERICAN MEDICAL CENTER DMCC — customer order (PO).
 *
 *   00051 HR³ Matrix Hair Tonic 70ml ×1 @ 145 AED
 *   00050 HR³ Matrix Scalp Peeling 100ml ×1 @ 145 AED
 *   Excellent Delivery Dubai ×1 @ 45 AED
 *
 *   node --import dotenv/config scripts/moysklad-create-american-medical-center-hair-order-20260701.js
 *   node --import dotenv/config scripts/moysklad-create-american-medical-center-hair-order-20260701.js --commit
 */
...
const ORDER = {
  ...
  moment: uaeMomentNow(),
  marker: `American Medical Center hair tonic scalp peeling delivery ${uaeToday()}`,
}
...
const EXPECTED_TOTAL_AED = 335 // 145 + 145 + 45
...
function buildShipmentAddress(agent) {
  ...
  return {
    country: countryHref(),
    city: 'Dubai',
    street: addInfo || 'JLT Red Diamond Building — Dubai',
  }
}
...
function buildPositions(stock) {
  ...
  for (const [code, qty] of PRODUCT_LINES) {
    ...
    if (item.price !== 14500) {
      console.warn(`  Note: ${code} salePrice ${money(item.price)} (expected 145.00)`)
    }
...
async function main() {
  console.log('====================================================================')
  console.log('  AMERICAN MEDICAL CENTER DMCC — customer order (PO)')
  console.log('====================================================================')
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      'HR3 Hair Tonic 00051 x1, Scalp Peeling 00050 x1, delivery 45 AED.',
    ].join(' | '),
