...
/**
 * Miss Nadi — new customer + sales order + proforma PDF → ~/Desktop/orders/
 *
 * Phone: +971521667701
 * Address: Beach Resort Tower 1, Dubai
 *
 *   node --import dotenv/config scripts/moysklad-create-miss-nadi-retail-order-20260612.js
 *   node --import dotenv/config scripts/moysklad-create-miss-nadi-retail-order-20260612.js --commit
 */
...
const CUSTOMER = {
  ...
  phone: '+971521667701',
  city: 'Dubai',
  street: 'Beach Resort Tower 1',
}

const ORDER = {
  name: `GENCardM${uaeShortDate()}7701`,
  moment: uaeMomentNow(),
  marker: `MISS-NADI-RETAIL-ORDER-${uaeToday()}`,
}

/** [code, qty, unit AED VAT incl.] */
const PRODUCT_LINES = [
  ['00144', 1, 300], // BB cushion #2 Beige
  ['54464', 1, 300], // BB cushion #3 Camel
  ['54467', 1, 400], // Skin Reboot PDRN mask Pack
  ['00011', 1, 460], // EZ CO₂ MASK Professional Box
...
async function resolveProduct(code, stock) {
  ...
  return { id: row.id, code: row.code, name: row.name, available: 0 }
}
...
function ordersPdfPath(orderName) {
  ...
  return path.join(ordersDir, `GENOSYS_Miss_Nadi_${safe}.pdf`)
}

async function main() {
  console.log('====================================================================')
  console.log('  Miss Nadi — customer order + proforma PDF')
  console.log('====================================================================')
  ...
  for (const [code, qty, unitAed] of PRODUCT_LINES) {
    const item = await resolveProduct(code, stock)
    console.log(
      `    ${code} ${item.name.slice(0, 50)} x${qty} @ ${unitAed.toFixed(2)} → ${(unitAed * qty).toFixed(2)} AED`
    )
  }
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      `Phone ${CUSTOMER.phone}. Beige+Camel cushion, PDRN pack, EZ CO₂ box, overnight mask, delivery 45.`,
    ].join(' | '),
