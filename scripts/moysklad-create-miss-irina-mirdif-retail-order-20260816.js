...
/**
 * Miss Irina (Mirdif) — new retail customer + unpaid SO → invoice → shipment.
 *
 *   00021 Snow O₂ Cleanser 180ml ×1 @ 330
 *   00145 Problem Control Toner 200ml ×1 @ 260
 *   00029 Problem Control Serum 30ml ×1 @ 330
 *   00035 Problem Control Cream 50g ×1 @ 290
 *   00189 Skin Rescue Overnight Cream Mask 100g ×1 @ 340
 *   00065 Power Solution PCS vial 2ml ×10 @ 58 (580)
 *   Delivery Dubai ×1 @ 45
 *   Total 2,175 AED
 *
 * Invoice retail PDF → ~/Desktop/orders/ (no print, no payment).
 *
 *   node --import dotenv/config scripts/moysklad-create-miss-irina-mirdif-retail-order-20260816.js
 *   node --import dotenv/config scripts/moysklad-create-miss-irina-mirdif-retail-order-20260816.js --commit
 */
...
const CUSTOMER = {
  ...
  city: 'Dubai',
  street: 'Villa 45c, Street 45c, Mirdif',
}

const ORDER = {
  name: `GENCardM${uaeShortDate()}0063`,
  moment: uaeMomentNow(),
  marker: `MISS-IRINA-MIRDIF-PC-SET-${uaeToday()}`,
}
...
const PRODUCT_LINES = [
  ...
  ['00029', 1, 330], // Problem Control Serum 30ml
  ['00035', 1, 290], // Intensive Problem Control Cream 50g
  ['00189', 1, 340], // Skin Rescue Overnight Cream Mask 100g
  ['00065', 10, 58], // Power Solution PCS 1 Vial 2ml ×10
]
const DELIVERY_AED = 45
const EXPECTED_SUM_MINOR = 217500
...
function isThisIrina(r) {
  const digits = phoneDigits(r.phone)
  return digits.endsWith('543010063') || digits.endsWith('0543010063')
}

async function findOrCreateCustomer() {
  for (const q of ['543010063', '+971 54 301 0063', 'Miss Irina Mirdif']) {
    const d = await api('GET', `/entity/counterparty?search=${encodeURIComponent(q)}&limit=20`)
    const hit = (d.rows || []).find(isThisIrina)
    if (hit) {
...
async function exportInvoicePdf(invoiceId, invoiceName) {
  ...
  const outPath = path.join(ORDERS_DIR, `GENOSYS_Miss_Irina_Mirdif_${safe}.pdf`)
...
async function main() {
  console.log('====================================================================')
  console.log('  Miss Irina Mirdif — problem-control retail set + PCS ×10 (unpaid)')
  console.log('====================================================================')
  ...
  for (const p of positions) {
    if (p._name) {
      console.log(
        `    ${p._code} ${p._name} x${p.quantity} @ ${p._retail} (avail ${p._avail})`,
      )
    } else {
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      '00021 cleanser 180 @330; 00145 PC toner 200 @260; 00029 PC serum @330; 00035 PC cream 50g @290; 00189 overnight @340; 00065 PCS vial x10 @58; delivery 45.',
