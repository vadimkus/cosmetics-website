 ...
 *   - No ingredients or INCI section. This is a device consumable, not a
 *     formula. The product record holds a single "ingredient" describing the
 *     stamp head, which would look absurd under an actives heading.
 *   - A specification table instead, because what a buyer needs here is
 *     compatibility and box contents, not a composition.
 *   - An "automatic, not manual" section, since the whole reason to buy this
 *     rather than a hand roller is that the Booster drives it at a fixed speed.
 *   - No size selector. Single SKU, one box of eight.
 *   - The routine strip is the argument for the product: the stamp is useless
...
import { getHairStampCopy } from './hairstampCopy'
...
export default function HairStampProductPage({
  product,
  unitsSold = 0,
  routineProducts = [],
}: Props) {
  ...
  const routineSteps = useMemo(() => {
    const routine = PRODUCT_ROUTINES['64']
    if (!routine) return []
  ...
  const addToCart = useCallback(
    async (qty: number) => {
      ...
      try {
        ...
        errorLog('HairStamp: add to cart failed', error)
        throw error
      }
    },
  ...
  const handleAddRoutineProduct = useCallback(
    async (item: Product, selection?: ProductOptionSelection, quantity = 1) => {
      ...
      try {
        ...
        errorLog('HairStamp: routine add to cart failed', error)
      } finally {
  ...
  return (
    <div className={`cera-page hairstamp-page min-h-[100dvh]`} dir={dir}>
                ...
                <button
                  ...
                  className={`inline-flex h-[54px] min-w-[220px] flex-1 items-center justify-center gap-2.5 rounded-full px-8 text-[15px] font-semibold tracking-wide transition-all duration-300 disabled:cursor-not-allowed disabled:opacity-45 ${
                    justAdded
                      ...
                      : 'bg-[var(--cera-cta)] text-white hover:bg-[var(--cera-rose-ink)] hover:shadow-[0_18px_38px_-20px_rgba(18,22,26,0.8)]'
                  }`}
                >
      ...
      {/* ──────────────── Specification (device, so no INCI) ────────────── */}
      <section className="bg-white py-16 lg:py-24">
        <div className="mx-auto max-w-[1200px] px-4 sm:px-6">
          <div className="grid grid-cols-1 gap-10 lg:grid-cols-[minmax(0,0.85fr)_minmax(0,1fr)] lg:gap-16">
            ...
            <CeraReveal className="cera-card overflow-hidden">
              <table className="hairstamp-spec w-full border-collapse text-start">
                <caption className="sr-only">{copy.spec.title}</caption>
                <tbody>
