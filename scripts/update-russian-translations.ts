...
async function updateRussianTranslations() {
  try {
  ...
  <div class="feature-section mt-8 text-center">
    ...
    try {
      ...
      if (hasRussianColumns) {
        console.log('✅ Russian columns found')
      } else {
        console.log('❌ Russian columns not found')
        console.log('   Please add them via: /api/admin/add-russian-blog-fields')
    ...
    console.log('')
    console.log('📝 Updating blog post with Russian translations...')
    
    // Update the blog post with Russian translations
    ...
    console.log('✅ Blog post updated with Russian translations!')
    ...
    const post = await prisma.$queryRaw<Array<{ 
      ...
      SELECT title, "titleAr", "titleRu", published 
      FROM blog_posts 
      WHERE slug = ${slug} 
      LIMIT 1
    `
    ...
    if (post.length > 0) {
      ...
      console.log(`   Russian: ${p.titleRu ? '✅ Set' : '❌ Not set'}`)
