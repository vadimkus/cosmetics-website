...
/**
 * Admin Egoistka Salon — new customer + clinic chain:
 *   order → invoice → shipment → paymentin
 *
 * Delphine Tower, Shop 2, Dubai Marina
 *
 *   BB Cushion #3 Camel (54464) ×4 @ 150 AED clinic = 600 AED
 *
 *   node --import dotenv/config scripts/moysklad-create-admin-egoistka-camel-order-invoice-demand-paymentin-20260619.js
...
const CUSTOMER = {
  ...
  city: 'Dubai',
  street: 'Delphine Tower, Shop 2, Dubai Marina',
}

const ORDER = {
  name: `GENCardM${uaeShortDate()}EGOK`,
  moment: uaeMomentNow(),
  marker: `ADMIN-EGOISTKA-CAMEL-CLINIC-${uaeToday()}`,
}

/** [code, qty, clinicAed VAT incl.] */
const PRODUCT_LINES = [['54464', 4, 150]]
...
async function findOrCreateCounterparty() {
  ...
  const created = await api('POST', '/entity/counterparty', {
    name: CUSTOMER.name,
    companyType: 'legal',
    description: `Salon Egoistka admin account — created with order ${ORDER.name}`,
...
function ordersPdfPath(invoiceName) {
  ...
  return path.join(ordersDir, `GENOSYS_Admin_Egoistka_Salon_${safe}.pdf`)
}

async function main() {
  console.log('====================================================================')
  console.log('  Admin Egoistka Salon — customer + order → invoice → shipment → paymentin')
  console.log('====================================================================')
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      'Salon Egoistka admin. Camel cushion 54464 x4 @ 150 AED clinic (600 total).',
      'Deliver: Delphine Tower, Shop 2, Dubai Marina.',
