...
/**
 * Miss Vlada — retail SO → invoice → shipment from invoice (+ PDF). No paymentin.
 *
 * Existing CP (Marina Plaza): 1fea28d7-f507-11f0-0a80-17d90029b4df · 0544382968
 * Ship: Marina Plaza Building, Office 2301, Dubai Marina, Dubai
 *   00144 Beige cushion ×1 @ 300 retail, 10% discount → 270
 *   Delivery Dubai ×1 @ 45
 *   Total: 315 AED
 *
 *   node --import dotenv/config scripts/moysklad-create-miss-vlada-beige-order-20260731.js
 *   node --import dotenv/config scripts/moysklad-create-miss-vlada-beige-order-20260731.js --commit
 */
...
const AGENT_ID = '1fea28d7-f507-11f0-0a80-17d90029b4df' // Miss Vlada — Marina Plaza

const CUSTOMER = {
  name: 'Miss Vlada',
  phone: '0544382968',
  city: 'Dubai',
  street: 'Marina Plaza Building, Office 2301, Dubai Marina',
}

const ORDER = {
  name: `GENCardM${uaeShortDate()}2968`,
  moment: uaeMomentNow(),
  marker: `MISS-VLADA-BEIGE-10PCT-${uaeToday()}`,
}

/** [code, qty, retailAed, discountPct] */
const PRODUCT_LINES = [['00144', 1, 300, 10]]
const DELIVERY_AED = 45
const DELIVERY_DISCOUNT = 0
const EXPECTED_SUM_MINOR = 31500 // 270 + 45
...
async function loadAndUpdateCounterparty() {
  ...
  console.log(`  Counterparty: ${agent.name} (${agent.id}) | phone ${agent.phone}`)
  console.log(`  Was address: ${agent.actualAddress || '(none)'}`)
  if (COMMIT) {
    ...
    console.log(`  Updated address → ${CUSTOMER.street}`)
  }
  return agent
}
...
async function exportInvoicePdf(invoiceId, invoiceName) {
  ...
  const outPath = path.join(ORDERS_DIR, `GENOSYS_Miss_Vlada_${safe}.pdf`)
...
async function main() {
  console.log('====================================================================')
  console.log('  Miss Vlada — beige cushion −10% + delivery')
  console.log('====================================================================')
  ...
  for (const [code, qty, retailAed, discountPct] of PRODUCT_LINES) {
    ...
    console.log(
      `    ${code} ${item.name.slice(0, 52)} x${qty} @ ${retailAed} −${discountPct}% → ${net.toFixed(2)}`,
    )
  }
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      '00144 beige cushion x1 @300 −10%; delivery 45.',
