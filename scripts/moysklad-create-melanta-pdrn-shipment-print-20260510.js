...
/**
 * Melanta Poly Clinic L.L.C — commission replenishment Отгрузка only (no sales report).
 *
 * Lines: Skin Reboot PDRN mask Pack (54467) × qty
 * Contract: 14
 *
 * After --commit: generates PDF via custom print template Genosys_Consignment_Stock_Note,
 * saves to Desktop, sends to default printer (`lp` on macOS) or opens the PDF.
 *
 * Dry-run:
 *   set -a; source .env; set +a
 *   node scripts/moysklad-create-melanta-pdrn-shipment-print-20260510.js
 *
 * Commit + print:
 *   node scripts/moysklad-create-melanta-pdrn-shipment-print-20260510.js --commit
 *
 * Skip system print / open:
 *   node scripts/moysklad-create-melanta-pdrn-shipment-print-20260510.js --commit --no-print
 */
...
/** MoySklad custom template: Genosys_Consignment_Stock_Note */
...
const DEMAND = {
  moment: '2026-05-10 11:00:00',
  stateShippedId: '50d70717-4582-11ea-0a80-05e3001273a2',
  marker: 'Melanta PDRN replenishment shipment 2026-05-10',
}

const LINES = [['54467', 2]] // Skin Reboot PDRN mask Pack (30 sheets)
...
async function createDemand(resolved) {
  const payload = {
    ...
    description: [
      ...
      'Contract: 14',
      'Replenishment only (no paired commissioner sales report for this movement).',
      'PDRN mask pack 54467 × 2.',
    ].join('\n'),
...
async function main() {
  console.log('====================================================================')
  console.log('  Melanta — PDRN shipment + Genosys Consignment Stock Note print')
  console.log('====================================================================')
