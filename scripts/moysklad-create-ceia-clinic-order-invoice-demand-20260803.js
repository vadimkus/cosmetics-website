...
/**
 * CEIA CLINIC L.L.C — SO → invoice → shipment (clinic prices, unpaid).
 *
 *   Hydro Cool Modeling Mask 1kg ×1 @ 300
 *   Snow O₂ Cleanser 500ml ×1 @ 255
 *   Snow Booster Toner 1000ml ×1 @ 245
 *   SRS vial 2ml ×10 FREE
 *   EZ CO₂ MASK box ×2 @ 230
 *   Multi Sun Cream SPF40 ×1 @ 105
 *   Skin Defender Lip & Eye Makeup Remover ×1 @ 145
 *   Peptide Gel Mask ×10 @ 38
 *   Total: 1,890 AED
 *
 *   node --import dotenv/config scripts/moysklad-create-ceia-clinic-order-invoice-demand-20260803.js
 *   node --import dotenv/config scripts/moysklad-create-ceia-clinic-order-invoice-demand-20260803.js --commit
 */
...
const CUSTOMER = {
  city: 'Dubai',
  street: 'Villa No. 2, Al Manara',
}

const ORDER = {
  name: `GENCardM${uaeShortDate()}CEIA`,
  moment: uaeMomentNow(),
  marker: `CEIA-CLINIC-HYDROCOOL-PRO-ORDER-${uaeToday()}`,
}

/** [code, qty, clinicAed, discountPct] */
const PRODUCT_LINES = [
  ['00013', 1, 300, 0], // Hydro Cool Modeling Mask 1kg
  ['00024', 1, 255, 0], // Snow O₂ Cleanser 500ml
  ['00025', 1, 245, 0], // Snow Booster Toner 1000ml
  ['00015', 10, 40.5, 100], // SRS vials FREE
  ['00011', 2, 230, 0], // EZ CO₂ MASK Professional Box
  ['00041', 1, 105, 0], // SPF40
  ['54461', 1, 145, 0], // Skin Defender
  ['00012', 10, 38, 0], // Peptide Gel Mask
]
const EXPECTED_SUM_MINOR = 189000 // 300+255+245+460+105+145+380
...
function shipmentAddress() {
  return {
    ...
    addInfo: 'Villa No. 2, Al Manara, Dubai, United Arab Emirates',
  }
}
...
async function main() {
  console.log('====================================================================')
  console.log('  CEIA CLINIC — SO + invoice + shipment')
  console.log('====================================================================')
  ...
  for (const p of positions) {
    const net = (p._clinic * p.quantity * (100 - p._disc)) / 100
    const tag = p._disc === 100 ? 'FREE' : 'clinic'
    console.log(
      `    ${p._code} ${p._name.slice(0, 48)} x${p.quantity} @ ${p._clinic} ${tag} → ${net.toFixed(2)} (avail ${p._avail})`,
    )
  }
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      'Clinic: Hydro Cool + cleanser500 + booster1000 + EZ×2 + SPF40 + Defender + peptide×10.',
      'FOC: SRS vials ×10. Unpaid — no paymentin.',
      `Ship: ${CUSTOMER.street}, Dubai. Phone +971561149495.`,
    ].join('\n'),
