 ...
 *   Marina 01432 → 1,036 AED (agreement 00030)
 *   Elite  01433 →   166 AED (agreement 21)
 *   Clinic 01434 →   480 AED (agreement 26)
 *   Total: 1,682 AED
 *
 *   node --import dotenv/config scripts/moysklad-create-shakirovna-01432-01433-01434-paymentins-20260812.js
 *   node --import dotenv/config scripts/moysklad-create-shakirovna-01432-01433-01434-paymentins-20260812.js --commit
 */
...
const { uaeMomentAddMinutes, uaeToday } = require('./lib/moysklad-uae-date')
...
const MARKER = `SHAKIROVNA-3LOC-PAYMENTIN-01432-01433-01434-${uaeToday()}`
...
async function main() {
  console.log('====================================================================')
  console.log('  Shakirovna — paymentin ×3 (Marina / Elite / Clinic)')
  console.log('====================================================================')
  ...
  for (const cfg of REPORTS) {
    ...
    if ((report.sum || 0) !== cfg.expectedMinor) {
      throw new Error(`${cfg.reportName}: sum ${money(report.sum)} ≠ ${money(cfg.expectedMinor)}`)
    }
    if ((report.payedSum || 0) >= cfg.expectedMinor) {
      throw new Error(`${cfg.reportName} already paid: ${money(report.payedSum)}`)
    }

    const agentId = report.agent?.meta?.href?.split('/').pop()?.split('?')[0] || cfg.agentId
    ...
    prepared.push({ cfg: { ...cfg, agentId }, report, siteMarker })
    console.log(
  ...
  for (let i = 0; i < prepared.length; i++) {
    ...
    results.push({
      ...
      sum: money(cfg.expectedMinor),
      state: final.state?.name,
      payId: paymentIn.id,
    })

    console.log(
      `\n  ✓ ${cfg.label}: paymentin ${paymentIn.name} | ${money(cfg.expectedMinor)} AED → ${final.state?.name}`,
    )
  ...
  for (const r of results) {
    console.log(`  ${r.label}: report ${r.report} → pay ${r.paymentin} | ${r.sum} AED | ${r.state}`)
  }
}
