...
/**
 * Persona Palm Jumeirah — First Person Ladies Salon LLC (Palm Jumeirah)
 *
 * 1. Полученный отчет комиссионера (sold items)
 * 2. Отгрузка (demand) under the same agreement with the same lines
 *
 * Agreement: 00078
 *
 * Dry-run:
 *   set -a; source .env; set +a
 *   node scripts/moysklad-create-persona-palm-jumeirah-sales-20260506.js
 *
 * Commit:
 *   node scripts/moysklad-create-persona-palm-jumeirah-sales-20260506.js --commit
 *
 * Mapping notes:
 *   - "Hair solution" → 00048 HR³ Matrix Hair Solution Professional Box (8pcs); if they meant single vial, use 00049 in MoySklad.
 */
...
const REPORT = {
  moment: '2026-05-06 11:00:00',
  stateNotPaidId: '3203736c-c43b-11eb-0a80-093a002b59a6',
  marker: 'Persona Palm Jumeirah consignment sales 2026-05-06',
}

const DEMAND = {
  moment: '2026-05-06 11:05:00',
  stateShippedId: '50d70717-4582-11ea-0a80-05e3001273a2',
  marker: 'Persona Palm Jumeirah shipment 2026-05-06',
}

const LINES = [
  ['00030', 1], // All For Sensitive Serum 30ml
  ['00021', 5], // Snow O₂ Cleanser 180ml
  ['54467', 3], // Skin Reboot PDRN mask Pack (30 sheets)
  ['00063', 4], // Intensive Repair Collagen Mask 23g
  ['00140', 9], // Soothing Bomb Sea Algae Mask 23g
  ...
  ['00052', 3], // HR³ Matrix Scalp & Hair Shampoo 300ml
  ...
  ['00051', 4], // HR³ Matrix Hair Tonic 70ml
...
async function createReport(resolved) {
  const payload = {
    ...
    description: [
      REPORT.marker,
      'Customer: First Person Ladies Salon LLC (Palm Jumeirah) / Persona Palm Jumeirah',
      'Agreement: 00078 — user-provided sold-items list.',
    ].join('\n'),
...
async function createDemand(resolved, report) {
  const payload = {
    ...
    description: [
      ...
      'Customer: First Person Ladies Salon LLC (Palm Jumeirah) / Persona Palm Jumeirah',
      'Agreement: 00078',
      `Created after sales report ${report.name} — same items and quantities.`,
    ].join('\n'),
...
async function main() {
  console.log('====================================================================')
  console.log('  MoySklad Persona Palm Jumeirah — commission report + shipment')
  console.log('====================================================================')
  ...
  printLines('  Commission report + shipment lines (same set)', resolved)
