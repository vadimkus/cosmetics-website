...
/**
 * Miss Laura Abizova — retail SO → invoice → shipment + PDF.
 * Order > 1,000 AED → free Abu Dhabi delivery (no delivery line).
 *
 *   00195 Hyaluron Serum ×1 @ 330
 *   54471 Scalp Brush ×1 @ 50
 *   00035 Problem Control Cream 50g ×1 @ 290
 *   00052 HR³ Shampoo 300ml ×1 @ 340
 *   00144 Cushion #2 Beige ×1 @ 300
 *
 *   node --import dotenv/config scripts/moysklad-create-laura-abizova-retail-order-invoice-demand-20260705.js
 *   node --import dotenv/config scripts/moysklad-create-laura-abizova-retail-order-invoice-demand-20260705.js --commit
 */
...
const AGENT_ID = 'a90bd5e1-7580-11ef-0a80-18b7002017fc' // Miss Laura Abizova (Abu Dhabi)
...
const ORDER = {
  name: `GENCardM${uaeShortDate()}9596`,
  moment: uaeMomentNow(),
  marker: `LAURA-ABIZOVA-RETAIL-${uaeToday()}`,
}
...
const EXPECTED_SUM_MINOR = 131000
...
function buildShipmentAddress(agent) {
  ...
  const street = full?.addInfo || agent.actualAddress || 'Al Zhahiya Street, Waterfront Towers, Tower A, apt 509'
  return { country: countryHref(), city: 'Abu Dhabi', street }
}
...
function buildPositions(stock) {
  ...
  for (const [code, qty, retailAed] of PRODUCT_LINES) {
    ...
    lines.push({ code, name: item.name, qty, retailAed, lineMinor })
  }
...
async function main() {
  console.log('====================================================================')
  console.log('  Miss Laura Abizova — retail SO + invoice + shipment')
  console.log('====================================================================')
  ...
  console.log(`\n  Ship: ${shipmentAddressFull.street}, ${shipmentAddressFull.city}`)
  console.log(`  Order: ${ORDER.name}`)
  for (const l of lines) {
    console.log(`    ${l.code} ${l.name} x${l.qty} @ ${l.retailAed.toFixed(2)} → ${money(l.lineMinor)} AED`)
  }
  console.log(`  Delivery Abu Dhabi: FREE (order > 1,000 AED)`)
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      'Retail: hyaluron serum, scalp brush, problem cream, HR3 shampoo, Beige cushion.',
      'Free Abu Dhabi delivery — order > 1,000 AED.',
    ].join(' | '),
