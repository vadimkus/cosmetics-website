...
/**
 * Bespoke product page for the HR³ MATRIX SCALP BRUSH (product 61).
 *
 ...
 * slate-teal palette on top via scalpbrush.css.
 *
 * This is the shortest of the bespoke layouts, on purpose. The entire
 * manufacturer record for this product is a four-slide DTS MG deck with six
 * feature lines in it (quoted in scalpBrushCopy.ts). A 50 AED silicone brush
 * does not have a formula, a clinical study, a COA or a protocol, and the
 * previous version of this product's content filled that gap by inventing a
 * "+50% absorption" figure and a KFDA claim belonging to a different product.
 * The correct response to thin source material is a shorter page, not a padded
 * one, so the sections here map one-to-one onto what DTS MG actually documents:
 *
 *   effects  the four documented benefit lines (foam, cleansing, blood flow,
 *            volume) - the whole product argument
 *   design   the two documented construction lines (soft silicone, stable grip)
 *   howTo    the manufacturer's own wet-with-shampoo instruction, expanded into
 *            four steps without adding anything to it
 *   care     cleaning, replacement and the standard cautions
 *   routine  where a wash-time tool sits relative to the leave-on treatments
 *
 * Where it deliberately diverges from the other bespoke layouts:
 *   - No ingredients or INCI section. It is a moulded silicone object. The
 *     product record's two "ingredients" describe the head and the handle, which
 *     would look absurd under an actives heading, so they appear as the design
 *     section instead.
 *   - No lab or quality section. There is no COA for a brush and no clinical
 *     study for it either; a "quality" block here could only be padding.
 *   - No brochure link. The DTS MG deck is four slides, one of which says
 *     "Thank You" - it is a sourcing document for us, not a customer download.
 *   - No size selector. Single SKU, one brush.
 *   - Only one inline figure. Of the five gallery images, s1 and s2 have the
 *     removed claims baked into the artwork and s3 has a garbled shampoo label,
 *     so s4 is the only one promoted to large inline art. The rest stay in the
 *     record-driven gallery, which was a deliberate call (see the copy file).
 *
 * See scalpBrushCopy.ts for the sourcing rules and the list of claims that must
 * not come back without a manufacturer document.
 */

import '../cerabarrier/cerabarrier.css'
import PageBreadcrumb from '@/components/PageBreadcrumb'
import './scalpbrush.css'
...
import { getScalpBrushCopy } from './scalpBrushCopy'
...
export default function ScalpBrushProductPage({
  product,
  unitsSold = 0,
  routineProducts = [],
}: Props) {
  ...
  const isRtl = dir === 'rtl'
  const copy = getScalpBrushCopy(locale)

  // Single SKU - one brush - so no size is ever passed to the cart.
  ...
  const routineSteps = useMemo(() => {
    const routine = PRODUCT_ROUTINES['61']
    if (!routine) return []
  ...
  const addToCart = useCallback(
    async (qty: number) => {
      ...
      try {
        ...
        errorLog('ScalpBrush: add to cart failed', error)
        throw error
      }
    },
  ...
  const handleAddRoutineProduct = useCallback(
    async (item: Product, selection?: ProductOptionSelection, quantity = 1) => {
      ...
      try {
        ...
        errorLog('ScalpBrush: routine add to cart failed', error)
      } finally {
  ...
  return (
    <div className={`cera-page scalpbrush-page ${ceraSerif.variable} min-h-[100dvh]`} dir={dir}>
      {/* ───────────────────────────── Hero ─────────────────────────────── */}
      ...
      <section className="bg-white py-16 lg:py-24">
        <div className="mx-auto max-w-[1200px] px-4 sm:px-6">
          <div className="grid grid-cols-1 gap-10 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1fr)] lg:gap-16">
            ...
            <div>
              <CeraReveal>
                <p className="cera-eyebrow">{copy.design.eyebrow}</p>
                <h2 className="cera-serif mt-3 text-[30px] leading-[1.12] sm:text-[40px]">{copy.design.title}</h2>
              ...
              <div className="mt-8 space-y-4">
                {copy.design.points.map((point, i) => (
                  <CeraReveal
                    key={point.title}
                    delay={i * 80}
                    as="article"
      ...
      {/* ─────────────────────────── Video ──────────────────────────────── */}
      {product.videoUrl ? (
        // Keeps the page's default background rather than taking the white slot
        // the care section already owns; a rule separates it from the how-to
        // block above so the alternation below is untouched.
      ...
      <section className="bg-white py-16 lg:py-24">
        <div className="mx-auto max-w-[1000px] px-4 sm:px-6">
          <CeraSectionHeader eyebrow={copy.care.eyebrow} title={copy.care.title} />
          <div className="mt-10 grid grid-cols-1 gap-4 lg:mt-14 lg:grid-cols-2 lg:gap-6">
            {/* Upkeep: routine maintenance, so plain cards with ticks. */}
            <CeraReveal as="ul" className="cera-card space-y-3 p-6 lg:p-7">
              {copy.care.upkeep.map(point => (
            ...
            {/* Cautions: reasons to stop, so tinted and flagged. */}
            <CeraReveal as="ul" delay={90} className="cera-card scalpbrush-cautions space-y-3 p-6 lg:p-7">
              {copy.care.cautions.map(point => (
                <li key={point} className="flex items-start gap-3">
