...
/**
 * Miss Elena Zakharova — new customer + retail chain:
 *   order → invoice → shipment → paymentin
 *
 * Salon Egoistka
 * Marina Promenade, Delphine Tower, Shop No 2 — Dubai
 * +971505471776
 *
 *   Beige BB cushion (00144) ×1 @ 150 AED
 *   Excellent Delivery Dubai ×1 @ 20 AED
 *
 *   node --import dotenv/config scripts/moysklad-create-miss-elena-zakharova-order-invoice-demand-paymentin-20260614.js
 *   node --import dotenv/config scripts/moysklad-create-miss-elena-zakharova-order-invoice-demand-paymentin-20260614.js --commit
 */
...
const CUSTOMER = {
  name: 'Miss Elena Zakharova',
  phone: '+971505471776',
  salon: 'Salon Egoistka',
  city: 'Dubai',
  street: 'Marina Promenade, Delphine Tower, Shop No 2',
}

const ORDER = {
  name: `GENCardM${uaeShortDate()}1776`,
  moment: uaeMomentNow(),
  marker: `MISS-ELENA-ZAKHAROVA-BEIGE-CUSHION-RETAIL-${uaeToday()}`,
}

/** [code, qty, retailAed VAT incl.] */
const PRODUCT_LINES = [['00144', 1, 150]]
...
async function findExistingCounterparty() {
  ...
  const bySearch = await api(
    'GET',
    `/entity/counterparty?search=${encodeURIComponent('505471776')}&limit=10`
  )
  return (bySearch?.rows || []).find((r) => (r.phone || '').includes('505471776')) || null
}
...
async function findOrCreateCounterparty() {
  ...
  const created = await api('POST', '/entity/counterparty', {
    ...
    description: `${CUSTOMER.salon} — retail customer; created with order ${ORDER.name}`,
...
function ordersPdfPath(invoiceName) {
  ...
  return path.join(ordersDir, `GENOSYS_Miss_Elena_Zakharova_${safe}.pdf`)
}

async function main() {
  console.log('====================================================================')
  console.log('  Miss Elena Zakharova — customer + order → invoice → shipment → paymentin')
  console.log('====================================================================')
  ...
  console.log(`  Salon: ${CUSTOMER.salon}`)
  ...
  const order = await api('POST', '/entity/customerorder', {
    ...
    description: [
      ORDER.marker,
      `${CUSTOMER.salon}. Beige cushion 00144 x1 @ 150 AED; delivery 20 AED.`,
